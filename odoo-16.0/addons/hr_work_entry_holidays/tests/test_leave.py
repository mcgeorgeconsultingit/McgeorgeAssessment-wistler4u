...
class TestWorkEntryLeave(TestWorkEntryHolidaysBase):
    ...
    def test_resource_leave_different_calendars(self):
        ...
        resource_leave = leave._create_resource_leave()
        self.assertEqual(
            len(resource_leave), 2, "it should have created one resource leave per calendar"
        )
        self.assertEqual(
    ...
    def test_validate_leave_with_overlap(self):
        ...
        leave_work_entry = (
            self.env["hr.work.entry"].search([("leave_id", "=", leave.id)]) - work_entry_1
        )
        self.assertTrue(
        ...
        self.assertEqual(
            leave_work_entry[:1].state, "conflict", "The leave work entry should conflict"
        )
    ...
    def test_validate_leave_without_overlap(self):
        ...
        self.assertNotEqual(
            leave_work_entry[:1].state, "conflict", "The leave work entry should not conflict"
        )
    ...
    def test_refuse_leave(self):
        ...
        self.assertNotEqual(
            adjacent_work_entry.state, "conflict", "Non overlapping work entry should not conflict"
        )
        leave.action_refuse()
        self.assertTrue(
            all(work_entries.mapped(lambda w: w.state != "conflict")),
    ...
    def test_archived_work_entry_conflict(self):
        ...
        self.assertEqual(
            work_entry.state, "conflict", "Attendance work entries should conflict with the leave"
        )
