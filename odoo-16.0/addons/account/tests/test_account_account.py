...
class TestAccountAccount(AccountTestInvoicingCommon):
    ...
    def test_name_create_account_code_only(self):
        ...
        """
        account_id = (
            self.env["account.account"].with_context(import_file=True).name_create("550003")[0]
        )
        account = self.env["account.account"].browse(account_id)
        self.assertEqual(account.code, "550003")
        ...
        account_id = (
            self.env["account.account"].with_context(import_file=True).name_create("550004 ")[0]
        )
        account = self.env["account.account"].browse(account_id)
    ...
    def test_name_create_account_name_with_number(self):
        ...
        """
        account_id = (
            self.env["account.account"].with_context(import_file=True).name_create("550005 CO2")[0]
        )
        account = self.env["account.account"].browse(account_id)
