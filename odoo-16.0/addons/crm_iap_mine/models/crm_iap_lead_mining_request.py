...
class CRMLeadMiningRequest(models.Model):
    ...
    contact_filter_type = fields.Selection(
        [("role", "Role"), ("seniority", "Seniority")], string="Filter on", default="role"
    )
    ...
    def _compute_tooltip(self):
        for record in self:
            ...
            record.lead_credits = _("%d credits will be consumed to find %d companies.") % (
                company_credits,
                record.lead_number,
            )
    ...
    def _compute_lead_count(self):
        if self.ids:
            ...
            leads_data = []
        mapped_data = dict(
            (m["lead_mining_request_id"][0], m["lead_mining_request_id_count"]) for m in leads_data
        )
        for request in self:
    ...
    def _iap_contact_mining(self, params, timeout=300):
        endpoint = (
            self.env["ir.config_parameter"].sudo().get_param("reveal.endpoint", DEFAULT_ENDPOINT)
    ...
    def action_submit(self):
        self.ensure_one()
        if self.name == _("New"):
            self.name = self.env["ir.sequence"].next_by_code("crm.iap.lead.mining.request") or _(
                "New"
            )
