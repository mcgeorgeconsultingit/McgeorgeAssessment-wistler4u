...
class IrActionsActWindowView(models.Model):
    ...
    view_mode = fields.Selection(selection_add=[("qweb", "QWeb")], ondelete={"qweb": "cascade"})
...
class Base(models.AbstractModel):
    ...
    @api.model
    def web_search_read(
        self, domain=None, fields=None, offset=0, limit=None, order=None, count_limit=None
    ):
        """
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        if field.type not in supported_types:
            types = dict(
                self.env["ir.model.fields"]._fields["ttype"]._description_selection(self.env)
            )
            raise UserError(
                _(
        ...
        if field.type == "selection":
            return {
                ...
                "values": self._search_panel_selection_range(
                    field_name, model_domain=model_domain, extra_domain=extra_domain, **kwargs
                ),
            }
