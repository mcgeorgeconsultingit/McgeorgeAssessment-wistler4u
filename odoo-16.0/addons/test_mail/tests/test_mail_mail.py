...
class TestMailMail(TestMailCommon):
    ...
    def _reset_data(self):
        ...
        self.test_notification.write(
            {"failure_reason": False, "failure_type": False, "notification_status": "ready"}
        )
    ...
    def test_mail_mail_attachment_access(self):
        ...
        with patch.object(type(self.env["ir.attachment"]), "check", _patched_check):
            ...
            self.assertEqual(
                mail.unrestricted_attachment_ids.mapped("name"), ["file 1", "file 3", "new file"]
            )
    ...
    def test_mail_mail_send_exceptions_origin(self):
        ...
        self.assertFalse(
            mail.failure_type, "Mail: bugged from (ascii): no failure type, should be updated"
        )
        self.assertEqual(mail.state, "exception")
        self.assertEqual(
            notification.failure_reason,
    ...
    def test_mail_mail_send_exceptions_recipients_emails(self):
        ...
        for email_to in self.emails_falsy:
            ...
            self.assertFalse(
                mail.failure_type, "Mail: missing email_to: no failure type, should be updated"
            )
            self.assertEqual(mail.state, "exception")
            if email_to == " ":
        ...
        for email_to, failure_type in zip(
            ...
            self.assertFalse(
                mail.failure_type, "Mail: invalid email_to: no failure type, should be updated"
            )
            self.assertEqual(mail.state, "exception")
            self.assertFalse(notification.failure_reason, "Mail: failure reason not propagated")
            self.assertEqual(
    ...
    def test_mail_mail_send_exceptions_recipients_partners(self):
        ...
        partners_invalid = self.env["res.partner"].create(
            [{"name": "Name %s" % email, "email": email} for email in self.emails_invalid]
        )
        partners_invalid_ascii = self.env["res.partner"].create(
            [{"name": "Name %s" % email, "email": email} for email in self.emails_invalid_ascii]
        )
    ...
    def test_mail_mail_send_exceptions_recipients_partners_mixed(self):
        ...
        mail.write(
            {"recipient_ids": [(5, 0), (4, partners_valid[1].id), (4, partners_falsy[0].id)]}
        )
    ...
    def test_mail_mail_send_server(self):
        ...
        self.connect_mocked.assert_has_calls(
            calls=[
                call(
                    smtp_from="notifications@test.com", mail_server_id=self.server_notification.id
                ),
