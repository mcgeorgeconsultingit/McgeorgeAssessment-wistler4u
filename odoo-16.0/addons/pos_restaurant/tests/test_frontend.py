...
class TestFrontend(odoo.tests.HttpCase):
    def setUp(self):
        ...
        second_cash_journal = self.env["account.journal"].create(
            {"name": "Cash 2", "code": "CSH2", "type": "cash", "company_id": main_company.id}
        )
    ...
    def test_01_pos_restaurant(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "pos_restaurant_sync", login="admin"
        )
    ...
    def test_02_others(self):
        self.pos_config.with_user(self.env.ref("base.user_admin")).open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "SplitBillScreenTour", login="admin"
        )
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "ControlButtonsTour", login="admin"
        )
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "FloorScreenTour", login="admin"
        )

    def test_04_ticket_screen(self):
        self.pos_config.with_user(self.env.ref("base.user_admin")).open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "PosResTicketScreenTour", login="admin"
        )
    ...
    def test_05_tip_screen(self):
        ...
        self.pos_config.with_user(self.env.ref("base.user_admin")).open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "PosResTipScreenTour", login="admin"
        )
    ...
    def test_06_split_bill_screen(self):
        self.pos_config.with_user(self.env.ref("base.user_admin")).open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.pos_config.id, "SplitBillScreenTour2", login="admin"
        )
