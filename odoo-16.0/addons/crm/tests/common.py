            ...
            for idx, lead_data in enumerate(leads_data):
                if partner_count and idx < partner_count:
                    ...
                    lead_data["email_from"] = tools.formataddr(
                        ("TestCustomer_%02d" % (idx), "customer_email_%04d@example.com" % (idx))
                    )
            ...
            for idx, lead_data in enumerate(leads_data):
                ...
                if lead_data["country_id"]:
                    lead_data["phone"] = phone_validation.phone_format(
                        "0456%04d99" % (idx), country.code, country.phone_code, force_format="E164"
                    )
                else:
        ...
        try:
            ...
            for fname, expected in expected.items():
                if expected is False:
                    ...
                    self.assertEqual(
                        opportunity[fname], expected, "%s must be equal to %s" % (fname, expected)
                    )
...
class TestLeadConvertCommon(TestCrmCommon):
    ...
    def assertMemberAssign(self, member, count):
        ...
        if member.assignment_domain:
            self.assertEqual(
                member_leads.filtered_domain(literal_eval(member.assignment_domain)), member_leads
            )
