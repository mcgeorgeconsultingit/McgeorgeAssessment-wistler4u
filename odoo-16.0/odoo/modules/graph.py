...
class Graph(dict):
    ...
    def update_from_db(self, cr):
        ...
        additional_data = {
            key: {"id": 0, "state": "uninstalled", "dbdemo": False, "installed_version": None}
...
class Node(object):
    ...
    def __iter__(self):
        return itertools.chain(self.children, itertools.chain.from_iterable(self.children))
    ...
    def parents(self):
        ...
        return (
            node for node in self.graph.values() if node.depth < self.depth if self in node.children
        )
