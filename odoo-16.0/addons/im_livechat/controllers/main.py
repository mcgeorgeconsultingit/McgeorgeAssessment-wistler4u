...
class LivechatController(http.Controller):
    ...
    def livechat_init(self, channel_id):
        operator_available = len(
            request.env["im_livechat.channel"].sudo().browse(channel_id)._get_available_users()
        )
        rule = {}
        ...
        if country_code:
            country_id = (
                request.env["res.country"].sudo().search([("code", "=", country_code)], limit=1).id
            )
        # extract url
        url = request.httprequest.headers.get("Referer")
        # find the first matching rule for the given country and url
        matching_rule = (
            request.env["im_livechat.channel.rule"].sudo().match_rule(channel_id, url, country_id)
        )
        if matching_rule and (
            not matching_rule.chatbot_script_id or matching_rule.chatbot_script_id.script_step_ids
        ):
            rule = {
            ...
            if (
                ...
                and (
                    ...
                    or (not operator_available and matching_rule.chatbot_only_if_no_operator)
                )
    ...
    @http.route(
        "/im_livechat/operator/<int:operator_id>/avatar", type="http", auth="public", cors="*"
    )
        ...
        **kwargs
        ...
        if request.session.uid:
            ...
            if request.geoip:
                ...
                country_code = request.geoip.get("country_code", "")
                country = (
                    request.env["res.country"].sudo().search([("code", "=", country_code)], limit=1)
                    if country_code
                    else None
                )
                if country:
    ...
    def history_pages(self, pid, channel_uuid, page_history=None):
        ...
        channel = (
            ...
            .sudo()
            .search([("uuid", "=", channel_uuid), ("channel_partner_ids", "in", partner_ids)])
        )
        if channel:
