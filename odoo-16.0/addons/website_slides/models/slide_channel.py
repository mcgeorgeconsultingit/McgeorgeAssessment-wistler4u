...
class ChannelUsersRelation(models.Model):
    ...
    completed = fields.Boolean(
        "Is Completed", help="Channel validated, even if slides / lessons are added once done."
    )
    ...
    def _recompute_completion(self):
        ...
        for record in self:
            record.completed_slides_count = mapped_data.get(record.channel_id.id, dict()).get(
            ...
            if not record.channel_id.active:
                ...
                record.completed and record.completed_slides_count < record.channel_id.total_slides
            ):
        ...
        for record in self:
            ...
            email_values["body_html"] = template._render_encapsulate(
                ...
                add_context={
                    ...
                    .sudo()
                    .new(dict(body=email_values["body_html"], record_name=record.channel_id.name)),
                    "model_description": _(
                        "Completed Course"
...
class Channel(models.Model):
    ...
    rating_avg_stars = fields.Float(
        "Rating Average (Stars)", compute="_compute_rating_stats", digits=(16, 1), compute_sudo=True
    )
    # configuration
    ...
    has_requested_access = fields.Boolean(
        string="Access Requested", compute="_compute_has_requested_access", compute_sudo=False
    )
    ...
    karma_slide_comment = fields.Integer(
        "Add Comment", default=3, help="Karma needed to add a comment on a slide of this course"
    )
    ...
    @api.depends(
        "can_publish", "is_member", "karma_review", "karma_slide_comment", "karma_slide_vote"
    )
    @api.depends_context("uid")
    ...
    def action_channel_invite(self):
        ...
        template = self.env.ref(
            "website_slides.mail_template_slide_channel_invite", raise_if_not_found=False
        )
    ...
    def _action_add_members(self, target_partners, **member_values):
        ...
        if to_join:
            existing = (
                self.env["slide.channel.partner"]
                .sudo()
                .search([("channel_id", "in", self.ids), ("partner_id", "in", target_partners.ids)])
            )
    ...
    def action_view_slides(self):
        ...
        action["context"] = {"search_default_published": 1, "default_channel_id": self.id}
    ...
    def _move_category_slides(self, category, new_category):
        if not category.slide_ids:
            return
        truncated_slide_ids = [
            slide_id for slide_id in self.slide_ids.ids if slide_id not in category.slide_ids.ids
        ]
        if new_category:
    ...
    def _resequence_slides(self, slide, force_category=False):
        ...
        if self.slide_category_ids:
            ...
            index_of_category = (
                self.slide_category_ids.ids.index(force_category_id) if force_category_id else None
            )
