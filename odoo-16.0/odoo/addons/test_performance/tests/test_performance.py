...
class TestPerformance(SavepointCaseWithUserDemo):
    ...
    def test_write_base_one2many(self):
        ...
        with self.assertQueryCount(25):
            ...
            rec1.write(
                {
                    "line_ids": [
                        Command.update(line.id, {"value": 42 + line.id}) for line in lines[1:]
                    ]
                }
            )
        self.assertEqual(rec1.line_ids, lines)
    ...
    def test_write_base_one2many_with_constraint(self):
        ...
        rec.write(
            {"line_ids": [Command.clear()] + [Command.create({"value": val}) for val in range(6)]}
        )
    ...
    def test_several_prefetch(self):
        ...
        self.env.cr.execute(
            "delete from test_performance_base where id not in %s", (tuple(initial_records.ids),)
        )
...
class TestIrPropertyOptimizations(TransactionCase):
    ...
    def test_with_truthy_default(self):
        ...
        self.assertEqual(
            eggs, self.env["ir.property"]._get("property_eggs", "test_performance.bacon")
        )
...
class TestIncrementFieldsSkipLock(TransactionCase):
    ...
    def test_increment_fields_skiplock_one_field(self):
        ...
        with self.assertQueryCount(1):
            ...
            _logger.info(
                "increment_fields_skiplock did %supdate the field", "" if did_update else "not "
            )
        ...
        self.assertEqual(
            self.other_record.value_plus_one, 11, "other_record should not have been updated."
        )
    ...
    def test_increment_fields_skiplock_multiple_fields(self):
        ...
        with self.assertQueryCount(1):
            ...
            _logger.info(
                "increment_fields_skiplock did %supdate the fields", "" if did_update else "not "
            )
