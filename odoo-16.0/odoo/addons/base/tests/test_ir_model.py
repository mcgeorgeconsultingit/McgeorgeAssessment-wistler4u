...
class TestXMLID(TransactionCase):
    ...
    def test_create_xmlids(self):
        ...
        self.assertEqual(
            foo.partner_id, self.env.ref("test_convert.foo_res_partner", raise_if_not_found=False)
        )
        self.assertEqual(
            bar.partner_id, self.env.ref("test_convert.bar_res_partner", raise_if_not_found=False)
        )
