...
class ServerActions(models.Model):
    ...
    mail_post_autofollow = fields.Boolean(
        "Subscribe Recipients", compute="_compute_mail_post_autofollow", readonly=False, store=True
    )
    ...
    activity_date_deadline_range = fields.Integer(
        string="Due Date In", compute="_compute_activity_info", readonly=False, store=True
    )
    ...
    def _compute_template_id(self):
        to_reset = self.filtered(
            lambda act: act.state != "mail_post" or (act.model_id != act.template_id.model_id)
        )
        if to_reset:
    ...
    def _run_action_mail_post_multi(self, eval_context=None):
        ...
        if (
            ...
            or (not self._context.get("active_ids") and not self._context.get("active_id"))
