...
class TestEdiJson(TestEGEdiCommon):
                ...
                invoice_line_ids=[
                    {
                        ...
                        "tax_ids": [
                            (
                                6,
                                0,
                                self.env.ref(f"l10n_eg.{self.env.company.id}_eg_exempt_sale").ids,
                            )
                        ],
                    },
                    {
                        ...
                        "invoiceLines": [
                            {
                                ...
                                "discount": {"rate": 10.0, "amount": 12.09889},
                                "taxableItems": [
                                    {"taxType": "T1", "amount": 0.0, "subType": "V003", "rate": 0.0}
                                ],
                                "salesTotal": 120.98889,
                                "netTotal": 108.89,
                                "total": 108.89,
    ...
    def test_8_test_serialization_function(self):
        with freeze_time(self.frozen_today), patch(
            ...
            self.assertTrue(generated_files)
            json_file = json.loads(generated_files[0])
            serialized_string = self.env["l10n_eg_edi.thumb.drive"]._serialize_for_signing(
                json_file["request"]
            )
            self.assertEqual(
                serialized_string,
