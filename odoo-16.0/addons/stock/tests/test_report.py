...
class TestReports(TestReportsCommon):
    ...
    def test_report_quantity_1(self):
        ...
        self.env["stock.quant"].with_context(inventory_mode=True).create(
            {"product_id": product.id, "location_id": stock.id, "inventory_quantity": 50}
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_tomorrow if r["state"] == "forecast"]),
            30.0,
        )
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_tomorrow if r["state"] == "out"]), -20.0
        )
        report_records_today = self.env["report.stock.quantity"].read_group(
            [("product_id", "=", product.id), ("date", "=", date.today())],
            ["product_qty", "state"],
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_today if r["state"] == "forecast"]), 50.0
        )
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_tomorrow if r["state"] == "in"]), 10.0
        )
        report_records_today = self.env["report.stock.quantity"].read_group(
            [("product_id", "=", product.id), ("date", "=", date.today())],
            ["product_qty", "state"],
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_yesterday if r["state"] == "forecast"]),
            -30.0,
        )
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_yesterday if r["state"] == "out"]), -30.0
        )
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_yesterday if r["state"] == "in"]), 0.0
        )
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_today if r["state"] == "out"]), 0.0
        )
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_today if r["state"] == "in"]), 0.0
        )
    ...
    def test_report_quantity_2(self):
        ...
        self.env["stock.quant"].with_context(inventory_mode=True).create(
            {"product_id": product.id, "location_id": stock_without_wh.id, "inventory_quantity": 50}
        ).action_apply_inventory()
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records if r["state"] == "forecast"]), 40.0
        )
        report_records = self.env["report.stock.quantity"].read_group(
            [("product_id", "=", product.id), ("date", "=", date.today())],
            ["product_qty", "state"],
    ...
    def test_report_forecast_4_intermediate_transfers(self):
        ...
        self.assertEqual(
            lines[0]["document_in"].id, receipt.id, "The report must only show the receipt."
        )
    ...
    def test_report_forecast_11_non_reserved_order(self):
        ...
        all_delivery = (
            delivery_by_date | delivery_at_confirm | delivery_by_date_priority | delivery_manual
        )
        self.assertEqual(
            all_delivery.move_ids.mapped("forecast_availability"), [-3.0, -3.0, -3.0, -3.0]
        )
    ...
    def test_report_reception_1_one_receipt(self):
        ...
        self.assertEqual(
            len(sources_to_lines), 2, "The report has wrong number of outgoing pickings."
        )
        all_lines = []
        ...
        self.assertEqual(len(all_lines), 3, "The report has wrong number of outgoing moves.")
        self.assertEqual(
            all_lines[0]["quantity"], 5, "The first move has wrong incoming qty to reserve."
        )
        self.assertEqual(
            all_lines[0]["product"]["id"],
            self.product.id,
        ...
        self.assertEqual(
            all_lines[1]["quantity"], 5, "The second move has wrong incoming qty to reserve."
        )
        self.assertEqual(
            all_lines[1]["product"]["id"],
            product2.id,
        ...
        self.assertEqual(
            all_lines[2]["quantity"], 2, "The last move has wrong incoming qty to reserve."
        )
        self.assertEqual(
            all_lines[2]["product"]["id"],
            self.product.id,
    ...
    def test_report_reception_2_two_receipts(self):
        ...
        sources_to_lines = report_values["sources_to_lines"]
        self.assertEqual(
            len(sources_to_lines), 1, "The report has wrong number of outgoing pickings."
        )
        ...
        self.assertEqual(
            len(all_lines), 2, "The report has wrong number of lines (1 assignable + 1 not)."
        )
        self.assertEqual(
            all_lines[0]["quantity"], 5, "The first move has wrong incoming qty to assign."
        )
        self.assertTrue(
            all_lines[0]["is_qty_assignable"], "1 receipt is done => should have 1 reservable move."
        )
        self.assertEqual(
            all_lines[1]["quantity"], 3, "The second move has wrong (expected) incoming qty."
        )
        self.assertFalse(
    ...
    def test_report_reception_4_pick_pack(self):
        ...
        report_values = report._get_report_values(docids=[receipt.id])
        self.assertEqual(
            len(report_values["sources_to_lines"]), 1, "There should only be 1 line (pick move)"
        )
    ...
    def test_report_reception_5_move_splitting(self):
        ...
        self.assertEqual(
            mto_move.product_uom_qty, incoming_qty, "Incorrect quantity split for MTO move"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            non_mto_move.state, "assigned", "Fully reserved move has not correctly set state"
        )
        ...
        self.assertEqual(
            mto_move.product_uom_qty, incoming_qty, "Move quantities should be unchanged"
        )
        self.assertEqual(
            mto_move.procure_method, "make_to_stock", "Procure method not correctly reset"
        )
        self.assertEqual(
            mto_move.state, "confirmed", "Move state not correctly reset (to non-MTO state)"
        )
    ...
    def test_report_reception_6_backorders(self):
        ...
        self.assertEqual(
            len(reserved_move), 1, "Move w/reserved qty should have full demand reserved"
        )
        self.assertEqual(
            reserved_move.state, "assigned", "Move w/reserved qty should have full demand reserved"
        )
        self.assertEqual(
        ...
        all_lines = list(sources_to_lines.values())[0]
        self.assertEqual(
            len(all_lines), 1, "The report should only contain the remaining non-reserved move"
        )
        self.assertEqual(
            all_lines[0]["quantity"],
        ...
        for move in delivery.move_ids_without_package:
            self.assertEqual(
                move.state, "assigned", "All delivery moves should be fully reserved now"
            )
