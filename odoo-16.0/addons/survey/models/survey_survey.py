...
class Survey(models.Model):
    ...
    def _get_default_session_code(self):
        ...
        for digits_count in range(4, 10):
            ...
            code_candidates = set(
                [str(random.randint(range_lower_bound, range_upper_bound)) for i in range(20)]
            )
    ...
    description = fields.Html(
        "Description",
        translate=True,
        sanitize=False,  # TDE FIXME: find a way to authorize videos
    ...
    access_token = fields.Char(
        "Access Token", default=lambda self: self._get_default_access_token(), copy=False
    )
    ...
    def _compute_survey_statistic(self):
        ...
        for survey_stats in stat.values():
            ...
            survey_stats["success_ratio"] = (
                survey_stats["success_count"] / (survey_stats["answer_done_count"] or 1.0)
            ) * 100
    ...
    @api.depends(
        "user_input_ids.survey_id", "user_input_ids.start_datetime", "user_input_ids.end_datetime"
    )
    ...
    def _compute_is_attempts_limited(self):
        for survey in self:
            if (
                ...
                or any(question.is_conditional for question in survey.question_and_page_ids)
            ):
    ...
    def _compute_session_show_leaderboard(self):
        for survey in self:
            survey.session_show_leaderboard = survey.scoring_type != "no_scoring" and any(
    ...
    def copy(self, default=None):
        ...
        answers_map = {
            ...
            for source_answer, copy_answer in zip(
                self.question_ids.suggested_answer_ids, clone.question_ids.suggested_answer_ids
            )
        }
    ...
    def _check_answer_creation(
        self, user, partner, email, test_entry=False, check_attempts=True, invite_token=False
    ):
    ...
    def _get_number_of_attempts_lefts(self, partner, email, invite_token):
        ...
        domain = [("survey_id", "=", self.id), ("test_entry", "=", False), ("state", "=", "done")]
    ...
    def _is_first_page_or_question(self, page_or_question):
        ...
        ) or (not first_section_has_description and page_or_question == self.question_ids[0])
    ...
    def _prepare_leaderboard_values(self):
        ...
        leaderboard = self.env["survey.user_input"].search_read(
            [("survey_id", "=", self.id), ("create_date", ">=", self.session_start_time)],
            [
                "id",
                "nickname",
                "scoring_total",
        ...
        if (
            ...
            and any(answer.answer_score for answer in self.session_question_id.suggested_answer_ids)
        ):
            question_scores = {}
    ...
    def action_print_survey(self, answer=None):
        ...
        return {"type": "ir.actions.act_url", "name": "Print Survey", "target": "self", "url": url}
    ...
    def action_survey_user_input_completed(self):
        ...
        ctx.update({"search_default_survey_id": self.ids[0], "search_default_completed": 1})
        action["context"] = ctx
        return action
    ...
    def action_survey_user_input_certified(self):
        action = self.env["ir.actions.act_window"]._for_xml_id("survey.action_survey_user_input")
        ctx = dict(self.env.context)
        ctx.update({"search_default_survey_id": self.ids[0], "search_default_scoring_success": 1})
        action["context"] = ctx
        return action
    ...
    def _prepare_statistics(self, user_input_lines=None):
        ...
        count_data_success = (
            ...
            .sudo()
            ._read_group(
                user_input_domain, ["scoring_success", "id:count_distinct"], ["scoring_success"]
            )
        )
        completed_count = (
            self.env["survey.user_input"]
        ...
        success_graph = json.dumps(
            [
                {"text": _("Passed"), "count": scoring_success_count, "color": "#2E7D32"},
                {"text": _("Failed"), "count": scoring_failed_count, "color": "#C62828"},
            ]
        )
