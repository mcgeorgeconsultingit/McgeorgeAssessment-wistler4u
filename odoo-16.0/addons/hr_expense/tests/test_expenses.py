...
class TestExpenses(TestExpenseCommon):
    def test_expense_sheet_changing_employee(self):
        ...
        self.assertEqual(
            expense_sheet.employee_id, employee, "Employee should have changed on the sheet"
        )
    ...
    def test_expense_sheet_payment_state(self):
        ...
        _st_liquidity_lines, st_suspense_lines, _st_other_lines = statement_line.with_context(
            ...
            self.assertDictEqual(
                stub_line[0],
                {
                    ...
                    "amount_total": formatLang(
                        self.env, move.amount_total, currency_obj=self.env.company.currency_id
                    ),
                    "amount_residual": "-",
                    "amount_paid": formatLang(
                        self.env, payment.amount_total, currency_obj=self.env.company.currency_id
                    ),
