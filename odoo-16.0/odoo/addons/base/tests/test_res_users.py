...
class TestUsers(TransactionCase):
    ...
    def test_deactivate_portal_users_access(self):
        ...
        with self.assertRaises(
            UserError, msg="Internal users should not be able to deactivate their account"
        ):
    ...
    def test_deactivate_portal_users_archive_and_remove(self):
        ...
        self.assertTrue(
            portal_user.exists() and not portal_user.active, "Should have archived the user 1"
        )
...
class TestUsers2(TransactionCase):
    ...
    def test_read_group_with_reified_field(self):
        ...
        reified_fname = next(
            fname for fname in User.fields_get() if fname.startswith(("in_group_", "sel_groups_"))
        )
