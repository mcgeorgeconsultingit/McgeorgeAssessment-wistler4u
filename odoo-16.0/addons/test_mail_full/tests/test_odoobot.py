...
class TestOdoobot(TestMailCommon, TestRecipients):
    ...
    def test_odoobot_ping(self):
        ...
        with patch("random.choice", lambda x: x[0]):
            self.assertNextMessage(
                self.test_record_employe.with_context({"mail_post_autofollow": True}).message_post(
                    **kwargs
                ),
                sender=self.odoobot,
                answer=False,
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            channel.message_post(**kwargs), sender=self.odoobot, answer=("too human for me",)
        )
        kwargs["body"] = "Go fuck yourself"
        self.assertNextMessage(
            channel.message_post(**kwargs), sender=self.odoobot, answer=("I have feelings",)
        )
        kwargs["body"] = "help me"
        self.assertNextMessage(
            channel.message_post(**kwargs), sender=self.odoobot, answer=("If you need help",)
        )
    ...
    def test_odoobot_no_default_answer(self):
        ...
        self.assertNextMessage(self.test_record_employe.message_post(**kwargs), answer=False)
