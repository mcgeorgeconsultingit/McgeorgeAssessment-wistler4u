...
class TestEmailParsing(TestMailCommon):
    ...
    def test_message_parse_body(self):
        ...
        multipart = self.format(
            MAIL_TEMPLATE, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>'
        )
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        self.assertEqual(
            msg["bounced_email"], partner.email, "The sender email should be correctly parsed"
        )
        ...
        self.assertEqual(
            msg["bounced_message"], message, "An existing message with this message_id should exist"
        )
...
class TestMailAlias(TestMailCommon):
    ...
    def test_alias_creation(self):
        ...
        self.assertEqual(
            record.alias_id.alias_model_id, self.env["ir.model"]._get("mail.test.container")
        )
        self.assertEqual(record.alias_id.alias_force_thread_id, record.id)
        self.assertEqual(
            record.alias_id.alias_parent_model_id, self.env["ir.model"]._get("mail.test.container")
        )
        ...
        record.write(
            {"alias_name": "better.alias.test", "alias_defaults": "{'default_name': 'defaults'}"}
        )
    ...
    def test_alias_setup(self):
        ...
        self.assertEqual(
            alias.alias_name, "b4r+_-_r3wl-", "Disallowed chars should be replaced by hyphens"
        )
...
class TestMailgateway(TestMailCommon):
    ...
    def test_message_process_followers(self):
        ...
        with self.mock_mail_gateway():
            record2 = self.format_and_process(
                MAIL_TEMPLATE, self.email_from, "groups@test.com", subject="Another Email"
            )
    ...
    def test_message_process_email_email_from(self):
        ...
        self.assertFalse(
            record.message_ids[0].author_id, "message_process: unrecognized email -> no author_id"
        )
    ...
    def test_message_process_email_author(self):
        ...
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE, self.partner_1.email_formatted, "groups@test.com", subject="Test1"
            )
    ...
    def test_message_process_alias_config_bounced_content(self):
        ...
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE, self.email_from, "groups@test.com", subject="Should Bounce"
            )
    ...
    def test_message_process_create_uid_crash(self):
        ...
        with patch.object(
            MailTestGateway, "check_access_rights", autospec=True, side_effect=_employee_crash
        ):
            record = self.format_and_process(
                MAIL_TEMPLATE,
    ...
    def test_message_route_alias_no_domain(self):
        ...
        self.assertEqual(
            len(new_record), 1, "message_process: a new mail.test.simple should have been created"
        )
    ...
    def test_message_route_alias_forward_bypass_reply_first(self):
        ...
        new_rec = self.format_and_process(
            ...
            self.partner_1.email_formatted,
            "%s@%s, %s@%s"
            % (new_alias_2.alias_name, self.alias_domain, self.alias.alias_name, self.alias_domain),
            subject="Test Subject",
        ...
        self.assertEqual(
            len(new_simple), 0, "message_process: a new mail.test should not have been created"
        )
    ...
    def test_message_route_alias_forward_bypass_reply_second(self):
        ...
        new_rec = self.format_and_process(
            ...
            self.partner_1.email_formatted,
            "%s@%s, %s@%s"
            % (self.alias.alias_name, self.alias_domain, new_alias_2.alias_name, self.alias_domain),
            subject="Test Subject",
            extra="In-Reply-To:\r\n\t%s\n" % self.fake_email.message_id,
            target_model=new_alias_2.alias_model_id.model,
        )
    ...
    def test_message_route_alias_multiple_new(self):
        ...
        new_simple = self.env["mail.test.gateway"].search([("name", "=", "Test Subject")])
        self.assertEqual(
            len(new_simple), 1, "message_process: a new mail.test should have been created"
        )
    ...
    def test_message_route_bounce_other_recipients(self):
        ...
        with self.mock_mail_gateway():
            new_recs = self.format_and_process(
                ...
                self.partner_1.email_formatted,
                "%s@%s, %s@%s"
                % (self.alias.alias_name, self.alias_domain, self.alias_bounce, self.alias_domain),
                subject="Should bounce",
            )
        self.assertFalse(new_recs)
        self.assertNotSentEmail()
    ...
    def test_message_process_references_multi_parent(self):
        ...
        self.assertEqual(
            new_msg.parent_id, self.fake_email, "Mail: flattening attach to original message"
        )
        self.assertEqual(
            new_msg.subtype_id,
    ...
    def test_message_process_references_multi_parent_notflat(self):
        ...
        self.assertEqual(
            new_msg.subtype_id, self.env.ref("mail.mt_comment"), "Mail: parent should be a comment"
        )
        ...
        self.assertFalse(
            new_thread.parent_id, "Mail: pseudo no flattening: no parent means new thread"
        )
