...
class TestEventNotifications(TransactionCase):
    ...
    def test_attendee_added(self):
        ...
        self.assertEqual(
            self.event.attendee_ids.partner_id, self.partner, "It should be linked to the partner"
        )
        self.assertIn(
            self.partner,
    ...
    def test_attendee_added_create(self):
        ...
        self.assertEqual(
            event.attendee_ids.partner_id, self.partner, "It should be linked to the partner"
        )
        self.assertIn(
            self.partner,
    ...
    def test_existing_attendee_added(self):
        ...
        self.assertEqual(
            self.event.attendee_ids, attendee, "It should not have created an new attendee record"
        )
    ...
    def test_attendee_add_self(self):
        ...
        self.assertEqual(
            self.event.attendee_ids.state, "accepted", "It should be accepted for the current user"
        )
    ...
    def test_attendee_removed(self):
        ...
        self.event.partner_ids -= self.partner
        self.assertEqual(
            attendee, self.event.attendee_ids, "It should not have re-created an attendee record"
        )
        self.assertNotIn(
            self.partner, self.event.attendee_ids.partner_id, "It should have removed the attendee"
        )
        self.assertNotIn(
            self.partner,
            self.event.message_follower_ids.partner_id,
        ...
        self.assertIn(
            partner_bis, self.event.attendee_ids.partner_id, "It should have left the attendee"
        )
    ...
    def test_default_attendee(self):
        ...
        self.assertNotIn(
            partner_bis, event.attendee_ids.partner_id, "Partner bis should not be in attendee"
        )
