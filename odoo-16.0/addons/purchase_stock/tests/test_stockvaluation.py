...
class TestStockValuationWithCOA(AccountTestInvoicingCommon):
    ...
    def test_anglosaxon_valuation(self):
        ...
        self.assertEqual(
            len(stock_valuation_aml), 1, "For now, only one line for the stock valuation account"
        )
    ...
    def test_average_realtime_with_delivery_anglo_saxon_valuation_multicurrency_different_dates(
        ...
        (
            picking.move_ids.filtered(lambda l: l.purchase_line_id == line_product_avg).write(
                {"quantity_done": 1.0}
            )
        )
    ...
    def test_price_diff_with_partial_bills_and_delivered_qties(self):
        ...
        for qty, new_svl_expected, expected_valuations, expected_expenses in [
            ...
            new_expense_amls = self.env["account.move.line"].search(
                [("account_id", "=", expense_account.id), ("id", "not in", expense_amls.ids)]
            )
            new_input_amls = self.env["account.move.line"].search(
                [("account_id", "=", self.stock_input_account.id), ("id", "not in", input_amls.ids)]
            )
            ...
            self.assertEqual(
                new_input_amls.filtered(lambda aml: aml.debit > 0).debit, qty * 60, err_msg
            )
