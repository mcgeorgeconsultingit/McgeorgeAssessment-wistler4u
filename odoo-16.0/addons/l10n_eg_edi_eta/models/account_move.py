...
class AccountMove(models.Model):
    ...
    l10n_eg_submission_number = fields.Char(
        string="Submission ID", compute="_compute_eta_response_data", store=True, copy=False
    )
    l10n_eg_uuid = fields.Char(
        string="Document UUID", compute="_compute_eta_response_data", store=True, copy=False
    )
    ...
    def _compute_eta_qr_code_str(self):
        for move in self:
            if move.invoice_date and move.l10n_eg_uuid and move.l10n_eg_long_id:
                ...
                base_url = self.env["account.edi.format"]._l10n_eg_get_eta_api_domain(
                    production_enviroment=is_prod
                )
                qr_code_str = "%s/documents/search/%s/share/%s" % (
                    base_url,
                    move.l10n_eg_uuid,
            ...
            attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "description": _(
                        "Egyptian Tax authority JSON invoice generated for %s.", invoice.name
                    ),
                }
            )
    ...
    def _l10n_eg_edi_exchange_currency_rate(self):
        ...
        return (
            abs(self.invoice_line_ids[0].balance / self.invoice_line_ids[0].amount_currency)
