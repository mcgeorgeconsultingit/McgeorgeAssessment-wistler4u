...
class TestSurveyRandomize(TransactionCase):
    def test_01_generate_randomized_questions(self):
        ...
        self.assertEqual(
            len(generated_questions.filtered(lambda question: question.page_id == page_1)),
            3,
            msg="Expected 3 questions in page 1",
        )
        self.assertEqual(
            len(generated_questions.filtered(lambda question: question.page_id == page_2)),
            5,
            msg="Expected 5 questions in page 2",
        )
        self.assertEqual(
            len(generated_questions.filtered(lambda question: question.page_id == page_3)),
            2,
    ...
    def _add_questions(self, question_and_pages, page, count):
        for i in range(count):
            question_and_pages |= (
                ...
                .sudo()
                .create(
                    {"title": page.title + " Q" + str(i + 1), "sequence": page.sequence + (i + 1)}
                )
            )
