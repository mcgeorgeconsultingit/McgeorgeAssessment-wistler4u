...
try:
    ...
    def phone_format(
        number, country_code, country_phone_code, force_format="INTERNATIONAL", raise_exception=True
    ):
...
def phone_sanitize_numbers_w_record(
    numbers, record, country=False, record_country_fname="country_id", force_format="E164"
):
    if not isinstance(numbers, (list)):
        raise NotImplementedError()
    if not country:
