...
class ResCompany(models.Model):
    ...
    currency_exchange_journal_id = fields.Many2one(
        "account.journal", string="Exchange Gain or Loss Journal", domain=[("type", "=", "general")]
    )
    ...
    bank_journal_ids = fields.One2many(
        "account.journal", "company_id", domain=[("type", "=", "bank")], string="Bank Journals"
    )
    ...
    account_setup_bank_data_state = fields.Selection(
        ONBOARDING_STEP_STATES, string="State of the onboarding bank data step", default="not_done"
    )
    ...
    account_setup_taxes_state = fields.Selection(
        ONBOARDING_STEP_STATES, string="State of the onboarding Taxes step", default="not_done"
    )
    ...
    account_onboarding_sale_tax_state = fields.Selection(
        ONBOARDING_STEP_STATES, string="State of the onboarding sale tax step", default="not_done"
    )
    ...
    account_setup_bill_state = fields.Selection(
        ONBOARDING_STEP_STATES, string="State of the onboarding bill step", default="not_done"
    )
    ...
    def _compute_account_onboarding_create_invoice_state(self):
        for record in self:
            if record.account_onboarding_create_invoice_state_flag:
                ...
                [("company_id", "=", record.id), ("move_type", "=", "out_invoice")], limit=1
            ):
    ...
    def write(self, values):
        ...
        for company in self:
            if values.get("bank_account_code_prefix"):
                new_bank_code = (
                    values.get("bank_account_code_prefix") or company.bank_account_code_prefix
                )
                company.reflect_code_prefix_change(company.bank_account_code_prefix, new_bank_code)

            if values.get("cash_account_code_prefix"):
                new_cash_code = (
                    values.get("cash_account_code_prefix") or company.cash_account_code_prefix
                )
    ...
    def get_unaffected_earnings_account(self):
        ...
        account = self.env["account.account"].search(
            [("company_id", "=", self.id), ("account_type", "=", unaffected_earnings_type)]
        )
        if account:
            return account[0]
        ...
        for journal in journals:
            ...
            for move in moves:
                if move.inalterable_hash != move._compute_hash(previous_hash=previous_hash):
                    rslt.update(
                        {"msg_cover": _("Corrupted data on journal entry with id %s.", move.id)}
                    )
                    results_by_journal["results"].append(rslt)
                    hash_corrupted = True
                    break
                ...
                rslt.update(
                    {
                        ...
                        % (start_move_info[0], format_date(self.env, start_move_info[2]))
                    }
                )
            results_by_journal["results"].append(rslt)
