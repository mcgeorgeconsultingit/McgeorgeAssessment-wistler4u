...
class TestAccountDebitNote(AccountTestInvoicingCommon):
    ...
    def test_10_debit_note_in_refund(self):
        ...
        self.assertFalse(
            debit_note.invoice_line_ids, "We should not copy lines by default on debit notes"
        )
        self.assertEqual(
            debit_note.move_type, "in_invoice", "Type of debit note should not be refund anymore"
        )
        self.assertEqual(debit_note.state, "draft", "We should create debit notes in draft state")
