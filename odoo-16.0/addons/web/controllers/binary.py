...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if odoo.tools.config["x_sendfile"]:
            ...
            _logger.error(BAD_X_SENDFILE_ERROR.format(data_dir=odoo.tools.config["data_dir"]))
    ...
    def content_assets(self, id=None, filename=None, unique=False, extra=None, nocache=False):
        if not id:
            ...
            attachments = (
                request.env["ir.attachment"].sudo().search_read(domain, fields=["id"], limit=1)
            )
            if not attachments:
    ...
    def upload_attachment(self, model, id, ufile, callback=None):
        ...
        return (
            out % (json.dumps(clean(callback)), json.dumps(args)) if callback else json.dumps(args)
        )
    ...
    @http.route(
        ["/web/sign/get_fonts", "/web/sign/get_fonts/<string:fontname>"], type="json", auth="public"
    )
    def get_fonts(self, fontname=None):
        ...
        if fontname:
            ...
            font_filenames = sorted(
                [fn for fn in os.listdir(fonts_directory) if fn.endswith(supported_exts)]
            )
            for filename in font_filenames:
                font_file = file_open(
                    os.path.join(fonts_directory, filename), "rb", filter_ext=supported_exts
                )
                font = base64.b64encode(font_file.read())
                fonts.append(font)
        return fonts
