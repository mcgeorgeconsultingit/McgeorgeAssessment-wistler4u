...
class HrEmployeeBase(models.AbstractModel):
    ...
    is_absent = fields.Boolean(
        "Absent Today", compute="_compute_leave_status", search="_search_absent_employee"
    )
    ...
    def _compute_allocation_count(self):
        ...
        for employee in self:
            ...
            employee.allocation_count = (
                float_round(result["number_of_days"], precision_digits=2) if result else 0.0
            )
        ...
        for employee in self:
            ...
            for leave_type in leaves_taken[employee.id]:
                ...
                for allocation in leaves_taken[employee.id][leave_type]:
                    if allocation:
                        virtual_remaining_leaves = leaves_taken[employee.id][leave_type][
                            allocation
    ...
    def write(self, values):
        if "parent_id" in values:
            ...
            if manager:
                to_change = self.filtered(
                    lambda e: e.leave_manager_id == e.parent_id.user_id or not e.leave_manager_id
                )
        ...
        if "parent_id" in values or "department_id" in values:
            ...
            allocations = (
                ...
                .sudo()
                .search([("state", "in", ["draft", "confirm"]), ("employee_id", "in", self.ids)])
            )
        ...
        stress_days = self.env["hr.leave.stress.day"].search(
            [
                ...
                (
                    ...
                    "in",
                    (self.resource_calendar_id | self.env.company.resource_calendar_id).ids,
                ),
