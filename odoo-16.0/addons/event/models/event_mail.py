...
class EventTypeMail(models.Model):
    ...
    template_model_id = fields.Many2one(
        "ir.model", string="Template Model", compute="_compute_template_model_id", compute_sudo=True
    )
...
class EventMailScheduler(models.Model):
    ...
    @api.depends(
        "event_id.date_begin", "event_id.date_end", "interval_type", "interval_unit", "interval_nbr"
    )
    def _compute_scheduled_date(self):
        for scheduler in self:
            ...
            scheduler.scheduled_date = (
                date + _INTERVALS[scheduler.interval_unit](sign * scheduler.interval_nbr)
                if date
                else False
            )
                ...
                scheduler.update(
                    {
                        ...
                        >= (scheduler.event_id.seats_reserved + scheduler.event_id.seats_used),
    ...
    def _warn_template_error(self, scheduler, exception):
        ...
        if random.random() < 0.1666 or scheduler.interval_unit in ("now", "hours"):
            ...
            try:
                ...
                emails = list(
                    set([event.organizer_id.email, event.user_id.email, template.write_uid.email])
                )
