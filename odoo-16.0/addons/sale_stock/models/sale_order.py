...
class SaleOrder(models.Model):
    ...
    def _compute_expected_date(self):
        ...
        for order in self:
            ...
            if dates_list:
                expected_date = (
                    min(dates_list) if order.picking_policy == "direct" else max(dates_list)
                )
        ...
        if values.get("partner_shipping_id"):
            ...
            for record in self:
                ...
                picking.activity_schedule(
                    "mail.mail_activity_data_warning", note=message, user_id=self.env.user.id
                )
                ...
                for order_line in order.order_line:
                    ...
                    if (
                        float_compare(
                            ...
                            precision_rounding=order_line.product_uom.rounding or rounding,
                        )
                        < 0
                    ):
                        to_log[order_line] = (
