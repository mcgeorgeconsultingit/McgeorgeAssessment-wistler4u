...
class Website(models.Model):
    ...
    salesteam_id = fields.Many2one(
        "crm.team", string="Sales Team", ondelete="set null", default=_default_salesteam_id
    )
    ...
    shop_default_sort = fields.Selection(
        selection="_get_product_sort_mapping", default="website_sequence asc", required=True
    )
    ...
    prevent_zero_price_sale_text = fields.Char(
        string="Text to show instead of price", translate=True, default="Not Available For Sale"
    )
    ...
    def get_current_pricelist(self):
        ...
        if request and request.session.get("website_sale_current_pl"):
            ...
            pricelist = (
                ProductPricelist.browse(request.session["website_sale_current_pl"]).exists().sudo()
            )
            country_code = self._get_geoip_country_code()
            if (
                not pricelist
                ...
                if sale_order_sudo.pricelist_id not in available_pricelists:
                    ...
                    fpos = (
                        ...
                        ._get_fiscal_position(
                            sale_order_sudo.partner_id, delivery=sale_order_sudo.partner_shipping_id
                        )
                    )
    ...
    def _prepare_sale_order_values(self, partner_sudo):
        ...
        if not salesperson_user_sudo:
            salesperson_user_sudo = (
                self.salesperson_id or partner_sudo.parent_id.user_id or partner_sudo.user_id
            )
    ...
    def _get_current_pricelist_id(self, partner_sudo):
        return self.get_current_pricelist().id or partner_sudo.property_product_pricelist.id
    ...
    def _get_current_fiscal_position_id(self, partner_sudo):
        ...
        if request and request.website.partner_id.id == partner_sudo.id:
            ...
            if country_code:
                country_id = (
                    self.env["res.country"].search([("code", "=", country_code)], limit=1).id
                )
    ...
    def _send_abandoned_cart_email(self):
        for website in self.search([]):
            ...
            for sale_order in abandoned_carts:
                ...
                template.send_mail(
                    sale_order.id, email_values=dict(email_to=sale_order.partner_id.email)
                )
