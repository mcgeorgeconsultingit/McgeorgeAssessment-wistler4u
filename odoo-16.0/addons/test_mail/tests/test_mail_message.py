...
class TestMessageValues(TestMailCommon):
    ...
    def test_mail_message_values_fromto_long_name(self):
        ...
        self.assertEqual(
            msg.reply_to, reply_to_email, "Reply-To: use only email when formataddr > 78 chars"
        )
        ...
        sanitized_alias_name = (
            "waaaay-too-long-alias-name-that-should-make-any-reply-to-blow-the-78-characters-limit"
        )
        msg = self.env["mail.message"].create(
            {"model": test_record._name, "res_id": test_record.id}
        )
    ...
    def test_mail_message_values_fromto_no_document(self):
        ...
        self.assertEqual(
            msg.email_from, formataddr((self.user_employee.name, self.user_employee.email))
        )
        ...
        self.assertEqual(
            msg.reply_to, formataddr((self.user_employee.name, self.user_employee.email))
        )
        self.assertEqual(
            msg.email_from, formataddr((self.user_employee.name, self.user_employee.email))
        )
    ...
    def test_mail_message_values_fromto_document_alias(self):
        msg = self.Message.create({"model": "mail.test.container", "res_id": self.alias_record.id})
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        msg = self.Message.create({"model": "mail.test.simple", "res_id": test_record.id})
...
class TestMessageAccess(TestMailCommon):
    ...
    def test_mail_message_access_search(self):
        ...
        msg7 = (
            self.env["mail.message"]
            .with_user(self.user_employee)
            .create({"subject": "_ZTest", "body": "B", "subtype_id": self.ref("mail.mt_comment")})
        )
        msg8 = (
            self.env["mail.message"]
            .with_user(self.user_employee)
    ...
    def test_mail_message_access_read_ok_portal(self):
        self.message.write(
            {"subtype_id": self.ref("mail.mt_comment"), "res_id": self.public_channel.id}
        )
        self.message.with_user(self.user_portal).read(["body", "message_type", "subtype_id"])
    ...
    def test_mail_message_access_create_crash(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.message"].with_user(self.user_employee).create(
                {"model": "mail.channel", "res_id": self.private_group.id, "body": "Test"}
            )
