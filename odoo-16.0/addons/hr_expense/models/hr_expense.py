...
class HrExpense(models.Model):
    ...
    product_has_tax = fields.Boolean(
        "Whether tax is defined on a selected product", compute="_compute_product_has_cost"
    )
    same_currency = fields.Boolean(
        "Is currency_id different from the company_currency_id", compute="_compute_same_currency"
    )
    ...
    def _compute_same_currency(self):
        for expense in self:
            expense.same_currency = bool(
                ...
                or (expense.currency_id and expense.currency_id == expense.company_currency_id)
            )
    ...
    def _compute_amount_tax(self):
        ...
        for expense in self:
            ...
            expense.amount_tax = (
                taxes["total_included"] - taxes["total_excluded"] if expense.tax_ids else 0.0
            )
    ...
    def _compute_amount_residual(self):
        for expense in self:
            ...
            payment_term_lines = expense.sheet_id.account_move_id.sudo().line_ids.filtered(
    ...
    def _compute_product_description(self):
        for expense in self:
            expense.product_description = (
                not is_html_empty(expense.product_id.description) and expense.product_id.description
            )
    ...
    def _compute_unit_amount_display(self):
        for expense in self:
            expense.unit_amount_display = (
                expense.unit_amount if expense.product_has_cost else expense.total_amount_company
            )
    ...
    def _check_product_uom_category(self):
        for expense in self:
            if (
                expense.product_id
                and expense.product_uom_id.category_id != expense.product_id.uom_id.category_id
            ):
                raise UserError(
                    _(
    ...
    def create_expense_from_attachments(self, attachment_ids=None, view_type="list"):
        ...
        if any(
            attachment.res_id or attachment.res_model != "hr.expense" for attachment in attachments
        ):
    ...
    def write(self, vals):
        ...
        if "reference" in vals:
            if any(not expense.is_ref_editable for expense in self):
                raise UserError(
                    _("You are not authorized to edit the reference of this expense report.")
                )
    ...
    def _get_empty_list_mail_alias(self):
        use_mailgateway = (
            self.env["ir.config_parameter"].sudo().get_param("hr_expense.use_mailgateway")
        )
...
Or send your receipts at <a href="mailto:%(email)s?subject=Lunch%%20with%%20customer%%3A%%20%%2412.32">%(email)s</a>.
    ...
    def _get_default_expense_sheet_values(self):
        ...
        for todo in sheets:
            if len(todo) == 1:
                ...
                expense_name = (
                    min_date if max_date == min_date else "%s - %s" % (min_date, max_date)
                )
    ...
    def _get_expense_account_destination(self):
        ...
        if self.payment_mode == "company_account":
            ...
            account_dest = (
                partner.property_account_payable_id or partner.parent_id.property_account_payable_id
            )
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        vals = {
            ...
            "tax_ids": [
                (4, tax.id, False)
                for tax in product.supplier_taxes_id.filtered(lambda r: r.company_id == company)
            ],
            "quantity": 1,
...
class HrExpenseSheet(models.Model):
    ...
    payment_mode = fields.Selection(
        related="expense_line_ids.payment_mode", readonly=True, string="Paid By", tracking=True
    )
    ...
    untaxed_amount = fields.Monetary(
        "Untaxed Amount", currency_field="currency_id", compute="_compute_amount", store=True
    )
    ...
    is_multiple_currency = fields.Boolean(
        "Handle lines with different currencies", compute="_compute_is_multiple_currency"
    )
    ...
    def _compute_can_reset(self):
        ...
        for sheet in self:
            sheet.can_reset = (
                is_expense_user if is_expense_user else sheet.employee_id.user_id == self.env.user
            )
    ...
    def _compute_from_employee_id(self):
        for sheet in self:
            ...
            sheet.user_id = (
                sheet.employee_id.expense_manager_id or sheet.employee_id.parent_id.user_id
            )
    ...
    def _check_expense_lines_company(self):
        for sheet in self:
            if any(expense.company_id != sheet.company_id for expense in sheet.expense_line_ids):
                raise ValidationError(
                    _("An expense report must contain only lines from the same company.")
                )
    ...
    def create(self, vals_list):
        ...
        context.update({"mail_create_nosubscribe": True, "mail_auto_subscribe_no_notify": True})
    ...
    def _get_mail_thread_data_attachments(self):
        ...
        expense_attachments = self.env["ir.attachment"].search(
            [("res_id", "in", expense_ids.ids), ("res_model", "=", "hr.expense")], order="id desc"
        )
