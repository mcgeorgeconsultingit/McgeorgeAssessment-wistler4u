...
def _exec_pipe(prog, args, env=None):
    ...
    pop = subprocess.Popen(
        cmd, bufsize=-1, stdin=subprocess.PIPE, stdout=subprocess.PIPE, close_fds=close_fds, env=env
    )
...
def file_path(file_path, filter_ext=("",)):
    ...
    for addons_dir in addons_paths:
        ...
        fpath = (
            normalized_path
            if is_abs
            else os.path.normpath(os.path.normcase(os.path.join(parent_path, normalized_path)))
        )
...
def detect_ip_addr():
    ...
    def _detect_ip_addr():
        ...
        if not fcntl:  # not UNIX:
            ...
            # print 'names: ', names
            outbytes = unpack(
                "iL", fcntl.ioctl(s.fileno(), 0x8912, pack("iL", nbytes, names.buffer_info()[0]))
            )[0]
...
def attrgetter(*items):
    warnings.warn(
        "Since 16.0, super old backport of Python 2.6's `operator.attrgetter`.", stacklevel=2
    )
    if len(items) == 1:
        attr = items[0]
...
class lower_logging(logging.Handler):
    ...
    def emit(self, record):
        if record.levelno > self.max_level:
            ...
            record.args = tuple(
                arg.replace(
                    "Traceback (most recent call last):", "_Traceback_ (most recent call last):"
                )
                if type(arg) is str
                else arg
