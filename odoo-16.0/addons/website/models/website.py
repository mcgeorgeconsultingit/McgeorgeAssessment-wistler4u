...
class Website(models.Model):
    ...
    robots_txt = fields.Html(
        "Robots.txt", translate=False, groups="website.group_website_designer", sanitize=False
    )
    ...
    def configurator_apply(self, **kwargs):
        ...
        def configure_page(page_code, snippet_list, pages_views, cta_data):
            ...
            for i, snippet in enumerate(snippet_list, start=1):
                try:
                    ...
                    logger.warning(e)
            page_view_id.save(
                value="".join(rendered_snippets), xpath="(//div[hasclass('oe_structure')])[last()]"
            )
        ...
        if cta_data["cta_btn_text"]:
            ...
            parent_view = (
                self.env["website"].with_context(website_id=website.id).viewref(xpath_view)
            )
    ...
    def _get_plausible_script_url(self):
        return (
            ...
            .sudo()
            .get_param("website.plausible_script", "https://plausible.io/js/plausible.js")
        )
    ...
    def _get_plausible_share_url(self):
        embed_url = (
            f"/share/{self.plausible_site}?auth={self.plausible_shared_key}&embed=true&theme=system"
        )
        return (
    ...
    def viewref(self, view_id, raise_if_not_found=True):
        ...
        if not view and raise_if_not_found:
            raise ValueError(
                "No record found for unique ID %s. It may have been deleted." % (view_id)
            )
        return view
    ...
    def _is_snippet_used(
        self, snippet_module, snippet_id, asset_version, asset_type, html_fields_attributes
    ):
        snippet_occurences = []
    ...
    def _disable_unused_snippets_assets(self):
        ...
        for snippet_module, snippet_id, asset_version, asset_type, _asset_id in snippets_assets:
            is_snippet_used = self._is_snippet_used(
                snippet_module, snippet_id, asset_version, asset_type, html_fields_attributes
            )
    ...
    def _trigram_enumerate_words(self, search_details, search, limit):
        ...
        for search_detail in search_details:
            ...
            for field in fields:
                ...
                table = (
                    inherits_fields[field]["table"] if field in inherits_fields else model._table
                )
                similarities.append(
