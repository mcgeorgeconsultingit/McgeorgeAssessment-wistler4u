...
class MrpWorkorder(models.Model):
    ...
    product_id = fields.Many2one(
        related="production_id.product_id", readonly=True, store=True, check_company=True
    )
    product_tracking = fields.Selection(related="product_id.tracking")
    product_uom_id = fields.Many2one("uom.uom", "Unit of Measure", required=True, readonly=True)
    production_id = fields.Many2one(
        "mrp.production", "Manufacturing Order", required=True, check_company=True, readonly=True
    )
    ...
    qty_production = fields.Float(
        "Original Production Quantity", readonly=True, related="production_id.product_qty"
    )
    ...
    is_produced = fields.Boolean(string="Has Been Produced", compute="_compute_is_produced")
    ...
    operation_id = fields.Many2one("mrp.routing.workcenter", "Operation", check_company=True)
    ...
    time_ids = fields.One2many("mrp.workcenter.productivity", "workorder_id", copy=False)
    ...
    working_user_ids = fields.One2many(
        "res.users", string="Working user on this work order.", compute="_compute_working_users"
    )
    ...
    @api.depends(
        "production_availability", "blocked_by_workorder_ids", "blocked_by_workorder_ids.state"
    )
    def _compute_state(self):
        ...
        for workorder in self:
            if workorder.state == "pending":
                if all(
                    [wo.state in ("done", "cancel") for wo in workorder.blocked_by_workorder_ids]
                ):
                    workorder.state = (
                        "ready"
    ...
    def name_get(self):
        ...
        for wo in self:
            if len(wo.production_id.workorder_ids) == 1:
                res.append(
                    (wo.id, "%s - %s - %s" % (wo.production_id.name, wo.product_id.name, wo.name))
                )
            else:
                res.append(
                    (
    ...
    def _compute_is_produced(self):
        ...
        for order in self.filtered(lambda p: p.production_id and p.production_id.product_uom_id):
            ...
            order.is_produced = (
                float_compare(
                    order.qty_produced, order.production_id.product_qty, precision_rounding=rounding
                )
                >= 0
            )
        ...
        for order in self:
            ...
            if order.duration_expected:
                order.duration_percent = (
                    100 * (order.duration_expected - order.duration) / order.duration_expected
                )
            else:
            ...
            for workorder in self:
                if workorder.workcenter_id.id != values["workcenter_id"]:
                    ...
                    workorder.leave_id.resource_id = (
                        self.env["mrp.workcenter"].browse(values["workcenter_id"]).resource_id
                    )
            ...
            for workorder in self:
                ...
                if "duration_expected" not in values and not self.env.context.get(
                    ...
                    if values.get("date_planned_start") and values.get("date_planned_finished"):
                        computed_finished_time = workorder._calculate_date_planned_finished(
                            start_date
                        )
    ...
    def _plan_workorder(self, replan=False):
        ...
        if best_finished_date == datetime.max:
            raise UserError(
                _("Impossible to plan the workorder. Please check the workcenter availabilities.")
            )
    ...
    def button_start(self):
        ...
        if any(
            not time.date_end
            for time in self.time_ids.filtered(lambda t: t.user_id.id == self.env.user.id)
        ):
            return True
    ...
    def _get_duration_expected(self, alternative_workcenter=False, ratio=1):
        ...
        if not self.operation_id:
            ...
            return (
                ...
                + duration_expected_working * ratio * 100.0 / self.workcenter_id.time_efficiency
            )
    ...
    def _update_finished_move(self):
        ...
        if production_move.product_id.tracking != "none":
            ...
            if move_line:
                ...
                putaway_location = production_move.location_dest_id._get_putaway_strategy(
