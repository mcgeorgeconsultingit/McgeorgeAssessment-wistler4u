...
class TestSalePurchase(TestCommonSalePurchaseNoChart):
    ...
    def test_sale_create_purchase(self):
        ...
        purchase_order = self.env["purchase.order"].search(
            [("partner_id", "=", self.supplierinfo1.partner_id.id), ("state", "=", "draft")]
        )
        ...
        self.assertEqual(
            len(purchase_order), 1, "Only one PO should have been created, from the 2 Sales orders"
        )
        self.assertEqual(
            len(purchase_order.order_line), 2, "The purchase order should have 2 lines"
        )
        self.assertEqual(
            len(purchase_lines_so1), 1, "Only one SO line from SO 1 should have create a PO line"
        )
        self.assertEqual(
            len(purchase_lines_so2), 1, "Only one SO line from SO 2 should have create a PO line"
        )
        self.assertEqual(
            len(purchase_order.activity_ids), 0, "No activity should be scheduled on the PO"
        )
    ...
    def test_reconfirm_sale_order(self):
        ...
        self.assertEqual(
            len(purchase_lines), 1, "Only one purchase line should be created on SO confirmation"
        )
        self.assertEqual(
            len(purchase_order), 1, "One purchase order should have been created on SO confirmation"
        )
        self.assertEqual(
            len(purchase_order.order_line), 1, "Only one line on PO, after SO confirmation"
        )
        self.assertEqual(
            purchase_order,
        ...
        self.assertEqual(
            len(purchase_lines), 1, "Always one purchase line even after SO cancellation"
        )
        self.assertTrue(purchase_order, "Always one purchase order even after SO cancellation")
        self.assertEqual(
            len(purchase_order.order_line), 1, "Still one line on PO, even after SO cancellation"
        )
        self.assertEqual(
            purchase_order,
            purchase_lines.order_id,
        ...
        self.assertEqual(
            purchase_order.state, "draft", "Generated purchase should still be in draft state"
        )
        self.assertEqual(
            purchase_line.price_unit,
            self.supplierinfo1.price,
    ...
    def test_update_ordered_sale_quantity(self):
        ...
        self.assertEqual(
            purchase_order.state, "draft", "The created purchase should be in draft state"
        )
        ...
        self.assertEqual(
            len(purchase_order.activity_ids), 1, "One activity should have been created on the PO"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            purchase_order.activity_ids.state, "today", "Activity is for today, as it is urgent"
        )
    ...
    def test_pol_description(self):
        ...
        po = self.env["purchase.order"].search(
            [("partner_id", "=", self.partner_vendor_service.id)], order="id desc", limit=1
        )
