...
class TestProcurement(TestMrpCommon):
    def test_procurement(self):
        ...
        # produce product
        self.assertEqual(
            produce_product_4.reservation_state, "confirmed", "Consume material not available"
        )
        ...
        self.assertEqual(
            produce_product_4.product_uom_id, self.uom_unit, "Wrong quantity of finish product."
        )
        self.assertEqual(
            produce_product_4.reservation_state, "assigned", "Consume material not available"
        )
        ...
        self.assertEqual(
            production_product_6.reservation_state, "assigned", "Consume material not available"
        )
        ...
        self.assertEqual(
            production_product_6.state, "done", "Production order should be in state done"
        )
        self.assertEqual(
    ...
    def test_link_date_mo_moves(self):
        ...
        mo_form = Form(mo)
        self.assertEqual(
            mo_form.product_qty, 10, "the quantity to produce is not good relative to the move"
        )

        mo = mo_form.save()

        # Confirming mo create finished move
        move_orig = self.env["stock.move"].search([("move_dest_ids", "in", move_dest.ids)], limit=1)

        self.assertEqual(len(move_orig), 1, "the move orig is not created")
        self.assertEqual(
            move_orig.product_qty, 10, "the quantity to produce is not good relative to the move"
        )
        ...
        self.assertAlmostEqual(
            mo.move_finished_ids.date, mo.date_planned_finished, delta=timedelta(seconds=1)
        )
    ...
    def test_auto_assign(self):
        ...
        mo.is_locked = False
        self.assertEqual(
            mo.move_raw_ids.reserved_availability, 0, "No components should be reserved yet"
        )
        self.assertEqual(
            mo.product_qty,
            15,
        ...
        self.assertEqual(
            mo.move_raw_ids.reserved_availability, 15, "Components should have been auto-reserved"
        )
        ...
        self.assertEqual(
            mo3.product_qty, 6, "Quantity to produce should be 1 + reordering rule max qty"
        )
    ...
    def test_check_update_qty_mto_chain(self):
        ...
        self.assertEqual(
            len(manufacturing_orders), 2, "A new MO should have been created for missing demand."
        )
    ...
    def test_rr_with_dependance_between_bom(self):
        ...
        route_manufacture = self.warehouse.manufacture_pull_id.route_id
        product_1 = self.env["product.product"].create(
            {"name": "Product A", "type": "product", "route_ids": [(6, 0, [route_manufacture.id])]}
        )
        product_2 = self.env["product.product"].create(
            {
                "name": "Product B",
        ...
        product_3 = self.env["product.product"].create(
            {"name": "Product B", "type": "product", "route_ids": [(6, 0, [route_manufacture.id])]}
        )
        product_4 = self.env["product.product"].create(
            {
                "name": "Product C",
