...
def _create_empty_database(name):
    ...
    with closing(db.cursor()) as cr:
        ...
        cr.execute(
            "SELECT datname FROM pg_database WHERE datname = %s", (name,), log_exceptions=False
        )
        if cr.fetchall():
...
def exp_create_database(
    db_name, demo, lang, user_password="admin", login="admin", country_code=None, phone=None
):
...
def dump_db(db_name, stream, backup_format="zip"):
    ...
    if backup_format == "zip":
        with tempfile.TemporaryDirectory() as dump_dir:
            ...
            subprocess.run(
                cmd, env=env, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT, check=True
            )
            if stream:
...
def exp_list_countries():
    list_countries = []
    root = ET.parse(
        os.path.join(odoo.tools.config["root_path"], "addons/base/data/res_country_data.xml")
    ).getroot()
