...
class TestMrpOrder(TestMrpCommon):
    ...
    def test_basic(self):
        ...
        man_order.action_confirm()
        self.assertEqual(
            man_order.state, "confirmed", "Production order should be in confirmed state."
        )
        ...
        self.assertEqual(
            ...
            test_quantity / self.bom_1.product_qty * self.product_4.uom_id.factor_inv * 2,
        )
        first_move = man_order.move_raw_ids.filtered(lambda move: move.product_id == self.product_1)
        self.assertEqual(
            first_move.product_qty,
            test_quantity / self.bom_1.product_qty * self.product_4.uom_id.factor_inv * 4,
        )
        ...
        self.assertEqual(
            mo_copy.move_finished_ids.product_uom_qty, 2, "Incorrect qty of products to produce"
        )
        ...
        self.assertEqual(
            mo_copy_2.move_finished_ids.product_uom_qty, 2, "Incorrect qty of products to produce"
        )
    ...
    def test_update_quantity_2(self):
        ...
        self.assertEqual(
            sum(mo.move_raw_ids.filtered(lambda m: m.product_id == p1).mapped("quantity_done")), 20
        )
    ...
    def test_rounding(self):
        ...
        self.assertEqual(
            production.move_raw_ids[0].product_qty, 41, "The quantity should be rounded up"
        )
        self.assertEqual(
            production.move_raw_ids[1].product_qty, 84, "The quantity should be rounded up"
        )
    ...
    def test_product_produce_1(self):
        ...
        self.assertEqual(
            mo_form.move_raw_ids._records[0]["product_uom_qty"], 5, "Wrong quantity to consume"
        )
        self.assertEqual(
            mo_form.move_raw_ids._records[0]["quantity_done"], 3, "Wrong quantity done"
        )
        self.assertEqual(
            mo_form.move_raw_ids._records[1]["product_uom_qty"], 20, "Wrong quantity to consume"
        )
        self.assertEqual(
    ...
    def test_product_produce_3(self):
        ...
        ml_to_shelf_1 = move_1.move_line_ids.filtered(
            lambda ml: ml.lot_id == first_lot_for_p1 and ml.location_id == self.stock_shelf_1
        )
        ml_to_shelf_2 = move_1.move_line_ids.filtered(
            lambda ml: ml.lot_id == first_lot_for_p1 and ml.location_id == self.stock_shelf_2
        )
    ...
    def test_product_produce_4(self):
        ...
        self.assertEqual(len(ml_p1), 2)
        self.assertEqual(
            sorted(ml_p1.mapped("qty_done")), [2.0, 3.0], "Quantity done should be 1.0, 2.0 or 3.0"
        )
        self.assertEqual(m_p1.quantity_done, 5.0, "Total qty done should be 6.0")
        self.assertEqual(
            sum(ml_p1.mapped("reserved_uom_qty")), 5.0, "Total qty reserved should be 5.0"
        )
    ...
    def test_product_produce_10(self):
        ...
        self.assertEqual(
            move_lines_byproduct_1.filtered(lambda ml: ml.lot_id == self.serial_1).qty_done, 1.0
        )
        self.assertEqual(
            move_lines_byproduct_1.filtered(lambda ml: ml.lot_id == self.serial_2).qty_done, 1.0
        )
        self.assertEqual(
            move_lines_byproduct_2.filtered(lambda ml: ml.lot_id == self.lot_1).qty_done, 2.0
        )
        self.assertEqual(
            move_lines_byproduct_2.filtered(lambda ml: ml.lot_id == self.lot_2).qty_done, 2.0
        )
    ...
    def test_product_produce_11(self):
        ...
        mo.bom_id.consumption = (
            "flexible"  # Because we'll over-consume with a product not defined in the BOM
        )
        mo.action_assign()
        mo.is_locked = False
    ...
    def test_product_produce_uom(self):
        ...
        self.assertEqual(
            move_line_raw.product_uom_id, unit, "Should be 1 unit since the tracking is serial."
        )
        ...
        bom = self.env["mrp.bom"].create(
            {
                ...
                "consumption": "flexible",
                "type": "normal",
                "bom_line_ids": [(0, 0, {"product_id": self.product_2.id, "product_qty": 1})],
                "operation_ids": [
                    (
                        0,
                        0,
    ...
    def test_products_with_variants(self):
        ...
        product = self.env["product.template"].create(
            {
                "attribute_line_ids": [[0, 0, {"attribute_id": 2, "value_ids": [[6, 0, [3, 4]]]}]],
        ...
        self.assertEqual(
            len(move_finished_ids_1), 1, "There should only be 1 produced product for this MO"
        )
        ...
        self.assertEqual(
            len(move_finished_ids_2), 1, "There should only be 1 produced product for this MO"
        )
        ...
        self.assertEqual(
            len(move_finished_ids_3), 1, "There should only be 1 produced product for this MO"
        )
    ...
    def test_move_finished_onchanges(self):
        ...
        mo = mo_form.save()
        self.assertEqual(
            len(mo.move_finished_ids), 1, "Wrong number of finished product moves created"
        )
        self.assertEqual(
        ...
        mo2 = mo_form.save()
        self.assertEqual(
            len(mo2.move_finished_ids), 1, "Wrong number of finished product moves created"
        )
        self.assertEqual(
        ...
        mo2 = mo_form.save()
        self.assertEqual(
            len(mo2.move_finished_ids), 1, "Wrong number of finish product moves created"
        )
        self.assertEqual(
            mo2.move_finished_ids.product_qty,
            5,
        ...
        mo3 = mo_form.save()
        self.assertEqual(
            len(mo3.move_finished_ids), 1, "Wrong number of finish product moves created"
        )
        self.assertEqual(
        ...
        bom2 = self.env["mrp.bom"].create(
            {
                "product_id": False,
                "product_tmpl_id": product1.product_tmpl_id.id,
                "bom_line_ids": [(0, 0, {"product_id": component.id, "product_qty": 10})],
            }
        )
        ...
        self.assertEqual(
            len(mo4.move_finished_ids), 1, "Wrong number of finished product moves created"
        )
        self.assertEqual(
        ...
        mo4 = mo_form.save()
        self.assertEqual(
            len(mo4.move_finished_ids), 1, "Wrong number of finish product moves created"
        )
        self.assertEqual(
            mo4.move_finished_ids.product_id,
            product1,
    ...
    def test_compute_tracked_time_1(self):
        ...
        production = production_form.save()
        self.assertEqual(
            production.workorder_ids[0].duration_expected, 60.0, "Default duration is 0+0+1*60.0"
        )
        production.action_confirm()
        ...
        production = production_form.save()
        self.assertEqual(
            production.workorder_ids[0].duration_expected, 15.0, "Duration is now 0+0+1*15"
        )
        production.action_confirm()
        production.button_plan()
        # Production planned, time to start, I produce all the 1 product
        ...
        self.assertEqual(
            production.workorder_ids[0].duration_expected, 10.0, "Duration is now 0+0+1*10"
        )
    ...
    def test_compute_tracked_time_2_under_capacity(self):
        ...
        self.assertEqual(
            production.workorder_ids[0].duration_expected, 10.0, "Duration is indeed (10+10)/2"
        )
    ...
    def test_workorder_set_duration(self):
        ...
        self.assertEqual(
            len(workorder.time_ids), 3, "Two more time tracking values should have been created"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(workorder.time_ids), 2, "One time tracking values should have been deleted"
        )
        self.assertEqual(
            workorder.time_ids[1].loss_type,
            "productive",
    ...
    def test_source_and_child_mo(self):
        ...
        for steps, case_description, in [
            ...
            for source_mo, mo, product, child_mo in [
                (none_production, grandparent_production, grandparent, parent_production),
