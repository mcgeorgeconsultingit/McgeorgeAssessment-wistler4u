...
class TestAr(AccountTestInvoicingCommon):
    ...
    def _create_test_invoices_like_demo(self, use_current_date=True):
        ...
        for key, values in invoices_to_create.items():
            with Form(
                self.env["account.move"].with_context(default_move_type=values["move_type"])
            ) as invoice_form:
        ...
        res = (
            ...
            .search(
                [
                    ...
                    ("company_id", "=", self.env.company.id),
                    ("tax_group_id", "=", self.env.ref("l10n_ar.tax_group_" + tax_type).id),
                ],
                limit=1,
            )
        )
    ...
    def _set_today_rate(self, currency, value):
        ...
        if rate:
            ...
            rate_obj.create(
                {"company_id": self.env.company.id, "currency_id": currency.id, "rate": value}
            )
