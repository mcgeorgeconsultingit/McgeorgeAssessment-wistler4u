...
class SaleOrder(models.Model):
    ...
    def _name_search(self, name="", args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        if (
            ...
            domain = expression.AND(
                [
                    args or [],
                    ["&", ("state", "=", "sale"), ("company_id", "in", self.env.companies.ids)],
                ]
            )
            return super(SaleOrder, self.sudo())._name_search(
                name=name, args=domain, operator=operator, limit=limit, name_get_uid=SUPERUSER_ID
            )
    ...
    def _compute_expense_count(self):
        ...
        mapped_data = dict(
            [(item["sale_order_id"][0], item["sale_order_id_count"]) for item in expense_data]
        )
