...
class HrExpenseSplitWizard(models.TransientModel):
    ...
    split_possible = fields.Boolean(
        help="The sum of after split shut remain the same", compute="_compute_split_possible"
    )
    ...
    def _compute_split_possible(self):
        for wizard in self:
            wizard.split_possible = wizard.total_amount_original and (
                float_compare(wizard.total_amount_original, wizard.total_amount, precision_digits=2)
                == 0
            )
