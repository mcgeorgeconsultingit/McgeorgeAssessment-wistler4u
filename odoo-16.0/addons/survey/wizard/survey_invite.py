...
class SurveyInvite(models.TransientModel):
    ...
    author_id = fields.Many2one(
        "res.partner", "Author", index=True, ondelete="set null", default=_get_default_author
    )
    # recipients
    ...
    existing_emails = fields.Text(
        "Existing emails", compute="_compute_existing_emails", readonly=True, store=False
    )
    ...
    def _compute_existing_partner_ids(self):
        self.existing_partner_ids = list(
            set(self.survey_id.user_input_ids.partner_id.ids) & set(self.partner_ids.ids)
        )
    ...
    def _send_mail(self, answer):
        ...
        if email_layout_xmlid:
            template_ctx = {
                ...
                .sudo()
                .new(dict(body=mail_values["body_html"], record_name=self.survey_id.title)),
