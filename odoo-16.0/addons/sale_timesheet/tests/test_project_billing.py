...
class TestProjectBilling(TestCommonSaleTimesheet):
    ...
    def test_make_billable_at_task_rate(self):
        ...
        wizard = (
            ...
            .with_context(active_id=self.project_non_billable.id, active_model="project.project")
            .create(
                {
                    "line_ids": [
                        Command.create(
    ...
    def test_make_billable_at_employee_rate(self):
        ...
        self.assertFalse(
            line1.project_id, "Sale line 1 should be linked to the 'non billable' project"
        )
        self.assertEqual(
            line2.project_id,
            self.project_non_billable,
    ...
    def test_billing_employee_rate(self):
        ...
        self.assertFalse(
            subtask.allow_billable, "Subtask in non billable project should be non billable too"
        )
        self.assertFalse(
        ...
        self.assertEqual(
            subtask.project_id, timesheet2.project_id, "The timesheet is in the subtask project"
        )
        self.assertNotEqual(
        ...
        self.assertTrue(
            task.allow_billable, "Task in project 'task rate' should be billed at task rate"
        )
        self.assertEqual(
            task.sale_line_id,
        ...
        task2 = Task.with_context(default_project_id=self.project_employee_rate.id).create(
            {"name": "first task", "partner_id": self.partner_a.id, "sale_line_id": False}
        )
