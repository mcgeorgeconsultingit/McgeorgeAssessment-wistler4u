...
class SurveyUserInput(models.Model):
    ...
    survey_id = fields.Many2one(
        "survey.survey", string="Survey", required=True, readonly=True, ondelete="cascade"
    )
    ...
    deadline = fields.Datetime(
        "Deadline", help="Datetime until customer can open the survey and submit answers"
    )
    state = fields.Selection(
        [("new", "Not started yet"), ("in_progress", "In Progress"), ("done", "Completed")],
        string="Status",
        default="new",
        readonly=True,
    )
    ...
    scoring_success = fields.Boolean(
        "Quizz Passed", compute="_compute_scoring_success", store=True, compute_sudo=True
    )  # stored for perf reasons
    # live sessions
            ...
            for question in user_input.predefined_question_ids:
                if question.question_type == "simple_choice":
                    total_possible_score += max(
                        [
                            score
                            for score in question.mapped("suggested_answer_ids.answer_score")
                            if score > 0
                        ],
                        default=0,
                    )
    ...
    def _compute_scoring_success(self):
        for user_input in self:
            user_input.scoring_success = (
                user_input.scoring_percentage >= user_input.survey_id.scoring_success_min
            )
    ...
    def _compute_attempts_info(self):
        ...
        if attempts_to_compute:
            self.flush_model(
                ["email", "invite_token", "partner_id", "state", "survey_id", "test_entry"]
            )
    ...
    def _save_line_choice(self, question, old_answers, answers, comment):
        ...
        if question.question_type == "simple_choice":
            ...
            vals_list = [
                self._get_line_answer_values(question, answer, "suggestion") for answer in answers
            ]
        ...
        if compute_speed_score and answer_score > 0:
            ...
            if session_speed_rating:
                ...
                if question_remaining_time < 0:  # if no time left
                    ...
                    time_limit -= (
                        max_score_delay  # we remove the max_score_delay to have all possible values
                    )
