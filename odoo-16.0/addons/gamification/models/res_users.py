...
class Users(models.Model):
    ...
    karma_tracking_ids = fields.One2many(
        "gamification.karma.tracking", "user_id", string="Karma Changes", groups="base.group_system"
    )
        ...
        if "karma" in vals:
            for user in self:
                if user.karma != vals["karma"]:
                    karma_trackings.append(
                        {"user_id": user.id, "old_value": user.karma, "new_value": vals["karma"]}
                    )
...
WHERE sub.user_id IN %%s""" % {
        ...
        if template:
            for u in self:
                if u.rank_id.karma_min > 0:
                    template.send_mail(
                        u.id, force_send=False, email_layout_xmlid="mail.mail_notification_light"
                    )
    ...
    def _recompute_rank(self):
        ...
        ranks = [
            ...
            for rank in self.env["gamification.karma.rank"].search([], order="karma_min DESC")
        ]
