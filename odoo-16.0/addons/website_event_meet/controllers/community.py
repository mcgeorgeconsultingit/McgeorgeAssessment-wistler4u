...
class WebsiteEventMeetController(EventCommunityController):
    ...
    def community(self, event, page=1, lang=None, **kwargs):
        ...
        return request.render(
            "website_event_meet.event_meet", self._event_meeting_rooms_get_values(event, lang=lang)
        )
    ...
    def _event_meeting_rooms_get_values(self, event, lang=None):
        ...
        if lang:
            search_domain = expression.AND([search_domain, [("room_lang_id", "=", int(lang))]])
    ...
    @http.route(
        "/event/<model('event.event'):event>/meeting_room_create",
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def create_meeting_room(self, event, **post):
        if (
            not event
            or (not event.is_published and not request.env.user.user_has_groups("base.group_user"))
            or not event.meeting_room_allow_creation
        ):
            raise Forbidden()
        ...
        _logger.info(
            "New meeting room (%s) created by %s (uid %s)"
            % (name, request.httprequest.remote_addr, request.env.uid)
        )
