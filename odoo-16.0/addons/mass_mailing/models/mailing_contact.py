...
class MassMailingContact(models.Model):
    ...
    list_ids = fields.Many2many(
        "mailing.list", "mailing_contact_list_rel", "contact_id", "list_id", string="Mailing Lists"
    )
        ...
        if (
            ...
            return [
                (
                    "id",
                    "in",
                    [record.contact_id.id for record in contacts if record.opt_out == value],
                )
            ]
    ...
    def create(self, vals_list):
        ...
        return super(MassMailingContact, self.with_context(default_list_ids=False)).create(
            vals_list
        )
