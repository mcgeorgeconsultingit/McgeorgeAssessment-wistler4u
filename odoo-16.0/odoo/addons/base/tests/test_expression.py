...
class TestExpression(SavepointCaseWithUserDemo):
    ...
    def _search(self, model, domain, init_domain=None):
        ...
        self.assertEqual(
            sql._ids, fil._ids, f"filtered_domain do not match SQL search for domain: {domain}"
        )
        return sql
    ...
    def test_00_in_not_in_m2m(self):
        ...
        a = partners.create(
            {"name": "test_expression_partner_A", "category_id": [Command.set([cat_a.id])]}
        )
        b = partners.create(
            {"name": "test_expression_partner_B", "category_id": [Command.set([cat_b.id])]}
        )
        ab = partners.create(
            {
        ...
        self.assertEqual(
            a + b + ab, with_a_or_b, "Search for category_id contains cat_a or cat_b failed."
        )

        # Show that `contains list` is really `contains element or contains element`.
        with_a_or_with_b = self._search(
            partners, ["|", ("category_id", "in", [cat_a.id]), ("category_id", "in", [cat_b.id])]
        )
        self.assertEqual(
            a + b + ab,
            with_a_or_with_b,
        ...
        self.assertTrue(
            c in without_a_or_b, "Search for category_id doesn't contain cat_a or cat_b failed (2)."
        )
        ...
        without_a_and_without_b = self._search(
            partners, [("category_id", "not in", [cat_a.id]), ("category_id", "not in", [cat_b.id])]
        )
        self.assertFalse(
        ...
        self.assertTrue(
            a not in without_a, "Search for category_id doesn't contain cat_a failed (1)."
        )
        self.assertTrue(
            ab not in without_a, "Search for category_id doesn't contain cat_a failed (2)."
        )
        ...
        self.assertTrue(
            b not in without_b, "Search for category_id doesn't contain cat_b failed (1)."
        )
        self.assertTrue(
            ab not in without_b, "Search for category_id doesn't contain cat_b failed (2)."
        )
        self.assertLessEqual(
    ...
    def test_05_not_str_m2m(self):
        ...
        def test(op, value, expected):
            ...
            self.assertItemsEqual(
                found_ids, expected_ids, "%s %r should return %r" % (op, value, expected)
            )
    ...
    def test_15_m2o(self):
        ...
        self.assertTrue(
            len(partners) >= 4, "We should have at least 4 partners not related to company #1"
        )
        ...
        self.assertTrue(
            len(partners) >= 8, "We should have at least 8 partners not related to company #1"
        )
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        self.assertEqual(
            set(all_ids) - set([p1]), set(res.ids), "o2m NOT IN matches none on the right side"
        )
        res = self._search(Partner, [("user_ids", "!=", "Dédé Boitaclou")])
        self.assertEqual(
            set(all_ids) - set([p1]), set(res.ids), "o2m NOT IN matches none on the right side"
        ...
        self.assertEqual(
            set(all_ids) - set([p1, p2]), set(res.ids), "o2m NOT IN matches none on the right side"
        )
    ...
    def test_active(self):
        ...
        vals = {
            ...
            "child_ids": [
                Command.create(
                    {"name": "address of OpenERP Test", "country_id": self.ref("base.be")}
                )
            ],
        }
        Partner.create(vals)
    ...
    def test_lp1071710(self):
        ...
        self.assertNotEqual(
            be.name, "Belgique", "Setting a translation should not impact other languages"
        )
...
class TestAutoJoin(TransactionCase):
    def test_auto_join(self):
        ...
        partners = partner_obj.search(
            ["|", ("name", "like", "C"), ("bank_ids.sanitized_acc_number", "like", name_test)]
        )
        self.assertIn(
            p_aa,
            partners,
        ...
        self.assertEqual(
            partners, p_aa + p_ab, "_auto_join on: ('bank_ids.id', 'in', [..]) incorrect result"
        )
        ...
        self.assertFalse(
            (p_ab + p_ba) & partners, "_auto_join on one2many with domains incorrect result"
        )
...
class TestOne2many(TransactionCase):
    ...
    def test_autojoin(self):
        ...
        self.patch(
            self.Partner._fields["bank_ids"], "domain", [("sanitized_acc_number", "like", "2")]
        )
        self.patch(
