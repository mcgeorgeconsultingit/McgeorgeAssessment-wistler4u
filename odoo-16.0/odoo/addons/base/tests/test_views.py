...
class TestApplyInheritanceSpecs(ViewCase):
    ...
    def test_insert_after(self):
        ...
        self.assertEqual(
            self.base_arch, E.form(E.field(name="target"), E.field(name="inserted"), string="Title")
        )
    ...
    def test_insert_before(self):
        ...
        self.assertEqual(
            self.base_arch, E.form(E.field(name="inserted"), E.field(name="target"), string="Title")
        )
    ...
    def test_insert_inside(self):
        ...
        self.assertEqual(
            self.base_arch,
            E.form(
                E.field(E.field(name="inserted"), E.field(name="inserted 2"), name="target"),
                string="Title",
            ),
        )
...
class TestApplyInheritanceMoveSpecs(ViewCase):
    def setUp(self):
        ...
        self.wrapped_arch = E.template(
            E.div("aaaa", E.p("Content", {"class": "some"}), "bbbb"), E.div({"class": "target"})
        )
    ...
    def test_move_replace(self):
        spec = E.xpath(
            E.xpath(expr="//p", position="move"), expr="//div[@class='target']", position="replace"
        )

        self.apply_spec(self.base_arch, spec)
        self.assertEqual(self.base_arch, E.template(E.div(), E.p("Content", {"class": "some"})))
        self.apply_spec(self.wrapped_arch, spec)
        self.assertEqual(
            self.wrapped_arch, E.template(E.div("aaaabbbb"), E.p("Content", {"class": "some"}))
        )

    def test_move_inside(self):
        spec = E.xpath(
            E.xpath(expr="//p", position="move"), expr="//div[@class='target']", position="inside"
        )

        self.apply_spec(self.base_arch, spec)
        self.assertEqual(
            self.base_arch,
            E.template(E.div(), E.div(E.p("Content", {"class": "some"}), {"class": "target"})),
        )
        self.apply_spec(self.wrapped_arch, spec)
        self.assertEqual(
            self.wrapped_arch,
            E.template(
                E.div("aaaabbbb"), E.div(E.p("Content", {"class": "some"}), {"class": "target"})
            ),
        )

    def test_move_before(self):
        spec = E.xpath(
            E.xpath(expr="//p", position="move"), expr="//div[@class='target']", position="before"
        )
    ...
    def test_move_after(self):
        spec = E.xpath(
            E.xpath(expr="//p", position="move"), expr="//div[@class='target']", position="after"
        )
...
class TestTemplating(ViewCase):
    ...
    def test_branding_inherit(self):
        ...
        self.assertEqual(
            str(view1.id), initial.get("data-oe-id"), "initial should come from the root view"
        )
        self.assertEqual(
            "/root[1]/item[1]",
        ...
        self.assertEqual(
            str(view2.id), second.get("data-oe-id"), "second should come from the extension view"
        )
    ...
    def test_branding_inherit_replace_node(self):
        ...
        self.assertEqual(
            "/xpath/world[1]", initial.get("data-oe-xpath"), "Inherited nodes have correct xpath"
        )

        # Second world added by inheritance
        [initial] = arch.xpath("/hello[1]/world[2]")
        self.assertEqual(
            "/xpath/world[2]", initial.get("data-oe-xpath"), "Inherited nodes have correct xpath"
        )

        # Third world - is not editable
        [initial] = arch.xpath("/hello[1]/world[3]")
        self.assertFalse(initial.get("data-oe-xpath"), "node containing t-esc is not branded")
    ...
    def test_branding_inherit_replace_node2(self):
        ...
        self.assertEqual(
            "/xpath/war", initial.get("data-oe-xpath"), "Inherited nodes have correct xpath"
        )

        # First world: from inheritance
        [initial] = arch.xpath("/hello[1]/world[1]")
        self.assertEqual(
            "/xpath/world", initial.get("data-oe-xpath"), "Inherited nodes have correct xpath"
        )
    ...
    def test_branding_inherit_remove_node_processing_instruction(self):
        ...
        self.assertEqual(
            len(head), 0, "The processing instruction of the <head> should have been removed"
        )
        self.assertEqual(
            len(body), 0, "The processing instruction of the <body> should have been removed"
        )
    ...
    def test_branding_inherit_top_t_field(self):
        ...
        self.assertEqual(
            node.get("data-oe-xpath"), "/hello[1]/world[2]", "First t-field has indication of xpath"
        )

        # Second t-field, from inheritance, should also have an indication of xpath
        [node] = arch.xpath('//*[@t-field="b"]')
        self.assertEqual(
            node.get("data-oe-xpath"), "/xpath/world", "Inherited t-field has indication of xpath"
        )
        ...
        self.assertEqual(node.get("t-field"), "z", "The node has properly been replaced")
    ...
    def test_branding_primary_inherit(self):
        ...
        [initial] = arch.xpath("//item[@order=1]")
        self.assertEqual(
            initial.get("data-oe-id"), str(view1.id), "initial should come from the root view"
        )
        self.assertEqual(
            initial.get("data-oe-xpath"),
            "/root[1]/item[1]",
        ...
        [second] = arch.xpath("//item[@order=2]")
        self.assertEqual(
            second.get("data-oe-id"), str(view2.id), "second should come from the extension view"
        )
        self.assertEqual(
...
class TestViews(ViewCase):
    ...
    def test_view_inheritance(self):
        ...
        self.assertEqual(
            etree.fromstring(view["arch"], parser=etree.XMLParser(remove_blank_text=True)),
            E.form(
    ...
    def test_view_inheritance_divergent_models(self):
        ...
        self.assertEqual(
            ...
            E.form(
                E.p("Replacement data"),
                E.footer(E.button(name="action_unarchive", type="object", string="New button")),
                string="Replacement title",
            ),
        )
    ...
    def test_attrs_groups_behavior(self):
        ...
        self.assertFalse(
            self.env["res.partner"].with_user(user_demo).env.user.has_group("base.group_system")
        )
        ...
        self.assertTrue(
            self.env["res.partner"].with_user(user_admin).env.user.has_group("base.group_system")
        )
    ...
    def test_button(self):
        ...
        self.assertInvalid(
            arch % "base.random_xmlid", "Invalid xmlid base.random_xmlid for button of type action"
        )
        self.assertInvalid('<form><button type="action"/></form>', "Button must have a name")
        self.assertInvalid(
            '<form><button special="dummy"/></form>', "Invalid special 'dummy' in button"
        )
    ...
    def test_view_ref(self):
        ...
        self.assertEqual(
            view.id, view_data["id"], "The view returned should be test_views_test_view_ref"
        )
        view_data = (
            self.env["ir.ui.view"]
            .with_context(form_view_ref="base.test_views_test_view_ref")
...
class TestXPathExtentions(common.BaseCase):
    def test_hasclass(self):
        tree = E.node(
            E.node({"class": "foo bar baz"}), E.node({"class": "foo bar"}), {"class": "foo"}
        )
...
class TestQWebRender(ViewCase):
    def test_render(self):
        ...
        content1 = (
            self.env["ir.qweb"].with_context(check_view_ids=[view1.id, view2.id])._render(view1.id)
        )
        content2 = (
            self.env["ir.qweb"].with_context(check_view_ids=[view1.id, view2.id])._render(view2.id)
        )
...
class TestRenderAllViews(common.TransactionCase):
    ...
    def test_render_all_views(self):
        ...
        _logger.info(
            "Rendered %d views as %s using (best of 5) %ss", count, self.env.user.name, elapsed
        )
