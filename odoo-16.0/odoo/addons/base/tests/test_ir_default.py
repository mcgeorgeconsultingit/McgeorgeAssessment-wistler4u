...
class TestIrDefault(TransactionCase):
    def test_defaults(self):
        ...
        user3 = user1.create(
            {"name": "u3", "login": "u3", "company_id": companyB.id, "company_ids": companyB.ids}
        )
    ...
    def test_conditions(self):
        ...
        self.assertEqual(IrDefault.get_model_defaults("res.partner.title"), {"shortcut": "X"})
        self.assertEqual(
    ...
    def test_multi_company_defaults(self):
        ...
        self.assertEqual(
            IrDefault.with_context(allowed_company_ids=company_a.ids).get_model_defaults(
                "res.partner"
            )["ref"],
            "CADefault",
        )
        self.assertEqual(
            IrDefault.with_context(allowed_company_ids=company_b.ids).get_model_defaults(
                "res.partner"
            )["ref"],
            "CBDefault",
        )
        self.assertEqual(
            IrDefault.with_context(allowed_company_ids=company_a_b.ids).get_model_defaults(
                "res.partner"
            )["ref"],
            "CADefault",
        )
        self.assertEqual(
            IrDefault.with_context(allowed_company_ids=company_b_a.ids).get_model_defaults(
                "res.partner"
            )["ref"],
            "CBDefault",
        )
