...
class TestPartner(TransactionCase):
    ...
    def test_lang_computation_form_view(self):
        ...
        self.assertEqual(
            partner_form.lang, default_lang_code, "New partner's lang should be default one"
        )
        ...
        # default from context > default from installed
        partner_form = Form(
            self.env["res.partner"].with_context(default_lang="de_DE"), "base.view_partner_form"
        )
        ...
        partner_form.name = "Test Company"
        self.assertEqual(
            partner_form.lang, "de_DE", "New partner's lang should take default from context"
        )
        with partner_form.child_ids.new() as child:
            child.name = "First Child"
            self.assertEqual(
                child.lang, "de_DE", "Child contact's lang should be the same as its parent."
            )
        ...
        with partner_form.child_ids.new() as child:
            ...
            self.assertEqual(
                child.lang, "fr_FR", "Child contact's lang should be the same as its parent."
            )
        partner = partner_form.save()
    ...
    def test_partner_merge_wizard_dst_partner_id(self):
        ...
        partner_merge_wizard = (
            ...
            .with_context({"partner_show_db_id": True, "default_dst_partner_id": test_partner})
            .new()
        )
        self.assertEqual(
