...
class MailThread(models.AbstractModel):
    ...
    message_attachment_count = fields.Integer(
        "Attachment Count", compute="_compute_message_attachment_count", groups="base.group_user"
    )
    ...
    def _search_has_message(self, operator, value):
        ...
        return [
            ("id", operator_new, ("SELECT res_id FROM mail_message WHERE model=%s", [self._name]))
        ]
    ...
    def _check_can_update_message_content(self, messages):
        ...
        if any(message.subtype_id.id != note_id for message in messages):
            raise exceptions.UserError(
                _("Only logged notes can have their content updated on model '%s'", self._name)
            )
    ...
    def _message_track(self, fields_iter, initial_values_dict):
        ...
        for record in self:
            ...
            subtype = record._track_subtype(
                dict((col_name, initial_values_dict[record.id][col_name]) for col_name in changes)
            )
            if subtype:
                ...
                record._message_log(
                    body=bodies.get(record.id) or "", tracking_value_ids=tracking_value_ids
                )
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        if bounced_email:
            ...
            for model in [
                ...
                rec_bounce_w_email = (
                    self.env[model.model].sudo().search([("email_normalized", "=", bounced_email)])
                )
    ...
    def message_route(self, message, message_dict, model=None, thread_id=None, custom_values=None):
        ...
        mail_messages = (
            ...
            .sudo()
            .search([("message_id", "in", msg_references)], limit=1, order="id desc, message_id")
        )
        ...
        rcpt_tos_localparts = [
            e.split("@")[0].lower() for e in tools.email_split(message_dict["recipients"])
        ]
        ...
        if is_a_reply:
            ...
            user_id = (
                self._mail_find_user_for_gateway(email_from, alias=dest_aliases).id or self._uid
            )
        ...
        if rcpt_tos_localparts:
            ...
            if dest_aliases:
                ...
                for alias in dest_aliases:
                    user_id = (
                        self._mail_find_user_for_gateway(email_from, alias=alias).id or self._uid
                    )
                    route = (
    ...
    def _message_parse_extract_payload_postprocess(self, message, payload_dict):
        ...
        for node in root.iter():
            if "o_mail_notification" in (node.get("class") or "") or "o_mail_notification" in (
                node.get("summary") or ""
            ):
                postprocessed = True
        ...
        if message.get_content_maintype() == "text":
            ...
            for part in message.walk():
                ...
                if filename and part.get("content-id"):
                    ...
                    attachments.append(
                        self._Attachment(filename, part.get_content(), {"cid": inner_cid})
                    )
                    continue
                # 1) Explicit Attachments -> attachments
                if filename or part.get("content-disposition", "").strip().startswith("attachment"):
                    attachments.append(
                        self._Attachment(filename or "attachment", part.get_content(), {})
                    )
                    continue
                # 2) text/plain -> <pre/>
    ...
    def _message_parse_extract_bounce(self, email_message, message_dict):
        ...
        if email_part:
            ...
            if bounced_msg_id:
                bounced_message = (
                    self.env["mail.message"].sudo().search([("message_id", "in", bounced_msg_id)])
                )
    ...
    def _message_get_suggested_recipients(self):
        ...
        if "user_id" in self._fields:
            for (
                obj
            ) in self.sudo():  # SUPERUSER because of a read on res.users that would crash otherwise
                if not obj.user_id or not obj.user_id.partner_id:
                    continue
                obj._message_add_suggested_recipient(
                    result, partner=obj.user_id.partner_id, reason=self._fields["user_id"].string
                )
        return result
    ...
    def _mail_find_user_for_gateway(self, email, alias=None):
        ...
        if not matching_user:
            std_users = (
                self.env["res.users"]
                .sudo()
                .search([("email_normalized", "=", normalized_email)], limit=1, order="name ASC")
            )
    ...
    def _mail_find_partner_from_emails(
        ...
        normalized_emails = [
            tools.email_normalize(contact) for contact in emails if tools.email_normalize(contact)
        ]
        if catchall_domain:
            domain_left_parts = [
                email.split("@")[0]
        ...
        done_partners = [
            follower for follower in followers if follower.email_normalized in normalized_emails
        ]
        remaining = [
            email
            for email in normalized_emails
    ...
    def _message_partner_info_from_emails(self, emails, link_mail=False):
        ...
        for idx, contact in enumerate(emails):
            ...
            if link_mail and partner:
                MailMessage.search(
                    [("email_from", "=ilike", partner.email_normalized), ("author_id", "=", False)]
    ...
    def message_post(
        ...
        **kwargs,
        ...
        msg_kwargs = dict(
            (key, val) for key, val in kwargs.items() if key in self.env["mail.message"]._fields
        )
    ...
    def _message_compose_with_view(self, views_or_xmlid, message_log=False, **kwargs):
        ...
        view_ref = (
            views_or_xmlid.id if isinstance(views_or_xmlid, models.BaseModel) else views_or_xmlid
        )
    ...
    def _message_log_batch(
        self, bodies, author_id=None, email_from=None, subject=False, message_type="notification"
    ):
    ...
    def _message_compute_author(self, author_id=None, email_from=None, raise_on_email=True):
        ...
        if not email_from and raise_on_email and not self.env.su:
            raise exceptions.UserError(
                _("Unable to send message, please configure the sender's email address.")
            )
    ...
    def _message_compute_parent_id(self, parent_id):
        ...
        if self._mail_flat_thread and not parent_id:
            ...
            if self._mail_flat_thread:
                ...
                parent_id = (
                    current_ancestor.parent_id.id if current_ancestor.parent_id else parent_id
                )
        return parent_id
    ...
    def _notify_thread_by_email(
        ...
        **kwargs,
        ...
        email_layout_xmlid = (
            msg_vals.get("email_layout_xmlid") if msg_vals else message.email_layout_xmlid
        )
        template_xmlid = (
            email_layout_xmlid if email_layout_xmlid else "mail.mail_notification_layout"
        )
                    ...
                    if resend_existing:
                        ...
                        if existing_notifications:
                            tocreate_recipient_ids = [
                                ...
                                if rid not in existing_notifications.mapped("res_partner_id.id")
                            ]
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        if not model_description:
            ...
            if model:
                model_description = (
                    self.env["ir.model"].with_context(lang=lang)._get(model).display_name
                )
        record_name = (
            msg_vals.get("record_name") if "record_name" in msg_vals else message.record_name
        )
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        for pid, pdata in res.items():
            if (
                pid and pid == author_id and not self.env.context.get("mail_notify_author")
        ...
        if kwargs.pop("skip_existing", False):
            ...
            if pids:
                existing_notifications = (
                    ...
                    .sudo()
                    .search(
                        [("res_partner_id", "in", pids), ("mail_message_id", "in", message.ids)]
                    )
                )
                recipients_data = [
                    r
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        """
        is_thread_notification = self._notify_get_recipients_thread_info(msg_vals=msg_vals)[
    ...
    def _notify_get_recipients_thread_info(self, msg_vals=None):
        ...
        return {"is_thread_notification": res_model and (res_model != "mail.thread") and res_id}
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        if (
            ...
            and (getattr(field, "track_visibility", False) or getattr(field, "tracking", False))
        ):
