...
class TestBurndownChartCommon(TestProjectCommon):
    ...
    def set_create_date(cls, table, res_id, create_date):
        cls.env.cr.execute(
            "UPDATE {} SET create_date=%s WHERE id=%s".format(table), (create_date, res_id)
        )
...
class TestBurndownChart(TestBurndownChartCommon):
    ...
    def test_burndown_chart(self):
        ...
        self.check_read_group_results(
            AND([burndown_chart_domain, all_projects_domain_with_ilike]), project_expected_dict
        )
        ...
        complex_domain = AND(
            [burndown_chart_domain, all_projects_domain_with_ilike, date_and_user_domain]
        )
