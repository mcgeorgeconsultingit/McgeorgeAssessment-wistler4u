...
class DiscussController(http.Controller):
    ...
    def _response_discuss_channel_invitation(self, channel_sudo, is_channel_token_secret=True):
        ...
        channel_member_sudo = channel_sudo.env["mail.channel.member"]._get_as_sudo_from_request(
                    ...
                    guest = channel_sudo.env["mail.guest"].create(
                        {
                            ...
                            .search([("code", "=", request.geoip.get("country_code"))], limit=1)
                            .id,
                            "lang": get_lang(channel_sudo.env).code,
                            "name": _("Guest"),
                            "timezone": channel_sudo.env["mail.guest"]._get_timezone_from_request(
                                request
                            ),
                        }
                    )
            ...
            "mail.discuss_public_channel_template",
            {
                "data": {
                    ...
                    "discussPublicViewData": dict(
                        {
                            "channel": [
                                ("insert", {"id": channel_sudo.id, "model": "mail.channel"})
                            ],
    ...
    def mail_channel_partner_avatar_128(self, channel_id, partner_id, **kwargs):
        channel_member_sudo = request.env["mail.channel.member"]._get_as_sudo_from_request(
        ...
        if channel_member_sudo and channel_member_sudo.env["mail.channel.member"].search(
        ...
        return (
            request.env["ir.binary"]
            ._get_image_stream_from(
                partner_sudo.sudo(False), field_name="avatar_128", placeholder=placeholder
            )
            .get_response()
        )
    ...
    def discuss_inbox_messages(self, max_id=None, min_id=None, limit=30, **kwargs):
        return (
            ...
            ._message_fetch(
                domain=[("needaction", "=", True)], max_id=max_id, min_id=min_id, limit=limit
            )
            .message_format()
        )
    ...
    def discuss_history_messages(self, max_id=None, min_id=None, limit=30, **kwargs):
        return (
            request.env["mail.message"]
            ._message_fetch(
                domain=[("needaction", "=", False)], max_id=max_id, min_id=min_id, limit=limit
            )
            .message_format()
        )
    ...
    def mail_attachment_upload(self, ufile, thread_id, thread_model, is_pending=False, **kwargs):
        ...
        if thread_model == "mail.channel":
            channel_member = request.env["mail.channel.member"]._get_as_sudo_from_request_or_raise(
    ...
    def mail_channel_add_guest_as_member(self, channel_id, channel_uuid, **kwargs):
        ...
        if not guest:
            raise NotFound()
        channel_member = channel_sudo.env["mail.channel.member"]._get_as_sudo_from_request(
            request=request, channel_id=channel_id
        )
    ...
    def channel_ping(self, channel_id, rtc_session_id=None, check_rtc_session_ids=None):
        ...
        current_rtc_sessions, outdated_rtc_sessions = channel_member_sudo._rtc_sync_sessions(
    ...
    @http.route(
        "/mail/rtc/session/notify_call_members", methods=["POST"], type="json", auth="public"
    )
    def session_call_notify(self, peer_notifications):
        ...
        for sender_session_id, target_session_ids, content in peer_notifications:
            session_sudo = (
                guest.env["mail.channel.rtc.session"].sudo().browse(int(sender_session_id)).exists()
            )
            if (
                not session_sudo
    ...
    @http.route(
        "/mail/rtc/session/update_and_broadcast", methods=["POST"], type="json", auth="public"
    )
    def session_update_and_broadcast(self, session_id, values):
        ...
        if request.env.user._is_public():
            ...
            if guest:
                session = (
                    guest.env["mail.channel.rtc.session"].sudo().browse(int(session_id)).exists()
                )
    ...
    @http.route(
        "/mail/rtc/channel/cancel_call_invitation", methods=["POST"], type="json", auth="public"
    )
    ...
    def mail_link_preview_delete(self, link_preview_id):
        guest = request.env["mail.guest"]._get_guest_from_request(request)
        link_preview_sudo = (
            guest.env["mail.link.preview"].sudo().search([("id", "=", int(link_preview_id))])
        )
