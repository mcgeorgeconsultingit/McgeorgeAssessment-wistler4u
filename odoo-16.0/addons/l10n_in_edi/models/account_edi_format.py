...
class AccountEdiFormat(models.Model):
        ...
        for line in move.invoice_line_ids.filtered(
            ...
            if line.product_id:
                ...
                if not hsn_code:
                    ...
                    error_message.append(
                        _("Invalid HSN Code (%s) in product %s", hsn_code, line.product_id.name)
                    )
            else:
        ...
        if response.get("error"):
            ...
            if "no-credit" in error_codes:
                ...
                error_message = "<br/>".join(
                    ["[%s] %s" % (e.get("code"), html_escape(e.get("message"))) for e in error]
                )
                return {
                    invoice: {
                        "success": False,
    ...
    def _get_l10n_in_edi_partner_details(
        self, partner, set_vat=True, set_phone_and_email=True, is_overseas=False, pos_state_id=False
    ):
        """
    ...
    def _l10n_in_edi_generate_invoice_json_managing_negative_lines(self, invoice, json_payload):
        ...
        def put_discount_on(discount_line_vals, other_line_vals):
            ...
            if (
                float_compare(
                    discount_to_allow, discount, precision_rounding=invoice.currency_id.rounding
                )
                < 0
            ):
                ...
                discount_line_vals.update(
                    {
                        ...
                        "CesNonAdvlAmt": self._l10n_in_round_value(
                            discount_line_vals["CesNonAdvlAmt"] + other_line_vals["CesNonAdvlAmt"]
                        ),
                        "StateCesAmt": self._l10n_in_round_value(
                            discount_line_vals["StateCesAmt"] + other_line_vals["StateCesAmt"]
                        ),
                        "StateCesNonAdvlAmt": self._l10n_in_round_value(
                        ...
                        "TotItemVal": self._l10n_in_round_value(
                            discount_line_vals["TotItemVal"] + other_line_vals["TotItemVal"]
                        ),
                    }
                )
            ...
            other_line_vals.update(
                {
                    ...
                    "CesNonAdvlAmt": self._l10n_in_round_value(
                        other_line_vals["CesNonAdvlAmt"] + discount_line_vals["CesNonAdvlAmt"]
                    ),
                    "StateCesAmt": self._l10n_in_round_value(
                        other_line_vals["StateCesAmt"] + discount_line_vals["StateCesAmt"]
                    ),
                    "StateCesNonAdvlAmt": self._l10n_in_round_value(
                        other_line_vals["StateCesNonAdvlAmt"]
                        + discount_line_vals["StateCesNonAdvlAmt"]
    ...
    def _l10n_in_edi_generate_invoice_json(self, invoice):
        ...
        if is_overseas:
            ...
            if invoice.l10n_in_shipping_port_code_id:
                json_payload["ExpDtls"].update({"Port": invoice.l10n_in_shipping_port_code_id.code})
    ...
    def _get_l10n_in_tax_details_by_line_code(self, tax_details):
        ...
        for tax_detail in tax_details.values():
            ...
            l10n_in_tax_details["%s_amount_currency" % (tax_detail["line_code"])] += tax_detail[
                "tax_amount_currency"
            ]
