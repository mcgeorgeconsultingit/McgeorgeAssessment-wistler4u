...
class IrAttachment(models.Model):
            ...
            for fname in names:
                ...
                if fname not in whitelist:
                    try:
                        ...
                        _logger.info(
                            "_file_gc could not unlink %s", self._full_path(fname), exc_info=True
                        )
                with contextlib.suppress(OSError):
                    os.unlink(filepath)
    ...
    def _check_contents(self, values):
        ...
        xml_like = "ht" in mimetype or (  # hta, html, xhtml, etc.
            "xml" in mimetype and not "openxmlformats" in mimetype  # other xml (svg, text/xml, etc)
        ...
        force_text = xml_like and (
            ...
            or not self.env["ir.ui.view"].with_user(user).check_access_rights("write", False)
        )
        if force_text:
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", change_default=True, default=lambda self: self.env.company
    )
    ...
    datas = fields.Binary(
        string="File Content (base64)", compute="_compute_datas", inverse="_inverse_datas"
    )
