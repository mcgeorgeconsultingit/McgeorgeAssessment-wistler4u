...
class TestUiHtmlEditor(odoo.tests.HttpCase):
    def test_html_editor_multiple_templates(self):
        ...
        self.assertTrue(
            generic_page.arch == oe_structure_layout, "Generic homepage view should be untouched"
        )
        self.assertEqual(
            len(generic_page.inherit_children_ids.filtered(lambda v: "oe_structure" in v.name)),
            0,
        ...
        self.assertEqual(
            len(specific_page.inherit_children_ids.filtered(lambda v: "oe_structure" in v.name)),
            1,
    ...
    def test_html_editor_scss(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "test_html_editor_scss_2", login="demo"
        )
...
class TestUiTranslate(odoo.tests.HttpCase):
    ...
    def test_translate_menu_name(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "translate_menu_name", login="admin"
        )
    ...
    def test_snippet_translation(self):
        ...
        self.start_tour(
            f"/website/force/{website_2.id}", "snippet_translation_changing_lang", login="admin"
        )
...
class TestUi(odoo.tests.HttpCase):
    ...
    def test_02_restricted_editor(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "restricted_editor", login="restricted"
        )
    ...
    def test_07_snippet_version(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "snippet_version", login="admin"
        )

    def test_08_website_style_custo(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "website_style_edition", login="admin"
        )
    ...
    def test_16_website_edit_megamenu(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "edit_megamenu", login="admin"
        )
