...
""",
    ...
    mo_bom_id = fields.Many2one(
        "mrp.bom", "Bill of Material used on the Production Order", related="mo_id.bom_id"
    )
    ...
    consume_line_ids = fields.One2many(
        "stock.move", "consume_unbuild_id", readonly=True, string="Consumed Disassembly Lines"
    )
    ...
    def action_unbuild(self):
        ...
        if any(
            produce_move.has_tracking != "none" and not self.mo_id for produce_move in produce_moves
        ):
            raise UserError(
                _(
        ...
        if any(
            consume_move.has_tracking != "none" and not self.mo_id for consume_move in consume_moves
        ):
            raise UserError(
                _(
        ...
        for move in produce_moves | consume_moves:
            if move.has_tracking != "none":
                ...
                for move_line in moves_lines:
                    ...
                    taken_quantity = min(
                        needed_quantity, move_line.qty_done - qty_already_used[move_line]
                    )
                    if taken_quantity:
                        self.env["stock.move.line"].create(
                            {
        ...
        if self.mo_id:
            ...
            self.mo_id.message_post(body=unbuild_msg, subtype_id=self.env.ref("mail.mt_note").id)
    ...
    def _generate_consume_moves(self):
        ...
        for unbuild in self:
            if unbuild.mo_id:
                ...
                factor = unbuild.product_qty / unbuild.mo_id.product_uom_id._compute_quantity(
                    unbuild.mo_id.product_qty, unbuild.product_uom_id
                )
                for finished_move in finished_moves:
                    moves += unbuild._generate_move_from_existing_move(
                        finished_move, factor, unbuild.location_id, finished_move.location_id
                    )
            else:
                factor = (
