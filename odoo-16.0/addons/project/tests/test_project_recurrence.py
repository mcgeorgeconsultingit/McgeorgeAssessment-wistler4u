...
class TestProjectrecurrence(TransactionCase):
    ...
    def test_recurrence_cron_repeat_after(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            self.assertEqual(
                self.env["project.task"].search_count(domain), 1, "no extra task should be created"
            )
        ...
        for f in self.env["project.task.recurrence"]._get_recurring_fields():
            self.assertTrue(
                tasks[0][f] == tasks[1][f] == tasks[2][f], "Field %s should have been copied" % f
            )
    ...
    def test_recurrence_cron_repeat_after_subtasks(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            project_tasks, project_task_count, project_subtask_count = get_task_and_subtask_counts(
                domain
            )
            ...
            self.env["project.task.recurrence"]._cron_create_recurring_tasks()
            self.assertEqual(
                self.env["project.task"].search_count(domain), 2, "no extra task should be created"
            )
    ...
    def test_recurrence_cron_repeat_after_subsubtasks(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            self.env["project.task.recurrence"]._cron_create_recurring_tasks()
            self.assertEqual(
                self.env["project.task"].search_count(domain), 8, "no extra task should be created"
            )
