...
class ir_cron(models.Model):
    ...
    ir_actions_server_id = fields.Many2one(
        "ir.actions.server", "Server action", delegate=True, ondelete="restrict", required=True
    )
    cron_name = fields.Char("Name", related="ir_actions_server_id.name", store=True, readonly=False)
    user_id = fields.Many2one(
        "res.users", string="Scheduler User", default=lambda self: self.env.user, required=True
    )
    ...
    def _process_jobs(cls, db_name):
        ...
        try:
            ...
            _logger.warning(
                "Skipping database %s because of modules to install/upgrade/remove.", db_name
            )
