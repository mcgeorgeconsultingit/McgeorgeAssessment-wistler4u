...
class Location(models.Model):
    ...
    posy = fields.Integer(
        "Shelves (Y)", default=0, help="Optional localization details, for information purpose only"
    )
    posz = fields.Integer(
        "Height (Z)", default=0, help="Optional localization details, for information purpose only"
    )
                ...
                try:
                    if location.last_inventory_date:
                        ...
                        if days_until_next_inventory <= 0:
                            ...
                            location.next_inventory_date = location.last_inventory_date + timedelta(
    ...
    def _check_scrap_location(self):
        for record in self:
            if record.scrap_location and self.env["stock.picking.type"].search(
                [("code", "=", "mrp_operation"), ("default_location_dest_id", "=", record.id)]
            ):
                raise ValidationError(
                    _(
    ...
    def write(self, values):
        ...
        if "usage" in values or "scrap_location" in values:
            modified_locations = self.filtered(
                lambda l: any(
                    l[f] != values[f] if f in values else False for f in {"usage", "scrap_location"}
                )
            )
        ...
        if self.storage_category_id:
            ...
            if package and package.package_type_id:
                ...
                if self.storage_category_id.max_weight < self.forecast_weight + sum(
                    package_smls.mapped(lambda sml: sml.reserved_qty * sml.product_id.weight)
                ):
                    return False
                # check if enough space
                package_capacity = self.storage_category_id.package_capacity_ids.filtered(
                ...
                if (
                    ...
                    return False
                product_capacity = self.storage_category_id.product_capacity_ids.filtered(
