...
class PricelistItem(models.Model):
    ...
    percent_price = fields.Float(
        string="Percentage Price", help="You can apply a mark-up by setting a negative discount."
    )
    ...
    def _compute_rule_tip(self):
        base_selection_vals = {
            elem[0]: elem[1] for elem in self._fields["base"]._description_selection(self.env)
        }
        self.rule_tip = False
        for item in self:
    ...
    def _onchange_rule_content(self):
        if not self.user_has_groups("product.group_sale_pricelist") and not self.env.context.get(
