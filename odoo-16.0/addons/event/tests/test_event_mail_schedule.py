...
class TestMailSchedule(EventCase, MockEmail):
    ...
    def test_event_mail_schedule(self):
        ...
        self.assertEqual(
            after_sub_scheduler_2.scheduled_date, test_event.create_date + relativedelta(hours=1)
        )
        ...
        self.assertEqual(
            event_prev_scheduler.scheduled_date, event_date_begin + relativedelta(days=-1)
        )
        ...
        self.assertTrue(
            after_sub_scheduler.mail_done, "event: all subscription mails should have been sent"
        )
        self.assertEqual(after_sub_scheduler.mail_state, "running")
        ...
        self.assertEqual(
            len(self._new_mails), 2, "event: should have 2 scheduled emails (1 / registration)"
        )
        ...
        self.assertFalse(
            any(mail_reg.mail_sent for mail_reg in after_sub_scheduler_2.mail_registration_ids)
        )
        self.assertFalse(after_sub_scheduler_2.mail_done)
        self.assertEqual(after_sub_scheduler_2.mail_count_done, 0)
        self.assertEqual(
            len(self._new_mails), 0, "event: should not send mails before scheduled date"
        )
        ...
        self.assertTrue(
            all(mail_reg.mail_sent for mail_reg in after_sub_scheduler_2.mail_registration_ids)
        )
        self.assertTrue(
            after_sub_scheduler_2.mail_done, "event: all subscription mails should have been sent"
        )
        self.assertEqual(after_sub_scheduler_2.mail_state, "running")
        ...
        self.assertEqual(
            event_prev_scheduler.mail_state, "sent", "event: reminder scheduler should have run"
        )

        # check emails effectively sent
        self.assertEqual(
            len(self._new_mails), 2, "event: should have scheduled 2 mails (1 / registration)"
        )
        self.assertMailMailWEmails(
            [formataddr((reg1.name, reg1.email)), formataddr((reg2.name, reg2.email))],
            "outgoing",
        ...
        # check emails effectively sent
        self.assertEqual(
            len(self._new_mails), 2, "event: should have 1 scheduled emails (new registration only)"
        )
        ...
        # check that scheduler is finished
        self.assertTrue(
            event_next_scheduler.mail_done, "event: reminder scheduler should should have run"
        )
        self.assertEqual(
            event_next_scheduler.mail_state, "sent", "event: reminder scheduler should have run"
        )
    ...
    def test_unique_event_mail_ids(self):
                    ...
                    Command.create(
                        {
                            ...
                            "interval_type": "before_event",
                            "template_ref": "mail.template,%i"
                            % self.env["ir.model.data"]._xmlid_to_res_id("event.event_reminder"),
                        }
                    ),
                ],
            }
        ...
        self.assertEqual(
            len(test_event.event_mail_ids), 2, "Should now have only two communication lines"
        )
