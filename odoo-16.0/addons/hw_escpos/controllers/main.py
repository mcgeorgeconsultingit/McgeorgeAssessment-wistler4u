...
class EscposDriver(Thread):
    ...
    def connected_usb_devices(self):
        ...
        for printer in printers:
            ...
            connected.append(
                {"vendor": printer.idVendor, "product": printer.idProduct, "name": description}
            )
    ...
    def set_status(self, status, message=None):
        ...
        if status == self.status["status"]:
            if message != None and (
                len(self.status["messages"]) == 0 or message != self.status["messages"][-1]
            ):
    ...
    def print_receipt_body(self, eprint, receipt):
        ...
        def print_taxes():
            ...
            for tax in taxes:
                eprint.text(
                    printline(tax["tax"]["name"], price(tax["amount"]), width=40, ratio=0.6)
                )
        ...
        for line in receipt["orderlines"]:
            ...
            if line["discount"] == 0 and line["unit_name"] == "Units" and line["quantity"] == 1:
                ...
                if line["discount"] != 0:
                    eprint.text(
                        printline("Discount: " + str(line["discount"]) + "%", ratio=0.6, indent=2)
                    )
        ...
        eprint.text(
            printline(_("         TOTAL"), money(receipt["total_with_tax"]), width=40, ratio=0.6)
        )
        eprint.text("\n\n")
        ...
        if receipt["total_discount"] != 0:
            eprint.text(
                printline(_("Discounts"), money(receipt["total_discount"]), width=40, ratio=0.6)
            )
        if taxincluded:
            print_taxes()
            # eprint.text(printline(_('Taxes'),money(receipt['total_tax']),width=40, ratio=0.6))
