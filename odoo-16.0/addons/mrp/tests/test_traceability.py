...
class TestTraceability(TestMrpCommon):
    ...
    def test_tracking_types_on_mo(self):
        ...
        quants |= self.env["stock.quant"].create(
            {
                ...
                "lot_id": Lot.create(
                    {"name": "L1", "product_id": consumed_lot.id, "company_id": self.env.company.id}
                ).id,
            }
        )
        quants |= self.env["stock.quant"].create(
        ...
        for finished_product in [finished_no_track, finished_lot, finished_serial]:
            ...
            details_operation_form = Form(
                mo.move_raw_ids[2], view=self.env.ref("stock.view_stock_move_operations")
            )
            with details_operation_form.move_line_ids.edit(0) as ml:
                ml.qty_done = 1
            details_operation_form.save()
            ...
            self.assertEqual(
                final_product["unfoldable"], True, "Final product should always be unfoldable"
            )
    ...
    def test_tracking_on_byproducts(self):
        ...
        self.assertEqual(
            finished_move_line_lot_2.consume_line_ids, raw_line_raw_1_lot_2 | raw_line_raw_2_lot_2
        )
