...
class Challenge(models.Model):
    ...
    def _compute_user_count(self):
        ...
        if self.ids:
            ...
            mapped_data = dict(
                (challenge_id, user_count) for challenge_id, user_count in self.env.cr.fetchall()
            )
        ...
        for user in self.user_ids:
            ...
            for goal in goal_ids:
                ...
                if goal.definition_condition == "higher":
                    # can be over 100
                    total_completeness += (
                        (100.0 * goal.current / goal.target_goal) if goal.target_goal else 0
                    )
            ...
            challengers.append(
                {"user": user, "all_reached": all_reached, "total_completeness": total_completeness}
            )
