...
class HrWorkEntry(models.Model):
    ...
    def _get_duration_batch(self):
        ...
        for work_entry in self - super_work_entries:
            ...
            calendar = work_entry.contract_id.resource_calendar_id
            employee = work_entry.contract_id.employee_id
            result[work_entry.id] = mapped_contract_data[(date_start, date_stop)][calendar][
                employee.id
            ]["hours"]
        return result
    ...
    def _get_leaves_entries_outside_schedule(self):
        return self.filtered(
            lambda w: w.work_entry_type_id.is_leave and w.state not in ("validated", "cancelled")
        )
    ...
    def _to_intervals(self):
        return WorkIntervals(
            (w.date_start.replace(tzinfo=pytz.utc), w.date_stop.replace(tzinfo=pytz.utc), w)
            for w in self
        )
