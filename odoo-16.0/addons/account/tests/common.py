...
class AccountTestInvoicingCommon(TransactionCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        if not chart_template:
            ...
            cls.skipTest(
                cls, "Accounting Tests skipped because the user's company has no chart of accounts."
            )
        ...
        cls.partner_b = cls.env["res.partner"].create(
            {
                ...
                "property_account_receivable_id": cls.company_data["default_account_receivable"]
                .copy()
                .id,
                "property_account_payable_id": cls.company_data["default_account_payable"]
                .copy()
                .id,
                "company_id": False,
            }
    ...
    def setup_company_data(cls, company_name, chart_template=None, **kwargs):
        ...
        return {
            ...
            "default_account_assets": cls.env["account.account"].search(
                [("company_id", "=", company.id), ("account_type", "=", "asset_current")], limit=1
            ),
    ...
    def init_invoice(
        ...
        move_form = Form(
            ...
            .with_context(
                default_move_type=move_type, account_predictive_bills_disable_prediction=True
            )
        )
    ...
    def _turn_node_as_dict_hierarchy(self, node):
        ...
        return {
            ...
            "attrib": attrib_wo_ns,
            "children": [
                self._turn_node_as_dict_hierarchy(child_node) for child_node in node.getchildren()
            ],
        }
    ...
    def assertXmlTreeEqual(self, xml_tree, expected_xml_tree):
        ...
        def assertNodeDictEqual(node_dict, expected_node_dict):
            ...
            # Check attributes.
            node_dict_attrib = {
                k: "___ignore___" if expected_node_dict["attrib"].get(k) == "___ignore___" else v
                for k, v in node_dict["attrib"].items()
            }
            expected_node_dict_attrib = {
                k: v for k, v in expected_node_dict["attrib"].items() if v != "___remove___"
            }
...
class TestAccountReconciliationCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.bank_journal_usd = cls.env["account.journal"].create(
            {"name": "Bank US", "type": "bank", "code": "BNK68", "currency_id": cls.currency_usd_id}
        )
