...
class AccountFiscalPosition(models.Model):
    ...
    account_ids = fields.One2many(
        "account.fiscal.position.account", "position_id", string="Account Mapping", copy=True
    )
    tax_ids = fields.One2many(
        "account.fiscal.position.tax", "position_id", string="Tax Mapping", copy=True
    )
    note = fields.Html(
        "Notes", translate=True, help="Legal mentions that have to be printed on the invoices."
    )
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        manual_fiscal_position = (
            delivery.property_account_position_id or partner.property_account_position_id
        )
...
class AccountFiscalPositionTax(models.Model):
    ...
    position_id = fields.Many2one(
        "account.fiscal.position", string="Fiscal Position", required=True, ondelete="cascade"
    )
    company_id = fields.Many2one(
...
class ResPartner(models.Model):
    ...
    def _invoice_total(self):
        ...
        for partner in self.filtered("id"):
            ...
            all_partners_and_children[partner] = (
                self.with_context(active_test=False).search([("id", "child_of", partner.id)]).ids
            )
    ...
    currency_id = fields.Many2one(
        "res.currency", compute="_get_company_currency", readonly=True, string="Currency"
    ...
    contract_ids = fields.One2many(
        "account.analytic.account", "partner_id", string="Partner Contracts", readonly=True
    )
    ...
    def _get_duplicated_bank_accounts(self):
        ...
        for bank in self.bank_ids:
            domains.append(
                [("acc_number", "=", bank.acc_number), ("bank_id", "=", bank.bank_id.id)]
            )
