...
def normalize_leaf(element):
    ...
    if isinstance(right, bool) and operator in ("in", "not in"):
        _logger.warning(
            "The domain term '%s' should use the '=' or '!=' operator." % ((left, original, right),)
        )
...
class expression(object):
    ...
    def __init__(self, domain, model, alias=None, query=None):
        ...
        self.query = (
            Query(model.env.cr, model._table, model._table_query) if query is None else query
        )
    ...
    def get_tables(self):
        warnings.warn(
            "deprecated expression.get_tables(), use expression.query instead", DeprecationWarning
        )
        ...
        while stack:
            ...
            # -------------------------------------------------
                ...
                if operator in ("=", "!=") and not right:
                    inselect_operator = (
                        "inselect" if operator in NEGATIVE_TERM_OPERATORS else "not inselect"
                    )
                    subselect = (
                    ...
                    push(
                        ("id", inselect_operator, (subselect, params)), model, alias, internal=True
                    )
                else:
                    _logger.error(
            ...
            # -------------------------------------------------
                ...
                if field.type == "datetime" and right:
                    ...
                    if not need_wildcard:
                        right = field.convert_to_column(right, model, validate=False).adapted[
                            "en_US"
                        ]
                    ...
                    if params:
                        params = [
                            field.convert_to_column(p, model, validate=False).adapted["en_US"]
                            for p in params
                        ]
                        lang = model.env.lang or "en_US"
                        if lang == "en_US":
    ...
    def __leaf_to_sql(self, leaf, model, alias):
        ...
        assert not isinstance(right, BaseModel), "Invalid value %r in domain term %r" % (
            right,
            leaf,
        )
        ...
        if leaf == TRUE_LEAF:
            ...
            if isinstance(right, bool):
                _logger.warning(
                    "The domain term '%s' should use the '=' or '!=' operator." % (leaf,)
                )
            ...
            and ((operator == "=" and right is False) or (operator == "!=" and right is True))
        ):
    ...
    def to_sql(self):
        warnings.warn(
            "deprecated expression.to_sql(), use expression.query instead", DeprecationWarning
        )
        return self.result
