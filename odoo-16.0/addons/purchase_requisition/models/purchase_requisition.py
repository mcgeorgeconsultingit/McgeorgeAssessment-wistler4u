...
class PurchaseRequisition(models.Model):
    ...
    def _onchange_vendor(self):
        ...
        if not self.vendor_id:
            ...
            self.currency_id = (
                self.vendor_id.property_purchase_currency_id.id or self.env.company.currency_id.id
            )
...
class PurchaseRequisitionLine(models.Model):
    ...
    requisition_id = fields.Many2one(
        "purchase.requisition", required=True, string="Purchase Agreement", ondelete="cascade"
    )
    company_id = fields.Many2one(
        "res.company",
