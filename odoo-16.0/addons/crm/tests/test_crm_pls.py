...
class TestCRMPLS(TransactionCase):
    ...
    def test_predictive_lead_scoring(self):
        ...
        team_ids = (
            self.env["crm.team"]
            .create([{"name": "Team Test 1"}, {"name": "Team Test 2"}, {"name": "Team Test 3"}])
            .ids
        )
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "crm.pls_fields", "country_id,state_id,email_state,phone_state,source_id,tag_ids"
        )
    ...
    def test_settings_pls_start_date(self):
        ...
        set_param(
            "crm.pls_start_date", "One does not simply walk into system parameters to corrupt them"
        )
        res_config_new = resConfig.new()
        self.assertEqual(
            Date.to_string(res_config_new.predictive_lead_scoring_start_date),
