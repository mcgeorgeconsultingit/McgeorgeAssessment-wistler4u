...
class ResUsersSettings(models.Model):
    ...
    volume_settings_ids = fields.One2many(
        "res.users.settings.volumes", "user_setting_id", string="Volumes of other partners"
    )
    ...
    def _res_users_settings_format(self, fields_to_format=None):
        ...
        if not fields_to_format:
            fields_to_format = [
                name for name, field in self._fields.items() if name == "id" or not field.automatic
            ]
