...
class AccountMove(models.Model):
    ...
    l10n_it_stamp_duty = fields.Float(
        default=0, string="Dati Bollo", readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    l10n_it_einvoice_id = fields.Many2one(
        "ir.attachment", string="Electronic invoice", compute="_compute_l10n_it_einvoice"
    )
    ...
    def _prepare_fatturapa_export_values(self):
        ...
        if is_self_invoice:
            document_total += sum(
                [abs(v["tax_amount_currency"]) for k, v in tax_details["tax_details"].items()]
            )
            if reverse_charge_refund:
