...
class TestViewSaving(TestViewSavingCommon):
    ...
    def setUp(self):
        ...
        self.arch = h.DIV(
            h.DIV(h.H3("Column 1"), h.UL(h.LI("Item 1"), h.LI("Item 2"), h.LI("Item 3"))),
            h.DIV(
                h.H3("Column 2"),
                h.UL(
                    h.LI("Item 1"),
    ...
    def test_embedded_extraction(self):
        ...
        expect = [
            ...
            h.SPAN(
                "+00 00 000 00 0 000", attrs(model="res.company", id=1, field="phone", type="char")
            ),
        ]
    ...
    def test_save_escaped_text(self):
        ...
        view = self.env["ir.ui.view"].create(
            {"arch": '<t t-name="dummy"><p><h1>hello world</h1></p></t>', "type": "qweb"}
        )
    ...
    def test_save_only_embedded(self):
        ...
        node = html.tostring(
            h.SPAN(
                "Acme Corporation",
                attrs(
                    model="res.company", id=company_id, field="name", expression="bob", type="char"
                ),
            ),
            encoding="unicode",
        )
        ...
        replacement = ET.tostring(
            h.LI(
                h.SPAN(
                    "+12 3456789",
                    attrs(
                        model="res.company", id=1, type="char", field="phone", expression="edmund"
                    ),
                ),
                "whop whop",
            ),
...
class TestCowViewSaving(TestViewSavingCommon):
    ...
    def test_cow_cross_inherit(self):
        ...
        generic_view_arch = (
            View.browse(generic_view).with_context(load_all_views=True).get_combined_arch()
        )
        specific_view_arch = (
    ...
    def test_get_related_views_tree(self):
        ...
        self.assertEqual(
            views.mapped("key"), ["B", "I", "II"], "Should only return the specific tree"
        )
    ...
    def test_module_new_inherit_view_on_parent_already_forked(self):
        ...
        w1_specific_child_view = Website.with_context(load_all_views=True, website_id=1).viewref(
        ...
        self.assertEqual(
            generic_child_view.priority, 3, "XML update should be written on the Generic View"
        )
        self.assertEqual(
    ...
    def test_soc_complete_flow(self):
        ...
        self.assertEqual(
            created_views.website_id.id, 1, "The created view should be specific to website 1"
        )
    ...
    def test_specific_view_module_update_inherit_change(self):
        ...
        self.assertEqual(
            self.base_view.inherit_children_ids, self.inherit_view, "D should be under A"
        )
        self.assertEqual(
            specific_view.inherit_children_ids, specific_child_view, "D' should be under A'"
        )
        ...
        self.assertTrue(
            specific_child_view in base_view_2.inherit_children_ids, "D' should be under B"
        )
...
class Crawler(HttpCase):
    ...
    def test_get_switchable_related_views(self):
        ...
        event_child_view.copy(
            {"name": "Quotes", "key": "_website_event.event_right_quotes", "priority": 30}
        )
