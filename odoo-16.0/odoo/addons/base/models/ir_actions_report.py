...
class IrActionsReport(models.Model):
    ...
    model_id = fields.Many2one(
        "ir.model", string="Model", compute="_compute_model_id", search="_search_model_id"
    )
    ...
    def retrieve_attachment(self, record):
        ...
        """
        attachment_name = (
            safe_eval(self.attachment, {"object": record, "time": time}) if self.attachment else ""
        )
    ...
    def _build_wkhtmltopdf_args(
        ...
        if paperformat_id:
            ...
            if specific_paperformat_args and specific_paperformat_args.get(
                ...
                command_args.extend(
                    ["--margin-top", str(specific_paperformat_args["data-report-margin-top"])]
                )
            else:
    ...
    def _prepare_html(self, html, report_model=False):
        ...
        if not bodies:
            body = "".join(
                lxml.html.tostring(c, encoding="unicode") for c in body_parent.getchildren()
            )
            bodies.append(body)
    ...
    def _run_wkhtmltopdf(
        ...
        """
        paperformat_id = (
            self._get_report(report_ref).get_paperformat() if report_ref else self.get_paperformat()
        )
    ...
    def _get_report(self, report_ref):
        ...
        if isinstance(report_ref, models.Model):
            if report_ref._name != self._name:
                raise ValueError(
                    "Expected report of type %s, got %s" % (self._name, report_ref._name)
                )
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        res_ids_wo_stream = [
            res_id for res_id, stream_data in collected_streams.items() if not stream_data["stream"]
        ]
        ...
        if report_sudo.attachment:
            ...
            if attachment_vals_list:
                ...
                try:
                    ...
                    _logger.info(
                        "The PDF documents %r are now saved in the database", attachment_names
                    )
