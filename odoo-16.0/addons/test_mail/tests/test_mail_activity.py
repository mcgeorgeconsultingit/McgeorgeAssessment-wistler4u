...
class TestActivityCommon(TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_record = (
            cls.env["mail.test.activity"].with_context(cls._test_context).create({"name": "Test"})
        )
        # reset ctx
...
class TestActivityRights(TestActivityCommon):
    ...
    def test_activity_security_user_noaccess_automated(self):
        ...
        with patch.object(
            MailTestActivity, "check_access_rights", autospec=True, side_effect=_employee_crash
        ):
...
class TestActivityFlow(TestActivityCommon):
    def test_activity_flow_employee(self):
        with self.with_user("employee"):
            ...
            self.assertEqual(test_record.activity_ids, self.env["mail.activity"])
            self.assertEqual(
                test_record.message_ids[0].subtype_id, self.env.ref("mail.mt_activities")
            )
...
class TestActivityMixin(TestActivityCommon):
    ...
    def test_activity_mixin_archive(self):
        rec = self.test_record.with_user(self.user_employee)
        new_act = rec.activity_schedule("test_mail.mail_act_test_todo", user_id=self.user_admin.id)
    ...
    def test_activity_mixin_reschedule_user(self):
        rec = self.test_record.with_user(self.user_employee)
        rec.activity_schedule("test_mail.mail_act_test_todo", user_id=self.user_admin.id)
    ...
    def test_mail_activity_mixin_search_state_basic(self):
        ...
        with patch("odoo.addons.mail.models.mail_activity.datetime", MockedDatetime), patch(
            ...
            self.assertEqual(
                result, all_activity_mixin_record.filtered(lambda p: p.activity_state == "today")
            )
            ...
            self.assertEqual(
                result,
                all_activity_mixin_record.filtered(lambda p: p.activity_state not in ("today",)),
            )

            result = self.env["mail.test.activity"].search([("activity_state", "=", False)])
            self.assertTrue(
                len(result) >= 3, "There is more than 3 records without an activity schedule on it"
            )
            self.assertEqual(
                result, all_activity_mixin_record.filtered(lambda p: not p.activity_state)
            )
            ...
            self.assertEqual(
                result,
                all_activity_mixin_record.filtered(lambda p: p.activity_state in ("today", False)),
            )
...
class TestReadProgressBar(tests.TransactionCase):
    ...
    def test_week_grouping(self):
        ...
        model.create({"date": "2021-05-09", "name": "Things we said today",}).activity_schedule(
            "test_mail.mail_act_test_todo",
            summary="Make another test asap",
            date_deadline=fields.Date.context_today(model),
        )
        model.create({"date": "2021-05-16", "name": "Tomorrow Never Knows",}).activity_schedule(
            "test_mail.mail_act_test_todo",
