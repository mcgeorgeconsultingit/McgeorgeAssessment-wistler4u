...
class Goal(models.Model):
    ...
    definition_id = fields.Many2one(
        "gamification.goal.definition", string="Goal Definition", required=True, ondelete="cascade"
    )
        ...
        for goal in self:
            if goal.definition_condition == "higher":
                if goal.current >= goal.target_goal:
                    ...
                    goal.completeness = (
                        round(100.0 * goal.current / goal.target_goal, 2) if goal.target_goal else 0
                    )
    ...
    def _check_remind_delay(self):
        ...
        body_html = self.env.ref("gamification.email_template_goal_reminder")._render_field(
            "body_html", self.ids, compute_lang=True
        )[self.id]
        self.message_notify(
            body=body_html,
    ...
    def _get_write_values(self, new_value):
        ...
        if (self.definition_id.condition == "higher" and new_value >= self.target_goal) or (
                    ...
                    for (start_date, end_date), query_goals in subqueries.items():
                        ...
                        if definition.computation_mode == "count":
                            ...
                            if field_name == "id":
                                ...
                                user_values = Obj.read_group(
                                    subquery_domain, fields=[field_name], groupby=[field_name]
                                )
                        ...
                        for goal in [g for g in goals if g.id in query_goals]:
                            for user_value in user_values:
                                queried_value = (
                                    field_name in user_value and user_value[field_name] or False
                                )
                                if (
