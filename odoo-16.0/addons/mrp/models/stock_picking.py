...
class StockPickingType(models.Model):
    ...
    code = fields.Selection(
        selection_add=[("mrp_operation", "Manufacturing")], ondelete={"mrp_operation": "cascade"}
    )
    ...
    def _get_mo_count(self):
        ...
        for field in domains:
            ...
            count = {
                x["picking_type_id"] and x["picking_type_id"][0]: x["picking_type_id_count"]
                for x in data
            }
...
class StockPicking(models.Model):
    ...
    def _less_quantities_than_expected_add_documents(self, moves, documents):
        documents = super(StockPicking, self)._less_quantities_than_expected_add_documents(
            moves, documents
        )
