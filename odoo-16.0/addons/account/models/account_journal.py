...
class AccountJournal(models.Model):
    ...
    payment_sequence = fields.Boolean(
        ...
        default="_compute_payment_sequence",
    )
    ...
    def _check_type_default_account_id_type(self):
        for journal in self:
            if journal.type in ("sale", "purchase") and journal.default_account_id.account_type in (
                "asset_receivable",
                "liability_payable",
            ):
                raise ValidationError(
    ...
    def _check_payment_method_line_ids_multiplicity(self):
        ...
        self.flush_model(
            ["inbound_payment_method_line_ids", "outbound_payment_method_line_ids", "company_id"]
        )
        ...
        if journal_type in ("bank", "cash"):
            ...
            if journal_type == "bank":
                ...
                liquidity_account_prefix = (
                    company.cash_account_code_prefix or company.bank_account_code_prefix or ""
                )
            ...
            if not has_liquidity_accounts:
                default_account_code = self.env["account.account"]._search_new_account_code(
    ...
    def create(self, vals_list):
        ...
        journals = super(AccountJournal, self.with_context(mail_create_nolog=True)).create(
            vals_list
        )
    ...
    def _get_journal_bank_account_balance(self, domain=None):
        ...
        journal_currency = (
            self.currency_id if self.currency_id and self.currency_id != company_currency else False
        )
    ...
    def _is_payment_method_available(self, payment_method_code):
        ...
        return self.filtered_domain(
            self.env["account.payment.method"]._get_payment_method_domain(payment_method_code)
        )
