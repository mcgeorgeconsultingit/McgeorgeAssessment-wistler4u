...
class TestProductConfiguratorUi(HttpCase, TestProductConfiguratorCommon):
    ...
    def test_02_product_configurator_advanced(self):
        ...
        product_attributes = self.env["product.attribute"].create(
            [
                ...
                {"name": "PA4", "display_type": "select", "create_variant": "no_variant"},
                {"name": "PA5", "display_type": "select", "create_variant": "no_variant"},
                {"name": "PA7", "display_type": "color", "create_variant": "no_variant"},
                {"name": "PA8", "display_type": "radio", "create_variant": "no_variant"},
            ]
        )

        self.env["product.attribute.value"].create(
            [
                {"name": "PAV" + str(i), "is_custom": i == 9, "attribute_id": product_attribute.id}
                for i in range(1, 11)
    ...
    def test_04_product_configurator_single_custom_value(self):
        ...
        self.start_tour(
            "/web", "sale_product_configurator_single_custom_attribute_tour", login="salesman"
        )
