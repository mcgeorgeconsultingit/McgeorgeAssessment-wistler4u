...
class TestAccountMoveOutInvoiceOnchanges(AccountTestInvoicingCommon):
    ...
    def test_out_invoice_line_onchange_rounding_price_subtotal_1(self):
        ...
        self.assertTrue(
            decimal_precision, "Decimal precision '%s' not found" % decimal_precision_name
        )
    ...
    def test_out_invoice_line_onchange_analytic_2(self):
        ...
        analytic_account = self.env["account.analytic.account"].create(
            {"name": "test_analytic_account1", "plan_id": analytic_plan.id, "code": "TEST1"}
        )
                ...
                "invoice_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "account_id": self.company_data["default_account_revenue"].id,
                            "tax_ids": [
                                (6, 0, (fixed_tax_price_include_1 + fixed_tax_price_include_2).ids)
                            ],
                        },
                    )
                ],
    ...
    def test_out_invoice_create_child_partner(self):
        ...
        self.assertEqual(
            partner_a_child.id, move.partner_id.id, "Keep child partner on the account move record"
        )
        self.assertEqual(
            self.partner_a.id,
    ...
    def test_out_invoice_post_1(self):
        ...
        with patch.object(fields.Date, "today", lambda *args, **kwargs: frozen_today), patch.object(
        ...
        move.write(
            {
                "line_ids": [
                    (
                        1,
                        move.line_ids.filtered(
                            lambda line: line.tax_line_id.id == self.tax_line_vals_1["tax_line_id"]
                        ).id,
                        {
                            "amount_currency": -200.0,
                        },
    ...
    def test_out_invoice_change_period_accrual_1(self):
        ...
        wizard = (
            ...
            .with_context(active_model="account.move.line", active_ids=move.invoice_line_ids.ids)
            .create(
                {
        ...
        accrual_lines = (
            self.env["account.move"].browse(wizard_res["domain"][0][2]).line_ids.sorted("date")
        )
        self.assertRecordValues(
            accrual_lines,
            [
        ...
        invoice = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    Command.create(
                        {"name": "line1", "price_unit": 38.73553, "quantity": 38.0, "tax_ids": []}
                    ),
                    Command.create(
                        {
                            "name": "line2",
                    ...
                    Command.create(
                        {"name": "line3", "price_unit": 49.45257, "quantity": 35.0, "tax_ids": []}
                    ),
                    Command.create(
                        {"name": "line4", "price_unit": 17.99000, "quantity": 1.0, "tax_ids": []}
                    ),
                ],
            }
        )
