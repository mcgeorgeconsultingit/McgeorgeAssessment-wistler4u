...
class Mailing(models.Model):
    ...
    # mailing options
    mailing_type = fields.Selection(selection_add=[("sms", "SMS")], ondelete={"sms": "set default"})
    ...
    def _compute_medium_id(self):
        ...
        for mailing in self:
            if mailing.mailing_type == "sms" and (
                not mailing.medium_id or mailing.medium_id == self.env.ref("utm.utm_medium_email")
            ):
    ...
    def _action_view_traces_filtered(self, view_filter):
        ...
        if self.mailing_type == "sms":
            action["views"] = [
                (self.env.ref("mass_mailing_sms.mailing_trace_view_tree_sms").id, "tree"),
    ...
    def _get_opt_out_list_sms(self):
        ...
        if hasattr(self.env[self.mailing_model_name], "_mailing_get_opt_out_list_sms"):
            ...
            _logger.info(
                "Mass SMS %s targets %s: optout: %s contacts", self, target._name, len(opt_out)
            )
        else:
    ...
    def _get_seen_list_sms(self):
        ...
        _logger.info(
            "Mass SMS %s targets %s: already reached %s SMS", self, target._name, len(seen_list)
        )
    ...
    def _get_default_mailing_domain(self):
        ...
        if (
            ...
            and "phone_sanitized_blacklisted" in self.env[self.mailing_model_name]._fields
        ):
