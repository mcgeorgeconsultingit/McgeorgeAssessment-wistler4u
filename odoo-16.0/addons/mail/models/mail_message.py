...
class Message(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    link_preview_ids = fields.One2many(
        "mail.link.preview", "message_id", string="Link Previews", groups="base.group_erp_manager"
    )
    # characteristics
    ...
    author_avatar = fields.Binary(
        "Author's avatar", related="author_id.avatar_128", depends=["author_id"], readonly=False
    )
    ...
    message_id = fields.Char(
        "Message-Id", help="Message unique identifier", index="btree", readonly=1, copy=False
    )
    ...
    reaction_ids = fields.One2many(
        "mail.message.reaction", "message_id", string="Reactions", groups="base.group_system"
    )
    ...
    def _compute_is_current_user_or_guest_author(self):
        ...
        for message in self:
            if not user._is_public() and (
                ...
                message.author_guest_id and message.author_guest_id == self.env.context.get("guest")
            ):
                message.is_current_user_or_guest_author = True
            else:
    ...
    def _find_allowed_model_wise(self, doc_model, doc_dict):
        ...
        allowed_doc_ids = (
            self.env[doc_model].with_context(active_test=False).search([("id", "in", doc_ids)]).ids
        )
        return set(
            [
                message_id
    ...
    def check_access_rule(self, operation):
        ...
        if operation == "read":
            ...
            self._cr.execute(
                ...
                WHERE m.id = ANY (%%(ids)s)"""
                % self._table,
                dict(pid=self.env.user.partner_id.id, uid=self.env.user.id, ids=self.ids),
            )
            for (
                mid,
                rmod,
    ...
    def create(self, values_list):
        ...
        for values in values_list:
            if "email_from" not in values:  # needed to compute reply_to
                _author_id, email_from = self.env["mail.thread"]._message_compute_author(
    ...
    def _read_group_raw(
        ...
        if not self.env.is_admin():
            raise AccessError(
                _("Only administrators are allowed to use grouped read on message model")
            )
            ...
            reaction_groups = [
                {
                    ...
                    "count": len(reactions),
                    "guests": [
                        {"id": guest.id, "name": guest.name} for guest in reactions.guest_id
                    ],
                    "message": {"id": message_sudo.id},
                    "partners": [
                        {"id": partner.id, "name": partner.name} for partner in reactions.partner_id
                    ],
                }
