...
class GoogleSync(models.AbstractModel):
    ...
    def _google_error_handling(self, http_error):
        ...
        if http_error.response.status_code in (403, 400):
            ...
            if not self.exists():
                reason = "Google gave the following explanation: %s" % response["error"].get(
                    "message"
                )
                error_log = (
            ...
            if self._name == "calendar.event":
                start = (
                    self.start and self.start.strftime("%Y-%m-%d at %H:%M") or _("undefined time")
                )
                event_ids = self.id
                name = self.name
                ...
                start = (
                    event.start and event.start.strftime("%Y-%m-%d at %H:%M") or _("undefined time")
                )
                event_ids = _(
                ...
                error_log = "Error while syncing recurrence [{id} - {name} - {rrule}]: ".format(
            ...
            if (
                ...
                reason = _(
                    "Google gave the following explanation: %s", response["error"].get("message")
                )
    ...
    def _get_records_to_sync(self, full_sync=False):
        ...
        if not full_sync:
            is_active_clause = (
                (self._active_name, "=", True) if self._active_name else expression.TRUE_LEAF
            )
    ...
    def _get_sync_partner(self, emails):
        ...
        partners = [
            user_partner for user_partner in user_partners if user_partner.type != "private"
        ]
        remaining = [
            email
        ...
        if remaining:
            partners += self.env["mail.thread"]._mail_find_partner_from_emails(
                remaining, records=self, force_create=True, extra_domain=[("type", "!=", "private")]
            )
        return partners
