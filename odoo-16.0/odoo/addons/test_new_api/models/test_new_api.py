...
class Category(models.Model):
    ...
    discussions = fields.Many2many(
        "test_new_api.discussion", "test_new_api_discussion_category", "category", "discussion"
    )
...
class Discussion(models.Model):
    ...
    categories = fields.Many2many(
        "test_new_api.category", "test_new_api_discussion_category", "discussion", "category"
    )
    ...
    very_important_messages = fields.One2many(
        "test_new_api.message", "discussion", domain=lambda self: self._domain_very_important()
    )
...
class Message(models.Model):
    ...
    author_partner = fields.Many2one(
        "res.partner", compute="_compute_author_partner", search="_search_author_partner"
    )
    ...
    def _search_size(self, operator, value):
        ...
        query = """SELECT id FROM "%s" WHERE char_length("body") %s %%s""" % (self._table, operator)
...
class EmailMessage(models.Model):
    ...
    message = fields.Many2one("test_new_api.message", "Message", required=True, ondelete="cascade")
...
class ModelImage(models.Model):
    ...
    image_512 = fields.Image(
        "Image 512", related="image", max_width=512, max_height=512, store=True, readonly=False
    )
    image_256 = fields.Image(
        "Image 256", related="image", max_width=256, max_height=256, store=False, readonly=False
    )
...
class PrecomputeCombo(models.Model):
    ...
    setter = fields.Char(
        compute="_compute_setter", precompute=True, inverse="_inverse_setter", store=True
    )
