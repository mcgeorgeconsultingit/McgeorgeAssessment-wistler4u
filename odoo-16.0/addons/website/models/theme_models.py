...
class ThemeAsset(models.Model):
    ...
    copy_ids = fields.One2many(
        "ir.asset", "theme_template_id", "Assets using a copy of me", copy=False, readonly=True
    )
...
class ThemeView(models.Model):
    ...
    copy_ids = fields.One2many(
        "ir.ui.view", "theme_template_id", "Views using a copy of me", copy=False, readonly=True
    )
    ...
    def _convert_to_base_model(self, website, **kwargs):
        ...
        if inherit and inherit.website_id != website:
            website_specific_inherit = (
                ...
                .search([("key", "=", inherit.key), ("website_id", "=", website.id)], limit=1)
            )
...
class ThemeMenu(models.Model):
    ...
    copy_ids = fields.One2many(
        "website.menu", "theme_template_id", "Menu using a copy of me", copy=False, readonly=True
    )
...
class ThemePage(models.Model):
    ...
    copy_ids = fields.One2many(
        "website.page", "theme_template_id", "Page using a copy of me", copy=False, readonly=True
    )
...
class Theme(models.AbstractModel):
    ...
    def _toggle_asset(self, key, active):
        ...
        if obj:
            ...
            has_specific = (
                obj.key
                and Asset.search_count([("key", "=", obj.key), ("website_id", "=", website.id)])
                >= 1
            )
    ...
    def _toggle_view(self, xml_id, active):
        ...
        if obj._name == "theme.ir.ui.view":
            ...
            has_specific = (
                obj.key
                and View.search_count([("key", "=", obj.key), ("website_id", "=", website.id)]) >= 1
            )
            if not has_specific and active == obj.active:
                return
        obj.write({"active": active})
