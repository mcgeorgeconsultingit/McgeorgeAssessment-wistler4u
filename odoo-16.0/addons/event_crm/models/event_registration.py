...
class EventRegistration(models.Model):
    ...
    lead_count = fields.Integer("# Leads", compute="_compute_lead_count", compute_sudo=True)
    ...
    def _update_leads(self, new_vals, lead_tracked_vals):
        ...
        for registration in self:
            ...
            upd_contact_fields = [
                field for field in self._get_lead_contact_fields() if field in new_vals.keys()
            ]
            ...
            upd_description_fields = [
                field for field in self._get_lead_description_fields() if field in new_vals.keys()
            ]
        ...
        for lead in leads_order:
            ...
            if new_vals.get("partner_id"):
                ...
                if not lead.partner_id:
                    lead_values["description"] = lead.registration_ids._get_lead_description(
    ...
    def _get_lead_tracked_values(self):
        ...
        tracked_fields = list(
            set(self._get_lead_contact_fields()) or set(self._get_lead_description_fields())
        )
        return dict(
            (
                registration.id,
    ...
    def _get_lead_grouping(self, rules, rule_to_new_regs):
        ...
        return dict(
            (
                rule,
                [
                    (False, event, (registrations & rule_to_new_regs[rule]).sorted("id"))
