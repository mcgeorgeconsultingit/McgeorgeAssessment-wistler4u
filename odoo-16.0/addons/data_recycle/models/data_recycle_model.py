...
class DataRecycleModel(models.Model):
    ...
    def _check_recycle_action(self):
        for model in self:
            if model.recycle_action == "archive" and "active" not in self.env[model.res_model_name]:
                raise UserError(
                    _("This model doesn't manage archived records. Only deletion is possible.")
                )
    ...
    def _compute_records_to_recycle_count(self):
        count_data = self.env["data_recycle.record"]._read_group(
            [("recycle_model_id", "in", self.ids)], ["recycle_model_id"], ["recycle_model_id"]
        )
        ...
        for recycle_model in self:
            ...
            if (
                ...
                rule_domain = expression.AND(
                    [rule_domain, [(recycle_model.time_field_id.name, "<=", now - delta)]]
                )
    ...
    def open_records(self):
        ...
        action["context"] = dict(
            ast.literal_eval(action.get("context")), searchpanel_default_recycle_model_id=self.id
        )
        return action
