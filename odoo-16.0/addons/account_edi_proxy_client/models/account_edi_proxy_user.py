...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _get_demo_state(self):
        demo_state = (
            self.env["ir.config_parameter"].sudo().get_param("account_edi_proxy_client.demo", False)
        )
        return (
            "prod" if demo_state in ["prod", False] else "test" if demo_state == "test" else "demo"
        )
    ...
    def _make_request(self, url, params=False):
        ...
        if "error" in response:
            ...
            if response["error"]["code"] == 404:
                message = _(
                    "The url that this service tried to contact does not exist. The url was %r", url
                )
    ...
    def _decrypt_data(self, data, symmetric_key):
        ...
        private_key = serialization.load_pem_private_key(
            base64.b64decode(self.sudo().private_key), password=None, backend=default_backend()
        )
