...
class TestWebsitePriceList(TransactionCase):
    def setUp(self):
        ...
        (self.env["product.pricelist"].search([]) - self.env.ref("product.list0")).write(
            {"website_id": False, "active": False}
        )
...
class TestWebsitePriceListMultiCompany(TransactionCaseWithUserDemo):
    def setUp(self):
        ...
        irp1 = (
            ...
            ._get("property_product_pricelist", "res.partner", self.demo_user.partner_id.id)
        )
        irp2 = (
            self.env["ir.property"]
