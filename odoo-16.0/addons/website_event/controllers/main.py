...
class WebsiteEventController(http.Controller):
    ...
    def events(self, page=1, **searches):
        ...
        pager = website.pager(
            url="/event", url_args=searches, total=event_count, page=page, step=step, scope=5
        )
        ...
        for registration_values in registration_data:
            ...
            if not registration_values.get("partner_id") and visitor_sudo.partner_id:
                ...
                registration_values["partner_id"] = (
                    False if request.env.user._is_public() else request.env.user.partner_id.id
                )
            ...
            .search(
                [
                    (
                        "id",
                        "in",
                        [str(registration_id) for registration_id in registration_ids.split(",")],
                    ),
                    ("event_id", "=", event.id),
    ...
    def get_formated_date(self, event):
        ...
        month = babel.dates.get_month_names("abbreviated", locale=get_lang(event.env).code)[
            start_date.month
        ]
        return ("%s %s%s") % (
            month,
