...
class TestProjectFlow(TestProjectCommon, MailCommon):
    def test_project_process_project_manager_duplicate(self):
        ...
        dogs = pigs.copy()
        self.assertEqual(
            len(dogs.tasks), 2, "project: duplicating a project must duplicate its tasks"
        )
    ...
    def test_task_process_without_stage(self):
        ...
        self.assertEqual(
            len(task), 1, "project: message_process: a new project.task should have been created"
        )
    ...
    def test_subtask_process(self):
        ...
        self.assertEqual(parent_task.subtask_count, 2, "Parent task should have 2 children")
    ...
    def test_rating(self):
        ...
        self.assertEqual(
            first_task.rating_count, 2, "Task should have two ratings associated with it"
        )
    ...
    def test_send_rating_review(self):
        ...
        tasks = (
            self.env["project.task"]
            .with_context(mail_create_nolog=True, default_project_id=self.project_goats.id)
            .create(
                [
