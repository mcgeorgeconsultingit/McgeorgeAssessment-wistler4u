...
class TestProjectSharingCommon(TestProjectCommon):
    ...
    def get_project_sharing_form_view(self, record, with_user=None):
        return Form(
            record.with_user(with_user or self.env.user), view=self.project_sharing_form_view_xml_id
        )
...
class TestProjectSharing(TestProjectSharingCommon):
    ...
    def test_project_sharing_access(self):
        ...
        self.assertTrue(
            self.project_portal.with_user(self.user_projectuser)._check_project_sharing_access(),
    ...
    def test_edit_task_in_project_sharing(self):
        ...
        with self.assertRaises(
            ...
            with self.get_project_sharing_form_view(
                self.task_cow.with_context(
                    {"tracking_disable": True, "default_project_id": self.project_cows.id}
                ),
                self.user_portal,
            ) as form:
                form.name = "Test"
        ...
        # 3.1) Try to change the project of the new task with this user.
        with self.assertRaises(
            AssertionError, msg="Should not accept the portal user changes the project of the task."
        ):
