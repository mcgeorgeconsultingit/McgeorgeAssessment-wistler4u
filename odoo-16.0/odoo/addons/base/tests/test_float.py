...
class TestFloatPrecision(TransactionCase):
    ...
    def test_rounding_02(self):
        ...
        def try_zero(amount, expected):
            self.assertEqual(
                currency.is_zero(amount), expected, "Rounding error: %s should be zero!" % amount
            )
    ...
    def test_rounding_03(self):
        ...
        def try_round(
            amount, expected, precision_rounding=None, method="HALF-UP"
        ):  # pylint: disable=function-redefined
            value = float_round(
                amount, precision_rounding=precision_rounding, rounding_method=method
            )
    ...
    def test_rounding_04(self):
        ...
        def try_roundtrip(value, expected, date):
            rate = currency_rate.create({"name": date, "rate": value, "currency_id": currency.id})
            self.assertEqual(
                rate.rate,
                expected,
    ...
    def test_float_split_05(self):
        ...
        def try_split(value, expected, split_fun, rounding=None):
            ...
            self.assertEqual(
                result, expected, "Split error: got %s, expected %s" % (result, expected)
            )
    ...
    def test_amount_to_text_10(self):
        ...
        self.assertNotEqual(
            amount_test, amount_target, "Amount in text should not depend on float representation"
        )
