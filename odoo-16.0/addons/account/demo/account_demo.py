        ...
        return (
            "account.move",
            {
                f"{cid}_demo_invoice_1": {
                    ...
                    "invoice_line_ids": [
                        Command.create(
                            {"product_id": ref("product.consu_delivery_02").id, "quantity": 5}
                        ),
                        Command.create(
                            {"product_id": ref("product.consu_delivery_03").id, "quantity": 5}
                        ),
                    ],
                },
                f"{cid}_demo_invoice_2": {
                    ...
                    "invoice_line_ids": [
                        ...
                        Command.create(
                            {"product_id": ref("product.consu_delivery_01").id, "quantity": 20}
                        ),
                    ],
                },
                f"{cid}_demo_invoice_3": {
                    ...
                    "invoice_line_ids": [
                        Command.create(
                            {"product_id": ref("product.consu_delivery_01").id, "quantity": 5}
                        ),
                        Command.create(
                            {"product_id": ref("product.consu_delivery_03").id, "quantity": 5}
                        ),
        ...
        return (
            "mail.message",
            {
                f"{cid}_mail_message_in_invoice_1": {
                    ...
                    "attachment_ids": [
                        Command.set([ref(f"account.{cid}_ir_attachment_in_invoice_1").id])
                    ],
                },
                f"{cid}_mail_message_in_invoice_2": {
                    ...
                    "author_id": ref("base.partner_demo").id,
                    "attachment_ids": [
                        Command.set([ref(f"account.{cid}_ir_attachment_in_invoice_2").id])
                    ],
                },
            },
        )
        ...
        return (
            "mail.activity",
            {
                f"{cid}_invoice_activity_1": {
                    ...
                    "date_deadline": (fields.Datetime.today() + relativedelta(days=5)).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
    ...
    def _get_demo_account(self, xml_id, account_type, company):
        ...
        return (
            ...
            or self.env["account.account"].search(
                [("account_type", "=", account_type), ("company_id", "=", company.id)], limit=1
            )
