...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def test_00_purchase_order_flow(self):
        ...
        if self.partner_a in self.product_id_2.seller_ids.partner_id:
            id_to_remove = (
                self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a).ids[
                    0
                ]
                if self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a)
                else False
            )
            if id_to_remove:
        ...
        self.assertFalse(
            self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a),
        ...
        self.assertEqual(
            self.po.invoice_status, "no", 'Purchase: PO invoice_status should be "Not purchased"'
        )
        self.assertEqual(
