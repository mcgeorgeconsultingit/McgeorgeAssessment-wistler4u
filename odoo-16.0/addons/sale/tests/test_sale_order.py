...
class TestSaleOrder(SaleCommon):
    ...
    def test_state_changes(self):
        ...
        self.sale_order.action_confirm()
        self.assertEqual(
            self.sale_order.state, "done", "The order wasn't automatically locked at confirmation."
        )
        with self.assertRaises(UserError):
            self.sale_order.action_confirm()
...
class TestSalesTeam(SaleCommon):
    ...
    def test_assign_sales_team_from_partner_team(self):
        ...
        self.assertEqual(
            sale_order.team_id.id, self.sale_team_2.id, "Should assign to team of partner"
        )
