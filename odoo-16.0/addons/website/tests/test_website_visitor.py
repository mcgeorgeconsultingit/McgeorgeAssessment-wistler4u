...
class WebsiteVisitorTests(MockVisitor, HttpCaseWithUserDemo):
    ...
    def test_visitor_creation_on_tracked_page(self):
        ...
        self.assertEqual(
            len(visitor_admin.website_track_ids), 5, "There should be 5 tracked page for the admin"
        )
        ...
        self.assertEqual(
            visitor_portal.visit_count, 2, "There should be 2 visits for the portal user"
        )
    ...
    def _test_unlink_old_visitors(self, inactive_visitors, active_visitors):
        ...
        if active_visitor_ids:
            ...
            self.assertEqual(
                active_visitors, WebsiteVisitor.search([("id", "in", active_visitor_ids)])
            )
    ...
    def _prepare_linked_visitor_data(self):
        return {
            ...
            "website_track_ids": [
                (0, 0, {"page_id": self.tracked_page_2.id, "url": self.tracked_page_2.url})
            ],
        }
    ...
    def test_merge_partner_with_visitor_both(self):
        ...
        # |    |      5013266 |    5013266 |
        self.assertTrue(
            visitor_demo.partner_id.id == int(visitor_demo.access_token) == self.partner_demo.id
        )
        self.assertTrue(
            visitor_admin.partner_id.id == int(visitor_admin.access_token) == self.partner_admin.id
        )
        ...
        # Track check
        self.assertEqual(
            visitor_admin.website_track_ids.sorted("url").mapped("url"), ["/admin", "/demo"]
        )
    ...
    def test_merge_partner_with_visitor_single(self):
        ...
        # |    |      5013266 |    5013266 |
        self.assertTrue(
            visitor_demo.partner_id.id == int(visitor_demo.access_token) == self.partner_admin.id,
