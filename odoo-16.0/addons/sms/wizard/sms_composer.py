...
class SendSMS(models.TransientModel):
    ...
    recipient_single_number = fields.Char(
        "Stored Recipient Number", compute="_compute_recipient_single", compute_sudo=False
    )
        ...
        for composer in self:
            ...
            if records and issubclass(type(records), self.pool["mail.thread"]):
                ...
                composer.recipient_invalid_count = (
                    0 if (composer.sanitized_numbers or composer.composition_mode == "mass") else 1
                )
    ...
    def _compute_recipient_single(self):
        for composer in self:
            ...
            composer.recipient_single_description = (
                res[records.id]["partner"].name or records._sms_get_default_partners().display_name
            )
    ...
    def _compute_recipient_single_valid(self):
        for composer in self:
            ...
            if value:
                records = composer._get_records()
                sanitized = phone_validation.phone_sanitize_numbers_w_record([value], records)[
                    value
                ]["sanitized"]
    ...
    def _compute_sanitized_numbers(self):
        for composer in self:
            if composer.numbers:
                ...
                if invalid_numbers:
                    raise UserError(
                        _("Following numbers are not correctly encoded: %s", repr(invalid_numbers))
                    )
    ...
    def _get_composer_values(self, composition_mode, res_model, res_id, body, template_id):
        ...
        if composition_mode == "comment":
            if not body and template_id and res_id:
                ...
                result["body"] = template._render_template(template.body, res_model, [res_id])[
                    res_id
                ]
            elif template_id:
                template = self.env["sms.template"].browse(template_id)
