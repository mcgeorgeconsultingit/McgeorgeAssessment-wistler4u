...
class TestEventSale(TestEventSaleCommon):
    ...
    def test_event_crm_sale(self):
        ...
        self.assertEqual(
            editor.event_registration_ids.sale_order_line_id, ticket1_line | ticket2_line
        )
        ...
        self.assertEqual(set(ticket1_new_reg.mapped("name")), set(["ManualEntry1", "ManualEntry2"]))
        self.assertEqual(
            set(ticket1_new_reg.mapped("email")),
            set(["manual.email.1@test.example.com", "manual.email.2@test.example.com"]),
        )
        self.assertEqual(
            set(ticket1_new_reg.mapped("phone")), set(["+32456111111", self.event_customer.phone])
        )
        self.assertEqual(
            set(ticket1_new_reg.mapped("mobile")), set(["+32456222222", self.event_customer.mobile])
        )
        for field in ["name", "email", "phone", "mobile"]:
    ...
    def test_event_sale_free_confirm(self):
        ...
        # Limiting seats
        self.event_0.write({"auto_confirm": False, "seats_limited": True, "seats_max": 5})
    ...
    def test_event_sale_free_full_event_no_confirm(self):
        ...
        self.event_0.write({"auto_confirm": False, "seats_limited": True, "seats_max": 2})
