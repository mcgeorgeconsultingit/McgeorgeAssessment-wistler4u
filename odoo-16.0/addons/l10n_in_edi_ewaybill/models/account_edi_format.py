...
class AccountEdiFormat(models.Model):
    ...
    def _check_move_configuration(self, move):
        ...
        if not move.l10n_in_mode:
            ...
            if not move.l10n_in_vehicle_no and move.l10n_in_vehicle_type:
                error_message.append(
                    _("- Vehicle Number and Type is required when Transportation Mode is By Road")
                )
        ...
        if error_message:
            error_message.insert(
                0, _("The following information are missing on the invoice (see eWayBill tab):")
            )
        if base == "irn":
        ...
        if not goods_line_is_available:
            error_message.append(
                _('You need at least one product having "Product Type" as stockable or consumable.')
            )
        if error_message:
                ...
                error_message = "<br/>".join(
                    [
                        ...
                        % (
                            ...
                            html_escape(
                                e.get("message")
                                or self._l10n_in_edi_ewaybill_get_error_message(e.get("code"))
                            ),
                        )
                        for e in error
                    ]
    ...
    def _get_l10n_in_edi_ewaybill_line_details(self, line, line_tax_details, sign):
        ...
        if tax_details_by_code.get("igst_rate"):
            line_details.update(
                {"igstRate": self._l10n_in_round_value(tax_details_by_code["igst_rate"])}
            )
        else:
            line_details.update(
                {
        ...
        if tax_details_by_code.get("cess_rate"):
            line_details.update(
                {"cessRate": self._l10n_in_round_value(tax_details_by_code.get("cess_rate"))}
            )
        return line_details
    ...
    def _l10n_in_edi_irn_ewaybill_generate(self, company, json_payload):
        ...
        return self._l10n_in_edi_connect_to_server(
            company, url_path="/iap/l10n_in_edi/1/generate_ewaybill_by_irn", params=params
        )
