...
class AccountFrFec(models.TransientModel):
    ...
    def _get_company_legal_data(self, company):
        ...
        * For non-french companies -> returns the complete vat number
        ...
        is_dom_tom = company.account_fiscal_country_id.code in dom_tom_group.country_ids.mapped(
            "code"
        )
    ...
    def generate_fec(self):
        ...
        unaffected_earnings_line = (
            True  # used to make sure that we add the unaffected earning initial balance only once
        )
        ...
        if (
            ...
            unaffected_earnings_account = self.env["account.account"].search(
                [("account_type", "=", "equity_unaffected"), ("company_id", "=", company.id)],
                limit=1,
            )
            if unaffected_earnings_account:
