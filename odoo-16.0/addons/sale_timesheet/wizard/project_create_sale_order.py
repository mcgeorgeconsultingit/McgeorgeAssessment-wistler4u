...
class ProjectCreateSalesOrder(models.TransientModel):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Customer", required=True, help="Customer of the sales order"
    )
    ...
    def _compute_info_invoice(self):
        for line in self:
            ...
            unit_amount = (
                round(sum(t.get("unit_amount", 0) for t in timesheet), 2) if timesheet else 0
            )
            if not unit_amount:
    ...
    def _make_billable_at_employee_rate(self, sale_order):
        ...
        task_id = (
            ...
            .search([("project_id", "=", self.project_id.id)], order="create_date DESC", limit=1)
            .id
        )
        project_id = self.project_id.id
        ...
        for map_entry in map_entries:
            ...
            # Avoid recomputing price_unit
            self.env.remove_to_compute(
                self.env["sale.order.line"]._fields["price_unit"], map_entry.sale_line_id
            )
        return map_entries
...
class ProjectCreateSalesOrderLine(models.TransientModel):
    ...
    employee_id = fields.Many2one(
        "hr.employee", string="Employee", help="Employee that has timesheets on the project."
    )
