...
class BaseModel(models.AbstractModel):
    ...
    def _valid_field_parameter(self, field, name):
        ...
        return name == "tracking" and self._abstract or super()._valid_field_parameter(field, name)
    ...
    def _notify_get_reply_to(self, default=None):
        ...
        if alias_domain:
            if model and res_ids:
                ...
                for alias in mail_aliases:
                    result_email.setdefault(
                        alias.alias_parent_thread_id, "%s@%s" % (alias.alias_name, alias_domain)
                    )
            ...
            if left_ids:
                ...
                if catchall:
                    result_email.update(
                        dict((rid, "%s@%s" % (catchall, alias_domain)) for rid in left_ids)
                    )
