        ...
        for move in self:
            ...
            for line in self.invoice_line_ids.filtered(lambda l: l.display_type == "product"):
                if not line.tax_ids or len(line.tax_ids) > 1:
                    move_errors.append(
                        _("On line %s, you must select one and only one tax.", line.name)
                    )
                else:
    ...
    def _l10n_ke_cu_lines_messages(self):
        ...
        for line in lines:
            ...
            candidate_vals_list = sorted(
                candidate_vals_list, key=lambda x: x.price_unit * x.quantity, reverse=True
            )
            ...
            for candidate in candidate_vals_list:
                ...
                if line_to_discount >= still_to_discount:
                    ...
                    rest_to_discount = abs(
                        (line_to_discount / (candidate.price_unit * candidate.quantity)) * 100.0
                    )
        ...
        return {
            ...
            "params": {
                "invoices": {
                    move.id: {
                        "messages": json.dumps(
                            [msg.decode("cp1251") for msg in move._l10n_ke_get_cu_messages()]
                        ),
