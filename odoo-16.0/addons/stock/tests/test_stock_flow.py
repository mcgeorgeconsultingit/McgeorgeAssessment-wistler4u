...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        lot2_productC = LotObj.create(
            {"name": "C Lot 2", "product_id": self.productC.id, "company_id": self.env.company.id}
        )
        ...
        total_qty = sum(
            self.StockPackObj.search([("move_id", "in", picking_in.move_ids.ids)]).mapped(
                "qty_done"
            )
        )
        ...
        c_done_qty = self.MoveObj.search(
            [("product_id", "=", self.productC.id), ("picking_id", "=", picking_in.id)], limit=1
        ).product_uom_qty
        self.assertEqual(
            c_done_qty,
            7.0,
        ...
        d_done_qty = self.MoveObj.search(
            [("product_id", "=", self.productD.id), ("picking_id", "=", picking_in.id)], limit=1
        ).product_uom_qty
        self.assertEqual(
            d_done_qty,
            7.0,
        ...
        moves = self.MoveObj.search(
            [("product_id", "=", self.productC.id), ("picking_id", "=", back_order_in.id)]
        )
        product_c_qty = [move.product_uom_qty for move in moves]
        self.assertEqual(
            sum(product_c_qty),
            3.0,
            "Wrong move quantity of product C (%s found instead of 3)" % (product_c_qty),
        )
        # Check back order should be created with 8 quantity of product D.
        moves = self.MoveObj.search(
            [("product_id", "=", self.productD.id), ("picking_id", "=", back_order_in.id)]
        )
        product_d_qty = [move.product_uom_qty for move in moves]
        self.assertEqual(
            sum(product_d_qty),
            8.0,
            "Wrong move quantity of product D (%s found instead of 8)" % (product_d_qty),
        )
        ...
        aval_a_qty = self.MoveObj.search(
            [("product_id", "=", self.productA.id), ("picking_id", "=", picking_out.id)], limit=1
        ...
        aval_b_qty = self.MoveObj.search(
            [("product_id", "=", self.productB.id), ("picking_id", "=", picking_out.id)], limit=1
        ).reserved_availability
        self.assertEqual(
            aval_b_qty,
            5.0,
            "Wrong move quantity availability of product B (%s found instead of 5)" % (aval_b_qty),
        )
        # Check availability for product C
        aval_c_qty = self.MoveObj.search(
            [("product_id", "=", self.productC.id), ("picking_id", "=", picking_out.id)], limit=1
        ).reserved_availability
        self.assertEqual(
            aval_c_qty,
            3.0,
            "Wrong move quantity availability of product C (%s found instead of 3)" % (aval_c_qty),
        )
        # Check availability for product D
        aval_d_qty = self.MoveObj.search(
            [("product_id", "=", self.productD.id), ("picking_id", "=", picking_out.id)], limit=1
        ).reserved_availability
        self.assertEqual(
            aval_d_qty,
            7.0,
            "Wrong move quantity availability of product D (%s found instead of 7)" % (aval_d_qty),
        )
        ...
        product_a_qty = self.MoveObj.search(
            [("product_id", "=", self.productA.id), ("picking_id", "=", back_order_out.id)], limit=1
        ).product_uom_qty
        self.assertEqual(
            product_a_qty,
            8.0,
            "Wrong move quantity of product A (%s found instead of 8)" % (product_a_qty),
        )
        # Check back order should be created with 4 quantity of product D.
        product_d_qty = self.MoveObj.search(
            [("product_id", "=", self.productD.id), ("picking_id", "=", back_order_out.id)], limit=1
        ).product_uom_qty
        self.assertEqual(
            product_d_qty,
            4.0,
            "Wrong move quantity of product D (%s found instead of 4)" % (product_d_qty),
        )
        ...
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.productA.id), ("location_id", "=", self.stock_location)]
        )
        ...
        self.assertEqual(
            ...
            2.0,
            "Wrong quantity available (%s found instead of 2.0)" % (self.productA.qty_available),
        )
        ...
        self.assertFalse(
            quants, "No quant should found as outgoing shipment took everything out of stock."
        )
        self.assertEqual(
            self.productB.qty_available, 0.0, "Product B should have zero quantity available."
        )
        ...
        self.assertEqual(
            ...
            2.0,
            "Wrong quantity available (%s found instead of 2.0)" % (self.productC.qty_available),
        )
        ...
        self.assertEqual(
            ...
            1.0,
            "Wrong quantity available (%s found instead of 1.0)" % (self.productD.qty_available),
        )
        ...
        lot3_productC = LotObj.create(
            {"name": "Lot 3", "product_id": self.productC.id, "company_id": self.env.company.id}
        )
        lot4_productC = LotObj.create(
            {"name": "Lot 4", "product_id": self.productC.id, "company_id": self.env.company.id}
        )
        lot5_productC = LotObj.create(
            {"name": "Lot 5", "product_id": self.productC.id, "company_id": self.env.company.id}
        )
        lot6_productC = LotObj.create(
            {"name": "Lot 6", "product_id": self.productC.id, "company_id": self.env.company.id}
        )
        lot1_productD = LotObj.create(
            {"name": "Lot 1", "product_id": self.productD.id, "company_id": self.env.company.id}
        )
        LotObj.create(
            {"name": "Lot 2", "product_id": self.productD.id, "company_id": self.env.company.id}
        )
        ...
        packCs = self.StockPackObj.search(
            [("product_id", "=", self.productC.id), ("picking_id", "=", back_order_in.id)], limit=1
        )
        packCs.write(
            {
                "qty_done": 1,
        ...
        movesA = self.MoveObj.search(
            [("product_id", "=", self.productA.id), ("picking_id", "=", back_order_in.id)]
        )
        self.assertEqual(
            movesA.product_uom_qty,
            10,
            "Wrong move quantity of product A (%s found instead of 10)" % (movesA.product_uom_qty),
        )
        ...
        self.assertEqual(
            sum(total_qty),
            12.0,
            "Wrong total stock location quantity (%s found instead of 12)" % (sum(total_qty)),
        )
        self.assertEqual(
            self.productA.qty_available,
            12.0,
            "Wrong quantity available (%s found instead of 12)" % (self.productA.qty_available),
        )
        # Check quants and available quantity for product B.
        quants = self.StockQuantObj.search(
            [
        ...
        self.assertEqual(
            sum(total_qty),
            8.0,
            "Wrong total stock location quantity (%s found instead of 8)" % (sum(total_qty)),
        )
        self.assertEqual(
            self.productC.qty_available,
            8.0,
            "Wrong quantity available (%s found instead of 8)" % (self.productC.qty_available),
        )
        # Check quants and available quantity for product D.
        quants = self.StockQuantObj.search(
            [
        ...
        self.assertEqual(
            sum(total_qty),
            9.0,
            "Wrong total stock location quantity (%s found instead of 9)" % (sum(total_qty)),
        )
        self.assertEqual(
            self.productD.qty_available,
            9.0,
            "Wrong quantity available (%s found instead of 9)" % (self.productD.qty_available),
        )
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        PackSdozAround = self.StockPackObj.search(
            [("product_id", "=", self.SDozARound.id), ("picking_id", "=", picking_in_A.id)], limit=1
        )
        self.assertEqual(
        ...
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.DozA.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty),
        ...
        self.assertEqual(
            ...
            10,
            "Wrong quantity available (%s found instead of 10)" % (self.DozA.qty_available),
        )
        # Check quants and available quantity for product SDozA
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozA.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty),
            10.5,
            "Expecting 10.5 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozA.qty_available,
            10.5,
            "Wrong quantity available (%s found instead of 10.5)" % (self.SDozA.qty_available),
        )
        # Check quants and available quantity for product SDozARound
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozARound.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty),
            11,
            "Expecting 11 SDozenRound , got %.4f SDozenRound on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            11,
            "Wrong quantity available (%s found instead of 11)" % (self.SDozARound.qty_available),
        )
        ...
        self.assertAlmostEqual(
            sum(total_qty),
            525.3,
            msg="Expecting 525.3 gram , got %.4f gram on location stock!" % (sum(total_qty)),
        )
        self.assertAlmostEqual(
            self.gB.qty_available,
            525.3,
            msg="Wrong quantity available (%s found instead of 525.3" % (self.gB.qty_available),
        )
        # Check quants and available quantity for product kgB
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.kgB.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty),
        ...
        self.assertEqual(
            ...
            0.020,
            "Wrong quantity available (%s found instead of 0.020)" % (self.kgB.qty_available),
        )
        ...
        PackdozA = self.StockPackObj.search(
            [("product_id", "=", self.DozA.id), ("picking_id", "=", picking_in_B.id)], limit=1
        )
        self.assertEqual(
        ...
        PackSdozA = self.StockPackObj.search(
            [("product_id", "=", self.SDozA.id), ("picking_id", "=", picking_in_B.id)], limit=1
        )
        self.assertEqual(
        ...
        PackSdozAround = self.StockPackObj.search(
            [("product_id", "=", self.SDozARound.id), ("picking_id", "=", picking_in_B.id)], limit=1
        )
        self.assertEqual(
        ...
        packgB = self.StockPackObj.search(
            [("product_id", "=", self.gB.id), ("picking_id", "=", picking_in_B.id)], limit=1
        )
        self.assertEqual(
        ...
        packkgB = self.StockPackObj.search(
            [("product_id", "=", self.kgB.id), ("picking_id", "=", picking_in_B.id)], limit=1
        )
        self.assertEqual(
        ...
        self.assertEqual(
            packkgB.product_uom_id.id, self.uom_gm.id, "Wrong uom in pack operation for product kgB"
        )
        ...
        self.assertEqual(
            ...
            96,
            "Wrong move quantity (%s found instead of 96)" % (moves_DozA.product_uom_qty),
        )
        self.assertEqual(
            moves_DozA.product_uom.id, self.uom_unit.id, "Wrong uom in move for product DozA."
        )
        ...
        self.assertEqual(
            ...
            1512,
            "Wrong move quantity (%s found instead of 1512)" % (moves_SDozA.product_uom_qty),
        )
        self.assertEqual(
            moves_SDozA.product_uom.id, self.uom_unit.id, "Wrong uom in move for product SDozA."
        )
        ...
        self.assertEqual(
            ...
            1584,
            "Wrong move quantity (%s found instead of 1584)" % (moves_SDozARound.product_uom_qty),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            20,
            "Wrong quantity in move (%s found instead of 20)" % (moves_kgB.product_uom_qty),
        )
        self.assertEqual(
            moves_kgB.product_uom.id, self.uom_gm.id, "Wrong uom in move for product kgB."
        )
        ...
        self.assertEqual(
            ...
            0.526,
            "Wrong move quantity (%s found instead of 0.526)" % (moves_gB_kg.product_uom_qty),
        )
        self.assertEqual(
            moves_gB_kg.product_uom.id, self.uom_kg.id, "Wrong uom in move for product gB."
        )
        ...
        moves_DozA = self.MoveObj.search(
            [("product_id", "=", self.DozA.id), ("picking_id", "=", bo_in_B.id)], limit=1
        )
        self.assertEqual(
            moves_DozA.product_uom_qty,
            24.0,
            "Wrong move quantity (%s found instead of 0.525)" % (moves_DozA.product_uom_qty),
        )
        self.assertEqual(
            moves_DozA.product_uom.id, self.uom_unit.id, "Wrong uom in move for product DozA."
        )
        ...
        self.assertEqual(
            self.DozA.qty_available,
            18,
            "Wrong quantity available (%s found instead of 18)" % (self.DozA.qty_available),
        )
        # Check quants and available quantity for product SDozA
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozA.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            sum(total_qty),
            21,
            "Expecting 21 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozA.qty_available,
            21,
            "Wrong quantity available (%s found instead of 21)" % (self.SDozA.qty_available),
        )
        # Check quants and available quantity for product SDozARound
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozARound.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            sum(total_qty),
            22,
            "Expecting 22 SDozenRound , got %.4f SDozenRound on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            22,
            "Wrong quantity available (%s found instead of 22)" % (self.SDozARound.qty_available),
        )
        # Check quants and available quantity for product gB.
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.gB.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            ...
            1051.3,
            "Wrong quantity available (%s found instead of 1051)" % (self.gB.qty_available),
        )
        # Check quants and available quantity for product kgB.
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.kgB.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            self.kgB.qty_available,
            0.040,
            "Wrong quantity available (%s found instead of 0.040)" % (self.kgB.qty_available),
        )
        ...
        self.assertEqual(
            self.kgB.qty_available,
            0.039,
            "Wrong quantity available (%s found instead of 0.039)" % (self.kgB.qty_available),
        )
        ...
        DozA_qty = self.MoveObj.search(
            [("product_id", "=", self.DozA.id), ("picking_id", "=", picking_out.id)], limit=1
        ).product_qty
        self.assertEqual(
            DozA_qty, 4.5, "Wrong move quantity availability (%s found instead of 4.5)" % (DozA_qty)
        )
        # Check product B available quantity
        SDozA_qty = self.MoveObj.search(
            [("product_id", "=", self.SDozA.id), ("picking_id", "=", picking_out.id)], limit=1
        ).product_qty
        self.assertEqual(
            SDozA_qty, 2, "Wrong move quantity availability (%s found instead of 2)" % (SDozA_qty)
        )
        # Check product C available quantity
        SDozARound_qty = self.MoveObj.search(
            [("product_id", "=", self.SDozARound.id), ("picking_id", "=", picking_out.id)], limit=1
        ).product_qty
        self.assertEqual(
            SDozARound_qty,
            3,
            "Wrong move quantity availability (%s found instead of 3)" % (SDozARound_qty),
        )
        # Check product D available quantity
        gB_qty = self.MoveObj.search(
            [("product_id", "=", self.gB.id), ("picking_id", "=", picking_out.id)], limit=1
        ).product_qty
        self.assertEqual(
            gB_qty, 503, "Wrong move quantity availability (%s found instead of 503)" % (gB_qty)
        )
        # Check product D available quantity
        kgB_qty = self.MoveObj.search(
            [("product_id", "=", self.kgB.id), ("picking_id", "=", picking_out.id)], limit=1
        ).product_qty
        self.assertEqual(
            kgB_qty,
            0.020,
        ...
        self.assertEqual(
            sum(total_qty),
            13.5,
            "Expecting 13.5 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.DozA.qty_available,
            13.5,
            "Wrong quantity available (%s found instead of 13.5)" % (self.DozA.qty_available),
        )
        # Check quants and available quantity for product SDozA
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozA.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            sum(total_qty),
            19,
            "Expecting 19 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozA.qty_available,
            19,
            "Wrong quantity available (%s found instead of 19)" % (self.SDozA.qty_available),
        )
        # Check quants and available quantity for product SDozARound
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.SDozARound.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            sum(total_qty),
            19,
            "Expecting 19 SDozRound , got %.4f SDozRound on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            19,
            "Wrong quantity available (%s found instead of 19)" % (self.SDozARound.qty_available),
        )
        # Check quants and available quantity for product gB.
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.gB.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            round(self.gB.qty_available, 1),
            548.3,
            "Wrong quantity available (%s found instead of 547.6)" % (self.gB.qty_available),
        )
        # Check quants and available quantity for product kgB.
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.kgB.id), ("location_id", "=", self.stock_location)]
        ...
        self.assertEqual(
            self.kgB.qty_available,
            0.019,
            "Wrong quantity available (%s found instead of 0.019)" % (self.kgB.qty_available),
        )
        ...
        self.assertEqual(
            sum(total_qty),
            15.5,
            "Expecting 15.5 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty)),
        )
        self.assertEqual(
            self.DozA.qty_available,
            15.5,
            "Wrong quantity available (%s found instead of 15.5)" % (self.DozA.qty_available),
        )
        ...
        packKG = self.StockPackObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", picking_in.id)], limit=1
        )
        self.assertEqual(
            packKG.reserved_qty,
            1000,
        ...
        self.assertEqual(
            packKG.product_uom_id.id, self.uom_tone.id, "Wrong product uom in pack operation."
        )
        # Transfer Incoming shipment.
        ...
        pack_opt = self.StockPackObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", picking_out.id)], limit=1
        )
        ...
        backorder_wizard.process()
        quants = self.StockQuantObj.search(
            [("product_id", "=", productKG.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        ...
        moves_KG = self.MoveObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", bo_out_1.id)], limit=1
        )
        # Check back order created with correct quantity and uom or not.
        self.assertEqual(
        ...
        moves_KG = self.MoveObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", bo_out_2.id)], limit=1
        )
        self.assertEqual(
            moves_KG.product_uom_qty,
            15,
        ...
        # Check back order created with correct quantity and uom or not.
        moves_KG = self.MoveObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", bo_out_3.id)], limit=1
        )
        self.assertEqual(
            moves_KG.product_uom_qty,
            10,
        ...
        # Check back order created with correct quantity and uom or not.
        moves_KG = self.MoveObj.search(
            [("product_id", "=", productKG.id), ("picking_id", "=", bo_out_4.id)], limit=1
        )
        self.assertEqual(
            moves_KG.product_uom_qty,
            5,
        ...
        self.assertAlmostEqual(
            sum(total_qty),
            999.975,
            msg="Expecting 999.975 kg , got %.4f kg on location stock!" % (sum(total_qty)),
        )
    ...
    def test_20_create_inventory_with_packs_and_lots(self):
        ...
        lot1 = lot_obj.create(
            {"name": "Lot001", "product_id": lotproduct.id, "company_id": self.env.company.id}
        )
        ...
        self.assertEqual(
            total_qty,
            0,
            "Expecting 0 units lot of lotproduct, but we got %.4f on location stock!" % (total_qty),
        )
    ...
    def test_30_check_with_no_incoming_lot(self):
        ...
        lot1 = self.LotObj.create(
            {"product_id": self.productA.id, "name": "LOT1", "company_id": self.env.company.id}
        )
        lot2 = self.LotObj.create(
            {"product_id": self.productA.id, "name": "LOT2", "company_id": self.env.company.id}
        )
        lot3 = self.LotObj.create(
            {"product_id": self.productA.id, "name": "LOT3", "company_id": self.env.company.id}
        )
    ...
    def test_40_pack_in_pack(self):
        ...
        quant = self.StockQuantObj.search(
            [("product_id", "=", self.productA.id), ("location_id", "=", self.customer_location)]
        )
        self.assertEqual(
            len(quant), 1, "There should be one quant with package for customer location"
        )
    ...
    def test_60_create_in_out_with_product_pack_lines(self):
        ...
        # Check the quants are in the package
        self.assertEqual(
            sum(x.quantity for x in pack1.quant_ids), 120, "Pack 1 should have 120 pieces"
        )
        self.assertEqual(
    ...
    def test_assign_qty_to_first_move(self):
        ...
        out02 = create_picking(
            self.picking_type_out, self.stock_location, self.customer_location, sequence=2, delay=1
        )
        in01 = create_picking(
    ...
    def test_assign_sm_to_existing_picking(self):
        ...
        self.assertEqual(
            out_moves.picking_id, out_picking, "All SM should be part of the same picking"
        )
        self.assertEqual(
