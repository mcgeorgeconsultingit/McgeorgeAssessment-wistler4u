...
class TestPersonalStages(TestProjectCommon):
    ...
    def test_personal_stage_read_group(self):
        ...
        self.assertEqual(
            1, total, "read_group should not have returned more tasks than the user is assigned to."
        )
        read_group_manager = (
            self.env["project.task"]
            .with_user(self.user_projectmanager)
        ...
        for group in read_group_manager:
            ...
            if group["personal_stage_type_ids"][0] == self.manager_stages[0].id:
                ...
                total_stage_1 += 1
        self.assertEqual(
            2, total, "read_group should not have returned more tasks than the user is assigned to."
        )
