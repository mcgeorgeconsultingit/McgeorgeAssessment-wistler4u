...
class TestSnippets(HttpCase):
    ...
    def test_04_countdown_preview(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "snippet_countdown", login="admin"
        )

    def test_05_social_media(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "snippet_social_media", login="admin"
        )
        self.assertEqual(
    ...
    def test_07_image_gallery(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "snippet_image_gallery", login="admin"
        )
