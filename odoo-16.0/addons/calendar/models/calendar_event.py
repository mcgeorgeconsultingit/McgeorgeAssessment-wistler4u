...
class Meeting(models.Model):
    ...
    videocall_source = fields.Selection(
        [("discuss", "Discuss"), ("custom", "Custom")], compute="_compute_videocall_source"
    )
    ...
    start_date = fields.Date(
        "Start Date", store=True, tracking=True, compute="_compute_dates", inverse="_inverse_dates"
    )
    stop_date = fields.Date(
        "End Date", store=True, tracking=True, compute="_compute_dates", inverse="_inverse_dates"
    )
    ...
    def _compute_is_organizer_alone(self):
        ...
        for event in self:
            ...
            all_declined = not any(
                (event.attendee_ids - organizer).filtered(lambda a: a.state != "declined")
            )
    ...
    def _inverse_dates(self):
        ...
        for meeting in self:
            if meeting.allday:
                ...
                meeting.write(
                    {"start": startdate.replace(tzinfo=None), "stop": enddate.replace(tzinfo=None)}
                )
    ...
    def create(self, vals_list):
        ...
        defaults = self.default_get(
            ["activity_ids", "res_model_id", "res_id", "user_id", "res_model", "partner_ids"]
        )
        ...
        model_ids = list(
            filter(
                None,
                {values.get("res_model_id", defaults.get("res_model_id")) for values in vals_list},
            )
        )
        ...
        vals_list = [
            dict(
                vals,
                attendee_ids=self._attendees_values(vals.get("partner_ids", default_partners_ids)),
            )
    ...
    def _compute_field_value(self, field):
        if field.compute_sudo:
            return super(Meeting, self.with_context(prefetch_fields=False))._compute_field_value(
                field
            )
    ...
    def write(self, values):
        ...
        update_recurrence = (
            recurrence_update_setting in ("all_events", "future_events") and len(self) == 1
        )
        ...
        recurrence_values = {
            field: values.pop(field) for field in self._get_recurrent_fields() if field in values
        }
        ...
        if "partner_ids" in values:
            ...
            (current_attendees - previous_attendees)._send_mail_to_attendees(
                self.env.ref(
                    "calendar.calendar_template_meeting_invitation", raise_if_not_found=False
                )
            )
        if not self.env.context.get("is_calendar_event_new") and "start" in values:
            ...
            if start_date and start_date >= fields.Datetime.now():
                (current_attendees & previous_attendees).with_context(
                    ...
                    self.env.ref(
                        "calendar.calendar_template_meeting_changedate", raise_if_not_found=False
                    )
                )
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if not self.env.su and private_fields:
            ...
            domain = AND(
                [domain, ["|", ("privacy", "!=", "private"), ("user_id", "=", self.env.user.id)]]
            )
    ...
    def _apply_recurrence_values(self, values, future=True):
        ...
        for event in self:
            if not event.recurrence_id:
                recurrence_vals += [
                    dict(values, base_event_id=event.id, calendar_event_ids=[(4, event.id)])
                ]
            elif future:
    ...
    def _rewrite_recurrence(self, values, time_values, recurrence_values):
        ...
        if start_update or stop_update:
            if start_update:
                ...
                update_dict.update(
                    {"start": start, "start_date": start_date, "stop": stop, "stop_date": stop_date}
                )
            if stop_update:
                if not start_update:
        ...
        for meeting in self:
            ...
            if not is_html_empty(meeting.description):
                if "appointment_type_id" in meeting._fields and self.appointment_type_id:
                    ...
                    event.add("description").value = self.convert_online_event_desc_to_text(
                        meeting.description
                    )
                else:
