...
class ChatbotScript(models.Model):
    ...
    operator_partner_id = fields.Many2one(
        "res.partner", string="Bot Operator", ondelete="restrict", required=True, copy=False
    )
    ...
    def _compute_livechat_channel_count(self):
        ...
        mapped_channels = {
            channel["chatbot_script_id"][0]: channel["channel_id"] for channel in channels_data
        }
        for script in self:
    ...
    def copy(self, default=None):
        ...
        answers_map = {
            ...
            for clone_answer, original_answer in zip(
                clone_chatbot_script.script_step_ids.answer_ids, self.script_step_ids.answer_ids
            )
        }
