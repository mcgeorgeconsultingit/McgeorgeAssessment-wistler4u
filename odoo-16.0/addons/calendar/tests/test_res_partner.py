...
class TestResPartner(TransactionCase):
    def test_meeting_count(self):
        test_user = new_test_user(
            self.env, login="test_user", groups="base.group_user, base.group_partner_manager"
        )
        ...
        Event.create(
            {"name": "event_2", "partner_ids": [(6, 0, [test_partner_1.id, test_partner_3.id])]}
        )
        Event.create(
            {"name": "event_2", "partner_ids": [(6, 0, [test_partner_2.id, test_partner_3.id])]}
        )
        Event.create(
            {"name": "event_3", "partner_ids": [(6, 0, [test_partner_3.id, test_partner_4.id])]}
        )
        ...
        Event.create(
            {"name": "event_9", "partner_ids": [(6, 0, [test_partner_2.id, test_partner_3.id])]}
        )
