...
class AccountEdiFormat(models.Model):
            ...
            if invoice.is_sale_document():
                ...
                if tax_values["l10n_es_type"] in ("sujeto", "sujeto_isp"):
                    ...
                    recargo = recargo_tax_details.get(
                        tax_values["group_tax_details"][0]["tax_repartition_line"].tax_id
                    )
                    if recargo:
        ...
        for invoice in invoices:
            ...
            if invoice.is_sale_document():
                ...
                if invoice.l10n_es_registration_date:
                    invoice_node["FechaRegContable"] = invoice.l10n_es_registration_date.strftime(
                        "%d-%m-%Y"
                    )
                else:
                    invoice_node["FechaRegContable"] = fields.Date.context_today(self).strftime(
                        "%d-%m-%Y"
                    )
            ...
            if invoice.is_sale_document():
                ...
                if tax_details_info_other_vals["tax_details_info"]:
                    invoice_node["DesgloseFactura"]["DesgloseIVA"] = tax_details_info_other_vals[
                        "tax_details_info"
                    ]
    ...
    def _l10n_es_edi_call_web_service_sign(self, invoices, info_list):
        ...
        connection_vals = getattr(self, f"_l10n_es_edi_web_service_{l10n_es_edi_tax_agency}_vals")(
            invoices
        )
        ...
        for respl in res.RespuestaLinea:
            ...
            if resp_line_state in ("Correcto", "AceptadoConErrores"):
                ...
                results[inv] = {
                    "error": _(
                        "[%s] %s", respl.CodigoErrorRegistro, respl.DescripcionErrorRegistro
                    ),
