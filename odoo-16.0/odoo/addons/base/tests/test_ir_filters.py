...
class TestGetFilters(FiltersCase):
    ...
    def test_global_filters(self):
        ...
        self.assertItemsEqual(
            ...
            [
                dict(
                    name="a", is_default=False, user_id=False, domain="[]", context="{}", sort="[]"
                ),
                dict(
                    name="b", is_default=False, user_id=False, domain="[]", context="{}", sort="[]"
                ),
                dict(
                    name="c", is_default=False, user_id=False, domain="[]", context="{}", sort="[]"
                ),
                dict(
                    name="d", is_default=False, user_id=False, domain="[]", context="{}", sort="[]"
                ),
            ],
        )
...
class TestGlobalDefaults(FiltersCase):
    def test_new_filter_not_default(self):
        ...
        self.assertItemsEqual(
            ...
            [
                dict(
                    name="a", user_id=False, is_default=False, domain="[]", context="{}", sort="[]"
                ),
                dict(
                    name="b", user_id=False, is_default=False, domain="[]", context="{}", sort="[]"
                ),
                dict(
                    name="c", user_id=False, is_default=True, domain="[]", context="{}", sort="[]"
                ),
            ],
        )
...
class TestReadGroup(TransactionCase):
    ...
    def test_read_group_1(self):
        ...
        self.assertEqual(
            Users._order, "name, login", "Model res.users must be ordered by name, login"
        )
...
class TestAllFilters(TransactionCase):
    def check_filter(self, name, model, domain, fields, groupby, order, context):
        if groupby:
            try:
                self.env[model].with_context(context).read_group(
                    domain, fields, groupby, orderby=order
                )
            except ValueError as e:
                raise self.failureException("Test filter '%s' failed: %s" % (name, e)) from None
            except KeyError as e:
                raise self.failureException(
                    "Test filter '%s' failed: field or aggregate %s does not exist" % (name, e)
                ) from None
        elif domain:
            try:
