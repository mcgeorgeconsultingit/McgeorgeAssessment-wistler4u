...
class Contract(models.Model):
    ...
    date_start = fields.Date(
        "Start Date", required=True, default=fields.Date.today, tracking=True, index=True
    )
    date_end = fields.Date(
        "End Date", tracking=True, help="End date of the contract (if it's a fixed-term contract)."
    )
    ...
    state = fields.Selection(
        [("draft", "New"), ("open", "Running"), ("close", "Expired"), ("cancel", "Cancelled")],
        string="Status",
    ...
    company_country_id = fields.Many2one(
        "res.country", string="Company country", related="company_id.country_id", readonly=True
    )
    ...
    def update_state(self):
        ...
        contract_ids = self.search(
            [("date_end", "=", False), ("state", "=", "close"), ("employee_id", "!=", False)]
        )
