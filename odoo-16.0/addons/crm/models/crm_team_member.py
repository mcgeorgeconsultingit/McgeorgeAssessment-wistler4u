...
class TeamMember(models.Model):
    ...
    def _assign_and_convert_leads(self, work_days=1):
        ...
        lead_limit = sum(member._get_assignment_quota(work_days=work_days) for member in members)
        ...
        while population:
            ...
            lead = next(
                (lead for lead in member_data["leads"] if lead.id not in leads_done_ids), False
            )
            if lead:
