...
def patch_proxy_user(func):
    @patch.object(
        AccountEdiProxyClientUser,
        "_make_request",
        MagicMock(spec=AccountEdiProxyClientUser._make_request),
    )
    @patch.object(
        AccountEdiProxyClientUser,
        "_decrypt_data",
        MagicMock(spec=AccountEdiProxyClientUser._decrypt_data),
    )
    @patch.object(
        AccountEdiProxyClientUser,
        "_get_demo_state",
        MagicMock(spec=AccountEdiProxyClientUser._get_demo_state),
    )
...
class TestItEdi(AccountEdiTestCommon):
    ...
    def _cleanup_etree(self, content, xpaths=None):
        ...
        return self.with_applied_xpath(
            ...
            "".join(
                [f"<xpath expr='{x}' position='replace'>{y}</xpath>" for x, y in xpaths.items()]
            ),
        )
