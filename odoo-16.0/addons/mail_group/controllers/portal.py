...
class PortalMailGroup(http.Controller):
    ...
    def _get_archives(self, group_id):
        ...
        results = request.env["mail.group.message"]._read_group_raw(
            domain, ["subject", "create_date"], groupby=["create_date"], orderby="create_date"
        )
    ...
    def group_view_messages(
        ...
        messages_sudo = GroupMessage.search(
            domain, limit=self._thread_per_page, offset=(page - 1) * self._thread_per_page
        ).sudo()
    ...
    def group_view_message(
        ...
        next_message = GroupMessage.search(
            expression.AND([base_domain, [("id", ">", message.id)]]), order="id ASC", limit=1
        )
        prev_message = GroupMessage.search(
            expression.AND([base_domain, [("id", "<", message.id)]]), order="id DESC", limit=1
        )
