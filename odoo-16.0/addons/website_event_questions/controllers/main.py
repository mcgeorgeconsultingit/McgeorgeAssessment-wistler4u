        ...
        for key, value in form_details.items():
            if "question_answer" in key and value:
                ...
                if question_sudo.question_type == "simple_choice":
                    answer_values = {"question_id": int(question_id), "value_answer_id": int(value)}
                elif question_sudo.question_type == "text_box":
                    answer_values = {"question_id": int(question_id), "value_text_box": value}

                if answer_values and not int(registration_index):
                    general_answer_ids.append((0, 0, answer_values))
                elif answer_values:
                    registrations[int(registration_index) - 1]["registration_answer_ids"].append(
                        (0, 0, answer_values)
                    )
