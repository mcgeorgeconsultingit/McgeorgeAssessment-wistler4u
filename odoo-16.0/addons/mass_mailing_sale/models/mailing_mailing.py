...
class MassMailing(models.Model):
    ...
    def _compute_sale_invoiced_amount(self):
        domain = expression.AND(
            [[("source_id", "in", self.source_id.ids)], [("state", "not in", ["draft", "cancel"])]]
        )
        moves_data = (
        ...
        mapped_data = {
            datum["source_id"][0]: datum["amount_untaxed_signed"] for datum in moves_data
        }
        for mass_mailing in self:
    ...
    def action_redirect_to_invoiced(self):
        domain = expression.AND(
            [[("source_id", "=", self.source_id.id)], [("state", "not in", ["draft", "cancel"])]]
        )
