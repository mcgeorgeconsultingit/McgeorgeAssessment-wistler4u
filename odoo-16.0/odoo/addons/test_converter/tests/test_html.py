...
class TestCurrencyExport(TestExport):
    ...
    def test_currency_pre(self):
        currency = self.create(self.Currency, name="Test", symbol="test", position="before")
        obj = self.create(self.Model, value=0.12)
        ...
        self.assertEqual(
            ...
            '<span class="oe_currency_value">0.12</span>'.format(symbol=currency.symbol),
        )
...
class TestDurationExport(TestBasicExport):
    ...
    def test_negative_digital(self):
        converter = self.get_converter("float", "duration")
        result = converter(
            -90, {"unit": "minute", "round": "minute", "digital": True}, {"lang": "fr_FR"}
        )
