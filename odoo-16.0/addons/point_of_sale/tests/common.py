...
class TestPoSCommon(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.pos_receivable_cash = cls.copy_account(
            cls.company.account_default_pos_receivable_account_id, {"name": "POS Receivable Cash"}
        )
        cls.pos_receivable_bank = cls.copy_account(
            cls.company.account_default_pos_receivable_account_id, {"name": "POS Receivable Bank"}
        )
        cls.outstanding_bank = cls.copy_account(
            cls.company.account_journal_payment_debit_account_id, {"name": "Outstanding Bank"}
        )
    ...
    def create_ui_order_data(
        self, pos_order_lines_ui_args, customer=False, is_invoiced=False, payments=None, uid=None
    ):
        ...
        fiscal_position = (
            customer.property_account_position_id if customer else default_fiscal_position
        )
        ...
        if payments is None:
            default_cash_pm = self.config.payment_method_ids.filtered(lambda pm: pm.is_cash_count)[
                :1
            ]
    ...
    @classmethod
    def create_product(
        cls, name, category, lst_price, standard_price=None, tax_ids=None, sale_account=None
    ):
    ...
    def _run_test(self, args):
        ...
        self._check_invoice_journal_entries(
            pos_session, orders_map, expected_values=args["journal_entries_before_closing"]
        )
        ...
        total_cash_payment = sum(
            ...
            .filtered(lambda payment: payment.payment_method_id.id == cash_payment_method.id)
            .mapped("amount")
        )
            ...
            for pos_payment in order.payment_ids:
                ...
                def predicate(args):
                    ...
                    second = tools.float_is_zero(
                        pos_payment.amount - amount, precision_rounding=currency_rounding
                    )
                    return first and second

                self._find_then_assert_values(
                    pos_payment.account_move_id, expected_values[uid]["payments"], predicate
                )
                _logger.info(
    ...
    def _check_session_journal_entries(self, pos_session, expected_values):
        ...
        for statement_line in pos_session.statement_line_ids:
            ...
            def statement_line_predicate(args):
                return tools.float_is_zero(
                    statement_line.amount - args[0], precision_rounding=currency_rounding
                )

            self._find_then_assert_values(
                statement_line.move_id, expected_values["cash_statement"], statement_line_predicate
            )
        ...
        for bank_payment in pos_session.bank_payment_ids:
            ...
            self._find_then_assert_values(
                bank_payment.move_id, expected_values["bank_payments"], bank_payment_predicate
            )
