...
def rename_column(cr, tablename, columnname1, columnname2):
    ...
    cr.execute(
        'ALTER TABLE "{}" RENAME COLUMN "{}" TO "{}"'.format(tablename, columnname1, columnname2)
    )
...
def drop_depending_views(cr, table, column):
    ...
    for v, k in get_depending_views(cr, table, column):
        cr.execute(
            "DROP {0} VIEW IF EXISTS {1} CASCADE".format("MATERIALIZED" if k == "m" else "", v)
        )
...
def add_constraint(cr, tablename, constraintname, definition):
    ...
    try:
        with cr.savepoint(flush=False):
            ...
            _schema.debug(
                "Table %r: added constraint %r as %s", tablename, constraintname, definition
            )
    except Exception:
        raise Exception(
            "Table %r: unable to add constraint %r as %s", tablename, constraintname, definition
        )
