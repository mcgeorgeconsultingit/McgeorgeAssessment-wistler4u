...
class AccountTax(models.Model):
    ...
    hide_tax_exigibility = fields.Boolean(
        string="Hide Use Cash Basis Option", related="company_id.tax_exigibility", readonly=True
    )
        ...
        for record in self:
            if (
                ...
                raise ValidationError(
                    _("The tax group must have the same country_id as the tax using it.")
                )
    ...
    def _validate_repartition_lines(self):
        for record in self:
            ...
            if len(invoice_repartition_line_ids) != len(refund_repartition_line_ids):
                raise ValidationError(
                    _("Invoice and credit note distribution should have the same number of lines.")
                )

            if not invoice_repartition_line_ids.filtered(
                lambda x: x.repartition_type == "tax"
            ) or not refund_repartition_line_ids.filtered(lambda x: x.repartition_type == "tax"):
                raise ValidationError(
                    _(
    ...
    def get_tax_tags(self, is_refund, repartition_type):
        ...
        return rep_lines.filtered(lambda x: x.repartition_type == repartition_type).mapped(
            "tag_ids"
        )
    ...
    def compute_all(
        ...
        for tax in taxes:
            ...
            tax_repartition_lines = (
                is_refund and tax.refund_repartition_line_ids or tax.invoice_repartition_line_ids
            ).filtered(lambda x: x.repartition_type == "tax")
            sum_repartition_factor = sum(tax_repartition_lines.mapped("factor"))
            ...
            total_rounding_error = float_round(
                factorized_tax_amount - sum(repartition_line_amounts), precision_rounding=prec
            )
    ...
    def _aggregate_taxes(
        ...
        for base_line, to_update_vals, tax_values_list in to_process:
            ...
            for tax_values in tax_values_list:
                ...
                add_tax_values(
                    record, global_tax_details, grouping_key, serialized_grouping_key, tax_values
                )
                add_tax_values(
                    record,
    ...
    @api.model
    def _compute_taxes(
        self, base_lines, tax_lines=None, handle_price_include=True, include_caba_tags=False
    ):
    ...
    def _prepare_tax_totals(self, base_lines, currency, tax_lines=None):
        ...
        tax_group_vals_list = sorted(
            tax_group_vals_list, key=lambda x: (x["tax_group"].sequence, x["tax_group"].id)
        )
        ...
        display_tax_base = (
            ...
            and currency.compare_amounts(tax_group_vals_list[0]["base_amount"], amount_untaxed) != 0
