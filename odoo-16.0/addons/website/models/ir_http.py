...
class Http(models.AbstractModel):
    ...
    def _generate_routing_rules(cls, modules, converters):
        ...
        rewrites = dict(
            [(x.url_from, x) for x in request.env["website.rewrite"].sudo().search(domain)]
        )
        ...
        for url, endpoint in super()._generate_routing_rules(modules, converters):
            if url in rewrites:
                ...
                if rewrite.redirect_type == "308":
                    ...
                    if url != url_to:
                        logger.debug(
                            "Redirect from %s to %s for website %s" % (url, url_to, website_id)
                        )
    ...
    def _serve_page(cls):
        ...
        if not page:
            ...
            if page:
                logger.info(
                    "Page %r not found, redirecting to existing page %r", req_page, page.url
                )
        ...
        if page and (
            request.env.user.has_group("website.group_website_designer") or page.is_visible
        ):
