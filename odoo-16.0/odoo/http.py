...
from werkzeug.exceptions import HTTPException, BadRequest, Forbidden, NotFound, InternalServerError
...
def db_filter(dbs, host=None):
    ...
    if config["dbfilter"]:
        ...
        dbfilter_re = re.compile(
            config["dbfilter"].replace("%h", re.escape(host)).replace("%d", re.escape(domain))
        )
...
def route(route=None, **routing):
    ...
    def decorator(endpoint):
        ...
        if wrong is not None:
            _logger.warning(
                "%s defined with invalid routing parameter 'method', assuming 'methods'", fname
            )
...
class Session(collections.abc.MutableMapping):
    ...
    def __getitem__(self, item):
        if item == "geoip":
            warnings.warn(
                "request.session.geoip have been moved to request.geoip", DeprecationWarning
            )
...
class Request:
    ...
    def geoip(self):
        ...
        if "_geoip" not in self.session:
            ...
            self.session._geoip = (
                self.registry["ir.http"]._geoip_resolve() if self.db else self._geoip_resolve()
            )
    ...
    def get_http_params(self):
        ...
        :rtype: dict
        """
        params = {**self.httprequest.args, **self.httprequest.form, **self.httprequest.files}
    ...
    def _set_request_dispatcher(self, rule):
        ...
        if not is_cors_preflight(self, rule.endpoint) and not dispatcher_cls.is_compatible_with(
            self
        ):
            compatible_dispatchers = [
                disp.routing_type for disp in _dispatchers.values() if disp.is_compatible_with(self)
            ]
            raise BadRequest(
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        if self.request.httprequest.method not in CSRF_FREE_METHODS and endpoint.routing.get(
            "csrf", True
        ):
            ...
            if not self.request.validate_csrf(token):
                if token is not None:
                    _logger.warning(
                        "CSRF validation failed on path '%s'", self.request.httprequest.path
                    )
                else:
