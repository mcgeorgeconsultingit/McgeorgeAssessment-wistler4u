...
class TestMailChannelMembers(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.public_channel = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_create(group_id=None, name="Public channel of user 1")[
                "id"
            ]
        )
    ...
    def test_group_01(self):
        ...
        channel_member_1 = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.group.id), ("partner_id", "=", self.user_1.partner_id.id)]
        )
        with self.assertRaises(AccessError):
    ...
    def test_unread_counter_with_message_post_multi_channel(self):
        ...
        members = self.env["mail.channel.member"].search(
            [("channel_id", "in", (channel_1_as_user_1 + channel_2_as_user_2).ids)], order="id"
        )
        self.assertEqual(
