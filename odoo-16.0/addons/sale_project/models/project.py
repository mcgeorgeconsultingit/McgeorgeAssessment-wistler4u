...
class Project(models.Model):
    ...
    invoice_count = fields.Integer(
        related="analytic_account_id.invoice_count", groups="account.group_account_readonly"
    )
    vendor_bill_count = fields.Integer(
        related="analytic_account_id.vendor_bill_count", groups="account.group_account_readonly"
    )
    ...
    def get_sale_items_data(self, domain=None, offset=0, limit=None, with_action=True):
        ...
        return [
            ...
            for sol_read in sols.with_context(with_price_unit=True).read(
                ["display_name", "product_uom_qty", "qty_delivered", "qty_invoiced", "product_uom"]
            )
        ]
    ...
    def _get_revenues_items_from_sol(self, domain=None, with_action=True):
        ...
        return {
            "data": [
                {"id": invoice_type, "sequence": sequence_per_invoice_type[invoice_type], **vals}
    ...
    def _get_revenues_items_from_invoices(self, excluded_move_line_ids=None):
        ...
        query.add_where(
            "account_move_line.analytic_distribution ? %s", [str(self.analytic_account_id.id)]
        )
        ...
        query_string, query_param = query.select(
            "price_subtotal", "parent_state", "account_move_line__move_id.move_type"
        )
        self._cr.execute(query_string, query_param)
    ...
    def _get_profitability_items(self, with_action=True):
        ...
        revenue_items_from_invoices = self._get_revenues_items_from_invoices(
            ...
            .browse([sol_id for sol_read in sale_line_read_group for sol_id in sol_read["ids"]])
            .invoice_lines.ids
        )
