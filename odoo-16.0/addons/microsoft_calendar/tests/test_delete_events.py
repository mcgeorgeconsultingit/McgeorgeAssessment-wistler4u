...
class TestDeleteEvents(TestCommon):
    ...
    def test_archive_several_events_at_once(self, mock_delete):
        ...
        mock_delete.assert_has_calls(
            [call(e.ms_organizer_event_id, token=ANY, timeout=ANY) for e in self.several_events]
        )
    ...
    def test_delete_one_event_from_recurrence_from_outlook_calendar(self, mock_get_events):
        ...
        idx = 3
        rec_values = [
            dict(event, lastModifiedDateTime=_modified_date_in_the_future(self.recurrence))
    ...
    def test_delete_one_event_and_future_from_recurrence_from_outlook_calendar(
        ...
        self.assertEqual(
            len(self.recurrence.calendar_event_ids), self.recurrent_events_count - len(idx)
        )
    ...
    def test_delete_first_event_and_future_from_recurrence_from_outlook_calendar(
        ...
        mock_get_events.return_value = (
            MicrosoftEvent(
                [{"id": self.recurrence.ms_organizer_event_id, "@removed": {"reason": "deleted"}}]
            ),
            None,
        )
