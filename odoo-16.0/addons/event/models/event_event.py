...
class EventEvent(models.Model):
    ...
    tag_ids = fields.Many2many(
        "event.tag", string="Tags", readonly=False, store=True, compute="_compute_tag_ids"
    )
    # Kanban fields
    ...
    seats_reserved = fields.Integer(
        string="Number of Registrations", store=False, readonly=True, compute="_compute_seats"
    )
    seats_available = fields.Integer(
        string="Available Seats", store=False, readonly=True, compute="_compute_seats"
    )
    seats_unconfirmed = fields.Integer(
        string="Unconfirmed Registrations", store=False, readonly=True, compute="_compute_seats"
    )
    seats_used = fields.Integer(
        string="Number of Attendees", store=False, readonly=True, compute="_compute_seats"
    )
    seats_expected = fields.Integer(
        string="Number of Expected Attendees", store=False, readonly=True, compute="_compute_seats"
    )
    # Registration fields
    auto_confirm = fields.Boolean(
    ...
    def mail_attendees(
        self, template_id, force_send=False, filter_func=lambda self: self.state != "cancel"
    ):
        for event in self:
