...
class TaxReportTest(AccountTestInvoicingCommon):
    ...
    def test_write_single_line_tagname_not_shared(self):
        ...
        self.assertEqual(
            original_tags, new_tags, "The expression should still be linked to the same tags."
        )
        self.assertEqual(
    ...
    def test_write_single_line_tagname_shared(self):
        ...
        self.assertNotEqual(
            original_tags, new_tags, "New tags should have been assigned to the expression"
        )
    ...
    def test_tax_report_change_country(self):
        ...
        original_report_2_tags = {
            line: line.expression_ids._get_matching_tags() for line in self.tax_report_2.line_ids
        }
        ...
        test_invoice = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    (
                        0,
                        0,
                        {"quantity": 1, "price_unit": 42, "tax_ids": [Command.set([test_tax.id])]},
                    ),
                ],
            }
        )
