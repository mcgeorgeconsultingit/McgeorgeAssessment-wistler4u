...
class CalendarLeaves(models.Model):
    ...
    def _get_domain(self, time_domain_dict):
        ...
        for date in time_domain_dict:
            domain = expression.OR(
                [domain, [("date_to", ">", date["date_from"]), ("date_from", "<", date["date_to"])]]
            )
    ...
    def _reevaluate_leaves(self, time_domain_dict):
        ...
        leaves_to_cancel._force_cancel(
            _("a new public holiday completely overrides this leave."), "mail.mt_comment"
        )
...
class ResourceCalendar(models.Model):
    ...
    def _compute_associated_leaves_count(self):
        ...
        result = dict(
            (data["calendar_id"][0] if data["calendar_id"] else "global", data["calendar_id_count"])
