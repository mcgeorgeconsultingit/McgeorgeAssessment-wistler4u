    ...
    def mockSMSGateway(self, sms_allow_unlink=False, sim_error=None, nbr_t_error=None):
        ...
        def _sms_sms_send(records, unlink_failed=False, unlink_sent=True, raise_exception=False):
            if sms_allow_unlink:
                ...
                return sms_send_origin(
                    records, unlink_failed=False, unlink_sent=False, raise_exception=raise_exception
                )
            return True

        try:
            with patch.object(SmsApi, "_contact_iap", side_effect=_contact_iap), patch.object(
                SmsSms, "create", autospec=True, wraps=SmsSms, side_effect=_sms_sms_create
            ), patch.object(
...
class SMSCase(MockSMS):
    ...
    def _find_sms_sms(self, partner, number, status):
        ...
        if not sms:
            raise AssertionError(
                "sms.sms not found for %s (number: %s / status %s)" % (partner, number, status)
            )
        if len(sms) > 1:
    ...
    def assertSMS(
        self, partner, number, status, failure_type=None, content=None, fields_values=None
    ):
    ...
    def assertNoSMSNotification(self, messages=None):
        ...
        self.assertEqual(
            self.env["mail.notification"].search(base_domain), self.env["mail.notification"]
        )
