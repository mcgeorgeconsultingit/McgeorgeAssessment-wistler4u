...
class ProductTemplate(models.Model):
    ...
    seller_ids = fields.One2many(
        "product.supplierinfo", "product_tmpl_id", "Vendors", depends_context=("company",)
    )
    ...
    attribute_line_ids = fields.One2many(
        "product.template.attribute.line", "product_tmpl_id", "Product Attributes", copy=True
    )
    ...
    barcode = fields.Char(
        "Barcode", compute="_compute_barcode", inverse="_set_barcode", search="_search_barcode"
    )
    ...
    can_image_1024_be_zoomed = fields.Boolean(
        "Can Image 1024 be zoomed", compute="_compute_can_image_1024_be_zoomed", store=True
    )
    has_configurable_attributes = fields.Boolean(
        "Is a configurable product", compute="_compute_has_configurable_attributes", store=True
    )
    ...
    def _get_length_uom_id_from_ir_config_parameter(self):
        ...
        product_length_in_feet_param = (
            self.env["ir.config_parameter"].sudo().get_param("product.volume_in_cubic_feet")
        )
    ...
    def _name_search(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        while True:
            ...
            products_ids = Product._name_search(
                name, args + domain, operator=operator, name_get_uid=name_get_uid, **kwargs
            )
    ...
    def open_pricelist_rules(self):
        ...
        return {
            ...
            "view_mode": "tree,form",
            "views": [
                (self.env.ref("product.product_pricelist_item_tree_view_from_product").id, "tree"),
                (False, "form"),
            ],
    ...
    def _get_attribute_exclusions(self, parent_combination=None, parent_name=None):
        ...
        archived_products = self.with_context(active_test=False).product_variant_ids.filtered(
        ...
        return {
            ...
            "archived_combinations": [
                product.product_template_attribute_value_ids.ids for product in archived_products
            ],
    ...
    def _cartesian_product(self, product_template_attribute_values_per_line, parent_combination):
        ...
        while True:
            ...
            if current_exclusions[current_ptav] or any(
                intersection in partial_combination for intersection in all_exclusions[current_ptav]
            ):
                continue
