...
class TestRepair(AccountTestInvoicingCommon):
    ...
    def test_00_repair_afterinv(self):
        ...
        self.assertEqual(
            repair.state, "under_repair", 'Repair order should be in "Under_repair" state.'
        )
        ...
        self.assertEqual(
            len(repair.move_id.move_line_ids[0].consume_line_ids), 1, "Consume lines should be set"
        )
    ...
    def test_repair_state(self):
        ...
        self.assertEqual(
            len(repair.invoice_id), 0, "No invoice should be exists for this repair order"
        )
