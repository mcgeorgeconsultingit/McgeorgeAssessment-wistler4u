...
class ResPartnerBank(models.Model):
    ...
    def _is_isr_issuer(self):
        return _is_l10n_ch_isr_issuer(self.l10n_ch_postal, "CHF") or _is_l10n_ch_isr_issuer(
            self.l10n_ch_postal, "EUR"
        )
    ...
    def _compute_l10n_ch_show_subscription(self):
        for bank in self:
            if bank.partner_id:
                ...
                bank.l10n_ch_show_subscription = bank.company_id.account_fiscal_country_id.code in (
                    "CH",
                    "LI",
                )
            else:
    ...
    def _l10n_ch_get_qr_vals(
        self, amount, currency, debtor_partner, free_communication, structured_communication
    ):
        comment = ""
    ...
    def _get_qr_vals(
        ...
        if qr_method == "ch_qr":
            return self._l10n_ch_get_qr_vals(
                amount, currency, debtor_partner, free_communication, structured_communication
            )
    ...
    def _is_iso11649_reference(self, reference):
        ...
        return (
            ...
            and int(
                "".join(
                    str(int(x, 36))
                    for x in clean(reference[4:] + reference[:4], " -.,/:").upper().strip()
                )
            )
            % 97
            == 1
