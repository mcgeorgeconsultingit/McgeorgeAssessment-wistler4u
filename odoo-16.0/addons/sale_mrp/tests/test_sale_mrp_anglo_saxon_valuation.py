...
class TestSaleMRPAngloSaxonValuation(ValuationReconciliationTestCommon):
    ...
    def test_sale_mrp_kit_bom_cogs(self):
        ...
        self.assertAlmostEqual(
            stock_out_aml.credit, 1.53, "Should not include the value of consumable component"
        )
    ...
    def test_kit_avco_fully_owned_and_delivered_invoice_post_delivery(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            compo01, self.company_data["default_warehouse"].lot_stock_id, 1, owner_id=self.partner_b
        )
        self.env["stock.quant"]._update_available_quantity(
            compo02, self.company_data["default_warehouse"].lot_stock_id, 1, owner_id=self.partner_b
        )
