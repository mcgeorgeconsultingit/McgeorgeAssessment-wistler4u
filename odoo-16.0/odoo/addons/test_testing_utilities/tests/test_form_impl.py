...
class TestBasic(TransactionCase):
    ...
    def test_readonly_save(self):
        ...
        f = Form(
            self.env["test_testing_utilities.a"], view="test_testing_utilities.non_normalized_attrs"
        )
...
class TestM2O(TransactionCase):
    def test_default_and_onchange(self):
        ...
        f.f2 = "B"
        self.assertEqual(f.f, b, "The new m2o value should match the second field by name")
...
class TestM2M(TransactionCase):
    ...
    def test_attr(self):
        f = Form(
            self.env["test_testing_utilities.e"], view="test_testing_utilities.attrs_using_m2m"
        )
        with self.assertRaises(AssertionError):
            f.count = 5
...
class TestO2M(TransactionCase):
    def test_basic_alterations(self):
        ...
        """
        f = Form(
            self.env["test_testing_utilities.parent"], view="test_testing_utilities.o2m_parent"
        )
        ...
        self.assertEqual([get(s) for s in r.subs], [("2", 2, 2), ("5", 5, 5), ("2", 2, 2)])
    ...
    def test_o2m_editable_list(self):
        ...
        """
        f = Form(
            self.env["test_testing_utilities.parent"], view="test_testing_utilities.o2m_parent_ed"
        )
        ...
        self.assertEqual([get(s) for s in r.subs], [("1", 1, 1), ("3", 3, 3), ("7", 7, 7)])
    ...
    def test_readonly_o2m(self):
        ...
        """
        r = self.env["test_testing_utilities.parent"].create({"subs": [Command.create({})]})
    ...
    def test_o2m_widget(self):
        ...
        f = Form(
            self.env["test_testing_utilities.parent"], view="test_testing_utilities.o2m_widget_m2m"
        )
        f.subs.add(a)
        f.subs.add(b)
        f.subs.add(c)
...
class TestEdition(TransactionCase):
    ...
    def test_m2m_nonempty(self):
        ...
        r = self.env["test_testing_utilities.f"].create({"m2m": [Command.set((a | b | c).ids)]})
