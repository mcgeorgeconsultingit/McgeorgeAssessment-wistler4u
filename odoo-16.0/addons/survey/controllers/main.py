...
class Survey(http.Controller):
    ...
    def _fetch_from_access_token(self, survey_token, answer_token):
        ...
        if not answer_token:
            ...
            answer_sudo = (
                ...
                .sudo()
                .search(
                    [("survey_id", "=", survey_sudo.id), ("access_token", "=", answer_token)],
                    limit=1,
                )
            )
    ...
    def _redirect_with_error(self, access_data, error_key):
        ...
        if error_key == "survey_void" and access_data["can_answer"]:
            return request.render(
                "survey.survey_void_content", {"survey": survey_sudo, "answer": answer_sudo}
            )
            ...
            return request.render(
                "survey.survey_auth_required", {"survey": survey_sudo, "redirect_url": redirect_url}
            )
    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        ...
        page_or_question_key = (
            "question" if survey_sudo.questions_layout == "page_per_question" else "page"
        )
    ...
    def survey_print(self, survey_token, review=False, answer_token=None, **post):
        ...
        return request.render(
            "survey.survey_page_print",
            {
                ...
                "scoring_display_correction": survey_sudo.scoring_type == "scoring_with_answers"
                and answer_sudo,
                "format_datetime": lambda dt: format_datetime(request.env, dt, dt_format=False),
                "format_date": lambda date: format_date(request.env, date),
            },
    ...
    @http.route(
        '/survey/results/<model("survey.survey"):survey>', type="http", auth="user", website=True
    )
    ...
    def _generate_report(self, user_input, download=True):
        report = (
            ...
            .sudo()
            ._render_qweb_pdf(
                "survey.certification_report", [user_input.id], data={"report_type": "pdf"}
            )[0]
        )
    ...
    def _get_user_input_domain(self, survey, line_filter_domain, **post):
        ...
        if line_filter_domain:
            matching_line_ids = (
                request.env["survey.user_input.line"].sudo().search(line_filter_domain).ids
            )
