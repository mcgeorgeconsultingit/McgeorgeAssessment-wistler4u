...
class AccountReport(models.Model):
    ...
    filter_multi_company = fields.Selection(
        ...
        compute=lambda x: x._compute_report_option_filter("filter_multi_company", "disabled"),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    )
    ...
    filter_hierarchy = fields.Selection(
        ...
        compute=lambda x: x._compute_report_option_filter("filter_hierarchy", "optional"),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    )
        ...
        if "country_id" in vals:
            ...
            for expression in tax_tags_expressions:
                tax_tags = self.env["account.account.tag"]._get_tax_tags(
                    expression.formula, expression.report_line_id.report_id.country_id.id
                )
                tag_reports = (
                ...
                if all(report in self for report in tag_reports):
                    ...
                    if not existing_tax_tags:
                        tag_vals = self.env["account.report.expression"]._get_tags_create_vals(
                            expression.formula, vals["country_id"]
                        )
    ...
    def name_get(self):
        ...
        for report in self:
            result.append(
                (
                    report.id,
                    report.name + (f" ({report.country_id.code})" if report.country_id else ""),
                )
            )
        return result
...
class AccountReportLine(models.Model):
    ...
    children_ids = fields.One2many(
        string="Child Lines", comodel_name="account.report.line", inverse_name="parent_id"
    )
    groupby = fields.Char(
        string="Group By",
    ...
    def _copy_hierarchy(self, copied_report, parent=None, code_mapping=None):
        ...
        for expression in self.expression_ids:
            ...
            if expression.engine == "aggregation":
                ...
                copy_defaults[
                    "formula"
                ] = copied_formula.strip()  # Remove the spaces introduced for lookahead/lookbehind
...
class AccountReportExpression(models.Model):
    ...
    report_line_id = fields.Many2one(
        string="Report Line", comodel_name="account.report.line", required=True, ondelete="cascade"
    )
    ...
    def _unlink_archive_used_tags(self):
        ...
        if tags_to_archive or tags_to_unlink:
            ...
            rep_lines_with_tag.write(
                {"tag_ids": [Command.unlink(tag.id) for tag in tags_to_archive + tags_to_unlink]}
            )
    ...
    def _get_aggregation_terms_details(self):
        ...
        for expression in self:
            if expression.engine != "aggregation":
                raise UserError(
                    _("Cannot get aggregation details from a line not using 'aggregation' engine")
                )
    ...
    def action_view_carryover_lines(self, options):
        ...
        return {
            ...
            "views": [
                (self.env.ref("account_reports.account_report_external_value_tree").id, "list")
            ],
            "domain": [
...
class AccountReportExternalValue(models.Model):
    ...
    target_report_expression_id = fields.Many2one(
        string="Target Expression", comodel_name="account.report.expression", required=True
    )
    ...
    def _check_fiscal_position(self):
        for record in self:
            if (
                ...
                and record.foreign_vat_fiscal_position_id.country_id != record.report_country_id
            ):
                raise ValidationError(
                    _(
