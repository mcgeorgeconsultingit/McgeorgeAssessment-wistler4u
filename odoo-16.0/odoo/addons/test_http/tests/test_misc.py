    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                ...
                res_rpc = res.json()
                self.assertNotIn(
                    "error", res_rpc.keys(), res_rpc.get("error", {}).get("data", {}).get("message")
                )
                self.assertIn(
                    milky_way.name, res_rpc["result"], "QWeb template was correctly rendered"
                )
...
class TestHttpCors(TestHttpBase):
    def test_cors0_http_default(self):
        res_opt = self.opener.options(
            f"{self.base_url()}/test_http/cors_http_default", timeout=10, allow_redirects=False
        )
    ...
    def test_cors1_http_methods(self):
        res_opt = self.opener.options(
            f"{self.base_url()}/test_http/cors_http_methods", timeout=10, allow_redirects=False
        )
        self.assertIn(res_opt.status_code, (200, 204))
        self.assertEqual(res_opt.headers.get("Access-Control-Allow-Origin"), "*")
