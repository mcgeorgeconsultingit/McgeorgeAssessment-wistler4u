...
class AccountMoveLine(models.Model):
        ...
        for line in product_lines:
            if line.product_id:
                ...
                accounts = line.with_company(
                    line.company_id
                ).product_id.product_tmpl_id.get_product_accounts(fiscal_pos=fiscal_position)
                if line.move_id.is_sale_document(include_receipts=True):
    ...
    def _compute_amount_residual(self):
        ...
        for line in need_residual_lines:
            ...
            line.reconciled = comp_curr.is_zero(line.amount_residual) and foreign_curr.is_zero(
    ...
    def _compute_tax_audit(self):
        ...
        for record in self:
            ...
            for tag in record.tax_tag_ids:
                ...
                audit_str += (
                    tag_name + ": " + formatLang(self.env, tag_amount, currency_obj=currency)
                )
                ...
                frozendict(
                    {
                        ...
                        "currency_id": line.currency_id.id,
                        "analytic_distribution": (tax["analytic"] or not tax["use_in_tax_closing"])
                    ...
                    percentages_to_apply.append(
                        {
                            ...
                            "term_percentage": abs(
                                term["amount_currency"] / line.move_id.amount_total
                            )
                            if line.move_id.amount_total
                            else 0,
                        }
                    )
    ...
    def _check_constrains_account_id_journal_id(self):
        for line in self.filtered(lambda x: x.display_type not in ("line_section", "line_note")):
            ...
            if account.deprecated:
                raise UserError(
                    _("The account %s (%s) is deprecated.") % (account.name, account.code)
                )
            ...
            is_account_control_ok = (
                not journal.account_control_ids or account in journal.account_control_ids
            )
    ...
    def _check_caba_non_caba_shared_tags(self):
        ...
        def get_base_repartition(base_aml, taxes):
            ...
            repartition_field = (
                is_refund and "refund_repartition_line_ids" or "invoice_repartition_line_ids"
            )
        ...
        for aml in self:
            ...
            if not common_tags:
                ...
                comparison_tags = (
                    non_caba_base_tags
                    if aml.tax_repartition_line_id.tax_id.tax_exigibility == "on_payment"
                    else caba_base_tags
                )
    ...
    def write(self, vals):
        ...
        account_to_write = (
            self.env["account.account"].browse(vals["account_id"]) if "account_id" in vals else None
        )
        ...
        with self.move_id._check_balanced(move_container), self.move_id._sync_dynamic_lines(
            move_container
                ...
                for move_id, modified_lines in move_initial_values.items():
                    for line in self.filtered(lambda l: l.move_id.id == move_id):
                        ...
                        if tracking_value_ids:
                            msg = _(
                                "Journal Item %s updated", line._get_html_link(title=f"#{line.id}")
                            )
        ...
        if not self.env.context.get("dynamic_unlink"):
            for line in self:
                if line.display_type == "tax" and line.move_id.line_ids.tax_ids:
                    raise ValidationError(
                        _("You cannot delete a tax line as it would impact the tax report")
                    )
    ...
    def _prepare_exchange_difference_move_vals(
        ...
        move_vals = {
            "move_type": "entry",
            "date": max(
                exchange_date or date.min, company._get_user_fiscal_lock_date() + timedelta(days=1)
            ),
    ...
    def _create_exchange_difference_move(self, exchange_diff_vals):
        ...
        # Create the move.
        exchange_move = (
            self.env["account.move"].with_context(skip_invoice_sync=True).create(move_vals)
        )
    ...
    def _add_exchange_difference_cash_basis_vals(self, exchange_diff_vals):
        ...
        for move in self.move_id:
            ...
            for transition_line in filter(
                lambda x: x.account_id in caba_transition_accounts, cash_basis_moves.line_ids
            ):
                caba_reconcile_key = (
            ...
            currency_line = last_caba_move.line_ids.filtered(lambda x: x.currency_id == currency)[
                :1
            ]
            currency_rate = (
        ...
        for line in self.filtered(lambda line: line.display_type == "product"):
            line._validate_distribution(
                **{
                    ...
                    and "invoice"
                    or line.move_id.move_type in ["in_invoice", "in_refund", "in_receipt"]
                    and "bill"
                    or "general",
                    "company_id": line.company_id.id,
                }
    ...
    def _get_attachment_domains(self):
        ...
        if self.payment_id:
            domains.append(
                [("res_model", "=", "account.payment"), ("res_id", "=", self.payment_id.id)]
            )
        return domains
