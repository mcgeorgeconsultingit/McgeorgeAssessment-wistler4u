...
class SaleOrder(models.Model):
        ...
        for order in self:
            ...
            if any(order._get_real_points_for_coupon(coupon) < 0 for coupon in all_coupons):
                raise ValidationError(
                    _("One or more rewards on the sale order is invalid. Please check them.")
                )
    ...
    def _get_real_points_for_coupon(self, coupon, post_confirm=False):
        ...
        if (
            coupon.program_id.applies_on != "future" and self.state not in ("sale", "done")
        ) or post_confirm:
    ...
    def _update_programs_and_rewards(self):
        ...
        self.applied_coupon_ids = self.applied_coupon_ids.filtered(
            lambda c: (not c.expiration_date or c.expiration_date >= fields.Date.today())
        )
        ...
        for line in self.order_line:
            ...
            if line.reward_id.program_id.is_payment_program:
                payment_rewards.append(
                    (line.reward_id, line.coupon_id, line.reward_identifier_code, line.product_id)
                )
            else:
                line_rewards.append(
                    (line.reward_id, line.coupon_id, line.reward_identifier_code, line.product_id)
    ...
    def _program_check_compute_points(self, programs):
        ...
        amounts_per_program = {
            p: {"untaxed": base_untaxed_amount, "tax": base_tax_amount} for p in programs
        }
        for line in self.order_line:
