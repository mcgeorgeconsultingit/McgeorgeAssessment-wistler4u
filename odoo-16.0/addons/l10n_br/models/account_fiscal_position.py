...
class AccountFiscalPosition(models.Model):
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        if self.env.company.state_id == delivery.state_id:
            return self.search(
                [("l10n_br_fp_type", "=", "internal"), ("company_id", "=", self.env.company.id)],
                limit=1,
            )
        if (
            ...
            return self.search(
                [("l10n_br_fp_type", "=", "ss_nnm"), ("company_id", "=", self.env.company.id)],
                limit=1,
            )
        return self.search(
            [("l10n_br_fp_type", "=", "interstate"), ("company_id", "=", self.env.company.id)],
            limit=1,
        )
