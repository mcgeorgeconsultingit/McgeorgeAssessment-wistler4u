...
class TestSubcontractingBasic(TransactionCase):
    def test_subcontracting_location_1(self):
        ...
        self.assertTrue(
            self.env.company.subcontracting_location_id != company2.subcontracting_location_id
        )
...
class TestSubcontractingFlows(TestMrpSubcontractingCommon):
    def test_flow_1(self):
        ...
        self.assertTrue(
            all(m.product_uom_qty == m.reserved_availability for m in picking_receipt.move_ids)
        )
    ...
    def test_flow_2(self):
        ...
        avail_qty_comp1_in_global_location = self.env["stock.quant"]._get_available_quantity(
            self.comp1, self.env.company.subcontracting_location_id, allow_negative=True
        )
        avail_qty_comp2_in_global_location = self.env["stock.quant"]._get_available_quantity(
    ...
    def test_flow_9(self):
        ...
        (self.comp1 + self.comp2).write({"route_ids": [(4, resupply_sub_on_order_route.id)]})
    ...
    def test_flow_warning_bom_1(self):
        ...
        warning = Form(
            self.env["mrp.consumption.warning"].with_context(**action_warning["context"])
        )
...
class TestSubcontractingTracking(TransactionCase):
    ...
    def test_flow_tracked_backorder(self):
        ...
        self.assertEqual(mo.state, "done")
        self.assertEqual(
            mo.procurement_group_id.mrp_production_ids.mapped("state"), ["done"] * todo_nb
        )
        self.assertEqual(len(mo.procurement_group_id.mrp_production_ids), todo_nb)
        self.assertEqual(
            mo.procurement_group_id.mrp_production_ids.mapped("qty_produced"), [1] * todo_nb
        )
    ...
    def test_flow_tracked_backorder02(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            component, self.env.ref("stock.stock_location_stock"), todo_nb, lot_id=component_lot
        )
...
class TestSubcontractingSerialMassReceipt(TransactionCase):
    ...
    def test_receive_after_resupply(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.raw_material, self.env.ref("stock.stock_location_stock"), sum(quantities)
        )
        # Create a receipt picking from the subcontractor
        picking_form = Form(self.env["stock.picking"])
        picking_form.picking_type_id = self.env.ref("stock.picking_type_in")
        ...
        # Process the delivery of the components
        picking_deliver = (
            self.env["mrp.production"].search([("bom_id", "=", self.bom.id)]).picking_ids
        )
