...
class TestProductPricelist(ProductCommon):
    ...
    def test_10_calculation_price_of_products_pricelist(self):
        ...
        self.assertEqual(
            float_compare(
                price, (product.lst_price - product.lst_price * (0.10)), precision_digits=2
            ),
            0,
            msg,
        )
        ...
        self.assertEqual(
            float_compare(
                price, product.lst_price - product.lst_price * (0.05), precision_digits=2
            ),
            0,
            msg,
        )
        ...
        self.assertEqual(
            float_compare(
                price, product.lst_price - product.lst_price * (0.30), precision_digits=2
            ),
            0,
            msg,
        )
        ...
        context.update(
            {"quantity": 1, "date": datetime.strptime("2020-04-05 08:00:00", "%Y-%m-%d %H:%M:%S")}
        )
        ...
        self.assertEqual(float_compare(price, product.lst_price / 2, precision_digits=2), 0, msg)
        ...
        pricelist = self.env["product.pricelist"].create(
            {
                ...
                "item_ids": [
                    (
                        0,
                        0,
                        {"compute_price": "formula", "base": "list_price", "price_surcharge": 100},
                    )
                ],
            }
        )
