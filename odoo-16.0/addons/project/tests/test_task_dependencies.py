...
class TestTaskDependencies(TestProjectCommon):
    ...
    def test_task_dependencies(self):
        ...
        """
        self.assertEqual(
            len(self.task_1.depend_on_ids), 0, "The task 1 should not have any dependency."
        )
        self.task_1.write(
            {
    ...
    def test_cyclic_dependencies(self):
        ...
        self.assertTrue(
            self.task_1.allow_task_dependencies, "The task dependencies feature should be enable."
        )
        self.assertTrue(
            self.task_2.allow_task_dependencies, "The task dependencies feature should be enable."
        )
        self.assertTrue(
            self.task_3.allow_task_dependencies, "The task dependencies feature should be enable."
        )
    ...
    def test_task_dependencies_settings_change(self):
        ...
        self.project_chickens = self.env["project.project"].create({"name": "My Chicken Project"})
        self.assertTrue(
        ...
        self.project_ducks = self.env["project.project"].create({"name": "My Ducks Project"})
        self.assertFalse(
    ...
    def test_duplicate_project_with_subtask_dependencies(self):
        ...
        self.assertEqual(
            node1.dependent_ids.ids, node2.ids, "Only Node1 - Node2 relation should be present"
        )
        self.assertEqual(len(node2.dependent_ids), 1)
        self.assertEqual(
            node2.dependent_ids.ids, node3.ids, "Only Node2 - Node3 relation should be present"
        )
