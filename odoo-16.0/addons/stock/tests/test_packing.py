...
class TestPacking(TestPackingCommon):
    def test_put_in_pack(self):
        ...
        self.assertEqual(
            pick_picking.package_level_ids[0].state, "new", 'A new pack should be in state "new"'
        )
        pick_picking.move_line_ids.filtered(
    ...
    def test_pick_a_pack_confirm(self):
        ...
        self.assertEqual(len(package_level.move_ids), 1, "The move should be in the package level")
        self.assertEqual(
            package_level.state,
            "confirmed",
        ...
        self.assertEqual(
            len(picking.move_ids), 1, "You still have only one move when the picking is assigned"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            package_level.state, "assigned", "The package level must be in state assigned"
        )
    ...
    def test_pick_a_pack_cancel_is_done(self):
        ...
        self.assertEqual(package_level.state, "cancel")
        self.assertTrue(package_level.move_line_ids)
        self.assertTrue(all(package_level.move_line_ids.mapped(lambda l: l.state == "cancel")))
    ...
    def test_put_in_pack_to_different_location(self):
        ...
        self.assertEqual(
            pack2.location_id.id, shelf2_location.id, "The package must be stored  in shelf2"
        )
        self.assertEqual(
            pack1.location_id.id, shelf1_location.id, "The package must be stored  in shelf1"
        )
    ...
    def test_pack_in_receipt_two_step_single_putway(self):
        ...
        * In this internal transfer, adds the package then valid it.
        ...
        self.assertEqual(
            receipt_package.move_line_ids[0].location_dest_id.id, receipt.location_dest_id.id
        )
        self.assertEqual(
            receipt_package.move_line_ids[1].location_dest_id.id, receipt.location_dest_id.id
        )
        ...
        internal_transfer = self.env["stock.picking"].search(
            [("picking_type_id", "=", self.warehouse.int_type_id.id)], order="id desc", limit=1
        )
    ...
    def test_2_steps_and_backorder(self):
        ...
        picking, moveA, moveB = create_picking(
            pick_type, pick_type.default_location_src_id, pick_type.default_location_dest_id
        )
        ...
        backorder_wizard = Form(
            self.env["stock.backorder.confirmation"].with_context(action_data["context"])
        ).save()
    ...
    def test_pack_in_receipt_two_step_multi_putaway_02(self):
        ...
        self.env.user.write(
            {"groups_id": [(4, self.env.ref("stock.group_stock_storage_categories").id)]}
        )
        self.env.user.write(
    ...
    def test_put_out_of_pack_transfer(self):
        ...
        self.assertEqual(
            pack.location_id.id, loc_2.id, "Package should have been moved to Location B."
        )
        ...
        self.assertEqual(
            quantB.package_id.id, pack.id, "Product B should still be in the initial package."
        )
