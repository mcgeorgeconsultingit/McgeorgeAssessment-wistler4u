...
class TestPortalControllers(TestPortal):
    def test_redirect_to_records(self):
        ...
        response = self.url_open(
            "/mail/view?model=%s&res_id=%s" % (self.record_portal._name, self.record_portal.id),
            timeout=15,
        )
    ...
    def test_redirect_to_records_norecord(self):
        ...
        for model, res_id in [
            ...
            response = self.url_open("/mail/view?model=%s&res_id=%s" % (model, res_id), timeout=15)
            path = url_parse(response.url).path
            self.assertEqual(path, "/web/login", "Failed with %s - %s" % (model, res_id))
    ...
    def test_portal_message_fetch(self):
        ...
        def get_chatter_message_count():
            res = self.url_open(
                url=message_fetch_url, data=payload, headers={"Content-Type": "application/json"}
            )
