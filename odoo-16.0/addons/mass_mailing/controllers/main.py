...
class MassMailController(http.Controller):
    ...
    @http.route(
        ["/mail/mailing/<int:mailing_id>/unsubscribe"], type="http", website=True, auth="public"
    )
    def mailing(self, mailing_id, email=None, res_id=None, token="", **post):
        ...
        if mailing.exists():
            ...
            if mailing.mailing_model_real == "mailing.contact":
                ...
                opt_out_list_ids = subscription_list_ids.filtered(lambda rel: rel.opt_out).mapped(
                    "list_id"
                )
                ...
                opt_out_list_ids = set(
                    [list.id for list in opt_out_list_ids if list not in opt_in_list_ids]
                )
                ...
                unsubscribed_list = ", ".join(
                    str(list.name) for list in mailing.contact_list_ids if list.is_public
                )
                ...
                opt_in_lists = (
                    ...
                    .sudo()
                    .search([("contact_id.email_normalized", "=", email), ("opt_out", "=", False)])
                    .mapped("list_id")
                )
                blacklist_rec = request.env["mail.blacklist"].sudo()._add(email)
                self._log_blacklist_action(
                    blacklist_rec, mailing_id, _("""Requested blacklisting via unsubscribe link.""")
                )
                return request.render(
        ...
        if mailing.exists() and email:
            ...
            for record in records:
                record.sudo().message_post(
                    body=_("Feedback from %(email)s: %(feedback)s", email=email, feedback=feedback)
                )
            return bool(records)
        return "error"
    ...
    def track_mail_open(self, mail_id, token, **post):
        """Email tracking."""
        if not consteq(
            token, tools.hmac(request.env(su=True), "mass_mailing-mail_mail-open", mail_id)
        ):
            raise BadRequest()
    ...
    def view(self, mailing_id, email=None, res_id=None, token=""):
        ...
        if mailing.exists():
            ...
            ) and not request.env.user.has_group("mass_mailing.group_mass_mailing_user"):
                raise exceptions.AccessDenied()
    ...
    def blacklist_add(self, mailing_id, res_id, email, token):
        ...
        if email:
            blacklist_rec = request.env["mail.blacklist"].sudo()._add(email)
            self._log_blacklist_action(
                blacklist_rec, mailing_id, _("""Requested blacklisting via unsubscription page.""")
            )
            return True
        return "error"
