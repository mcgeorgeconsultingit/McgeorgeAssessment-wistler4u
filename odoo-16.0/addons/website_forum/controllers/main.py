...
class WebsiteForum(WebsiteProfile):
    ...
    def questions(
        self, forum, tag=None, page=1, filters="all", my=None, sorting=None, search="", **post
    ):
    ...
    @http.route(
        "/forum/get_tags", type="http", auth="public", methods=["GET"], website=True, sitemap=False
    )
    ...
    def old_question(self, forum, question, **post):
        ...
        return request.redirect(
            _build_url_w_params("/forum/%s/%s" % (slug(forum), slug(question)), request.params),
            code=301,
        )
    ...
    def post_create(self, forum, post_parent=None, **post):
        ...
        return request.redirect(
            "/forum/%s/%s" % (slug(forum), post_parent and slug(post_parent) or new_question.id)
        )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/flagged_queue', type="http", auth="user", website=True
    )
    ...
    def post_json_ask_for_mark_as_offensive(self, post, **kwargs):
        if not post.can_moderate:
            raise AccessError(
                _("%d karma required to mark a post as offensive.", post.forum_id.karma_moderate)
            )
    ...
    def post_http_ask_for_mark_as_offensive(self, forum, post, **kwargs):
        if not post.can_moderate:
            raise AccessError(
                _("%d karma required to mark a post as offensive.", forum.karma_moderate)
            )
        values = self._prepare_mark_as_offensive_values(post, **kwargs)
        return request.render("website_forum.close_post", values)
    ...
    def _prepare_user_profile_values(self, user, **post):
        ...
        if not post.get("no_forum"):
            ...
            values.update(
                self._prepare_user_values(forum=forums[0] if len(forums) == 1 else True, **post)
            )
            if forums:
