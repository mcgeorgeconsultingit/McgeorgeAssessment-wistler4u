...
class TestLeadAssignCommon(TestLeadConvertCommon):
    ...
    def setUpClass(cls):
        ...
        with mute_logger("odoo.models.unlink"):
            cls.env["crm.lead"].with_context(active_test=False).search(
                ["|", ("team_id", "=", False), ("user_id", "in", cls.sales_teams.member_ids.ids)]
            ).unlink()
        cls.bundle_size = 50
    ...
    def assertInitialData(self):
        ...
        self.assertEqual(
            self.sales_team_convert.assignment_domain, "[('priority', 'in', ['1', '2', '3'])]"
        )
        self.assertEqual(
...
class TestLeadAssign(TestLeadAssignCommon):
    ...
    def test_assign_configuration(self):
        ...
        with patch.object(fields.Datetime, "now", return_value=now_patch):
            ...
            self.assertEqual(
                self.assign_cron.nextcall, datetime(2020, 11, 2, 10, 0, 0) + relativedelta(hours=19)
            )
            ...
            self.assertTrue(self.assign_cron.active)
            self.assertEqual(
                self.assign_cron.nextcall, datetime(2020, 11, 2, 10, 0, 0) + relativedelta(days=2)
            )
    ...
    def test_assign_quota(self):
        ...
        existing_leads = self._create_leads_batch(
            lead_type="lead", user_ids=[self.user_sales_leads.id], probabilities=[10], count=30
        )
        self.assertEqual(
            existing_leads.team_id, self.sales_team_1, "Team should have lower sequence"
        )
    ...
    def test_assign_specific_won_lost(self):
        ...
        leads[5].update(
            {"team_id": self.sales_team_convert.id, "user_id": self.user_sales_manager.id}
        ...
        self.assertEqual(
            leads[5].team_id, self.sales_team_convert, "Assigned lead should not be reassigned"
        )
        self.assertEqual(
            leads[5].user_id, self.user_sales_manager, "Assigned lead should not be reassigned"
        )
    ...
    def test_merge_assign_keep_master_team(self):
        ...
        self.assertEqual(
            master_opp.team_id, self.sales_team_1, "Opportunity: should keep its sales team"
        )
        self.assertEqual(
            master_opp.user_id, self.user_sales_manager, "Opportunity: should keep its salesman"
        )
