        ...
        for wizard in self:
            ...
            if to_send:
                ...
                record._notify_thread_by_email(
                    message, email_partners_data, resend_existing=True, send_after_commit=False
                )
...
class PartnerResend(models.TransientModel):
    ...
    name = fields.Char(
        related="partner_id.name", string="Recipient Name", related_sudo=False, readonly=False
    )
    email = fields.Char(
        related="partner_id.email", string="Email Address", related_sudo=False, readonly=False
    )
