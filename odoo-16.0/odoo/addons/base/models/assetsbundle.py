...
class AssetsBundle(object):
            ...
            if js:
                if f["atype"] == "text/javascript":
                    self.javascripts.append(
                        JavascriptAsset(
                            self, url=f["url"], filename=f["filename"], inline=f["content"]
                        )
                    )
                elif f["atype"] == "text/xml":
                    self.templates.append(
                        XMLAsset(self, url=f["url"], filename=f["filename"], inline=f["content"])
                    )

    def to_node(
        self, css=True, js=True, debug=False, async_load=False, defer_load=False, lazy_load=False
    ):
        """
    ...
    def last_modified_combined(self):
        ...
        assets = [
            ...
            for f in self.files
            if f["atype"]
            in ["text/sass", "text/scss", "text/less", "text/css", "text/javascript", "text/xml"]
        ]
    ...
    def checksum(self):
        ...
        """
        check = "%s%s" % (json.dumps(self.files, sort_keys=True), self.last_modified_combined)
    ...
    def get_asset_url(self, id="%", unique="%", extra="", name="%", sep="%", extension="%"):
        return self._get_asset_template_url().format(
            **self._get_asset_url_values(
                id=id, unique=unique, extra=extra, name=name, sep=sep, extension=extension
            )
        )
    ...
    def clean_attachments(self, extension):
        ...
        url = self.get_asset_url(
            extra="%s"
            % ("rtl/" if extension in ["css", "min.css"] and self.user_direction == "rtl" else ""),
            name=self.name,
            sep="",
    ...
    def js_with_sourcemap(self, template_bundle=None):
        ...
        """
        sourcemap_attachment = self.get_attachments("js.map") or self.save_attachment("js.map", "")
            ...
            for template_tree in list(content_templates_tree):
                ...
                if "t-inherit" in template_tree.attrib:
                    ...
                    if parent_name not in template_dict[parent_addon]:
                        raise ValueError(
                            _("Cannot create %r because the template to inherit %r is not found.")
                            % (f"{addon}.{template_name}", f"{parent_addon}.{parent_name}")
                        )
                            ...
                            if position == "attributes":
                                if item.get("expr"):
                                    comment_node = etree.Element(
                                        "xpath", {"expr": item.get("expr"), "position": "before"}
                                    )
                                else:
                    ...
                    if template_name in template_dict[addon]:
                        raise ValueError(
                            _("Template %r already exists in module %r", template_name, addon)
                        )
    ...
    def css(self, is_minified=True):
        ...
        if not attachments:
            ...
            matches = []
            css = re.sub(
                self.rx_css_import, lambda matchobj: matches.append(matchobj.group(0)) and "", css
            )
    ...
    def css_with_sourcemap(self, content_import_rules):
        ...
        generator = SourceMapGenerator(
            source_root="/".join([".." for i in range(0, len(debug_asset_url.split("/")) - 2)])
            + "/",
        )
        ...
        for asset in self.stylesheets:
            if asset.content:
                ...
                content = re.sub(
                    self.rx_css_import, lambda matchobj: f"/* {matchobj.group(0)} */", content
                )
    ...
    def is_css_preprocessed(self):
        ...
        for atype in asset_types:
            outdated = False
            assets = dict(
                (asset.html_url, asset) for asset in self.stylesheets if isinstance(asset, atype)
            )
            if assets:
        ...
        if self.stylesheets:
            ...
            if self.user_direction == "rtl":
                stylesheet_assets = [
                    ...
                    for asset in self.stylesheets
                    if not isinstance(
                        asset, (SassStylesheetAsset, ScssStylesheetAsset, LessStylesheetAsset)
                    )
                ]
    ...
    def compile_css(self, compiler, source):
        ...
        compiled = re.sub(
            r"(appearance: (\w+);)", r"-webkit-appearance: \2; -moz-appearance: \2; \1", compiled
        )
    ...
    def stat(self):
        if not (self.inline or self._filename or self._ir_attach):
            ...
            try:
                ...
                self._ir_attach = (
                    self.bundle.env["ir.attachment"].sudo()._get_serve_attachment(self.url)
                )
