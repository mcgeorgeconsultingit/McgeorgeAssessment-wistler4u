...
class ProductAttribute(models.Model):
    ...
    create_variant = fields.Selection(
        [("always", "Instantly"), ("dynamic", "Dynamically"), ("no_variant", "Never (option)")],
        default="always",
    ...
    display_type = fields.Selection(
        [("radio", "Radio"), ("pills", "Pills"), ("select", "Select"), ("color", "Color")],
        default="radio",
        required=True,
...
class ProductAttributeValue(models.Model):
    ...
    is_custom = fields.Boolean(
        "Is custom value", help="Allow users to input custom values for this attribute value"
    )
...
class ProductTemplateAttributeLine(models.Model):
    ...
    product_template_value_ids = fields.One2many(
        "product.template.attribute.value", "attribute_line_id", string="Product Attribute Values"
    )
    ...
    def create(self, vals_list):
        ...
        for value in vals_list:
            ...
            if archived_ptal:
                ...
                archived_ptal.with_context(update_product_template_attribute_values=False).write(
                    vals
                )
            ...
            for pav in remaining_pav:
                ...
                if ptav:
                    ...
                    ptav_to_create.append(
                        {"product_attribute_value_id": pav.id, "attribute_line_id": ptal.id}
                    )
    ...
    def write(self, values):
        ...
        if pav_in_values or product_in_values:
            for ptav in self:
                if (
                    pav_in_values
                    and ptav.product_attribute_value_id.id != values["product_attribute_value_id"]
                ):
                    raise UserError(
