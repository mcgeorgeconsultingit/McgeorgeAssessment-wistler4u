...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option(
            "--test-file", dest="test_file", my_default=False, help="Launch a python test file."
        )
        group.add_option(
            "--test-enable",
            action="callback",
        ...
        group.add_option(
            "--smtp-port", dest="smtp_port", my_default=25, help="specify the SMTP port", type="int"
        )
        group.add_option(
            "--smtp-ssl",
            dest="smtp_ssl",
        ...
        group.add_option(
            "-d", "--database", dest="db_name", my_default=False, help="specify the database name"
        )
        group.add_option(
            "-r",
            "--db_user",
        ...
        group.add_option(
            "--db_host", dest="db_host", my_default=False, help="specify the database host"
        )
        group.add_option(
            "--db_port",
            dest="db_port",
    ...
    def _parse_config(self, args=None):
        ...
        die(bool(opt.syslog) and bool(opt.logfile), "the syslog and logfile options are exclusive")
        ...
        die(
            opt.overwrite_existing_translations and not (opt.translate_in or opt.update),
        ...
        self.options["upgrade_path"] = (
            ",".join(self._normalize(x) for x in self.options["upgrade_path"].split(","))
        ...
        for path in value.split(","):
            ...
            if not self._is_addons_path(res):
                raise optparse.OptionValueError(
                    "option %s: the path %r is not a valid addons directory" % (opt, path)
                )
            ad_paths.append(res)
    ...
    def _check_upgrade_path(self, option, opt, value, parser):
        ...
        for path in value.split(","):
            ...
            if not self._is_upgrades_path(res):
                raise optparse.OptionValueError(
                    "option %s: the path %r is not a valid upgrade directory" % (opt, path)
                )
    ...
    def _is_upgrades_path(self, res):
        return any(
            glob.glob(os.path.join(res, f"*/*/{prefix}-*.py")) for prefix in ["pre", "post", "end"]
        )
    ...
    def set_admin_password(self, new_password):
        hash_password = (
            crypt_context.hash if hasattr(crypt_context, "hash") else crypt_context.encrypt
        )
