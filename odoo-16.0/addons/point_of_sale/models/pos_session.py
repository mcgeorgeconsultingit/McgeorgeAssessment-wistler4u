...
class PosSession(models.Model):
    ...
    statement_line_ids = fields.One2many(
        "account.bank.statement.line", "pos_session_id", string="Cash Lines", readonly=True
    )
    ...
    payment_method_ids = fields.Many2many(
        "pos.payment.method", related="config_id.payment_method_ids", string="Payment Methods"
    )
        ...
        for session in self:
            ...
            session.failed_pickings = bool(
                self.env["stock.picking"].search(
                    [("pos_session_id", "=", session.id), ("state", "!=", "done")], limit=1
                )
            )
    ...
    def _check_start_date(self):
        for record in self:
            ...
            if (company.period_lock_date and start_date <= company.period_lock_date) or (
                company.fiscalyear_lock_date and start_date <= company.fiscalyear_lock_date
            ):
    ...
    def _check_invoices_are_posted(self):
        ...
        if unposted_invoices:
            raise UserError(
                ...
                % str.join(
                    "\n",
                    ["%s - %s" % (invoice.name, invoice.state) for invoice in unposted_invoices],
                )
            )
    ...
    def close_session_from_ui(self, bank_payment_method_diff_pairs=None):
        ...
        if isinstance(validate_result, dict):
            ...
            return {"successful": False, "message": validate_result.get("name"), "redirect": True}
    ...
    def _get_diff_vals(self, payment_method_id, diff_amount):
        ...
        source_vals = self._debit_amounts(
            {"account_id": source_account.id}, amounts["amount"], amounts["amount_converted"]
        )
        dest_vals = self._credit_amounts(
            {"account_id": destination_account.id}, amounts["amount"], amounts["amount_converted"]
        )
    ...
    def _prepare_balancing_line_vals(self, imbalance_amount, move, balancing_account):
        ...
        if not self.is_in_company_currency:
            imbalance_amount_session = self.company_id.currency_id._convert(
                imbalance_amount, self.currency_id, self.company_id, fields.Date.context_today(self)
            )
            ...
            if not order_is_invoiced:
                ...
                if self.company_id.anglo_saxon_accounting and order.picking_ids.ids:
                    ...
                    for move in stock_moves:
                        ...
                        amount = signed_product_qty * move.product_id._compute_average_price(
                            0, move.product_qty, move
                        )
    ...
    def _create_non_reconciliable_move_lines(self, data):
        ...
        MoveLine.create(
            ...
            + [
                self._get_stock_expense_vals(key, amounts["amount"], amounts["amount_converted"])
    ...
    def _get_rounding_difference_vals(self, amount, amount_converted):
        if self.config_id.cash_rounding:
            ...
            if (
                float_compare(0.0, amount, precision_rounding=self.currency_id.rounding) < 0
            ):  # profit
    ...
    def _credit_amounts(
        self, partial_move_line_vals, amount, amount_converted, force_company_currency=False
    ):
    ...
    def _get_attributes_by_ptal_id(self):
        ...
        for key, group in groupby(sorted(product_template_attribute_values, key=key), key=key):
            ...
            values = [
                {
                    **ptav.product_attribute_value_id.read(["name", "is_custom", "html_color"])[0],
    ...
    def _pos_data_process(self, loaded_data):
        ...
        for fiscal_position in loaded_data["account.fiscal.position"]:
            fiscal_position["fiscal_position_taxes_by_id"] = {
                tax_id: fiscal_position_by_id[tax_id] for tax_id in fiscal_position["tax_ids"]
            }
    ...
    def _get_pos_ui_res_users(self, params):
        ...
        user["role"] = (
            "manager"
            if any(id == self.config_id.group_pos_manager_id.id for id in user["groups_id"])
            else "cashier"
        )
        del user["groups_id"]
        return user
    ...
    def _get_pos_ui_product_category(self, params):
        ...
        for category in categories:
            category["parent"] = (
                category_by_id[category["parent_id"][0]] if category["parent_id"] else None
            )
        return categories
