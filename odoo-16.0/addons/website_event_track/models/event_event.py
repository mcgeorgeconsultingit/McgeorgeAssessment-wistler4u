...
class Event(models.Model):
    ...
    website_track = fields.Boolean(
        "Tracks on Website", compute="_compute_website_track", readonly=False, store=True
    )
    ...
    allowed_track_tag_ids = fields.Many2many(
        "event.track.tag", relation="event_allowed_track_tags_rel", string="Available Track Tags"
    )
    ...
    def _compute_website_track(self):
        ...
        for event in self:
            if event.event_type_id and event.event_type_id != event._origin.event_type_id:
                ...
                event.website_menu != event._origin.website_menu or not event.website_track
            ):
    ...
    def _compute_tracks_tag_ids(self):
        for event in self:
            event.tracks_tag_ids = (
                event.track_ids.mapped("tag_ids").filtered(lambda tag: tag.color != 0).ids
            )
    ...
    def _update_website_menus(self, menus_update_by_field=None):
        ...
        for event in self:
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("website_track")
            ):
            ...
            if event.menu_id and (
                ...
                event._update_website_menu_entry(
                    "website_track_proposal", "track_proposal_menu_ids", "track_proposal"
                )
