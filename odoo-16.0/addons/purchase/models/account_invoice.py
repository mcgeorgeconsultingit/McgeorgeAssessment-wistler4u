...
class AccountMove(models.Model):
    ...
    def _get_invoice_reference(self):
        self.ensure_one()
        vendor_refs = [
            ref for ref in set(self.line_ids.mapped("purchase_line_id.order_id.partner_ref")) if ref
        ]
        if self.ref:
            return [
    ...
    def _onchange_purchase_auto_complete(self):
        ...
        * If it's a purchase order, 'purchase_id' is set and this method will load lines.
        ...
        invoice_vals["currency_id"] = (
            self.invoice_line_ids and self.currency_id or invoice_vals.get("currency_id")
        )
    ...
    def find_matching_subset_invoice_lines(self, invoice_lines, goal_total, timeout):
        ...
        try:
            subsets = _find_matching_subset_invoice_lines(
                sorted(invoice_lines, key=lambda line: line["amount_to_invoice"], reverse=True),
                goal_total,
            )
        ...
        if po_references and amount_total:
            ...
            if matching_pos:
                ...
                if (
                    ...
                    < sum(line["amount_to_invoice"] for line in matching_pos_invoice_lines)
                    < amount_total + TOLERANCE
                ):
                    ...
                    if il_subset:
                        return "subset_total_match", self.env["purchase.order.line"].union(
                            *[line["line"] for line in il_subset]
                        )
                    else:
...
class AccountMoveLine(models.Model):
    ...
    purchase_line_id = fields.Many2one(
        "purchase.order.line", "Purchase Order Line", ondelete="set null", index="btree_not_null"
    )
    purchase_order_id = fields.Many2one(
        "purchase.order", "Purchase Order", related="purchase_line_id.order_id", readonly=True
    )
