...
class QwebTracker:
    ...
    def __init__(self, view_id, arch, cr):
        ...
        self.execution_context_enabled = getattr(current_thread, "profiler_params", {}).get(
            "execution_context_qweb"
        )
    ...
    def enter_directive(self, directive, attrib, xpath):
        ...
        if self.execution_context_enabled:
            ...
            if directive == "set":
                ...
                directive == "groups" and "groups" in attrib and not directive_info.get("t-groups")
            ):
...
class Profiler:
    ...
    def __enter__(self):
        ...
        try:
            ...
            message = (
                "Cannot start profiler, thread not found. Is the thread part of a thread pool?"
            )
        ...
        try:
            ...
            if self.db:
                ...
                with db_connect(self.db).cursor() as cr:
                    ...
                    _logger.info(
                        "ir_profile %s (%s) created", self.profile_id, self.profile_session
                    )
        finally:
            if self.disable_gc:
                gc.enable()
