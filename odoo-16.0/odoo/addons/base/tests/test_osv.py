...
class QueryTestCase(BaseCase):
    ...
    def test_long_aliases(self):
        query = Query(None, "product_product")
        tmp = query.join(
            "product_product", "product_tmpl_id", "product_template", "id", "product_tmpl_id"
        )
        ...
        self.assertEqual(
            tmp_cat_stm, "product_product__product_tmpl_id__product_category_id__953a466f"
        )
        # extend hashed identifiers
        tmp_cat_cmp_par = query.join(tmp_cat_cmp, "partner_id", "res_partner", "id", "partner_id")
        self.assertEqual(
            tmp_cat_cmp_par, "product_product__product_tmpl_id__product_category_id__56d55687"
        )
        tmp_cat_stm_par = query.join(tmp_cat_stm, "partner_id", "res_partner", "id", "partner_id")
        self.assertEqual(
            tmp_cat_stm_par, "product_product__product_tmpl_id__product_category_id__00363fdd"
        )
