...
class TestProductMargin(AccountTestInvoicingCommon):
    def test_product_margin(self):
            ...
            [
                {
                    ...
                    "invoice_line_ids": [
                        (0, 0, {"product_id": ipad.id, "quantity": 10.0, "price_unit": 300.0})
                    ],
                },
                {
                    "move_type": "in_invoice",
                    "partner_id": supplier.id,
                    "invoice_line_ids": [
                        (0, 0, {"product_id": ipad.id, "quantity": 4.0, "price_unit": 450.0})
                    ],
                },
                {
                    ...
                    "invoice_line_ids": [
                        (0, 0, {"product_id": ipad.id, "quantity": 20.0, "price_unit": 750.0})
                    ],
                },
                {
                    "move_type": "out_invoice",
        ...
        self.assertEqual(
            result[ipad.id]["expected_margin"], expected_margin, "Wrong Expected Margin."
        )
