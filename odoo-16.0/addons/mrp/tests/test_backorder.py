...
class TestMrpProductionBackorder(TestMrpCommon):
    ...
    def test_no_tracking_pbm_1(self):
        ...
        self.assertEqual(
            sum(
                pbm_move.filtered(lambda m: m.product_id.id == product_to_use_1.id).mapped(
                    "product_qty"
                )
            ),
            16,
        ...
        self.assertEqual(
            sum(
                pbm_move.filtered(lambda m: m.product_id.id == product_to_use_2.id).mapped(
                    "product_qty"
                )
            ),
            4,
    ...
    def test_tracking_backorder_immediate_production_serial_1(self):
        ...
        backorder_mo_form = Form(
            self.env[action["res_model"]].with_context(action["context"]).browse(action["res_id"])
        )
    ...
    def test_backorder_name(self):
        ...
        default_picking_type_id = self.env["mrp.production"]._get_default_picking_type_id(
            self.env.company.id
        )
        ...
        initial_mo_name = mo_sequence.prefix + str(mo_sequence.number_next_actual).zfill(
            mo_sequence.padding
        )
    ...
    def test_backorder_name_without_procurement_group(self):
        ...
        self.assertEqual(
            int(production.name.split("-")[1]) + 1, int(backorder_ids.name.split("-")[1])
        )
