...
class TestSaleStock(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def test_00_sale_stock_invoice(self):
        ...
        self.assertTrue(
            all([il.product_id.invoice_policy == "delivery" for il in inv_1.invoice_line_ids]),
    ...
    def test_01_sale_stock_order(self):
        ...
        self.assertTrue(
            pick.button_validate(), "Sale Stock: complete delivery should not need a backorder"
        )
        del_qties = [sol.qty_delivered for sol in self.so.order_line]
        del_qties_truth = [
            2.0 if sol.product_id.type in ["product", "consu"] else 0.0
    ...
    def test_02_sale_stock_return(self):
        ...
        self.assertEqual(
            len(self.inv_1),
            1,
            'Sale Stock: only one invoice instead of "%s" should be created' % len(self.inv_1),
        )
        self.assertEqual(
        ...
        stock_return_picking_form = Form(
            self.env["stock.return.picking"].with_context(
                active_ids=pick.ids, active_id=pick.sorted().ids[0], active_model="stock.picking"
            )
        )
    ...
    def test_03_sale_stock_delivery_partial(self):
        ...
        del_qty = sum(sol.qty_delivered for sol in self.so.order_line)
        self.assertEqual(
            del_qty, 4.0, "Sale Stock: delivered quantity should be 4.0 after partial delivery"
        )
    ...
    def test_04_create_picking_update_saleorderline(self):
        ...
        self.assertEqual(
            sum(
                backorder.move_ids.filtered(lambda m: m.product_id.id == item1.id).mapped(
                    "product_qty"
                )
            ),
            2,
    ...
    def test_17_qty_update_propagation(self):
        ...
        self.assertEqual(
            len(sale_order.picking_ids), 2, "No additionnal picking should have been created."
        )

        # Increase the quantity in the sale order and check the move has been updated.
        sale_order.write({"order_line": [(1, sale_order.order_line.id, {"product_uom_qty": 40})]})
        self.assertEqual(
            move_pick.product_uom_qty,
            40,
