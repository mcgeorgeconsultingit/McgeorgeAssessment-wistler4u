...
class ResourceCalendar(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not res.get("name") and res.get("company_id"):
            res["name"] = _(
                "Working Hours of %s", self.env["res.company"].browse(res["company_id"]).name
            )
    ...
    def _check_attendance_ids(self):
        for resource in self:
            if (
                ...
                and resource.attendance_ids.filtered(lambda a: a.display_type == "line_section")
    ...
    def _compute_hours_per_day(self, attendances):
        ...
        if self.two_weeks_calendar:
            number_of_days = len(
                set(attendances.filtered(lambda cal: cal.week_type == "1").mapped("dayofweek"))
            )
            number_of_days += len(
                set(attendances.filtered(lambda cal: cal.week_type == "0").mapped("dayofweek"))
            )
        else:
        ...
        for calendar in self:
            ...
            if calendar.two_weeks_calendar:
                calendar._check_overlap(
                    attendance_ids.filtered(lambda attendance: attendance.week_type == "0")
                )
                calendar._check_overlap(
                    attendance_ids.filtered(lambda attendance: attendance.week_type == "1")
                )
            else:
    ...
    def _attendance_intervals_batch(self, start_dt, end_dt, resources=None, domain=None, tz=None):
        ...
        bounds_per_tz = {
            tz: (start_dt.astimezone(tz), end_dt.astimezone(tz)) for tz in resources_per_tz.keys()
        }
    ...
    def _work_intervals_batch(
        self, start_dt, end_dt, resources=None, domain=None, tz=None, compute_leaves=True
    ):
    ...
    def _unavailable_intervals_batch(self, start_dt, end_dt, resources=None, domain=None, tz=None):
        ...
        for resource in resources_list:
            work_intervals = [
                (start, stop) for start, stop, meta in resources_work_intervals[resource.id]
            ]
    ...
    def get_work_duration_data(self, from_datetime, to_datetime, compute_leaves=True, domain=None):
        ...
        if compute_leaves:
            ...
            intervals = self._attendance_intervals_batch(from_datetime, to_datetime, domain=domain)[
                False
            ]
...
class ResourceCalendarAttendance(models.Model):
    ...
    calendar_id = fields.Many2one(
        "resource.calendar", string="Resource's Calendar", required=True, ondelete="cascade"
    )
    ...
    week_type = fields.Selection([("1", "Second"), ("0", "First")], "Week Number", default=False)
...
class ResourceResource(models.Model):
    ...
    resource_type = fields.Selection(
        [("user", "Human"), ("material", "Material")], string="Type", default="user", required=True
    )
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get("company_id") and not values.get("calendar_id"):
                values["calendar_id"] = (
                    self.env["res.company"].browse(values["company_id"]).resource_calendar_id.id
                )
            if not values.get("tz"):
                # retrieve timezone on user or calendar
                tz = (
                    self.env["res.users"].browse(values.get("user_id")).tz
                    or self.env["resource.calendar"].browse(values.get("calendar_id")).tz
                )
                if tz:
                    values["tz"] = tz
    ...
    def _adjust_to_calendar(self, start, end, compute_leaves=True):
        ...
        for resource in self:
            ...
            calendar_start = resource.calendar_id._get_closest_work_time(
                start, resource=resource, search_range=search_range, compute_leaves=compute_leaves
            )
    ...
    def _get_calendars_validity_within_period(self, start, end, default_company=None):
        ...
        for resource in self:
            calendar = (
                resource.calendar_id or resource.company_id.resource_calendar_id or default_calendar
            )
    ...
    def _get_valid_work_intervals(self, start, end, calendars=None):
        ...
        resource_calendar_validity_intervals = self.sudo()._get_calendars_validity_within_period(
            start, end
        )
        for resource in self:
