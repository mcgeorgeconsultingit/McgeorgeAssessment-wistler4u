...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        env["ir.property"]._set_default(
            "property_account_receivable_id", "res.partner", account_receivable, main_company
        )
                ...
                "fiscal_position_ids": [
                    (
                        ...
                        0,
                        {
                            "name": "FP-POS-2M",
                            "tax_ids": [
                                (0, 0, {"tax_src_id": src_tax.id, "tax_dest_id": src_tax.id}),
                                (0, 0, {"tax_src_id": src_tax.id, "tax_dest_id": dst_tax.id}),
                            ],
                        },
                    )
                ],
...
class TestUi(TestPointOfSaleHttpCommon):
    def test_01_pos_basic_order(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id, "pos_pricelist", login="accountman"
        )
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id, "pos_basic_order", login="accountman"
        )
        self.start_tour(
    ...
    def test_02_pos_with_invoiced(self):
        self.main_pos_config.open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id, "ChromeTour", login="accountman"
        )
    ...
    def test_05_ticket_screen(self):
        self.main_pos_config.open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id, "TicketScreenTour", login="accountman"
        )
    ...
    def test_fixed_tax_negative_qty(self):
        ...
        self.assertEqual(
            lines[1].account_id, zero_amount_product.categ_id.property_account_income_categ_id
        )
