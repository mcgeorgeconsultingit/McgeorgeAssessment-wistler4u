...
class WebsiteSlides(WebsiteProfile):
    ...
    def _get_slide_detail(self, slide):
        ...
        previous_slide = (
            slide.channel_id.slide_content_ids[slide_index - 1] if slide_index > 0 else None
        )
        next_slide = (
    ...
    def slides_channel_home(self, **post):
        ...
        if request.env.user._is_public():
            ...
            challenges = tools.lazy(
                ...
                .sudo()
                .search(
                    [("challenge_category", "=", "slides"), ("reward_id.is_published", "=", True)],
                    order="id asc",
                    limit=5,
                )
            )
        ...
        users = tools.lazy(
            ...
            .sudo()
            .search(
                [("karma", ">", 0), ("website_published", "=", True)], limit=5, order="karma desc"
            )
        )
    ...
    def slides_channel_all(self, slide_category=None, slug_tags=None, my=False, **post):
        if slug_tags and request.httprequest.method == "GET":
            ...
            if len(tag_list) > 1 and not post.get("search"):
                url = QueryURL(
                    "/slides/all", ["tag"], tag=tag_list[0], my=my, slide_category=slide_category
                )()
    ...
    def channel(
        ...
        **kw,
        ...
        slide_categories = dict(
            request.env["slide.slide"]._fields["slide_category"]._description_selection(request.env)
        )
        ...
        if request.params.get("access_error") == "course_content" and request.params.get(
            "access_error_slide_id"
        ):
    ...
    def _slide_channel_prepare_review_values(self, channel):
        ...
        if not request.env.user._is_public():
            ...
            if last_message:
                last_message_values = last_message.read(["body", "rating_value", "attachment_ids"])[
                    0
                ]
        ...
        values.update(
            {
                ...
                "search_tag": request.env["slide.tag"].browse(int(kwargs.get("search_tag")))
    ...
    def slide_shared_view(self, slide_id, **kwargs):
        ...
        if status == "authorized":
            return request.redirect(
                "%s?%s"
                % (user_slide_authorization["slide"].website_url, werkzeug.urls.url_encode(kwargs))
            )
    ...
    def slide_quiz_submit(self, slide_id, answer_ids):
        ...
        return {
            "answers": {
                answer.question_id.id: {"is_correct": answer.is_correct, "comment": answer.comment}
    ...
    @http.route(
        ["/slides/category/search_read"], type="json", auth="user", methods=["POST"], website=True
    )
    ...
    @http.route(
        ["/slides/prepare_preview"], type="json", auth="user", methods=["POST"], website=True
    )
    ...
    def create_slide(self, *args, **post):
        ...
        values = dict(
            (fname, post[fname]) for fname in self._get_valid_slide_post_values() if post.get(fname)
        )
    ...
    @http.route(
        ["/slides/tag/search_read"], type="json", auth="user", methods=["POST"], website=True
    )
    ...
    @http.route(
        "/slides/embed/<int:slide_id>", type="http", auth="public", website=True, sitemap=False
    )
    ...
    def _prepare_user_profile_values(self, user, **post):
        ...
        values.update(
            self._prepare_user_values(channel=channels[0] if len(channels) == 1 else True, **post)
        )
