            ...
            res["lines"].append(
                {
                    ...
                    "move_out": line["move_out"].read(self._fields_for_serialized_moves())[0]
                    if line["move_out"]
                    else False,
                    "move_in": line["move_in"].read(self._fields_for_serialized_moves())[0]
                    if line["move_in"]
                    else False,
    ...
    def _get_report_lines(self, product_template_ids, product_variant_ids, wh_location_ids):
        def _reconcile_out_with_ins(
            ...
            for index, in_ in enumerate(ins):
                ...
                lines.append(
                    self._prepare_report_line(taken_from_in, move_in=in_["move"], move_out=out)
                )
        ...
        for out in outs:
            ...
            for move in moves_orig:
                ...
                if (
                    float_compare(out_qty_reserved, out.product_qty, precision_rounding=rounding)
                    >= 0
                ):
                    break
        ...
        for in_ in ins:
            ins_per_product[in_.product_id.id].append(
                {"qty": in_.product_qty, "move": in_, "move_dests": in_._rollup_move_dests(set())}
            )
        ...
        for product in (ins | outs).product_id:
            ...
            for (demand, out) in unreconciled_outs:
                ...
                if not float_is_zero(demand, precision_rounding=product_rounding):
                    # Not reconciled
                    lines.append(
                        self._prepare_report_line(demand, move_out=out, replenishment_filled=False)
                    )
