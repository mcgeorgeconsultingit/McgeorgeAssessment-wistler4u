    ...
    def _get_translation(self, source, module=None):
        try:
            ...
            if lang and lang != "en_US":
                ...
                _logger.debug(
                    'no translation language detected, skipping translation for "%r" ', source
                )
        except Exception:
    ...
    def write_rows(self, rows):
        ...
        for src, row in sorted(grouped_rows.items()):
            if not self.lang:
                ...
                row["translation"] = ""
            self.add_entry(
                row["modules"], sorted(row["tnrs"]), src, row["translation"], row["comments"]
            )
...
def trans_load(cr, filepath, lang, verbose=True, overwrite=False):
    warnings.warn(
        "The function trans_load is deprecated in favor of TranslationImporter", DeprecationWarning
    )
