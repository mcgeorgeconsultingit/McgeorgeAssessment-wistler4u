...
class Company(models.Model):
    ...
    name = fields.Char(
        related="partner_id.name", string="Company Name", required=True, store=True, readonly=False
    )
    ...
    company_details = fields.Html(
        string="Company Details", help="Header text displayed at the top of all reports."
    )
    logo = fields.Binary(
        related="partner_id.image_1920", default=_get_logo, string="Company Logo", readonly=False
    )
    ...
    def create(self, vals_list):
        ...
        no_partner_vals_list = [
            vals for vals in vals_list if vals.get("name") and not vals.get("partner_id")
        ]
