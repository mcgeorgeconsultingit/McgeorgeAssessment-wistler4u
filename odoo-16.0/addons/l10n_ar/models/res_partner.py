...
class ResPartner(models.Model):
    ...
    def _compute_l10n_ar_vat(self):
        ...
        recs_ar_vat = self.filtered(
            lambda x: x.l10n_latam_identification_type_id.l10n_ar_afip_code == "80" and x.vat
        )
        for rec in recs_ar_vat:
    ...
    def l10n_ar_identification_validation(self):
        for rec in self.filtered("vat"):
            ...
            try:
                ...
                raise ValidationError(
                    _('Invalid length for "%s"', rec.l10n_latam_identification_type_id.name)
                )
            except module.InvalidFormat:
                raise ValidationError(
                    _('Only numbers allowed for "%s"', rec.l10n_latam_identification_type_id.name)
                )
            except Exception as error:
