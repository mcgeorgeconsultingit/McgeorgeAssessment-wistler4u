...
class TestMailPublicPage(HttpCase):
    ...
    def setUp(self):
        ...
        self.channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(group_id=None, name="Test channel")["id"]
        )
    ...
    def test_mail_channel_public_page_as_guest(self):
        self.start_tour(
            self.channel.invitation_url, "mail/static/tests/tours/mail_channel_as_guest_tour.js"
        )
