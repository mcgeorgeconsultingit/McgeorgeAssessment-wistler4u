...
def pager(url, total, page=1, step=30, scope=5, url_args=None):
    ...
    return {
        ...
        "page_previous": {"url": get_url(max(pmin, page - 1)), "num": max(pmin, page - 1)},
...
class CustomerPortal(Controller):
    ...
    @http.route(
        "/portal/attachment/add", type="http", auth="public", methods=["POST"], website=True
    )
    def attachment_add(self, name, file, res_model, res_id, access_token=None, **kwargs):
        ...
        return request.make_response(
            data=json.dumps(
                attachment.read(["id", "name", "mimetype", "file_size", "access_token"])[0]
            ),
    ...
    def details_form_validate(self, data, partner_creation=False):
        ...
        unknown = [
            k for k in data if k not in self.MANDATORY_BILLING_FIELDS + self.OPTIONAL_BILLING_FIELDS
        ]
        if unknown:
