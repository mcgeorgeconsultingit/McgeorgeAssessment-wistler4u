...
class IrActionsActWindow(models.Model):
    ...
    def _check_model(self):
        for action in self:
            ...
            if action.binding_model_id and action.binding_model_id.model not in self.env:
                raise ValidationError(
                    _("Invalid model name %r in action definition.", action.binding_model_id.model)
                )
    ...
    usage = fields.Char(
        string="Action Usage", help="Used to filter menu and home actions from the user form."
    )
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _run_action_code_multi(self, eval_context):
        safe_eval(
            self.code.strip(), eval_context, mode="exec", nocopy=True, filename=str(self)
...
class IrServerObjectLines(models.Model):
    ...
    evaluation_type = fields.Selection(
        [("value", "Value"), ("reference", "Reference"), ("equation", "Python expression")],
        "Evaluation Type",
        default="value",
        required=True,
        change_default=True,
    ...
    def _raise_many2many_error(self):
        if self.filtered(
            lambda line: line.col1.ttype == "many2many" and line.evaluation_type == "reference"
        ):
...
class IrActionsTodo(models.Model):
    ...
    state = fields.Selection(
        [("open", "To Do"), ("done", "Done")], string="Status", default="open", required=True
    )
    name = fields.Char()
