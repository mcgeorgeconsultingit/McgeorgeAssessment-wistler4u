...
class TestLeadMergeCommon(TestLeadConvertMassCommon):
    ...
    def setUpClass(cls):
        ...
        cls.assign_users = (
            cls.user_sales_manager + cls.user_sales_leads_convert + cls.user_sales_salesman
        )
...
class TestLeadMerge(TestLeadMergeCommon):
    ...
    def test_lead_merge_properties_formatting(self):
        ...
        result = self.env["ir.qweb"]._render(
            "crm.crm_lead_merge_summary", {"opportunities": lead, "is_html_empty": lambda x: True}
        )
    ...
    def test_merge_method_followers(self):
        ...
        self.assertIn(
            self.contact_2, new_partner_followers, "The partner must follow the destination lead"
        )
        ...
        self.assertNotIn(
            self.contact_company, new_partner_followers, "The partner was not active on the lead"
        )
        self.assertIn(
            self.contact_1,
