...
class ImLivechatChannel(models.Model):
    ...
    button_text = fields.Char(
        "Text of the Button",
        default=_("Have a Question? Chat with us."),
    ...
    input_placeholder = fields.Char(
        "Chat Input Placeholder", help="Text that prompts the user to initiate the chat."
    )
    header_background_color = fields.Char(
        default="#875A7B", help="Default background color of the channel header once open"
    )
    ...
    are_you_inside = fields.Boolean(
        string="Are you inside the matrix?", compute="_are_you_inside", store=False, readonly=True
    )
    ...
    nbr_channel = fields.Integer(
        "Number of conversation", compute="_compute_nbr_channel", store=False, readonly=True
    )
    ...
    def _get_livechat_mail_channel_vals(
        self, anonymous_name, operator=None, chatbot_script=None, user_id=None, country_id=None
    ):
        # partner to add to the mail.channel
        operator_partner_id = (
            operator.partner_id.id if operator else chatbot_script.operator_partner_id.id
        )
        ...
        if user_id:
            ...
            if (
                visitor_user and visitor_user.active and operator and visitor_user != operator
    ...
    def _open_livechat_mail_channel(
        ...
        mail_channel_vals = self._get_livechat_mail_channel_vals(
            anonymous_name, user_operator, chatbot_script, user_id=user_id, country_id=country_id
        )
        if persisted:
            ...
            operator_partner_id = (
                user_operator.partner_id if user_operator else chatbot_script.operator_partner_id
            )
            display_name = (
                operator_partner_id.user_livechat_username or operator_partner_id.display_name
            )
            return {
    ...
    def _get_random_operator(self):
        ...
        return next(
            operator for operator in operators if operator.partner_id.id == less_active_operator
        )
