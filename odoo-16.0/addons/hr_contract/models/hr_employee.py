...
class Employee(models.Model):
    ...
    def _compute_contracts_count(self):
        ...
        contract_histories = (
            self.env["hr.contract.history"].sudo().search([("employee_id", "in", self.ids)])
        )
        for employee in self:
    ...
    def _get_all_contracts(self, date_from, date_to, states=["open"]):
        ...
        """
        return self.search(["|", ("active", "=", True), ("active", "=", False)])._get_contracts(
            date_from, date_to, states=states
        )
