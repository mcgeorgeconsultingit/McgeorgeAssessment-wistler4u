...
class SelectionConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        if "selection" not in options:
            options = dict(
                options,
                selection=dict(record._fields[field_name].get_description(self.env)["selection"]),
            )
...
class ImageConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        return Markup(
            '<img src="data:%s;base64,%s">' % (Image.MIME[image.format], value.decode("ascii"))
        )
...
class MonetaryConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        lang = self.user_lang()
        formatted_amount = (
            lang.format(fmt, display_currency.round(value), grouping=True, monetary=True)
    ...
    def record_to_html(self, record, field_name, options):
        ...
        if not options.get("display_currency"):
            ...
            currency_fields = [
                k for k, v in fields if v.type == "many2one" and v.comodel_name == "res.currency"
            ]
            if currency_fields:
...
class DurationConverter(models.AbstractModel):
    ...
    def get_available_options(self):
        ...
        options.update(
            ...
            format=dict(
                type="selection",
                params=[("long", _("Long")), ("short", _("Short")), ("narrow", _("Narrow"))],
                string=_("Format"),
...
class RelativeDatetimeConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        return pycompat.to_text(
            babel.dates.format_timedelta(value - reference, add_direction=True, locale=locale)
        )
    ...
    def get_available_options(self):
        ...
        options.update(
            fields=dict(
                ...
                default_value=[
                    param.get("field_name") for param in contact_fields if param.get("default")
                ],
            ),
            separator=dict(
                type="selection",
