...
class WebsocketController(Controller):
    ...
    def websocket(self):
        ...
        """
        is_headful_browser = (
            request.httprequest.user_agent
            and "Headless" not in request.httprequest.user_agent.string
        )
    ...
    def peek_notifications(self, channels, last, is_first_poll=False):
        ...
        last_known_notification_id = (
            request.env["bus.bus"].sudo().search([], limit=1, order="id desc").id or 0
        )
    ...
    def update_bus_presence(self, inactivity_period, im_status_ids_by_model):
        if "is_websocket_session" not in request.session:
            raise SessionExpiredException()
        request.env["ir.websocket"]._update_bus_presence(
            int(inactivity_period), im_status_ids_by_model
        )
        return {}
    ...
    def get_websocket_worker_bundle(self, v=None):  # pylint: disable=unused-argument
        ...
        stream = request.env["ir.binary"]._get_stream_from(
            asset.js(is_minified="assets" not in request.session.debug)
        )
