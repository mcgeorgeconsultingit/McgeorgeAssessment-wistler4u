...
class TestPickShip(TestStockCommon):
    ...
    def test_mto_moves(self):
        ...
        self.assertEqual(
            picking_client.state, "assigned", "The state of the client should be assigned"
        )
    ...
    def test_mto_moves_transfer(self):
        ...
        self.assertEqual(
            picking_client.state, "waiting", "The picking should not assign what it does not have"
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, stock_location), 0.0
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, pack_location), 5.0
        )
        ...
        self.assertEqual(
            picking_client.state, "assigned", "The picking should not assign what it does not have"
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, stock_location), 0.0
        )
        ...
        self.assertEqual(
            sum(self.env["stock.quant"]._gather(self.productA, stock_location).mapped("quantity")),
            0.0,
        )
        self.assertEqual(len(self.env["stock.quant"]._gather(self.productA, pack_location)), 1.0)
    ...
    def test_edit_done_chained_move(self):
        ...
        self.assertEqual(
            picking_pick.move_ids.quantity_done, 10.0, "Wrong quantity_done for pick move"
        )
        self.assertEqual(
            picking_client.move_ids.product_qty, 10.0, "Wrong initial demand for client move"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            picking_pick.move_ids.quantity_done, 5.0, "Wrong quantity_done for pick move"
        )
        self.assertEqual(
            picking_client.move_ids.product_qty, 10.0, "Wrong initial demand for client move"
        )
    ...
    def test_edit_done_chained_move_with_lot(self):
        ...
        self.assertEqual(
            picking_client.move_ids.move_line_ids.lot_id, lot1, "Wrong lot for client move line"
        )
        self.assertEqual(
            picking_client.move_ids.reserved_availability,
            10.0,
        ...
        self.assertEqual(
            picking_client.move_ids.move_line_ids.lot_id, lot2, "Wrong lot for client move line"
        )
        self.assertEqual(
            picking_client.move_ids.reserved_availability,
            10.0,
    ...
    def test_pick_ship_return(self):
        ...
        """
        self.assertTrue(
            return_ship_picking.move_ids in return_pick_picking.move_ids.mapped("move_orig_ids"),
            "The pick return picking's moves should have the ship return picking's moves as origin",
        )

        self.assertTrue(
            return_pick_picking.move_ids in return_ship_picking.move_ids.mapped("move_dest_ids"),
        ...
        self.assertEqual(
            pack_quantity, 0, "It should be one product in pack location but is reserved"
        )
    ...
    def test_pick_pack_ship_return(self):
        ...
        self.assertEqual(
            len(picking_pick.move_ids.move_orig_ids), 0, "Picking pick should not have origin moves"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            set(picking_ship.move_ids.move_dest_ids.ids), set(return_ship_picking.move_ids.ids)
        )

        self.assertEqual(
            set(return_ship_picking.move_ids.move_orig_ids.ids), set(picking_ship.move_ids.ids)
        )
        self.assertEqual(
    ...
    def test_mto_cancel_move_line(self):
        ...
        self.assertEqual(
            picking_client.state, "waiting", "The picking state should not be ready anymore."
        )
    ...
    def test_return_location(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, pick_location), 5.0
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, return_location), 5.0
        )
        self.assertEqual(
            len(
    ...
    def test_return_lot(self):
        ...
        self.assertEqual(
            return_pick.picking_type_id, picking_client.location_id.warehouse_id.return_type_id
        )
...
class TestSinglePicking(TestStockCommon):
    def test_backorder_1(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, pack_location), 0.0
        )
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, pack_location), 1.0
        )
    ...
    def test_assign_deadline(self):
        ...
        self.env["ir.config_parameter"].create({"key": "stock.merge_only_same_date", "value": True})
        ...
        self.assertEqual(
            move1.reserved_availability, 2, "Earlier deadline should have reserved quantity"
        )
        self.assertEqual(
            move2.reserved_availability, 0, "Later deadline should not have reserved quantity"
        )
        ...
        delivery_order.action_assign()
        self.assertEqual(
            move1.reserved_availability, 4, "Earlier deadline should have reserved quantity"
        )
        self.assertEqual(
            move2.reserved_availability, 0, "Later deadline should not have reserved quantity"
        )
        ...
        self.assertEqual(
            move4.reserved_availability, 1, "Earlier move without deadline should take the priority"
        )
    ...
    def test_extra_move_3(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, stock_location), 2.0
        )
    ...
    def test_extra_move_5(self):
        ...
        delivery._action_done()
        self.assertEqual(
            delivery.move_ids.product_uom_qty, 5.95, "Move initial demand should be 5.95"
        )
    ...
    def test_use_create_lot_use_existing_lot_4(self):
        ...
        with self.assertRaises(UserError):
            self.env["stock.lot"].with_context(active_picking_id=delivery_order.id).create(
                {
                    "name": "lot1",
                    "product_id": self.productA.id,
                    "company_id": self.env.company.id,
    ...
    def test_merge_moves_1(self):
        ...
        self.assertEqual(
            receipt.move_ids.filtered(lambda m: m.product_id == self.productA).product_uom_qty,
            9,
            "Merged quantity is not correct",
        )
        self.assertEqual(
            receipt.move_ids.filtered(lambda m: m.product_id == self.productB).product_uom_qty,
            5,
    ...
    def test_additional_move_1(self):
        ...
        stock_location = self.env["stock.location"].browse(self.stock_location)
        self.assertEqual(
            self.env["stock.quant"]._gather(self.productA, stock_location).reserved_quantity, 10.0
        )
        self.assertEqual(
            self.env["stock.quant"]._gather(self.productB, stock_location).reserved_quantity, 10.0
        )
...
class TestStockUOM(TestStockCommon):
    ...
    def test_pickings_transfer_with_different_uom_and_back_orders(self):
        ...
        self.assertEqual(
            back_order_in.move_ids.product_qty, 91640.00, "There should be one back order created"
        )
    ...
    def test_move_product_with_different_uom(self):
        ...
        self.assertEqual(
            float_compare(
                move_line.reserved_qty, quant.quantity, precision_digits=precision_digits
            ),
            -1,
        ...
        self.assertTrue(
            float_is_zero(
                move_line.reserved_qty - quant.reserved_quantity, precision_digits=precision_digits
            )
        )
    ...
    def test_update_product_move_line_with_different_uom(self):
        ...
        self.assertEqual(
            len(product_LtDA.stock_quant_ids), 1, "One quant should exist for the product."
        )
        self.assertEqual(
            len(product_GtDA.stock_quant_ids), 1, "One quant should exist for the product."
        )
        ...
        self.assertEqual(
            float_compare(
                reserved_on_move_lines_LtDA, quant_LtDA.quantity, precision_digits=precision_digits
            ),
            -1,
            "We do not reserve more (in the same UOM) than the quantity in stock",
        )
        self.assertEqual(
            float_compare(
                reserved_on_move_lines_GtDA, quant_GtDA.quantity, precision_digits=precision_digits
            ),
            -1,
            "We do not reserve more (in the same UOM) than the quantity in stock",
        )
...
class TestRoutes(TestStockCommon):
    ...
    def test_replenish_pick_ship_1(self):
        ...
        self.assertEqual(
            move_line[0].product_uom_qty, self.product_uom_qty, "Quantities does not match"
        )
        self.assertEqual(
            move_line[1].product_uom_qty, self.product_uom_qty, "Quantities does not match"
        )
...
class TestAutoAssign(TestStockCommon):
    ...
    def test_auto_assign_0(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, stock_location), 0
        )
    ...
    def test_auto_assign_1(self):
        ...
        self.assertEqual(
            picking_client.state, "waiting", "MTO moves can't be automatically assigned."
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, pack_location), 10.0
        )
    ...
    def test_auto_assign_reservation_method(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(self.productA, stock_location), 40
        )
