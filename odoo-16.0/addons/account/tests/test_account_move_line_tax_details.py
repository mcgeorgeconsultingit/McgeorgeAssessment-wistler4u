...
class TestAccountTaxDetailsReport(AccountTestInvoicingCommon):
    def _dispatch_move_lines(self, moves):
        base_lines = moves.line_ids.filtered(lambda x: x.tax_ids and not x.tax_line_id).sorted(
            lambda x: (x.move_id.id, x.id, -abs(x.amount_currency))
        )
        tax_lines = moves.line_ids.filtered(lambda x: x.tax_line_id).sorted(
            lambda x: (x.move_id.id, x.tax_line_id.id, x.tax_ids.ids, x.tax_repartition_line_id.id)
        )
    ...
    def _get_tax_details(self, fallback=False, extra_domain=None):
        ...
        return sorted(
            tax_details_res,
            key=lambda x: (x["base_line_id"], abs(x["base_amount"]), abs(x["tax_amount"])),
        )
    ...
    def assertTotalAmounts(self, moves, tax_details):
        ...
        for tax in taxes:
            ...
            tax_details_amount = sum(
                x["tax_amount"] for x in tax_details if (x["group_tax_id"] or x["tax_id"]) == tax.id
            )
