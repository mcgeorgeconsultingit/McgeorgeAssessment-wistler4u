...
class TestCRUDVisibilityFollowers(TestAccessRights):
    ...
    def test_project_no_write(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to write on the project" % self.env.user.name
        ):
    ...
    def test_project_no_unlink(self):
        ...
        with self.assertRaises(
            AccessError, msg="%s should not be able to unlink the project" % self.env.user.name
        ):
    ...
    def test_project_no_read(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to read the project" % self.env.user.name
        ):
    ...
    def test_task_no_read(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to read the task" % self.env.user.name
        ):
    ...
    def test_task_no_write(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to write on the task" % self.env.user.name
        ):
    ...
    def test_task_no_create(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to create a task" % self.env.user.name
        ):
    ...
    def test_task_no_unlink(self):
        with self.assertRaises(
            AccessError, msg="%s should not be able to unlink the task" % self.env.user.name
        ):
...
class TestAllowedUsers(TestAccessRights):
    ...
    def test_project_specific_permission(self):
        ...
        self.assertIn(
            john.partner_id, task.message_partner_ids, "John should allowed to read the task"
        )
...
class TestProjectPortalCommon(TestProjectCommon):
    def setUp(self):
        ...
        self.task_5 = (
            self.env["project.task"]
            .with_context({"mail_create_nolog": True})
            .create({"name": "Test5", "user_ids": False, "project_id": self.project_pigs.id})
        )
        self.task_6 = (
            self.env["project.task"]
            .with_context({"mail_create_nolog": True})
