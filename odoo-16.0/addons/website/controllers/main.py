...
class Website(Home):
        ...
        if not isredir and website.domain:
            ...
            if domain_from != domain_to:
                ...
                url_to = werkzeug.urls.url_join(
                    website.domain, "/website/force/%s?isredir=1&path=%s" % (website.id, path)
                )
    ...
    def robots(self, **kwargs):
        return request.render(
            "website.robots", {"url_root": request.httprequest.url_root}, mimetype="text/plain"
        )

    @http.route(
        "/sitemap.xml", type="http", auth="public", website=True, multilang=False, sitemap=False
    )
    ...
    def website_info(self, **kwargs):
        ...
        values = {"apps": apps, "l10n": l10n, "version": odoo.service.common.exp_version()}
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        if request.env.lang != request.website.default_lang_id.code:
            return request.redirect(
                "/%s%s" % (request.website.default_lang_id.url_code, request.httprequest.path)
            )
        action_url = (
    ...
    @http.route(
        ["/website/social/<string:social>"], type="http", auth="public", website=True, sitemap=False
    )
    ...
    def pages_list(self, page=1, search="", **kw):
        ...
        pager = portal_pager(
            url="/pages", url_args={"search": search}, total=pages_count, page=page, step=step
        )
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(
                    "/web#id=" + str(page.get("view_id")) + "&view_type=form&model=ir.ui.view"
                )
    ...
    def seo_suggest(self, keywords=None, lang=None):
        ...
        return json.dumps(
            [sugg[0].attrib["data"] for sugg in xmlroot if len(sugg) and sugg[0].attrib["data"]]
        )
    ...
    def actions_server(self, path_or_xml_id_or_id, **post):
        ...
        if not action:
            action = ServerActions.sudo().search(
                [("website_path", "=", path_or_xml_id_or_id), ("website_published", "=", True)],
                limit=1,
            )
        if not action:
            try:
...
class WebsiteBinary(Binary):
    ...
    @http.route(
        ["/favicon.ico"], type="http", auth="public", website=True, multilang=False, sitemap=False
    )
    def favicon(self, **kw):
