...
class TestMailingContactToList(MassMailCommon):
    ...
    def test_mailing_contact_to_list(self):
        ...
        self.assertTrue(
            action.get("params", {}).get("next"), "Should return a notification with a next action"
        )
...
class TestMailingListMerge(MassMailCommon):
    ...
    def test_mailing_contact_create(self):
        ...
        self.assertEqual(
            new.list_ids, (self.mailing_list_1 | self.mailing_list_2 | self.mailing_list_3)
        )
        ...
        self.assertEqual(
            contact.subscription_list_ids[0].unsubscription_date, datetime(2022, 1, 1, 12, 0, 0)
        )
    ...
    def test_mailing_list_contact_copy_in_context_of_mailing_list(self):
        ...
        self.assertEqual(
            contact_1.list_ids, contact_2.list_ids, "Should copy the existing mailing list(s)"
        )
...
class TestMailingContactImport(MassMailCommon):
    ...
    def test_mailing_contact_import(self):
        ...
        self.assertIn(
            ("", "alice@example.com"), contacts, "Should have imported the right email address"
        )
        self.assertIn(
            ("Bob", "bob@example.com"), contacts, "Should have imported the name of the contact"
        )
        self.assertIn(
            ("", "already_exists_list_2@example.com"),
            contacts,
