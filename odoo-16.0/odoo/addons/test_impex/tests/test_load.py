...
def message(msg, type="error", from_=0, to_=0, record=0, field="value", **kwargs):
    return dict(
        kwargs, type=type, rows={"from": from_, "to": to_}, record=record, field=field, message=msg
    )
...
class ImporterCase(common.TransactionCase):
    ...
    def xid(self, record):
        ...
        name = name.replace(".", "-")
        ModelData.create(
            {"name": name, "model": record._name, "res_id": record.id, "module": "__test__"}
        )
...
class test_integer_field(ImporterCase):
    ...
    def test_negatives(self):
        result = self.import_(
            ["value"], [["-1"], ["-42"], [str(-(2**31 - 1))], [str(-(2**31))], ["-12345678"]]
        )
...
class test_float_field(ImporterCase):
    ...
    def test_positives(self):
        ...
        self.assertEqual([1, 42, 2**31 - 1, 12345678, 2.0**33, 0.000001], values(self.read()))
    ...
    def test_nonsense(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    "'foobar' does not seem to be a number for field 'Value'", field_name="Value"
                )
            ],
        )
...
class test_required_string_field(ImporterCase):
    ...
    def test_empty(self):
        result = self.import_(["value"], [[]])
        self.assertEqual(
            result["messages"], [message("Missing required value for the field 'Value' (value)")]
        )
        self.assertIs(result["ids"], False)
