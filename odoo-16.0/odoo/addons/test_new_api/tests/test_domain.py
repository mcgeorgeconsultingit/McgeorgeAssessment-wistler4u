...
class test_domain(common.TransactionCase):
    ...
    def test_00_test_bool_undefined(self):
        ...
        for f in ["bool_true", "bool_false", "bool_undefined", "x_bool_new_undefined"]:
            ...
            self.assertEqual(
                eq_1, neq_1, "`= False` (%s) <> `!= True` (%s) " % (len(eq_1), len(neq_1))
            )

            eq_2 = model.search([(f, "=", True)])
            neq_2 = model.search([(f, "!=", False)])
            self.assertEqual(
                eq_2, neq_2, "`= True` (%s) <> `!= False` (%s) " % (len(eq_2), len(neq_2))
            )
    ...
    def test_empty_char(self):
        ...
        self.assertListEqual(
            EmptyChar.search([("name", "ilike", False)]).mapped("name"), ["name", "", False]
        )
        self.assertListEqual(
        ...
        for length in range(len(values) + 1):
            for subset in combinations(values, length):
                ...
                self.assertListEqual(
                    EmptyChar.search([("name", "not in", sublist)]).mapped("name"), sublist_remained
                )
    ...
    def test_empty_translation(self):
        ...
        self.assertListEqual(
            records_fr.search([("name", "ilike", False)]).mapped("name"), ["name", "", False]
        )
        self.assertListEqual(
