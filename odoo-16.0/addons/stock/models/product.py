...
class Product(models.Model):
    ...
    reordering_min_qty = fields.Float(compute="_compute_nbr_reordering_rules", compute_sudo=False)
    reordering_max_qty = fields.Float(compute="_compute_nbr_reordering_rules", compute_sudo=False)
    ...
    def _get_description(self, picking_type_id):
        ...
        description = (
            html2plaintext(self.description) if not is_html_empty(self.description) else self.name
        )
    ...
    def action_view_orderpoints(self):
        ...
        if self and len(self) == 1:
            action["context"].update(
                {"default_product_id": self.ids[0], "search_default_product_id": self.ids[0]}
            )
        else:
...
class ProductTemplate(models.Model):
    ...
    type = fields.Selection(
        selection_add=[("product", "Storable Product")], ondelete={"product": "set consu"}
    )
    ...
    def write(self, vals):
        ...
        if (
            ...
            and any(
                p.type == "product"
                and not float_is_zero(p.qty_available, precision_rounding=p.uom_id.rounding)
                for p in self
            )
        ):
    ...
    def action_product_tmpl_forecast_report(self):
        self.ensure_one()
        if self.env.ref(
            "stock.stock_replenishment_product_template_action", raise_if_not_found=True
        ):
            action = self.env["ir.actions.actions"]._for_xml_id(
...
class ProductCategory(models.Model):
    ...
    total_route_ids = fields.Many2many(
        "stock.route", string="Total routes", compute="_compute_total_route_ids", readonly=True
    )
    ...
    def write(self, values):
        ...
        if "factor" in values or "factor_inv" in values or "category_id" in values:
            changed = self.filtered(
                lambda u: any(
                    u[f] != values[f] if f in values else False for f in {"factor", "factor_inv"}
                )
            ) + self.filtered(
                lambda u: any(
                    u[f].id != int(values[f]) if f in values else False for f in {"category_id"}
                )
            )
            if changed:
                error_msg = _(
