...
class TestEncoding(ImportCase):
    ...
    def _check_text(self, text, encodings, **options):
        ...
        for encoding in ["utf-8", "utf-16", "utf-32", *encodings]:
            if isinstance(encoding, tuple):
                ...
                es = [encoding]
            preview = self._make_import(test_text.encode(encoding)).parse_preview(dict(options))

            self.assertIsNone(preview.get("error"))
            guessed = preview["options"]["encoding"]
            self.assertIsNotNone(guessed)
            self.assertIn(codecs.lookup(guessed).name, [codecs.lookup(e).name for e in es])
    ...
    def test_autodetect_encoding(self):
        ...
        self._check_text(
            "やぶら小路の藪柑子。海砂利水魚の、食う寝る処に住む処、パイポパイポ パイポのシューリンガン。", ["eucjp", "shift_jis", "iso2022_jp"]
        )

        self._check_text(
            "대통령은 제4항과 제5항의 규정에 의하여 확정된 법률을 지체없이 공포하여야 한다, 탄핵의 결정.", ["euc_kr", "iso2022_kr"]
        )
