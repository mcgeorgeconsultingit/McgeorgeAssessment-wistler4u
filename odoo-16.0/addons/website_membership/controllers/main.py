...
class WebsiteMembership(http.Controller):
    ...
    def members(self, membership_id=None, country_name=None, country_id=0, page=1, **post):
        ...
        if country_id:
            ...
            if not any(x["country_id"][0] == country_id for x in countries if x["country_id"]):
                countries.append(
                    {"country_id_count": 0, "country_id": (country_id, current_country["name"])}
                )
                countries = [d for d in countries if d["country_id"]]
                countries.sort(key=lambda d: d["country_id"][1])

        countries.insert(
            0, {"country_id_count": countries_total, "country_id": (0, _("All Countries"))}
        )
        ...
        if request.website.is_view_active("website_membership.opt_index_google_map"):
            google_map_partner_ids = MembershipLine.search(line_domain)._get_published_companies(
                limit=2000
            )
        ...
        pager = request.website.pager(
            url=base_url, total=count_members, page=page, step=limit, scope=7, url_args=post
        )
    ...
    def partners_detail(self, partner_id, **post):
        ...
        if partner_id:
            ...
            if (
                partner.exists() and partner.website_published
