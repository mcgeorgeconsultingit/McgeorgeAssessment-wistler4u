...
class Module(models.Model):
    ...
    def get_module_info(cls, name):
        try:
            ...
            _logger.debug(
                "Error when trying to fetch information for module %s", name, exc_info=True
            )
            return {}
    ...
    def downstream_dependencies(
        self, known_deps=None, exclude_states=("uninstalled", "uninstallable", "to remove")
    ):
        ...
        query = """ SELECT DISTINCT m.id
                    ...
                    WHERE
                        ...
                        m.id NOT IN %s """
        self._cr.execute(
            query, (tuple(self.ids), tuple(exclude_states), tuple(known_deps.ids or self.ids))
        )
    ...
    def upstream_dependencies(
        self, known_deps=None, exclude_states=("installed", "uninstallable", "to remove")
    ):
    ...
    def button_uninstall(self):
        ...
        if un_installable_modules:
            raise UserError(
                _("Those modules cannot be uninstalled: %s", ", ".join(un_installable_modules))
            )
    ...
    def button_upgrade(self):
        ...
        while i < len(todo):
            ...
            if module.state not in ("installed", "to upgrade"):
                raise UserError(
                    _("Can not upgrade module '%s'. It is not installed.") % (module.name,)
                )
    ...
    def _installed(self):
        ...
        return {
            module.name: module.id for module in self.sudo().search([("state", "=", "installed")])
        }
...
class ModuleDependency(models.Model):
    ...
    depend_id = fields.Many2one(
        "ir.module.module", "Dependency", compute="_compute_depend", search="_search_depend"
    )
