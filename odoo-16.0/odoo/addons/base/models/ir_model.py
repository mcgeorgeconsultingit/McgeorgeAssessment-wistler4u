...
class IrModel(models.Model):
    ...
    def _default_field_id(self):
        ...
        return [
            Command.create(
                {"name": "x_name", "field_description": "Name", "ttype": "char", "copied": True}
            )
        ]
    ...
    def _compute_count(self):
        ...
        for model in self:
            ...
            if not records._abstract and records._auto:
                cr.execute(
                    sql.SQL("SELECT COUNT(*) FROM {}").format(sql.Identifier(records._table))
                )
    ...
    def _unlink_if_manual(self):
        ...
        for model in self:
            if model.state != "manual":
                raise UserError(
                    _("Model '%s' contains module data and cannot be removed.", model.name)
                )
    ...
    def _reflect_models(self, model_names):
        ...
        rows = [self._reflect_model_params(self.env[model_name]) for model_name in model_names]
...
class IrModelFields(models.Model):
    ...
    store = fields.Boolean(
        string="Stored", default=True, help="Whether the value is stored in the database."
    )
    ...
    def _related_field(self):
        ...
        for index, name in enumerate(names):
            ...
            if not field:
                raise UserError(
                    _("Unknown field name '%s' in related field '%s'") % (name, self.related)
                )
            model_name = field.relation
            if index < last and not field.relation:
                raise UserError(
                    _("Non-relational field name '%s' in related field '%s'") % (name, self.related)
                )
        return field
        ...
        for rec in self:
            if rec.state == "manual" and rec.related:
                ...
                if field.ttype != rec.ttype:
                    raise ValidationError(
                        _("Related field '%s' does not have type '%s'") % (rec.related, rec.ttype)
                    )
            ...
            for seq in record.depends.split(","):
                ...
                for index, name in enumerate(names):
                    ...
                    if index < last and not field.relational:
                        raise UserError(
                            _("Non-relational field %r in dependency %r") % (name, seq.strip())
                        )
                    model = model[name]
            ...
            if stored:
                self._cr.execute(
                    sql.SQL("ALTER TABLE {} RENAME COLUMN {} TO {}").format(
                        sql.Identifier(table), sql.Identifier(oldname), sql.Identifier(newname)
                    )
                )
                if index:
                    self._cr.execute(
    ...
    def _instanciate_attrs(self, field_data):
        ...
        if field_data["ttype"] in ("char", "text", "html"):
            ...
            attrs["selection"] = self.env["ir.model.fields.selection"]._get_selection_data(
                field_data["id"]
            )
            ...
            if not self.pool.loaded and not (
                ...
                and (
                    field_data["relation_field"] in self.env[field_data["relation"]]._fields
        ...
        for name, field_data in fields_data.items():
            if name not in model._fields and field_data["state"] == "manual":
                try:
                    ...
                    _logger.exception(
                        "Failed to load field %s.%s: skipped", model._name, field_data["name"]
                    )
...
class IrModelSelection(models.Model):
    ...
    def create(self, vals_list):
        ...
        if any(
            model in self.pool and name in self.pool[model]._fields for model, name in field_names
        ):
            # setup models; this re-initializes model in registry
            self.env.flush_all()
            self.pool.setup_models(self._cr)
    ...
    def _process_ondelete(self):
        ...
        for selection in self:
            ...
            if ondelete is None:
                ...
                raise ValueError(
                    _("The ondelete policy %r is not valid for field %r") % (ondelete, selection)
                )
...
class IrModelData(models.Model):
    ...
    res_id = fields.Many2oneReference(
        string="Record ID", help="ID of the target record in the database", model_field="model"
    )
    ...
    _sql_constraints = [
        ("name_nospaces", "CHECK(name NOT LIKE '% %')", "External IDs cannot contain spaces"),
    ]
    ...
    def _auto_init(self):
        ...
        tools.create_unique_index(
            self._cr, "ir_model_data_module_name_uniq_index", self._table, ["module", "name"]
        )
        tools.create_index(
            self._cr, "ir_model_data_model_res_id_index", self._table, ["model", "res_id"]
        )
        return res
    ...
    def check_object_reference(self, module, xml_id, raise_on_access_error=False):
        ...
        if raise_on_access_error:
            raise AccessError(
                _("Not enough access rights on the external ID:") + " %s.%s" % (module, xml_id)
            )
        return model, False
