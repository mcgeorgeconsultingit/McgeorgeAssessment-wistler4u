...
class TestMultiCompanyProject(TestMultiCompanyCommon):
    ...
    def setUpClass(cls):
        ...
        # setup users
        cls.user_employee_company_a.write({"groups_id": [(4, user_group_project_user.id)]})
        cls.user_manager_company_a.write({"groups_id": [(4, user_group_project_manager.id)]})
        cls.user_employee_company_b.write({"groups_id": [(4, user_group_project_user.id)]})
        cls.user_manager_company_b.write({"groups_id": [(4, user_group_project_manager.id)]})
    ...
    def test_move_task(self):
        with self.sudo("employee-a"):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(
                    task.company_id, self.company_a, "Moving a task should change its company."
                )
