...
class HrDepartureWizard(models.TransientModel):
    ...
    set_date_end = fields.Boolean(
        ...
        default=lambda self: self.env.user.user_has_groups("hr_contract.group_hr_contract_manager"),
    ...
    def action_register_departure(self):
        ...
        if current_contract and current_contract.date_start > self.departure_date:
            raise UserError(
                _("Departure date can't be earlier than the start date of current contract.")
            )

        super(HrDepartureWizard, self).action_register_departure()
        if self.set_date_end:
            self.sudo().employee_id.contract_ids.filtered(lambda c: c.state == "draft").write(
                {"state": "cancel"}
            )
            if current_contract:
