...
class MailActivity(models.Model):
    ...
    def _default_activity_type_for_model(self, model):
        ...
        if (
            ...
            and (activity_type_todo.res_model == model or not activity_type_todo.res_model)
        ):
    ...
    res_name = fields.Char(
        "Document Name", compute="_compute_res_name", compute_sudo=True, store=True, readonly=True
    )
    # activity
    ...
    user_id = fields.Many2one(
        "res.users", "Assigned to", default=lambda self: self.env.user, index=True, required=True
    )
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        super(MailActivity, self.with_user(access_rights_uid or self._uid)).check_access_rights(
            "read"
        )
    ...
    def activity_format(self):
        ...
        mail_template_dict = dict(
            [(mail_template["id"], mail_template) for mail_template in mail_template_info]
        )
        for activity in activities:
