...
class AccountPaymentRegister(models.TransientModel):
    ...
    amount = fields.Monetary(
        currency_field="currency_id", store=True, readonly=False, compute="_compute_amount"
    )
    ...
    def _get_batch_available_partner_banks(self, batch_result, journal):
        ...
        if payment_values["payment_type"] == "inbound":
            ...
            return (
                batch_result["lines"]
                .partner_id.bank_ids.filtered(lambda x: x.company_id.id in (False, company.id))
                ._origin
            )
    ...
    def _compute_available_partner_bank_ids(self):
        for wizard in self:
            if wizard.can_edit_wizard:
                batch = wizard._get_batches()[0]
                wizard.available_partner_bank_ids = wizard._get_batch_available_partner_banks(
                    batch, wizard.journal_id
                )
            else:
        ...
        for wizard in self:
            if wizard.journal_id.type == "cash":
                ...
                wizard.show_partner_bank_account = (
                    ...
                    in self.env["account.payment"]._get_method_codes_using_bank_account()
                )
    ...
    def _compute_amount(self):
        for wizard in self:
            if wizard.source_currency_id and wizard.can_edit_wizard:
                ...
                wizard.amount = wizard._get_total_amount_in_wizard_currency_to_full_reconcile(
                    batch_result
                )[0]
            else:
        ...
        for wizard in self:
            if wizard.can_edit_wizard:
                ...
                (
                    ...
                    mode,
                ) = wizard._get_total_amount_in_wizard_currency_to_full_reconcile(batch_result)
    ...
    def default_get(self, fields_list):
        ...
        if "line_ids" in fields_list and "line_ids" not in res:
            ...
            if self._context.get("active_model") == "account.move":
                lines = (
                    self.env["account.move"].browse(self._context.get("active_ids", [])).line_ids
                )
            ...
            if "journal_id" in res and not self.env["account.journal"].browse(
                ...
                [("company_id", "=", lines.company_id.id), ("type", "in", ("bank", "cash"))]
            ):
            ...
            if edit_mode:
                ...
                if payment.currency_id != lines.currency_id:
                    ...
                    if liquidity_lines[0].balance:
                        payment_rate = (
                            liquidity_lines[0].amount_currency / liquidity_lines[0].balance
                        )
                    else:
                        payment_rate = 0.0
