...
class Web_Editor(http.Controller):
    ...
    def update_checklist(self, res_model, res_id, filename, checklistId, checked, **kwargs):
        ...
        value = etree.tostring(htmlelem[0][0], encoding="utf-8", method="html")[5:-6].decode(
            "utf-8"
        )
    ...
    @http.route(
        "/web_editor/attachment/add_data", type="json", auth="user", methods=["POST"], website=True
    )
    def add_data(
        self,
        name,
    ...
    @http.route(
        "/web_editor/attachment/add_url", type="json", auth="user", methods=["POST"], website=True
    )
    ...
    def remove(self, ids, **kwargs):
        ...
        for attachment in Attachment.browse(ids):
            ...
            views = Views.search(
                ["|", ("arch_db", "like", '"%s"' % url), ("arch_db", "like", "'%s'" % url)]
            )
    ...
    def _attachment_create(
        ...
        if not request.env.is_admin() and IrAttachment._can_bypass_rights_on_media_dialog(
            **attachment_data
        ):
    ...
    def _update_svg_colors(self, options, svg):
        ...
        for key, value in options.items():
            ...
            if colorMatch:
                ...
                if not re.match(
                    r"(?i)^%s$|^%s$" % (regex_hex, regex_rgba), css_color_value.replace(" ", "")
                ):
                    if re.match("^o-color-([1-5])$", css_color_value):
                        ...
                        color_search = re.search(
                            r"(?i)--%s:\s+(%s|%s)" % (css_color_value, regex_hex, regex_rgba),
                            bundle_css,
                        )
                        if not color_search:
        ...
        color_mapping = {
            default_palette[palette_number]: color for color, palette_number in user_colors
        }
    ...
    @http.route(
        ["/web_editor/shape/<module>/<path:filename>"], type="http", auth="public", website=True
    )
    ...
    def save_library_media(self, media):
        ...
        for id, url in response.json().items():
            ...
            context = {
                "binary_field_real_user": request.env["res.users"].sudo().browse([SUPERUSER_ID])
            }
            attachment = (
                request.env["ir.attachment"]
                .sudo()
