...
class TestXMLRPC(common.HttpCase):
    ...
    def test_xmlrpc_name_search(self):
        self.xmlrpc_object.execute(
            common.get_db_name(), self.admin_uid, "admin", "res.partner", "name_search", "admin"
        )

    def test_xmlrpc_html_field(self):
        sig = '<p>bork bork bork <span style="font-weight: bork">bork</span><br></p>'
        r = self.env["res.users"].create({"name": "bob", "login": "bob", "signature": sig})
    ...
    def test_xmlrpc_attachment_raw(self):
        ...
        [att] = self.xmlrpc_object.execute(
            common.get_db_name(), self.admin_uid, "admin", "ir.attachment", "read", ids, ["raw"]
        )
...
class TestAPIKeys(common.HttpCase):
    ...
    def test_trivial(self):
        ...
        ctx = model.dispatch(
            "execute_kw", [self.env.cr.dbname, uid, "ananananan", "res.users", "context_get", []]
        )
    ...
    def test_wrongpw(self):
        ...
        with self.assertRaises(AccessDenied):
            model.dispatch(
                "execute_kw",
                [self.env.cr.dbname, self._user.id, "aws", "res.users", "context_get", []],
            )
    ...
    def test_disabled(self):
        ...
        with self.assertRaises(AccessDenied):
            model.dispatch(
                "execute_kw",
                [self.env.cr.dbname, self._user.id, "ananananan", "res.users", "context_get", []],
            )

        with self.assertRaises(AccessDenied):
            model.dispatch(
                "execute_kw", [self.env.cr.dbname, self._user.id, k, "res.users", "context_get", []]
            )
