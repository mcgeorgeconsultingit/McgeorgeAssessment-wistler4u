...
class PurchaseOrder(models.Model):
    ...
    picking_ids = fields.Many2many(
        "stock.picking", compute="_compute_picking_ids", string="Receptions", copy=False, store=True
    )
    ...
    def write(self, vals):
        if vals.get("order_line") and self.state == "purchase":
            for order in self:
                pre_order_line_qty = {
                    order_line: order_line.product_qty for order_line in order.mapped("order_line")
                }
    ...
    def _create_or_update_picking(self):
        for line in self:
            if line.product_id and line.product_id.type in ("product", "consu"):
                ...
                if (
                    float_compare(line.product_qty, line.qty_received, line.product_uom.rounding)
                    < 0
                ):
                    raise UserError(
                        _(
                ...
                if (
                    float_compare(line.product_qty, line.qty_invoiced, line.product_uom.rounding)
                    == -1
                ):
