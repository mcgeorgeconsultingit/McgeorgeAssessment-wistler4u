...
class SaleOrder(models.Model):
    ...
    def init(self):
        create_index(
            self._cr, "sale_order_date_order_id_idx", "sale_order", ["date_order desc", "id desc"]
        )
    ...
    def _compute_partner_invoice_id(self):
        for order in self:
            order.partner_invoice_id = (
                order.partner_id.address_get(["invoice"])["invoice"] if order.partner_id else False
            )
    ...
    def _compute_is_expired(self):
        ...
        for order in self:
            order.is_expired = (
                order.state == "sent" and order.validity_date and order.validity_date < today
            )
    ...
    def _compute_partner_credit_warning(self):
        for order in self:
            ...
            show_warning = (
                order.state in ("draft", "sent") and order.company_id.account_use_credit_limit
            )
            if show_warning:
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            for line in invoiceable_lines:
                ...
                invoice_line_vals.append(
                    Command.create(line._prepare_invoice_line(sequence=invoice_item_sequence)),
                )
                invoice_item_sequence += 1
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        if self._context.get("proforma"):
            for group in [
                g for g in groups if g[0] in ("portal_customer", "portal", "follower", "customer")
            ]:
        ...
        try:
            ...
            if self._has_to_be_signed(include_draft=True):
                if self._has_to_be_paid():
                    access_opt["title"] = (
                        _("View Quotation") if is_tx_pending else _("Sign & Pay Quotation")
                    )
                else:
