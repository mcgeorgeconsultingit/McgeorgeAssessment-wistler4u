...
class Lead(models.Model):
    ...
    def website_form_input_filter(self, request, values):
        ...
        values["team_id"] = values.get("team_id") or request.website.crm_default_team_id.id
        values["user_id"] = values.get("user_id") or request.website.crm_default_user_id.id
        if values.get("team_id"):
            ...
            values["type"] = (
                "lead"
                if self.with_user(SUPERUSER_ID).env["res.users"].has_group("crm.group_use_lead")
                else "opportunity"
            )
