...
class SlideChannelTag(models.Model):
    ...
    group_id = fields.Many2one(
        "slide.channel.tag.group", string="Group", index=True, required=True, ondelete="cascade"
    )
    group_sequence = fields.Integer(
        "Group sequence", related="group_id.sequence", index=True, readonly=True, store=True
    )
    channel_ids = fields.Many2many(
        "slide.channel", "slide_channel_tag_rel", "tag_id", "channel_id", string="Channels"
    )
