...
class MicrosoftService(models.AbstractModel):
    ...
    def _get_auth_endpoint(self):
        return (
            ...
            .sudo()
            .get_param("microsoft_account.auth_endpoint", DEFAULT_MICROSOFT_AUTH_ENDPOINT)
        )
    ...
    def _get_token_endpoint(self):
        return (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("microsoft_account.token_endpoint", DEFAULT_MICROSOFT_TOKEN_ENDPOINT)
        )
    ...
    def _do_request(
        ...
        _logger.debug(
            "Uri: %s - Type : %s - Headers: %s - Params : %s !" % (uri, method, headers, params)
        )
        ...
        try:
            if method.upper() in ("GET", "DELETE"):
                res = requests.request(
                    method.lower(), preuri + uri, headers=headers, params=params, timeout=timeout
                )
