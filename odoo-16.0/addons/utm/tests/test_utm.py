...
class TestUtm(TestUTMCommon):
    ...
    def test_name_generation(self):
        ...
        for utm_model in ("utm.source", "utm.medium", "utm.campaign"):
            ...
            self.assertEqual(
                utm_3.name, "UTM dup [2]", msg="Must add a counter as suffix to ensure uniqueness"
            )
            self.assertEqual(
                utm_4.name, "UTM dup [3]", msg="Must add a counter as suffix to ensure uniqueness"
            )
            self.assertEqual(
                utm_5.name, "UTM dup [4]", msg="Must add a counter as suffix to ensure uniqueness"
            )
            ...
            utm_8 = utm_7.copy()
            self.assertEqual(
                utm_8.name, "UTM d [2]", msg="Must add a counter as suffix to ensure uniqueness"
            )
