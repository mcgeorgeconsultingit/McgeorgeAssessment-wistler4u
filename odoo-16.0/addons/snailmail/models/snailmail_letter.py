...
class SnailmailLetter(models.Model):
    ...
    state = fields.Selection(
        [("pending", "In Queue"), ("sent", "Sent"), ("error", "Error"), ("canceled", "Canceled")],
        "Status",
        readonly=True,
        copy=False,
        default="pending",
    ...
    reference = fields.Char(
        string="Related Record", compute="_compute_reference", readonly=True, store=False
    )
                    ...
                    document.update(
                        {
                            ...
                            and self._count_pages_pdf(base64.b64decode(attachment.datas)),
                        }
                    )
                else:
                    letter.write(
    ...
    def _snailmail_print_valid_address(self):
        ...
        """
        endpoint = (
            self.env["ir.config_parameter"].sudo().get_param("snailmail.endpoint", DEFAULT_ENDPOINT)
        )
        timeout = int(
            self.env["ir.config_parameter"].sudo().get_param("snailmail.timeout", DEFAULT_TIMEOUT)
        )
        ...
        for doc in response["request"]["documents"]:
            if doc.get("sent") and response["request_code"] == 200:
                self.env["iap.account"]._send_iap_bus_notification(
                    service_name="snailmail", title=_("Snail Mails are successfully sent")
                )
                note = _(
