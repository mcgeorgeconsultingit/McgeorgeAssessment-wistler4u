...
class ProjectTaskType(models.Model):
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        project_ids = list(
            set([project["project_id"][0] for project in readgroup] + self.project_ids.ids)
        )
    ...
    def toggle_active(self):
        ...
        inactive_tasks = (
            ...
            .search([("active", "=", False), ("stage_id", "in", stage_active.ids)], limit=1)
        )
        if stage_active and inactive_tasks:
            ...
            return {
                ...
                "res_model": "project.task.type.delete.wizard",
                "views": [
                    (self.env.ref("project.view_project_task_type_unarchive_wizard").id, "form")
                ],
                "type": "ir.actions.act_window",
                "res_id": wizard.id,
                "target": "new",
...
class Project(models.Model):
    ...
    resource_calendar_id = fields.Many2one(
        "resource.calendar", string="Working Time", related="company_id.resource_calendar_id"
    )
    type_ids = fields.Many2many(
        "project.task.type", "project_task_type_rel", "project_id", "type_id", string="Tasks Stages"
    )
    ...
    user_id = fields.Many2one(
        "res.users", string="Project Manager", default=lambda self: self.env.user, tracking=True
    )
    ...
    allow_subtasks = fields.Boolean(
        "Sub-tasks", default=lambda self: self.env.user.has_group("project.group_subtask_project")
    )
    allow_recurring_tasks = fields.Boolean(
        "Recurring Tasks",
        default=lambda self: self.env.user.has_group("project.group_project_recurring_tasks"),
    )
    ...
    milestone_count_reached = fields.Integer(
        compute="_compute_milestone_reached_count", groups="project.group_project_milestone"
    )
    ...
    @api.depends(
        "milestone_ids", "milestone_ids.is_reached", "milestone_ids.deadline", "allow_milestones"
    )
    ...
    def _compute_collaborator_count(self):
        ...
        collaborator_count_by_project = {
            res["project_id"][0]: res["project_id_count"] for res in collaborator_read_group
        }
        for project in self:
...
class Task(models.Model):
    ...
    def _default_personal_stage_type_id(self):
        return (
            self.env["project.task.type"].search([("user_id", "=", self.env.user.id)], limit=1).id
        )
    ...
    def _default_company_id(self):
        if self._context.get("default_project_id"):
            return (
                self.env["project.project"].browse(self._context["default_project_id"]).company_id
            )
    ...
    date_deadline = fields.Date(
        string="Deadline", index=True, copy=False, tracking=True, task_dependency_tracking=True
    )
    ...
    portal_user_names = fields.Char(
        compute="_compute_portal_user_names", compute_sudo=True, search="_search_portal_user_names"
    )
    ...
    manager_id = fields.Many2one(
        "res.users", string="Project Manager", related="project_id.user_id", readonly=True
    )
    company_id = fields.Many2one(
        "res.company",
        string="Company",
    ...
    legend_blocked = fields.Char(
        related="stage_id.legend_blocked", string="Kanban Blocked Explanation", readonly=True
    )
    legend_done = fields.Char(
        related="stage_id.legend_done", string="Kanban Valid Explanation", readonly=True
    )
    legend_normal = fields.Char(
        related="stage_id.legend_normal", string="Kanban Ongoing Explanation", readonly=True
    )
    ...
    working_days_close = fields.Float(
        compute="_compute_elapsed", string="Working Days to Close", store=True, group_operator="avg"
    )
    ...
    repeat_day = fields.Selection(
        [(str(i), str(i)) for i in range(1, 32)], compute="_compute_repeat", readonly=False
    )
    ...
    def _compute_is_analytic_account_id_changed(self):
        for task in self:
            task.is_analytic_account_id_changed = (
                task.project_id and task.analytic_account_id != task.project_id.analytic_account_id
            )
            ...
            for user_id in user_ids:
                ...
                if not stage:
                    stages = (
                        self.env["project.task.type"]
                        .sudo()
                        .with_context(lang=user_id.partner_id.lang, default_project_id=False)
                        .create(
                            self.with_context(
    ...
    def _compute_recurrence_message(self):
        ...
        for task in self.filtered(lambda t: t.recurring_task and t._is_recurrence_valid()):
            ...
            recurrence_left = (
                task.recurrence_id.recurrence_left if task.recurrence_id else task.repeat_number
            )
            ...
            task.recurrence_message = (
                '<p><span class="fa fa-check-circle"></span> %s</p><ul>' % recurrence_title
            )
    ...
    def _compute_dependent_tasks_count(self):
        ...
        if tasks_with_dependency:
            ...
            dependent_tasks_count_dict = {
                group["depend_on_ids"][0]: group["depend_on_ids_count"] for group in group_dependent
            }
    ...
    def _compute_attachment_ids(self):
        for task in self:
            attachment_ids = (
                self.env["ir.attachment"].search(task._get_attachments_search_domain()).ids
            )
    ...
    def _compute_display_parent_task_button(self):
        accessible_parent_tasks = self.parent_id.with_user(self.env.user)._filter_access_rules(
            "read"
        )
        for task in self:
    ...
    def copy(self, default=None):
        ...
        if self.allow_task_dependencies:
            ...
            self.write(
                {
                    "depend_on_ids": [
                        Command.unlink(t.id) for t in self.depend_on_ids if t.id in new_tasks
                    ]
                }
            )
            self.write(
                {
                    "dependent_ids": [
                        Command.unlink(t.id) for t in self.dependent_ids if t.id in new_tasks
                    ]
                }
            )
            task_copy.write(
                {
                    "depend_on_ids": [
                        Command.link(task_mapping.get(t.id, t.id)) for t in self.depend_on_ids
                    ]
                }
            )
            task_copy.write(
                {
                    "dependent_ids": [
                        Command.link(task_mapping.get(t.id, t.id)) for t in self.dependent_ids
                    ]
                }
            )
        return task_copy
    ...
    def _ensure_fields_are_accessible(self, fields, operation="read", check_group_user=True):
        ...
        if (
            ...
            if unauthorized_fields:
                if operation == "read":
                    error_message = _(
                        "You cannot read %s fields in task.", ", ".join(unauthorized_fields)
                    )
                else:
                    error_message = _(
                        "You cannot write on %s fields in task.", ", ".join(unauthorized_fields)
                    )
    ...
    def _load_records_create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("recurring_task"):
                if vals.get("project_id") in projects_with_recurrence.ids and not vals.get(
                    "recurrence_id"
                ):
    ...
    def write(self, vals):
        ...
        if (
            ...
            raise UserError(
                _("Archived tasks cannot be recurring. Please unarchive the task first.")
            )
        ...
        if "stage_id" in vals and vals.get("stage_id"):
            tasks.filtered(
                lambda x: x.project_id.rating_active and x.project_id.rating_status == "stage"
    ...
    def _unlink_except_recurring(self):
        if any(self.mapped("recurrence_id")):
            ...
            raise UserError(
                _("You cannot delete recurring tasks. Please disable the recurrence first.")
            )
    ...
    def _search_has_late_and_unreached_milestone(self, operator, value):
        if operator not in ("=", "!=") or not isinstance(value, bool):
            raise NotImplementedError(
                _("The search does not support the %s operator or %s value.", operator, value)
            )
        domain = [
    ...
    def _mail_track(self, tracked_fields, initial_values):
        ...
        if (
            ...
            if parent_ids:
                ...
                child_subtype_info = (
                    child_subtype.description or child_subtype.name if child_subtype else False
                )
                ...
                for p in parent_ids:
                    p.message_post(
                        body=body, subtype_id=subtype, tracking_value_ids=depends_tracking_value_ids
                    )
    ...
    def _mail_get_message_subtypes(self):
        ...
        if len(self) == 1:
            ...
            if (
                ...
                or (
                    not self.project_id
                    and not self.user_has_groups("project.group_project_task_dependencies")
                )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        if self.project_privacy_visibility == "portal":
            groups.insert(0, ("allowed_portal_users", lambda pdata: pdata["type"] == "portal", {}))
