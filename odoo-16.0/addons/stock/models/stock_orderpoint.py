...
class StockWarehouseOrderpoint(models.Model):
    ...
    product_uom_name = fields.Char(
        string="Product unit of measure label", related="product_uom.display_name", readonly=True
    )
    ...
    qty_on_hand = fields.Float(
        "On Hand", readonly=True, compute="_compute_qty", digits="Product Unit of Measure"
    )
    qty_forecast = fields.Float(
        "Forecast", readonly=True, compute="_compute_qty", digits="Product Unit of Measure"
    )
    ...
    def _check_product_uom(self):
        ...
        if any(
            orderpoint.product_id.uom_id.category_id != orderpoint.product_uom.category_id
            for orderpoint in self
        ):
    ...
    def _compute_qty(self):
        ...
        for orderpoint_context, orderpoints_by_context in orderpoints_contexts.items():
            products_qty = {
                p["id"]: p
                for p in orderpoints_by_context.product_id.with_context(orderpoint_context).read(
    ...
    @api.depends(
        "qty_multiple", "qty_forecast", "product_min_qty", "product_max_qty", "visibility_days"
    )
    def _compute_qty_to_order(self):
        for orderpoint in self:
            ...
            if (
                float_compare(
                    orderpoint.qty_forecast, orderpoint.product_min_qty, precision_rounding=rounding
                )
                < 0
            ):
                ...
                qty_forecast_with_visibility = (
                    orderpoint.product_id.with_context(product_context).read(["virtual_available"])[
                        0
                    ]["virtual_available"]
                ...
                remainder = (
                    orderpoint.qty_multiple > 0 and qty_to_order % orderpoint.qty_multiple or 0.0
                )
    ...
    def _get_product_context(self, visibility_days=0):
        ...
        return {
            ...
            "to_date": datetime.combine(
                self.lead_days_date + relativedelta.relativedelta(days=visibility_days), time.max
            ),
        }
    ...
    def _get_orderpoint_action(self):
        ...
        for products in map(self.env["product.product"].browse, split_every(5000, all_product_ids)):
            for loc in all_replenish_location_ids:
                ...
                for product, quantity in zip(products, quantities):
                    ...
                    lead_days = rules.with_context(bypass_delay_description=True)._get_lead_days(
                        product
                    )[0]
        ...
        for (days, loc), product_ids in ploc_per_day.items():
            ...
            for qty in qties:
                if (
                    float_compare(
                        qty["virtual_available"], 0, precision_rounding=product.uom_id.rounding
                    )
                    < 0
                ):
        ...
        for (product, location_id), product_qty in to_refill.items():
            ...
            if orderpoint_id:
                ...
                orderpoint_values = self.env["stock.warehouse.orderpoint"]._get_orderpoint_values(
                    product, location_id
                )
    ...
    def _unlink_processed_orderpoints(self):
        ...
        orderpoints_to_remove = (
            self.env["stock.warehouse.orderpoint"].with_context(active_test=False).search(domain)
        )
        # Remove previous automatically created orderpoint that has been refilled.
