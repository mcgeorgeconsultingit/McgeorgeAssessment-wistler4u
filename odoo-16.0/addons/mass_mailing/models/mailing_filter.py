...
class MailingFilter(models.Model):
    ...
    create_uid = fields.Many2one(
        "res.users", "Saved by", index=True, readonly=True, default=lambda self: self.env.user
    )
        ...
        for mailing_filter in self:
            if mailing_filter.mailing_domain != "[]":
                try:
                    ...
                    raise ValidationError(_("The filter domain is not valid for this recipients."))
