...
class StockQuant(models.Model):
    ...
    def _domain_lot_id(self):
        ...
        if self.env.context.get("active_model") == "product.product":
            ...
            if product_template.exists():
                domain.insert(
                    0, "('product_id', 'in', %s)" % product_template.product_variant_ids.ids
                )
        else:
    ...
    owner_id = fields.Many2one(
        "res.partner", "Owner", help="This is the owner of the quant", check_company=True
    )
    ...
    inventory_quantity = fields.Float(
        "Counted Quantity", digits="Product Unit of Measure", help="The product's counted quantity."
    )
    ...
    inventory_quantity_set = fields.Boolean(
        store=True, compute="_compute_inventory_quantity_set", readonly=False, default=False
    )
    is_outdated = fields.Boolean(
        "Quantity has been moved since last count", compute="_compute_is_outdated"
    )
    user_id = fields.Many2one("res.users", "Assigned To", help="User assigned to do product count.")
    ...
    def _compute_inventory_date(self):
        quants = self.filtered(
            lambda q: not q.inventory_date and q.location_id.usage in ["internal", "transit"]
        )
    ...
    def _load_records_create(self, values):
        ...
        return super(StockQuant, self.with_context(inventory_mode=True))._load_records_create(
            values
        )
    ...
    def check_quantity(self):
        for quant in self:
            if (
                ...
                and float_compare(
                    abs(quant.quantity), 1, precision_rounding=quant.product_uom_id.rounding
                )
                > 0
            ):
    ...
    def _get_available_quantity(
        ...
        if product_id.tracking == "none":
            ...
            if allow_negative:
                ...
                return (
                    available_quantity
                    if float_compare(available_quantity, 0.0, precision_rounding=rounding) >= 0.0
                    else 0.0
                )
        else:
            availaible_quantities = {
                lot_id: 0.0 for lot_id in list(set(quants.mapped("lot_id"))) + ["untracked"]
            }
            for quant in quants:
                if not quant.lot_id:
    ...
    def _check_serial_number(
        self, product_id, lot_id, company_id, source_location_id=None, ref_doc_location_id=None
    ):
