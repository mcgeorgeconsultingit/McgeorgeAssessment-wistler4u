...
class StockMove(models.Model):
        ...
        for move in self:
            ...
            if self.env.user.has_group("base.group_portal"):
                move.show_details_visible = any(
                    not p._has_been_recorded() for p in move._get_subcontract_production()
                )
                continue
            productions = move._get_subcontract_production()
    ...
    def action_show_details(self):
        ...
        if self.state != "done" and (
            self._subcontrating_should_be_record() or self._subcontrating_can_be_record()
        ):
    ...
    def _set_quantities_to_reservation(self):
        move_untouchable = self.filtered(
            lambda m: m.is_subcontract and m._get_subcontract_production()._has_tracked_component()
        )
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        for move in self:
            ...
            if not bom:
                continue
            if (
                float_is_zero(move.product_qty, precision_rounding=move.product_uom.rounding)
            ...
            if (
                float_compare(move.product_qty, 0, precision_rounding=move.product_uom.rounding)
                <= 0
            ):
    ...
    def _has_tracked_subcontract_components(self):
        return any(
            m.has_tracking != "none" for m in self._get_subcontract_production().move_raw_ids
        )
