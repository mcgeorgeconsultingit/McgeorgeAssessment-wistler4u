...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_at_date(self):
        ...
        for line in self.filtered(lambda l: l.state == "sale"):
            ...
            line.forecast_expected_date = max(
                moves.filtered("forecast_expected_date").mapped("forecast_expected_date"),
                default=False,
            )
        ...
        for line in self.filtered(lambda l: l.state in ("draft", "sent")):
            if not (line.product_id and line.display_qty_widget):
                continue
            grouped_lines[
                (line.warehouse_id.id, line.order_id.commitment_date or line._expected_date())
            ] |= line
    ...
    @api.depends(
        "move_ids.state", "move_ids.scrapped", "move_ids.product_uom_qty", "move_ids.product_uom"
    )
