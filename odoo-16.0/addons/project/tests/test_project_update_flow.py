...
class TestProjectUpdate(TestProjectCommon):
    ...
    def test_project_update_form(self):
        with Form(
            self.env["project.milestone"].with_context({"default_project_id": self.project_pigs.id})
        ) as milestone_form:
        ...
        self.assertEqual(
            update.user_id, self.env.user, "The author is the user who created the update."
        )
        self.assertNotEqual(len(update.description), 0, "The description should not be empty.")
        self.assertTrue(
            "Activities" in update.description, "The description should contain 'Activities'."
        )
        self.assertEqual(update.status, "on_track", "The status should be the default one.")

        with Form(
            self.env["project.update"].with_context({"default_project_id": self.project_pigs.id})
        ) as update_form:
    ...
    def test_project_update_description(self):
        ...
        self.env["res.config.settings"].create({"group_project_milestone": False}).execute()
    ...
    def test_project_update_panel(self):
        ...
        self.assertEqual(
            len(panel_data["milestones"]["data"]), 3, "Panel data should contain 'milestone' entry"
        )
        self.assertFalse(
            panel_data["milestones"]["data"][0]["is_deadline_exceeded"], "Milestone is achieved"
        )
        self.assertTrue(
            panel_data["milestones"]["data"][1]["is_deadline_exceeded"], "Milestone is exceeded"
        )
        ...
        # sorting
        self.assertEqual(
            panel_data["milestones"]["data"][0]["name"], "Test 2", "Sorting isn't correct"
        )
        self.assertEqual(
            panel_data["milestones"]["data"][1]["name"], "Test 1", "Sorting isn't correct"
        )
        self.assertEqual(
            panel_data["milestones"]["data"][2]["name"], "Test 3", "Sorting isn't correct"
        )
