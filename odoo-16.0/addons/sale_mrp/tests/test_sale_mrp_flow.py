...
class TestSaleMrpFlow(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.uom_kg = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_kgm.id), ("uom_type", "=", "reference")], limit=1
        )
        ...
        cls.bom_kit_1 = cls.env["mrp.bom"].create(
            {"product_tmpl_id": cls.kit_1.product_tmpl_id.id, "product_qty": 1.0, "type": "phantom"}
        )
        ...
        bom_kit_2 = cls.env["mrp.bom"].create(
            {"product_tmpl_id": cls.kit_2.product_tmpl_id.id, "product_qty": 1.0, "type": "phantom"}
        )

        BomLine.create(
            {"product_id": cls.component_d.id, "product_qty": 1.0, "bom_id": bom_kit_2.id}
        )
        BomLine.create({"product_id": cls.kit_1.id, "product_qty": 2.0, "bom_id": bom_kit_2.id})
        ...
        bom_kit_3 = cls.env["mrp.bom"].create(
            {"product_tmpl_id": cls.kit_3.product_tmpl_id.id, "product_qty": 1.0, "type": "phantom"}
        )
    ...
    def test_00_sale_mrp_flow(self):
        ...
        self.assertEqual(
            mnf_product_a.product_qty, 10, "Wrong product quantity in manufacturing order."
        )
        self.assertEqual(
        ...
        self.assertEqual(
            list_qty, {6.0}, "Wrong product quantity in 'To consume line' of manufacturing order."
        )
        # Check state of consume line with product c and uom kg.
        for move in moves:
            self.assertEqual(
                move.state, "confirmed", "Wrong state in 'To consume line' of manufacturing order."
            )
        ...
        self.assertEqual(
            mnf_product_d.reservation_state, "assigned", "Availability should be assigned"
        )
        self.assertEqual(
            move.state, "assigned", "Wrong state in 'To consume line' of manufacturing order."
        )
        ...
        self.assertEqual(
            mnf_product_d.state, "done", "Manufacturing order should still be in progress state."
        )
    ...
    def test_01_sale_mrp_delivery_kit(self):
        ...
        product.write(
            {
                "route_ids": [
                    (6, 0, [self.company_data["default_warehouse"].manufacture_pull_id.route_id.id])
                ]
            }
        )
    ...
    def test_02_sale_mrp_anglo_saxon(self):
        ...
        account_income = self.env["account.account"].create(
            {"name": "Income", "code": "INC00", "account_type": "asset_current", "reconcile": True}
        )
    ...
    def test_04_sale_mrp_kit_qty_delivered(self):
        ...
        for return_move in return_wiz.product_return_moves:
            return_move.write(
                {"quantity": expected_quantities[return_move.product_id], "to_refund": True}
            )
        ...
        for move in return_of_return_pick.move_ids:
            move.write(
                {"quantity_done": expected_quantities[move.product_id] - 1, "to_refund": True}
            )
    ...
    def test_05_mrp_sale_kit_availability(self):
        ...
        warehouse_1 = self.env["stock.warehouse"].create({"name": "Warehouse 1", "code": "WH1"})
        warehouse_2 = self.env["stock.warehouse"].create({"name": "Warehouse 2", "code": "WH2"})
    ...
    def test_06_kit_qty_delivered_mixed_uom(self):
        ...
        # Then create a backorder for the missing components
        qty_to_process = {component_uom_unit: 48, component_uom_dozen: 3, component_uom_kg: 0.006}
        ...
        # Adding missing components
        qty_to_process = {component_uom_unit: 192, component_uom_dozen: 7, component_uom_kg: 0.024}
        self._process_quantities(backorder_1.move_ids, qty_to_process)
    ...
    def test_15_anglo_saxon_variant_price_unit(self):
        ...
        self.pt_attr_NOKIT = self.product_template.attribute_line_ids[0].product_template_value_ids[
            1
        ]
        ...
        aml_kit_expense = amls.filtered(
            lambda l: l.display_type == "cogs" and l.debit > 0 and l.product_id == self.variant_KIT
        )
        aml_kit_output = amls.filtered(
            lambda l: l.display_type == "cogs" and l.credit > 0 and l.product_id == self.variant_KIT
        )
    ...
    def test_reconfirm_cancelled_kit(self):
        ...
        # Check picking creation
        self.assertEqual(
            len(so.picking_ids), 1, "A picking should be created after the SO validation"
        )
