...
class OdooSuite(BackportSuite):
    ...
    def _tearDownPreviousClass(self, test, result):
        ...
        test_id = (
            f"{previous_test_class.__module__}.{previous_test_class.__qualname__}.tearDownClass"
        )
        with result.collectStats(test_id):
            super()._tearDownPreviousClass(test, result)
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
        ...
        def _compare_candidate(record, candidate, field_names):
            ...
            for field_name in field_names:
                ...
                if field_type == "monetary":
                    ...
                    if (
                        float_compare(candidate[field_name], record_value, precision_digits=prec)
                        != 0
                    ):
                        diff[field_name] = (record_value, candidate[field_name])
                    ...
                    elif (record_value or candidate[field_name]) and record_value.id != candidate[
                        field_name
                    ]:
                    ...
                    if field_name not in candidate:
                        diff[field_name] = (record_value, None)
                    elif (candidate[field_name] or record_value) and record_value != candidate[
                        field_name
                    ]:
                        diff[field_name] = (record_value, candidate[field_name])
            return diff
        ...
        for index, record in enumerate(records):
            ...
            if diff:
                different_values.append(
                    (index, "additional_record" if is_additional_record else "regular_diff", diff)
                )
    ...
    def profile(self, description="", **kwargs):
        ...
        return profiler.Profiler(
            ...
            % (test_method, self.env.user.id, "warm" if self.warm else "cold", description),
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        while tb:
            ...
            if code.co_filename.endswith("/unittest/case.py") and code.co_name in (
                "_callTestMethod",
                "_callSetUp",
                "_callTearDown",
                "_callCleanup",
...
class ChromeBrowser:
    ...
    def __init__(self, test_class):
        ...
        self._receiver = threading.Thread(
            target=self._receive, name="WebSocket events consumer", args=(get_db_name(),)
        )
...
which leads to stray network requests and inconsistencies."""
                ...
                try:
                    ...
                    if self._result.exception() is None:
                        self._result = Future()
                        self._result.set_exception(
                            ChromeBrowserException("Tried to make the tour successful twice.")
                        )
            ...
            with open(concat_script_path, "w") as concat_file:
                for i in range(nb_frames):
                    frame_file_path = os.path.join(
                        self.screencasts_frames_dir, self.screencast_frames[i]["file_path"]
                    )
                    end_time = (
                        time.time()
                        if i == nb_frames - 1
...
class HttpCase(TransactionCase):
    ...
    def url_open(
        self, url, data=None, files=None, timeout=12, headers=None, allow_redirects=True, head=False
    ):
        if url.startswith("/"):
            url = self.base_url() + url
        if head:
            return self.opener.head(
                url, data=data, files=files, timeout=timeout, headers=headers, allow_redirects=False
            )
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        def get_http_request_threads():
            return [
                t for t in threading.enumerate() if t.name.startswith("odoo.service.http.request.")
            ]
    ...
    def browser_js(
        ...
        **kw,
        ...
        try:
            ...
            self.assertTrue(
                self.browser._wait_ready(ready), 'The ready "%s" code was always falsy' % ready
            )
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        ready = kwargs.pop(
            "ready", "odoo.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name
        )
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(
            _profiler, patch("odoo.http.Request._get_profiler_context_manager", route_profiler)
        )
...
def users(*logins):
    ...
    def _users(func, *args, **kwargs):
        ...
        try:
            ...
            user_id = {
                user.login: user.id for user in Users.search([("login", "in", list(logins))])
            }
            for login in logins:
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    def _get_view_fields(self, node, model):
        ...
        fnames = set(
            el.get("name") for el in node.xpath(".//field[count(ancestor::field) = %s]" % level)
        )
    ...
    def _o2m_set_edition_view(self, descr, node, level):
        default_view = next((m for m in node.get("mode", "tree").split(",") if m != "form"), "tree")
        ...
        edition_view = (
            "tree" if default_view == "tree" and views["tree"].get("editable") else "form"
        )
        edition = {
    ...
    def __setattr__(self, field, value):
        ...
        assert not self._get_modifier(field, "readonly"), "can't write on readonly field {}".format(
            field
        )
    ...
    def _values_to_save(self, all_fields=False):
        ...
        for f in self._view["fields"]:
            if self._get_modifier(f, "required") and not (
                self._get_modifier(f, "column_invisible") or self._get_modifier(f, "invisible")
            ):
