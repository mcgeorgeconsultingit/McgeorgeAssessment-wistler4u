...
class ResConfigSettings(models.TransientModel):
    ...
    pos_config_id = fields.Many2one(
        "pos.config", string="Point of Sale", default=lambda self: self._default_pos_config()
    )
    ...
    pos_cash_rounding = fields.Boolean(
        related="pos_config_id.cash_rounding", readonly=False, string="Cash Rounding (PoS)"
    )
    ...
    pos_iface_cashdrawer = fields.Boolean(
        string="Cashdrawer", compute="_compute_pos_iface_cashdrawer", readonly=False, store=True
    )
    ...
    def _compute_pos_pricelist_id(self):
        for res_config in self:
            ...
            pricelists_in_current_currency = self.env["product.pricelist"].search(
                [
                    ("company_id", "in", (False, res_config.pos_config_id.company_id.id)),
