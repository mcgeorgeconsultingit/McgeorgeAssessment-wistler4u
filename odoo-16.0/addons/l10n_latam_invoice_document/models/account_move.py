        ...
        * If move use document and is in draft state and has not been posted before we restart name to '/' (this is
        ...
        (
            ...
            + manual_documents.filtered(
                lambda x: not x.name or x.name and x.state == "draft" and not x.posted_before
            )
        ).name = "/"
    ...
    def _check_invoice_type_document_type(self):
        for rec in self.filtered("l10n_latam_document_type_id.internal_type"):
            ...
            if internal_type in ["debit_note", "invoice"] and invoice_type in [
                ...
                raise ValidationError(
                    _("You can not use a %s document type with a refund invoice", internal_type)
                )
            elif internal_type == "credit_note" and invoice_type in ["out_invoice", "in_invoice"]:
                raise ValidationError(
                    _("You can not use a %s document type with a invoice", internal_type)
                )
