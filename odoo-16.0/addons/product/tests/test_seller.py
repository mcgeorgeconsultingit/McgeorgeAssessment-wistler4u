...
class TestSeller(TransactionCase):
    ...
    def test_10_sellers(self):
        ...
        context_code = self.product_service.with_context(partner_id=self.camptocamp.id).code
        self.assertEqual(
            "C2CCODE", context_code, "Partner's code not used in product name with context set"
        )
    ...
    def test_20_sellers_company(self):
        ...
        self.product_consu.write(
            {
                "seller_ids": [
                    ...
                    (
                        0,
                        0,
                        {"partner_id": self.asustec.id, "product_code": "NO", "company_id": False},
                    ),
                ]
            }
        )
        ...
        self.assertEqual(
            ref, {"[A] Boudin", "[B] Boudin", "[NO] Boudin"}, "Incorrect vendor reference list"
        )
        names = self.product_consu.with_context(
            partner_id=self.asustec.id,
            company_id=company_a.id,
    ...
    def test_30_select_seller(self):
        ...
        msg = (
            "Wrong cost price: LCD Monitor if more than 3 Unit.should be 785 instead of %s" % price
        )
