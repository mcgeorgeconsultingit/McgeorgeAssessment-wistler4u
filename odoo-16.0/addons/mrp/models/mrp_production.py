...
class MrpProduction(models.Model):
    ...
    reservation_state = fields.Selection(
        [("confirmed", "Waiting"), ("assigned", "Ready"), ("waiting", "Waiting Another Operation")],
        string="MO Readiness",
    ...
    move_byproduct_ids = fields.One2many(
        "stock.move", compute="_compute_move_byproduct_ids", inverse="_set_move_byproduct_ids"
    )
    ...
    move_dest_ids = fields.One2many(
        "stock.move", "created_production_id", string="Stock Movements of Produced Goods"
    )
    ...
    delay_alert_date = fields.Datetime(
        "Delay Alert Date", compute="_compute_delay_alert_date", search="_search_delay_alert_date"
    )
    ...
    production_location_id = fields.Many2one(
        "stock.location", "Production Location", compute="_compute_production_location", store=True
    )
    ...
    consumption = fields.Selection(
        [("flexible", "Allowed"), ("warning", "Allowed with warning"), ("strict", "Blocked")],
        required=True,
        readonly=True,
        default="flexible",
    )
    ...
    show_lot_ids = fields.Boolean(
        "Display the serial number shortcut on the moves", compute="_compute_show_lot_ids"
    )
    forecasted_issue = fields.Boolean(compute="_compute_forecasted_issue")
    show_serial_mass_produce = fields.Boolean(
        "Display the serial mass produce wizard action", compute="_compute_show_serial_mass_produce"
    )
    ...
    def _compute_locations(self):
        for production in self:
            ...
            production.location_dest_id = (
                production.picking_type_id.default_location_dest_id.id or fallback_loc.id
            )
                ...
                if forecast_date:
                    ...
                    if production.date_planned_start:
                        production.components_availability_state = (
                            "late" if forecast_date > production.date_planned_start else "expected"
                        )
    ...
    def _compute_date_deadline(self):
        for production in self:
            production.date_deadline = min(
                production.move_finished_ids.filtered("date_deadline").mapped("date_deadline"),
    ...
    def _compute_product_uom_qty(self):
        for production in self:
            if production.product_id.uom_id != production.product_uom_id:
                production.product_uom_qty = production.product_uom_id._compute_quantity(
        ...
        for production in self:
            if not production.state or not production.product_uom_id:
                ...
                or (
                    ...
                    and all(move.state in ("cancel", "done") for move in production.move_raw_ids)
                )
                ...
                production.qty_producing, precision_rounding=production.product_uom_id.rounding
            ):
                production.state = "progress"
            elif any(
                not float_is_zero(
    ...
    def _compute_workorder_ids(self):
        for production in self:
            ...
            workorders_list = [
                Command.link(wo.id)
                for wo in production.workorder_ids.filtered(lambda wo: not wo.operation_id)
            ]
            ...
            if production.bom_id and production.product_id and production.product_qty > 0:
                ...
                workorders_dict = {
                    ...
                    for wo in production.workorder_ids.filtered(lambda wo: wo.operation_id)
                }
    ...
    def _compute_unreserve_visible(self):
        for order in self:
            ...
            order.reserve_visible = order.state in ("confirmed", "progress", "to_close") and any(
                move.product_uom_qty and move.state in ["confirmed", "partially_available"]
    ...
    def _compute_show_serial_mass_produce(self):
        ...
        for order in self:
            if (
                ...
                and float_compare(
                    order.product_qty, 1, precision_rounding=order.product_uom_id.rounding
                )
                > 0
                and float_compare(
                    order.qty_producing,
        ...
        for mo in self:
            ...
            if lines:
                ...
                wh_location_ids = self.env["stock.location"]._search(
                    [
                        ("id", "child_of", mo.picking_type_id.warehouse_id.view_location_id.id),
                        ("usage", "!=", "supplier"),
                    ]
                )
    ...
    def _compute_move_raw_ids(self):
        for production in self:
            ...
            list_move_raw = [
                ...
                for move in production.move_raw_ids.filtered(lambda m: not m.bom_line_id)
            ]
            if not production.bom_id and not production._origin.product_id:
            ...
            if production.bom_id and production.product_id and production.product_qty > 0:
                ...
                for move_raw_values in moves_raw_values:
                    if move_raw_values["bom_line_id"] in move_raw_dict:
                        ...
                        list_move_raw += [
                            Command.update(
                                move_raw_dict[move_raw_values["bom_line_id"]].id, move_raw_values
                            )
                        ]
                    else:
                        # add new entries
        ...
        for production in self:
            ...
            if (
                ...
                if (
                    ...
                    production.date_planned_finished = new_date_planned_start + datetime.timedelta(
                        hours=1
                    )
        return res
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get("name", False) or vals["name"] == _("New"):
                ...
                vals["name"] = (
                    self.env["stock.picking.type"].browse(picking_type_id).sequence_id.next_by_id()
                )
    ...
    def _get_move_raw_values(
        self, product_id, product_uom_qty, product_uom, operation_id=False, bom_line=False
    ):
    ...
    def _get_origin(self):
        ...
        if self.orderpoint_id and self.origin:
            origin = self.origin.replace("%s - " % (self.orderpoint_id.display_name), "")
    ...
    def _set_qty_producing(self):
        ...
        for move in self.move_raw_ids | self.move_finished_ids.filtered(
            ...
            if self.use_auto_consume_components_lots and move.has_tracking in ("lot", "serial"):
                if (
                    float_compare(
                        move.reserved_availability, 0, precision_rounding=move.product_uom.rounding
                    )
                    <= 0
                ):
                    continue
    ...
    def _update_raw_moves(self, factor):
        ...
        for move in self.move_raw_ids.filtered(lambda m: m.state not in ("done", "cancel")):
            ...
            new_qty = float_round(
                old_qty * factor, precision_rounding=move.product_uom.rounding, rounding_method="UP"
            )
            if new_qty > 0:
    ...
    def _get_ready_to_produce_state(self):
        ...
        moves_in_first_operation = moves_in_first_operation.filtered(
            lambda move: move.bom_line_id and not move.bom_line_id._skip_bom_line(self.product_id)
        )
    ...
    def _autoconfirm_production(self):
        ...
        for production in self:
            if production.state in ("done", "cancel"):
                continue
            additional_moves = production.move_raw_ids.filtered(lambda move: move.state == "draft")
    ...
    def _link_workorders_and_moves(self):
        ...
        if self.allow_workorder_dependencies:
            ...
            for workorder in self.workorder_ids:
                if (
                    previous_workorder
                    and previous_workorder.operation_id.bom_id == workorder.operation_id.bom_id
                ):
        ...
        for move in self.move_raw_ids | self.move_finished_ids:
            if move.operation_id:
                move.write({"workorder_id": workorder_per_operation[move.operation_id].id})
            else:
                bom = (
        ...
        self.with_context(force_date=True).write(
            {
                "date_planned_start": min(
                    [workorder.leave_id.date_from for workorder in self.workorder_ids]
                ),
                "date_planned_finished": max(
                    [workorder.leave_id.date_to for workorder in self.workorder_ids]
                ),
            }
        )
    ...
    def button_unplan(self):
        if any(wo.state == "done" for wo in self.workorder_ids):
            raise UserError(
                _("Some work orders are already done, you cannot unplan this manufacturing order.")
            )
    ...
    def _get_consumption_issues(self):
        ...
        for order in self:
            ...
            for product, qty_to_consume in expected_qty_by_product.items():
                ...
                if (
                    float_compare(
                        qty_to_consume, qty_done, precision_rounding=product.uom_id.rounding
                    )
                    != 0
                ):
    ...
    def _get_quantity_produced_issues(self):
        ...
        for order in self:
            if not float_is_zero(
                order._get_quantity_to_backorder(), precision_rounding=order.product_uom_id.rounding
            ):
    ...
    def _action_cancel(self):
        ...
        self.filtered(
            lambda p: p.state not in ["done", "cancel"] and p.bom_id.consumption == "flexible"
        ).write({"state": "done"})
    ...
    def _post_inventory(self, cancel_backorder=False):
        ...
        moves_to_do = self.move_raw_ids.filtered(lambda x: x.state == "done") - self.env[
            "stock.move"
        ].browse(moves_not_to_do)
        ...
        for order in self:
            ...
            for workorder in order.workorder_ids:
                ...
                if workorder.duration == 0.0:
                    workorder.duration = (
                        workorder.duration_expected * order.qty_produced / order.product_qty
                    )
    ...
    def _get_name_backorder(self, name, sequence):
        if not sequence:
            return name
        seq_back = (
            "-" + "0" * (SIZE_BACK_ORDER_NUMERING - 1 - int(math.log10(sequence))) + str(sequence)
        )
    ...
    def _split_productions(self, amounts=False, cancel_remaining_qty=False, set_consumed_qty=False):
        ...
        for production in self:
            ...
            next_seq = max(
                production.procurement_group_id.mrp_production_ids.mapped("backorder_sequence"),
                default=1,
            )
        ...
        backorders = (
            self.env["mrp.production"].with_context(skip_confirm=True).create(backorder_vals_list)
        )
        ...
        for production in self:
            for move in production.move_raw_ids | production.move_finished_ids:
                ...
                for backorder in production_to_backorders[production]:
                    move_vals = dict(
                        initial_move_vals, product_uom_qty=backorder.product_qty * unit_factor
                    )
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            if (
                ...
                for move in list(initial_move + backorder_moves - backorder_move_to_ignore):
                    new_ml_vals = dict(ml_vals, qty_done=move.product_uom_qty, move_id=move.id)
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            for move_line in initial_move.move_line_ids:
                ...
                if (
                    float_compare(
                        available_qty, 0, precision_rounding=move_line.product_uom_id.rounding
                    )
                    <= 0
                ):
                    continue
            ...
            initial_move.move_line_ids.with_context(bypass_reservation_update=True).write(
                {"reserved_uom_qty": 0}
            )
            ...
            for quantity, move_line, ml_vals in ml_by_move:
                while (
                    float_compare(quantity, 0, precision_rounding=product_uom.rounding) > 0 and move
                ):
                    ...
                    if move == initial_move:
                        ...
                        new_ml_vals = dict(ml_vals, reserved_uom_qty=taken_qty_uom, move_id=move.id)
                        if set_consumed_qty:
                    ...
                    if (
                        float_compare(
                            move_qty_to_reserve, 0, precision_rounding=move.product_uom.rounding
                        )
                        <= 0
                    ):
    ...
    def _is_finished_sn_already_produced(self, lot, excluded_sml=None):
        ...
        if duplicates:
            ...
            # Either removed or unbuild
            if not (
                (duplicates_unbuild or removed) and duplicates - duplicates_unbuild - removed == 0
            ):
                return True
        # Check presence of same sn in current production
    ...
    def _check_immediate(self):
        ...
        for production in self:
            if all(
                ...
                for ml in production.move_raw_ids.move_line_ids.filtered(
                    lambda m: m.state not in ("done", "cancel")
                )
    ...
    def _pre_action_split_merge_hook(self, merge=False, split=False):
        ...
        if any(not production.bom_id for production in self):
            raise UserError(
                _("Only manufacturing orders with a Bill of Materials can be %s.", ope_str)
            )
        if split:
            return True
        ...
        if len(products) > 1:
            raise UserError(
                _("You can only merge manufacturing orders of identical products with same BoM.")
            )
