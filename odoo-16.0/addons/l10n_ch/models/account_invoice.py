...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ch_isr_subscription(self):
        ...
        def _format_isr_subscription_scanline(isr_subscription):
            ...
            return (
                isr_subscription[:2] + isr_subscription[2:-1].rjust(6, "0") + isr_subscription[-1:]
            )
    ...
    def _compute_l10n_ch_isr_number(self):
        ...
        * ISR-B (Indirect through a bank, requires a customer ID)
        ...
        for record in self:
            if (
                record.partner_bank_id.l10n_ch_qr_iban or record.l10n_ch_isr_subscription
            ) and record.name:
    ...
    def _compute_l10n_ch_isr_needs_fixing(self):
        for inv in self:
            if inv.move_type == "in_invoice" and inv.company_id.account_fiscal_country_id.code in (
                "CH",
                "LI",
            ):
