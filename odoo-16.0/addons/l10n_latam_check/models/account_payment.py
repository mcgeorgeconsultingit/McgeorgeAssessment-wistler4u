...
class AccountPayment(models.Model):
    ...
    def _compute_check_number(self):
        ...
        latam_checks = self.filtered(
            ...
            or (x.payment_method_line_id.code == "check_printing" and x.l10n_latam_manual_checks)
        )
    ...
    def _compute_l10n_latam_check_warning_msg(self):
        ...
        latam_draft_checks = self.filtered(
            ...
            and (
                ...
                in ["in_third_party_checks", "out_third_party_checks", "new_third_party_checks"]
            )
        )
        ...
        for rec in self.filtered("l10n_latam_check_id"):
            ...
            if rec.payment_method_line_id.code in [
                ...
                if rec.l10n_latam_check_id.state != "posted":
                    msgs.append(
                        _('Selected check "%s" is not posted', rec.l10n_latam_check_id.display_name)
                    )
                elif (
    ...
    @api.onchange(
        "payment_method_line_id", "is_internal_transfer", "journal_id", "destination_journal_id"
    )
            ...
            res[0].update(
                {
                    "name": document_name
                    + " - "
                    + "".join([item[1] for item in self._get_aml_default_display_name_list()]),
                }
            )
        return res
    ...
    def button_open_check_operations(self):
        ...
        operations = (
            self.l10n_latam_check_operation_ids.filtered(lambda x: x.state == "posted") + self
        )
        action = {
