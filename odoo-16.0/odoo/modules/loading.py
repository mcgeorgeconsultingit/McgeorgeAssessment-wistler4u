    ...
    def _get_files_of_kind(kind):
        ...
        for k in keys:
            for f in package.data[k]:
                if f in files:
                    _logger.warning(
                        "File %s is imported twice in module %s %s", f, package.name, kind
                    )
                files.append(f)
...
def _check_module_names(cr, module_names):
    ...
    if mod_names:
        cr.execute(
            "SELECT count(id) AS count FROM ir_module_module WHERE name in %s", (tuple(mod_names),)
        )
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if update_module:
            ...
            if module_names:
                modules = Module.search(
                    [("state", "in", ("installed", "to upgrade")), ("name", "in", module_names)]
                )
                if modules:
