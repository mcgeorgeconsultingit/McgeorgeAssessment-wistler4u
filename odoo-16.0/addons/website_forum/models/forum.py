...
class Forum(models.Model):
    ...
    @api.depends(
        "post_ids.state", "post_ids.views", "post_ids.child_count", "post_ids.favourite_count"
    )
    ...
    def create(self, vals_list):
        forums = super(
            Forum, self.with_context(mail_create_nolog=True, mail_create_nosubscribe=True)
        ).create(vals_list)
...
class Post(models.Model):
    ...
    karma_accept = fields.Integer(
        "Convert comment to answer", compute="_get_post_karma_rights", compute_sudo=False
    )
    ...
    can_post = fields.Boolean(
        "Can Automatically be Validated", compute="_get_post_karma_rights", compute_sudo=False
    )
    ...
    def _get_post_karma_rights(self):
        ...
        for post, post_sudo in zip(self, self.sudo()):
            ...
            post.karma_edit = (
                post.forum_id.karma_edit_own if is_creator else post.forum_id.karma_edit_all
            )
            post.karma_close = (
                post.forum_id.karma_close_own if is_creator else post.forum_id.karma_close_all
            )
            post.karma_unlink = (
                post.forum_id.karma_unlink_own if is_creator else post.forum_id.karma_unlink_all
            )
            post.karma_comment = (
                post.forum_id.karma_comment_own if is_creator else post.forum_id.karma_comment_all
            )
            ...
            post.can_upvote = (
                is_admin or user.karma >= post.forum_id.karma_upvote or post.user_vote == -1
            )
            post.can_downvote = (
                is_admin or user.karma >= post.forum_id.karma_downvote or post.user_vote == 1
            )
    ...
    def _default_website_meta(self):
        ...
        res["default_opengraph"]["og:image"] = res["default_twitter"]["twitter:image"] = self.env[
            "website"
        ].image_url(self.create_uid, "image_1024")
    ...
    def create(self, vals_list):
        ...
        for post in posts:
            ...
            if not post.parent_id and not post.can_ask:
                raise AccessError(
                    _("%d karma required to create a new question.", post.forum_id.karma_ask)
                )
            elif post.parent_id and not post.can_answer:
                raise AccessError(
                    _("%d karma required to answer a question.", post.forum_id.karma_answer)
                )
        ...
        for post in self:
            if "state" in vals:
                if vals["state"] in ["active", "close"]:
                    if not post.can_close:
                        raise AccessError(
                            _("%d karma required to close or reopen a post.", post.karma_close)
                        )
            ...
            if "active" in vals:
                if not post.can_unlink:
                    raise AccessError(
                        _("%d karma required to delete or reactivate a post.", post.karma_unlink)
                    )
            if "is_correct" in vals:
                if not post.can_accept:
                    raise AccessError(
                        _("%d karma required to accept or refuse an answer.", post.karma_accept)
                    )
    ...
    def reopen(self):
        ...
        for post in self:
            if post.closed_reason_id in (reason_offensive, reason_spam):
                _logger.info(
                    "Upvoting user <%s>, reopening spam/offensive question", post.create_uid
                )
    ...
    def close(self, reason_id):
        ...
        if reason_id in (reason_offensive, reason_spam):
            for post in self:
                _logger.info(
                    "Downvoting user <%s> for posting spam/offensive contents", post.create_uid
                )
                karma = post.forum_id.karma_gen_answer_flagged
    ...
    def validate(self):
        for post in self:
            if not post.can_moderate:
                raise AccessError(
                    _("%d karma required to validate a post.", post.forum_id.karma_moderate)
                )
    ...
    def refuse(self):
        for post in self:
            if not post.can_moderate:
                raise AccessError(
                    _("%d karma required to refuse a post.", post.forum_id.karma_moderate)
                )
    ...
    def bump(self):
        ...
        if (
            ...
            and (
                datetime.today()
                - datetime.strptime(self.write_date, tools.DEFAULT_SERVER_DATETIME_FORMAT)
            ).days
            > 9
        ):
    ...
    def convert_answer_to_comment(self):
        ...
        values = {
            ...
            "body": tools.html_sanitize(
                self.content, sanitize_attributes=True, strip_style=True, strip_classes=True
            ),
    ...
    def convert_comment_to_answer(self, message_id, default=None):
        ...
        if not can_convert:
            if is_author and karma_own < karma_all:
                raise AccessError(
                    _("%d karma required to convert your comment to an answer.", karma_own)
                )
            else:
                raise AccessError(
...
class Vote(models.Model):
    ...
    vote = fields.Selection(
        [("1", "1"), ("-1", "-1"), ("0", "0")], string="Vote", required=True, default="1"
    )
    create_date = fields.Datetime("Create Date", index=True, readonly=True)
    forum_id = fields.Many2one(
        "forum.forum", string="Forum", related="post_id.forum_id", store=True, readonly=False
    )
    recipient_id = fields.Many2one(
        "res.users", string="To", related="post_id.create_uid", store=True, readonly=False
    )
...
class Tags(models.Model):
    ...
    def create(self, vals_list):
        ...
        return super(
            Tags, self.with_context(mail_create_nolog=True, mail_create_nosubscribe=True)
        ).create(vals_list)
