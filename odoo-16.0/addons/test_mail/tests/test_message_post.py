...
class TestMailNotifyAPI(TestMessagePostCommon):
    ...
    def test_email_notifiction_layouts(self):
        ...
        for email_xmlid in [
            ...
            with self.mock_mail_gateway():
                test_record._notify_thread_by_email(test_message, recipients_data, force_send=False)
            self.assertEqual(
                len(self._new_mails),
                2,
    ...
    def test_notify_by_mail_add_signature(self):
        ...
        template = self.env.ref(
            "mail.mail_notification_layout_with_responsible_signature", raise_if_not_found=True
        ).sudo()
    ...
    def test_notify_by_email_prepare_rendering_context(self):
        ...
        other_company = (
            self.env["res.company"].with_user(self.user_admin).create({"name": "Company B"})
        )
    ...
    def test_notify_recipients_internals(self):
        ...
        emp_info = next(
            item for item in classify_res if item["recipients"] == self.partner_employee.ids
        )
...
class TestMessageNotify(TestMessagePostCommon):
    ...
    def test_notify(self):
        ...
        admin_mails = [
            mail for mail in self._mails if self.partner_admin.name in mail.get("email_to")[0]
        ]
        self.assertEqual(len(admin_mails), 1, "There should be exactly one email sent to admin")
        admin_mail_body = admin_mails[0].get("body")

        self.assertTrue(
            "model=" in admin_mail_body, "The email sent to admin should contain an access link"
        )
    ...
    def test_notify_from_user_id_batch(self):
        ...
        self.assertEqual(
            len(self._new_msgs), 20, "Should have 20 messages: 10 tracking and 10 assignments"
        )
        for test_record in test_records:
            assign_notif = self._new_msgs.filtered(
                lambda msg: msg.message_type == "user_notification" and msg.res_id == test_record.id
            )
            self.assertTrue(assign_notif)
            self.assertMessageFields(
                assign_notif,
...
class TestMessageLog(TestMessagePostCommon):
    ...
    def test_message_log(self):
        ...
        self.assertMessageFields(
            new_note,
            {
                ...
                "reply_to": formataddr(
                    (self.company_admin.name, "%s@%s" % (self.alias_catchall, self.alias_domain))
                ),
                "res_id": test_record.id,
                "subtype_id": self.env.ref("mail.mt_note"),
            },
    ...
    def test_message_log_batch(self):
        ...
        new_notes = test_records._message_log_batch(
            bodies=dict(
                (test_record.id, "<p>Test _message_log_batch</p>") for test_record in test_records
            ),
        )
...
class TestMessagePost(TestMessagePostCommon, CronMixinCase):
    ...
    def test_message_post(self):
        ...
        self.assertMessageFields(
            new_message, {"notified_partner_ids": self.partner_1 + self.partner_employee_2}
        )
        # notifications emails should have been deleted
        self.assertFalse(
            self.env["mail.mail"].sudo().search_count([("mail_message_id", "=", new_message.id)])
        )
    ...
    def test_message_post_keep_emails(self):
        ...
        self.assertEqual(
            self.env["mail.mail"].sudo().search_count([("mail_message_id", "=", msg.id)]), 2
        )
    ...
    def test_message_post_schedule(self):
        ...
        with freeze_time(now), self.assertMsgWithoutNotifications(), self.capture_triggers(
            cron_id
        ) as capt:
            msg = test_record.message_post(
                body="<p>Test</p>",
        ...
        schedules = (
            self.env["mail.message.schedule"].sudo().search([("mail_message_id", "=", msg.id)])
        )
    ...
    def test_message_post_schedule_update(self):
        ...
        with freeze_time(now), self.assertNoNotifications(), self.capture_triggers(cron.id) as capt:
    ...
    @users("employee")
    @mute_logger(
        "odoo.addons.mail.models.mail_mail", "odoo.addons.mail.models.mail_message_schedule"
    )
    ...
    def test_portal_acls(self):
        ...
        self.assertEqual(
            new_msg.sudo().notified_partner_ids, (self.partner_1 | self.user_employee.partner_id)
        )
...
class TestMessagePostGlobal(TestMessagePostCommon):
    ...
    def test_message_post_return(self):
        ...
        message_id = call_kw(
            self.env["mail.test.simple"], "message_post", [test_record.id], {"body": "test"}
        )
...
class TestMessagePostLang(TestMailCommon, TestRecipients):
    ...
    def test_composer_lang_template_comment(self):
        ...
        self.assertTrue(
            record0_customer, "Template usage should have created a contact based on record email"
        )
        ...
        self.assertIn(
            "Spanish Layout para Spanish Model Description", body, "Model name should be translated"
        )
        # check notification layout strings
        self.assertIn(
            "View Lang Chatter Model", body, 'Fixme: "View document" should be translated'
        )
        ...
        self.assertIn(
            "NotificationButtonTitle", body, "Fixme: Groups-based action names should be translated"
        )
    ...
    def test_layout_email_lang_context(self):
        ...
        # check notification layout strings
        self.assertIn(
            "SpanishView Spanish Model Description", body, '"View document" should be translated'
        )
