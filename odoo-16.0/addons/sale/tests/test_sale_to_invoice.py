...
class TestSaleToInvoice(TestSaleCommon):
    ...
    def test_downpayment(self):
        ...
        payment = (
            self.env["sale.advance.payment.inv"]
            .with_context(self.context)
            .create({"deposit_account_id": self.company_data["default_account_revenue"].id})
        )
        ...
        self.assertEqual(
            len(
                invoice.invoice_line_ids.filtered(
                    lambda l: not (l.display_type == "line_section" and l.name == "Down Payments")
                )
            ),
            len(
    ...
    def test_multiple_sale_orders_on_same_invoice(self):
        ...
        invoice = self.sale_order.invoice_ids[0]
        self.assertTrue(any(len(move_line.sale_line_ids) > 1 for move_line in invoice.line_ids))
    ...
    def test_invoice_analytic_rule_with_account_prefix(self):
        ...
        self.assertRecordValues(
            aml, [{"analytic_distribution": analytic_distribution_model.analytic_distribution}]
        )
    ...
    def test_cost_invoicing(self):
        ...
        self.assertTrue(
            sol, "Sale: cost invoicing does not add lines when confirming vendor invoice"
        )
        self.assertEqual(
