...
__all__ = ["convert_file", "convert_sql_import", "convert_csv_import", "convert_xml_import"]
    ...
    if node.tag in ("field", "value"):
        ...
        if t == "file":
            ...
            if not module.get_module_resource(self.module, path):
                raise IOError("No such file or directory: '%s' in %s" % (path, self.module))
...
form: module.record_id""" % (
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        if rec.get("action"):
            ...
            if (
                not values.get("name")
                and act.type.endswith(("act_window", "wizard", "url", "client", "server"))
                and act.name
            ):
    ...
    def _tag_template(self, el):
        ...
        if groups:
            ...
            record.append(
                Field(name="groups_id", eval="[Command.set([" + ", ".join(grp_lst) + "])]")
            )
