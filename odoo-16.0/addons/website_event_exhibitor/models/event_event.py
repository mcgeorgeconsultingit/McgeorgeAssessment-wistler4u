...
class EventEvent(models.Model):
    ...
    exhibitor_menu = fields.Boolean(
        string="Showcase Exhibitors", compute="_compute_exhibitor_menu", readonly=False, store=True
    )
    ...
    def _compute_exhibitor_menu(self):
        for event in self:
            if event.event_type_id and event.event_type_id != event._origin.event_type_id:
                ...
                event.website_menu != event._origin.website_menu or not event.exhibitor_menu
            ):
    ...
    def _update_website_menus(self, menus_update_by_field=None):
        ...
        for event in self:
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("exhibitor_menu")
            ):
