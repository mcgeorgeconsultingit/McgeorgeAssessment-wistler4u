...
class HolidaysAllocation(models.Model):
    ...
    lastcall = fields.Date(
        "Date of the last accrual allocation", readonly=True, default=fields.Date.context_today
    )
    ...
    def _compute_leaves(self):
        employee_days_per_allocation = self.holiday_status_id._get_employees_days_per_allocation(
        ...
        for allocation in self:
            ...
            allocation.leaves_taken = employee_days_per_allocation[allocation.employee_id.id][
    ...
    def _end_of_year_accrual(self):
        ...
        for allocation in self:
            ...
            lastcall = (
                ...
                if allocation.lastcall < current_level._get_previous_date(last_day_last_year)
            ...
            if current_level.action_with_unused_accruals == "lost":
                ...
                allocation.write(
                    {"number_of_days": number_of_days, "lastcall": lastcall, "nextcall": nextcall}
                )
    ...
    def _get_current_accrual_plan_level_id(self, date, level_ids=False):
        ...
        if current_level._get_next_date(level_start_date) < previous_level._get_next_date(
            level_start_date
        ):
    ...
    def _process_accrual_plan_level(self, level, start_period, start_date, end_period, end_date):
        ...
        if level.added_value_type == "hours":
            added_value = added_value / (
                self.employee_id.sudo().resource_id.calendar_id.hours_per_day or HOURS_PER_DAY
            )
        period_prorata = 1
        if (
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False):
        ...
        for allocation in self:
            ...
            while allocation.nextcall <= date_to:
                (current_level, current_level_idx) = allocation._get_current_accrual_plan_level_id(
                    allocation.nextcall
                )
            ...
            if days_added_per_level:
                ...
                allocation.number_of_days = (
                    min(
                        number_of_days_to_add, current_level_maximum_leave + allocation.leaves_taken
                    )
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if not values.get("department_id"):
                values.update(
                    {"department_id": self.env["hr.employee"].browse(employee_id).department_id.id}
                )
    ...
    def write(self, values):
        if not bool(values.get("active", True)):
            if any(allocation.state not in ["draft", "cancel", "refuse"] for allocation in self):
                raise UserError(
                    _("You cannot archive an allocation which is in confirm or validate state.")
                )
        employee_id = values.get("employee_id", False)
        if values.get("state"):
    ...
    def _unlink_if_no_leaves(self):
        if any(
            ...
            raise UserError(
                _("You cannot delete an allocation request which has some validated leaves.")
            )
    ...
    def action_confirm(self):
        ...
        self.activity_update()
        self.filtered(
            lambda holiday: holiday.validation_type == "no" and holiday.state != "validate"
        ).action_validate()
        return res
