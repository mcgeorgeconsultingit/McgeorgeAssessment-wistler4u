...
class HolidaysType(models.Model):
    ...
    group_days_leave = fields.Float(compute="_compute_group_days_leave", string="Group Time Off")
    ...
    leave_notif_subtype_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref("hr_holidays.mt_leave", raise_if_not_found=False),
    )
    ...
    def _compute_valid(self):
        ...
        employee_id = self._context.get(
            "default_employee_id", self._context.get("employee_id", self.env.user.employee_id.id)
        )
    ...
    def _get_employees_days_per_allocation(self, employee_ids, date=None):
        ...
        company_domain = [
            (
                "company_id",
                "in",
                list(set(self.env.company.ids + self.env.context.get("allowed_company_ids", []))),
            )
        ]
            ...
            for holiday_status_id in allocations_days_consumed[employee_id]:
                for allocation in allocations_days_consumed[employee_id][holiday_status_id]:
                    if allocation:
                        ...
                        ]["closest_allocation_to_expire"] = allocations_days_consumed[employee_id][
                            holiday_status_id
                        ][
                            False
                        ][
                        ...
                        for leave_key in leave_keys:
                            if allocations_days_consumed[employee_id][holiday_status_id][False].get(
                                leave_key
                            ):
                                result[employee_id][
                                    holiday_status_id
    ...
    def get_days_all_request(self):
        leave_types = sorted(
            self.search([]).filtered(lambda x: ((x.virtual_remaining_leaves > 0 or x.max_leaves))),
    ...
    def _get_days_request(self):
        ...
        closest_allocation_remaining = result[self.closest_allocation_to_expire.employee_id.id][
            self
        ][self.closest_allocation_to_expire]["virtual_remaining_leaves"]
        return (
            self.name,
            {
                ...
                .rstrip("."),
                "leaves_requested": (
                    "%.2f" % (self.max_leaves - self.virtual_remaining_leaves - self.leaves_taken)
                )
                .rstrip("0")
                .rstrip("."),
    ...
    def _compute_allocation_count(self):
        ...
        max_datetime = fields.Datetime.to_string(
            datetime.datetime.now().replace(month=12, day=31, hour=23, minute=59, second=59)
        )
        domain = [
            ("holiday_status_id", "in", self.ids),
        ...
        grouped_dict = dict(
            (data["holiday_status_id"][0], data["holiday_status_id_count"]) for data in grouped_res
        )
