...
class CrmTeamMember(models.Model):
        ...
        if duplicates:
            raise exceptions.ValidationError(
                _(
                    ...
                    duplicates=", ".join(
                        "%s (%s)" % (m.user_id.name, m.crm_team_id.name) for m in duplicates
                    ),
                )
            )
        ...
        for member in self:
            if member_user_ids:
                ...
                member.user_in_teams_ids = (
                    self.env["crm.team"].browse(self.env.context["default_crm_team_id"]).member_ids
                )
            else:
    ...
    def _compute_is_membership_multi(self):
        multi_enabled = (
            self.env["ir.config_parameter"].sudo().get_param("sales_team.membership_multi", False)
        )
    ...
    def create(self, values_list):
        ...
        return super(CrmTeamMember, self.with_context(mail_create_nosubscribe=True)).create(
            values_list
        )
