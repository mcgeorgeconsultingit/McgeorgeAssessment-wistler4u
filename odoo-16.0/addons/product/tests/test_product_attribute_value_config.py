...
class TestProductAttributeValueCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        (cls.ssd_attribute, cls.ram_attribute, cls.hdd_attribute, cls.size_attribute,) = cls.env[
            "product.attribute"
        ].create(
            [
                {
        ...
        cls.computer_case = cls.env["product.template"].create({"name": "Super Computer Case"})
                                ...
                                Command.set(
                                    [
                                        self._get_product_value_id(
                                            self.computer_hdd_attribute_lines, self.hdd_1
                                        ).id
                                    ]
                                )
                            ],
...
class TestProductAttributeValueConfig(TestProductAttributeValueCommon):
    ...
    def test_children_product_filtered_exclude_for(self):
        ...
        self.assertFalse(
            self.computer_case._get_variant_for_combination(computer_size_m)._is_variant_possible(
                computer_hdd_4
            )
        )
    ...
    def test_get_first_possible_combination(self):
        ...
        self._add_exclude(
            self._get_product_template_attribute_value(
                product_template.attribute_line_ids[1].value_ids[0], model=product_template
            ),
            self._get_product_template_attribute_value(
                product_template.attribute_line_ids[0].value_ids[0], model=product_template
            ),
            product_template,
        )
        self._add_exclude(
            ...
            self._get_product_template_attribute_value(
                product_template.attribute_line_ids[1].value_ids[1], model=product_template
            ),
            product_template,
        )
    ...
    def test_constraints(self):
        ...
        with self.assertRaises(
            UserError, msg="can't change variants creation mode of attribute used on product"
        ):
        ...
        with self.assertRaises(
            UserError, msg="can't change the value of a product template attribute value"
        ):
        ...
        with self.assertRaises(
            UserError, msg="can't change the product of a product template attribute value"
        ):
            self.computer_ram_attribute_lines.product_template_value_ids[
                0
            ].product_tmpl_id = self.computer_case.id
        ...
        with mute_logger("odoo.sql_db"), self.assertRaises(
            IntegrityError, msg="can't have two values with the same name for the same attribute"
        ):
            self.env["product.attribute.value"].create(
                {
                    "name": "32 GB",
