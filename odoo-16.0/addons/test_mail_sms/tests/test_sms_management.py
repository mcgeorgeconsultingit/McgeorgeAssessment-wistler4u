...
class TestSMSWizards(TestSMSActionsCommon):
    ...
    def test_sms_resend(self):
        ...
        with self.with_user("employee"):
            wizard = (
                self.env["sms.resend"].with_context(default_mail_message_id=self.msg.id).create({})
            )
            wizard.write(
                {"recipient_ids": [(1, r.id, {"resend": True}) for r in wizard.recipient_ids]}
            )
    ...
    def test_sms_resend_update_number(self):
        ...
        with self.with_user("employee"):
            ...
            wizard.write(
                {
                    "recipient_ids": [
                        (1, r.id, {"resend": True, "sms_number": self.random_numbers[idx]})
    ...
    def test_sms_resend_w_cancel(self):
        ...
        with self.with_user("employee"):
            ...
            wizard.write(
                {
                    "recipient_ids": [
                        (1, r.id, {"resend": True if r.partner_id == self.partner_1 else False})
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1, "state": "sent"}], "TEST BODY", self.msg, check_sms=True
        )
        self.assertSMSNotification(
            [
                {
