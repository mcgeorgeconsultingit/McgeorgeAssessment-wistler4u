...
class Currency(models.Model):
    ...
    def _compute_current_rate(self):
        ...
        company = (
            self.env["res.company"].browse(self._context.get("company_id")) or self.env.company
        )
...
class CurrencyRate(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Currency", readonly=True, required=True, ondelete="cascade"
    )
    ...
    def _get_last_rates_for_companies(self, companies):
        return {
            ...
            .filtered(lambda x: (x.rate and x.company_id == company or not x.company_id))
            .sorted("name")[-1:]
            .rate
            or 1
