...
class TestReorderingRule(TransactionCase):
    ...
    def test_reordering_rule_2(self):
        ...
        self.assertTrue(purchase_order, "No purchase order created.")
        self.assertEqual(
            len(purchase_order.order_line), 2, "Not enough purchase order lines created."
        )
        ...
        self.assertEqual(self.product_01.with_context(location=subloc_2.id).virtual_available, 0)
        self.assertEqual(
            self.product_01.with_context(location=warehouse_1.lot_stock_id.id).virtual_available, 10
        )  # 5 on the main loc, 5 on subloc_1

        self.assertEqual(purchase_order.picking_ids.move_ids[-1].product_qty, 5)
        self.assertEqual(
            purchase_order.picking_ids.move_ids[-1].location_dest_id, warehouse_1.lot_stock_id
        )
    ...
    def test_reordering_rule_3(self):
        ...
        # Check purchase order created or not
        purchase_order = (
            self.env["purchase.order.line"].search([("product_id", "=", product.id)]).order_id
        )
        self.assertTrue(purchase_order, "No purchase order created.")
        self.assertEqual(
            len(purchase_order.order_line), 1, "Not enough purchase order lines created."
        )
        purchase_order.button_confirm()
    ...
    def test_2steps_and_partner_on_orderpoint(self):
        ...
        po_line = self.env["purchase.order.line"].search(
            [("partner_id", "=", self.partner.id), ("product_id", "=", self.product_01.id)]
        )
    ...
    def test_decrease_qty_multi_step_receipt(self):
        ...
        moves = self.env["stock.move"].search(
            [("id", "!=", out_move.id), ("product_id", "=", self.product_01.id)], order="id desc"
        )
        self.assertRecordValues(
            moves,
            [
