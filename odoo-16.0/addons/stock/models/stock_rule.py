...
class StockRule(models.Model):
    ...
    action = fields.Selection(
        selection=[("pull", "Pull From"), ("push", "Push To"), ("pull_push", "Pull & Push")],
        string="Action",
        required=True,
    )
    ...
    location_dest_id = fields.Many2one(
        "stock.location", "Destination Location", required=True, check_company=True
    )
    ...
    @api.depends(
        "action", "location_dest_id", "location_src_id", "picking_type_id", "procure_method"
    )
    ...
    def _run_pull(self, procurements):
        ...
        procurements = sorted(
            ...
            key=lambda proc: float_compare(
                proc[0].product_qty, 0.0, precision_rounding=proc[0].product_uom.rounding
            )
            > 0,
        )
        for procurement, rule in procurements:
            ...
            if rule.procure_method == "mts_else_mto":
                ...
                if (
                    float_compare(
                        qty_needed, 0, precision_rounding=procurement.product_id.uom_id.rounding
                    )
                    <= 0
                ):
                    procure_method = "make_to_order"
                    ...
                    float_compare(
                        qty_needed,
                        forecasted_qties_by_loc[rule.location_src_id][procurement.product_id.id],
        ...
        if not partner and move_dest_ids:
            ...
            if location_dest_id == company_id.internal_transit_location_id:
                ...
                move_dest.partner_id = (
                    self.location_src_id.warehouse_id.partner_id or self.company_id.partner_id
                )
    ...
    def _get_lead_days(self, product, **values):
        ...
        if global_visibility_days:
            delay_description.append(
                (_("Global Visibility Days"), _("+ %d day(s)") % int(global_visibility_days))
            )
...
class ProcurementGroup(models.Model):
    ...
    def _search_rule(self, route_ids, packaging_id, product_id, warehouse_id, domain):
        ...
        if warehouse_id:
            domain = expression.AND(
                [
                    ["|", ("warehouse_id", "=", warehouse_id.id), ("warehouse_id", "=", False)],
                    domain,
                ]
            )
        ...
        if not res and packaging_id:
            ...
            if packaging_routes:
                res = Rule.search(
                    expression.AND([[("route_id", "in", packaging_routes.ids)], domain]),
                    order="route_sequence, sequence",
                    limit=1,
                )
        if not res:
    ...
    def _run_scheduler_tasks(self, use_new_cursor=False, company_id=False):
        ...
        moves_to_assign = self.env["stock.move"].search(
            domain, limit=None, order="reservation_date, priority desc, date asc, id asc"
        )
