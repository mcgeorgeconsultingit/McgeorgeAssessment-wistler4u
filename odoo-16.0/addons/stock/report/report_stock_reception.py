...
class ReceptionReport(models.AbstractModel):
    ...
    def get_report_data(self, docids, data):
        ...
        report_values["sources_to_formatted_scheduled_date"] = self._format_html_sources_to_date(
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if not docs:
            ...
            docs = False
            msg = _(
                "This report cannot be used for done and not done %s at the same time", doc_types
            )
        if not docs:
            ...
            for out in outs:
                ...
                for move_in_qty, move_in in product_to_qty_to_assign[out.product_id]:
                    ...
                    if move_in_qty:
                        ...
                        product_to_qty_to_assign[out.product_id] = product_to_qty_to_assign[
                            out.product_id
                        ][1:]
                    if (
                        float_compare(
                ...
                if float_compare(
                    qty_to_reserve, qty_done, precision_rounding=product_uom.rounding
                ) > 0 and not float_is_zero(qty_expected, precision_rounding=product_uom.rounding):
                    to_expect = min(qty_expected, qty_to_reserve - qty_done)
                    sources_to_lines[source].append(
                        self._prepare_report_line(
                            to_expect, product_id, out, source[0], is_qty_assignable=False
                        )
                    )
        ...
        for product_id, qty_and_ins in product_to_total_assigned.items():
            ...
            for out_move in out_moves:
                if float_is_zero(
                    total_assigned, precision_rounding=out_move.product_id.uom_id.rounding
                ):
        ...
        for source, dummy in sources_to_lines.items():
            sources_to_formatted_scheduled_date[source] = self._get_formatted_scheduled_date(
                source[0]
            )
    ...
    def action_assign(self, move_ids, qtys, in_ids):
        ...
        for out, qty_to_link in zip(outs, qtys):
            if (
                float_compare(
                    out.product_qty, qty_to_link, precision_rounding=out.product_id.uom_id.rounding
                )
                == 1
            ):
        ...
        for out, qty_to_link, ins in zip(outs, qtys, in_ids):
                ...
                if potential_ins[0].state != "done" and out.reserved_availability:
                    ...
                    for move_line_id in new_out.move_line_ids:
                        if assigned_amount + move_line_id.reserved_qty > qty_to_link:
                            ...
                            move_line_id.reserved_uom_qty = out.product_id.uom_id._compute_quantity(
            ...
            for in_move in reversed(potential_ins):
                ...
                if (
                    ...
                    or float_compare(
                        0, quantity_remaining, precision_rounding=in_move.product_id.uom_id.rounding
                    )
                    >= 0
                ):
    ...
    def action_unassign(self, move_id, qty, in_ids):
        ...
        for in_move in ins:
            ...
            if (
                float_compare(
                    qty, amount_unassigned, precision_rounding=out.product_id.uom_id.rounding
                )
                <= 0
            ):
                break
                ...
                if new_out.reserved_availability > new_out.product_qty:
                    ...
                    for move_line_id in new_out.move_line_ids:
                        ...
                        if move_line_id.reserved_qty > reserved_amount_to_remain:
                            ...
                            new_move_line.reserved_uom_qty = (
                                out.product_id.uom_id._compute_quantity(
                                    move_line_id.reserved_qty - reserved_amount_to_remain,
    ...
    def _format_html_sources_info(self, sources_to_lines):
        ...
        return {
            str(source): [self._format_html_source(s, s._name == "stock.picking") for s in source]
