...
class TestWithholdingAndPensionFundTaxes(TestItEdi):
    ...
    def test_withholding_taxes_export(self):
        ...
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(self.withholding_tax_invoice)
        )
    ...
    def test_withholding_taxes_import(self):
        ...
        for line in invoice.line_ids.filtered(
            ...
            self.assertEqual(
                [1, 1, 0], [len(x) for x in (vat_taxes, withholding_taxes, pension_fund_taxes)]
            )
    ...
    def test_pension_fund_taxes_export(self):
        ...
        """
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(self.pension_fund_tax_invoice)
        )
        invoice_data = self.get_real_client_invoice_data()
        expected_etree = self.with_applied_xpath(
            etree.fromstring(self.edi_basis_xml),
    ...
    def test_pension_fund_taxes_import(self):
        ...
        for line in invoice.line_ids.filtered(
            ...
            vat_taxes = line.tax_ids - withholding_taxes - pension_fund_taxes
            self.assertEqual(
                [1, 1, 1], [len(x) for x in (vat_taxes, withholding_taxes, pension_fund_taxes)]
            )
    ...
    def test_enasarco_tax_export(self):
        ...
        """
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(self.enasarco_tax_invoice)
        )
        invoice_data = self.get_real_client_invoice_data()
        expected_etree = self.with_applied_xpath(
            etree.fromstring(self.edi_basis_xml),
