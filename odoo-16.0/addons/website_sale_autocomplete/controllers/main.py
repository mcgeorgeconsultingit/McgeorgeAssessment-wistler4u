...
class AutoCompleteController(http.Controller):
    ...
    def _perform_place_search(
        self, partial_address, api_key=None, session_id=None, language_code=None, country_code=None
    ):
        ...
        try:
            results = requests.get(
                f"{GOOGLE_PLACES_ENDPOINT}/autocomplete/json", params=params, timeout=TIMEOUT
            ).json()
    ...
    def _perform_complete_place_search(
        ...
        results.sort(
            key=lambda r: FIELDS_PRIORITY.index(r["type"]) if r["type"] in FIELDS_PRIORITY else 100
        )
    ...
    @http.route(
        "/autocomplete/address_full", methods=["POST"], type="json", auth="public", website=True
    )
