...
class ProductTemplate(models.Model):
    ...
    used_in_bom_count = fields.Integer(
        "# of BoM Where is Used", compute="_compute_used_in_bom_count", compute_sudo=False
    )
...
class ProductProduct(models.Model):
    ...
    def _compute_is_kits(self):
        ...
        for product in self:
            product.is_kits = (
                product.id in kits_product_ids or product.product_tmpl_id.id in kits_template_ids
            )
    ...
    def _compute_mrp_product_qty(self):
        ...
        for product in self:
            ...
            product.mrp_product_qty = float_round(
                mapped_data.get(product.id, 0), precision_rounding=product.uom_id.rounding
            )
        ...
        for product in bom_kits:
            ...
            for bom_line, bom_line_data in bom_sub_lines:
                ...
                qty_per_kit = bom_line.product_uom_id._compute_quantity(
                    uom_qty_per_kit, bom_line.product_id.uom_id, round=False, raise_if_failure=False
                )
                if not qty_per_kit:
                    continue
    ...
    def action_compute_bom_days(self):
        ...
        for product in self:
            bom_data = (
                ...
                ._get_bom_data(bom_by_products[product], warehouse, product, ignore_stock=True)
            )
            availability_delay = bom_data.get("resupply_avail_delay")
            product.days_to_prepare_mo = (
                availability_delay - bom_data.get("lead_time", 0) if availability_delay else 0
            )
    ...
    def _match_all_variant_values(self, product_template_attribute_value_ids):
        ...
        return len(
            self.product_template_attribute_value_ids & product_template_attribute_value_ids
