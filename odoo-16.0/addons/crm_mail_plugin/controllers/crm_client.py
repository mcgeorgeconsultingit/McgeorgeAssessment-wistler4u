...
class CrmClient(MailPluginController):
    ...
    @http.route(
        "/mail_client_extension/lead/get_by_partner_id", type="json", auth="outlook", cors="*"
    )
    ...
    @http.route(
        "/mail_client_extension/lead/create_from_partner", type="http", auth="user", methods=["GET"]
    )
