...
class HrAttendance(models.Model):
    ...
    department_id = fields.Many2one(
        "hr.department", string="Department", related="employee_id.department_id", readonly=True
    )
                    ...
                    raise exceptions.ValidationError(
                        ...
                        % {
                            ...
                            "datetime": format_datetime(
                                self.env, no_check_out_attendances.check_in, dt_format=False
                            ),
                        }
                    )
            else:
                ...
                if (
                    ...
                    raise exceptions.ValidationError(
                        ...
                        % {
                            "empl_name": attendance.employee_id.name,
                            "datetime": format_datetime(
                                self.env, last_attendance_before_check_out.check_in, dt_format=False
                            ),
                        }
                    )
    ...
    def _get_attendances_dates(self):
        ...
        for attendance in self.filtered(
            ...
            if check_in_day_start[0] < datetime.combine(
                attendance.employee_id.company_id.overtime_start_date, datetime.min.time()
            ):
                continue
            ...
            for day_data in attendance_dates:
                    ...
                    # The employee usually work on that day
                        ...
                        for attendance in attendances:
                            ...
                            if local_check_in < planned_start_dt:
                                pre_work_time += (
                                    min(planned_start_dt, local_check_out) - local_check_in
                                ).total_seconds() / 3600.0
                            ...
                            if local_check_out > planned_end_dt:
                                post_work_time += (
                                    local_check_out - max(planned_end_dt, local_check_in)
                                ).total_seconds() / 3600.0
                        ...
                        overtime_duration_real = (
                            sum(attendances.mapped("worked_hours")) - planned_work_duration
                        )
                ...
                if not float_is_zero(overtime_duration, 2) or unfinished_shifts:
                    ...
                    if not overtime and overtime_duration:
                        ...
                        overtime.sudo().write(
                            {"duration": overtime_duration, "duration_real": overtime_duration}
                        )
                elif overtime:
