...
class TestCurrencyRates(TransactionCase):
    ...
    def test_currency_with_date(self):
        self.assertEqual(
            self.env["res.currency.rate"]._get_rate_for_spreadsheet("USD", "EUR", "2021-11-11"),
            CURRENT_EUR / USD_11,
        )
        self.assertEqual(
            self.env["res.currency.rate"]._get_rate_for_spreadsheet("EUR", "USD", "2021-11-11"),
            USD_11,
        )
        self.assertEqual(
            self.env["res.currency.rate"]._get_rate_for_spreadsheet("USD", "CAD", "2021-11-11"),
            CAD_11 / USD_11,
        )
