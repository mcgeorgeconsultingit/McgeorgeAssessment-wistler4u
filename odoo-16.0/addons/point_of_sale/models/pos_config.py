...
class PosConfig(models.Model):
    ...
    iface_print_via_proxy = fields.Boolean(
        string="Print via Proxy", help="Bypass browser printing and prints via the hardware proxy."
    )
    ...
    current_user_id = fields.Many2one(
        "res.users", string="Current Session Responsible", compute="_compute_current_session_user"
    )
        ...
        for pos_config in self:
            ...
            if session:
                ...
                pos_config.pos_session_duration = (
                    (datetime.now() - session[0].start_at).days if session[0].start_at else 0
                )
        ...
        if self.cash_control and self.payment_method_ids:
            for method in self.payment_method_ids:
                if method.is_cash_count and (
                    not method.journal_id.loss_account_id or not method.journal_id.profit_account_id
                ):
                    raise ValidationError(
    ...
    def _set_fiscal_position(self):
        for config in self:
            if (
                ...
                and (config.default_fiscal_position_id.id not in config.fiscal_position_ids.ids)
            ):
    ...
    def _force_http(self):
        enforce_https = (
            self.env["ir.config_parameter"].sudo().get_param("point_of_sale.enforce_https")
        )
