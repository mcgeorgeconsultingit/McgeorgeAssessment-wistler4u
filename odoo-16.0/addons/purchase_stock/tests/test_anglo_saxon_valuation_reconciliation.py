...
class TestValuationReconciliation(ValuationReconciliationTestCommon):
    ...
    def test_multiple_shipments_invoices(self):
        ...
        self._process_pickings(
            purchase_order.picking_ids.filtered(lambda x: x.state != "done"), quantity=3.0
        )
        picking = self.env["stock.picking"].search(
    ...
    def test_price_difference_exchange_difference_accounting_date(self):
        self.stock_account_product_categ.property_account_creditor_price_difference_categ = (
            self.company_data["default_account_stock_price_diff"]
        )
        test_product = self.test_product_delivery
        ...
        self.assertTrue(
            valuation_line.full_reconcile_id, "The reconciliation should be total at that point."
        )
