...
class ViewCustom(models.Model):
    ...
    ref_id = fields.Many2one(
        "ir.ui.view", string="Original View", index=True, required=True, ondelete="cascade"
    )
    ...
    def _auto_init(self):
        ...
        tools.create_index(
            self._cr, "ir_ui_view_custom_user_id_ref_id", self._table, ["user_id", "ref_id"]
        )
        return res
...
def get_view_arch_from_file(filepath, xmlid):
    ...
    for node in document.xpath(xpath):
        if node.tag == "record":
            ...
            if field_arch is not None:
                ...
                inner = "".join(
                    etree.tostring(child, encoding="unicode") for child in field_arch.iterchildren()
                )
    ...
    _logger.warning(
        "Could not find view arch definition in file '%s' for xmlid '%s'", filepath, xmlid
    )
    return None
...
class View(models.Model):
    ...
    arch_db = fields.Text(
        string="Arch Blob", translate=xml_translate, help="This field stores the view arch."
    )
    arch_fs = fields.Char(
    ...
    xml_id = fields.Char(
        string="External ID", compute="_compute_xml_id", help="ID of the view defined in xml file"
    )
...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def _compute_model_data_id(self):
        ...
        for data in (
            self.env["ir.model.data"].sudo().search_read(domain, ["res_id"], order="id desc")
        ):
    ...
    def _valid_inheritance(self, arch):
        ...
        for node in arch.xpath("//*[@position]"):
            ...
            if node.tag == "xpath":
                ...
                if match:
                    message = (
                        "View inheritance may not use attribute %r as a selector." % match.group(1)
                    )
        ...
        for view in self:
            ...
            try:
                ...
                for view_arch in combined_archs:
                    ...
                    if not check:
                        view_name = (
                            ("%s (%s)" % (view.name, view.xml_id)) if view.xml_id else view.name
                        )
    ...
    def _auto_init(self):
        ...
        tools.create_index(
            self._cr, "ir_ui_view_model_type_inherit_id", self._table, ["model", "inherit_id"]
        )
        return res
    ...
    def create(self, vals_list):
        ...
        self.clear_caches()
        result = super(View, self.with_context(ir_ui_view_partial_validation=True)).create(
            vals_list
        )
    ...
    def _get_view_refs(self, node):
        ...
        if not node.get("context"):
            return {}
        return {
            m.group("view_type"): m.group("view_id") for m in ref_re.finditer(node.get("context"))
        }
    ...
    def _get_x2many_missing_view_archs(self, field, field_node, node_info):
        ...
        if not any(
            ...
            missing_view_types.append(
                field_node.get("mode", "kanban" if node_info.get("mobile") else "tree").split(",")[
                    0
                ]
            )
        ...
        if node.get("name"):
            ...
            if field_info:
                transfer_field_to_modifiers(
                    field_info, node_info["modifiers"], node_info["view_modifiers_from_model"]
                )
                ...
                for child in e.iterchildren(etree.Element, etree.ProcessingInstruction):
                    if child.get("data-oe-xpath"):
                        ...
                        self.distribute_branding(
                            child, distributed_branding, parent_xpath=node_path, index_map=indexes
                        )
    ...
    def is_node_branded(self, node):
        ...
        """
        return any(
            (attr in ("data-oe-model", "groups") or (attr.startswith("t-"))) for attr in node.attrib
        ) or (
    ...
    def _validate_module_views(self, module):
        ...
        names = tuple(
            xmlid[prefix_len:] for xmlid in self.pool.loaded_xmlids if xmlid.startswith(prefix)
        )
        if not names:
            return
...
class ResetViewArchWizard(models.TransientModel):
    ...
    arch_diff = fields.Html(
        string="Architecture Diff", readonly=True, compute="_compute_arch_diff", sanitize_tags=False
    )
...
class Model(models.AbstractModel):
    ...
    def _get_default_calendar_view(self):
        ...
        set_first_of(["user_id", "partner_id", "x_user_id", "x_partner_id"], self._fields, "color")

        if not set_first_of(
            ["date_stop", "date_end", "x_date_stop", "x_date_end"], self._fields, "date_stop"
        ):
            if not set_first_of(
    ...
    def get_views(self, views, options=None):
        ...
        result["views"] = {
            v_type: self.get_view(v_id, v_type if v_type != "list" else "tree", **options)
    ...
    def _get_view_fields(self, view_type, models):
        ...
        if view_type in ("kanban", "tree", "form"):
            ...
            models[self._name].union(
                fname for fname, field in self._fields.items() if field.type in ("integer", "float")
            )
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        result = self.get_views([(view_id, view_type)], {"toolbar": toolbar, "submenu": submenu})[
            "views"
        ][view_type]
        node = etree.fromstring(result["arch"])
...
class NameManager:
    ...
    def _get_node_groups(self, node):
        return tuple(
            tuple(n.get("groups").split(",")) for n in node.xpath("ancestor-or-self::*[@groups]")
        )
        ...
        for name, groups_uses in self.mandatory_fields.items():
            ...
            def combinate(groups):
                ...
                if not groups:
                    return []
                positive = [
                    (group,) for group in groups[0] if not group or not group.startswith("!")
                ]
                ...
                if len(groups) > 1:
                    combinations = [
                        group + tuple(c) for c in combinate(groups[1:]) for group in combinations
                    ]
                return [
                    set(combination)
                    for combination in set(tuple(combination) for combination in combinations)
                ]
