...
class StockPickingType(models.Model):
    ...
    batch_group_by_destination = fields.Boolean(
        "Destination Country", help="Automatically group batches by destination country."
    )
    ...
    batch_group_by_dest_loc = fields.Boolean(
        "Destination Location", help="Automatically group batches by their destination location."
    )
...
class StockPicking(models.Model):
    ...
    def _find_auto_batch(self):
        ...
        possible_batches = (
            self.env["stock.picking.batch"].sudo().search(self._get_possible_batches_domain())
        )
    ...
    def _is_auto_batchable(self, picking=None):
        ...
        if self.picking_type_id.batch_max_lines:
            res = res and (
                len(self.move_ids) + len(picking.move_ids) <= self.picking_type_id.batch_max_lines
            )
    ...
    def _get_possible_pickings_domain(self):
        ...
        if self.picking_type_id.batch_group_by_destination:
            domain = expression.AND(
                [domain, [("partner_id.country_id", "=", self.partner_id.country_id.id)]]
            )
    ...
    def _get_possible_batches_domain(self):
        ...
        domain = [
            (
                "state",
                "in",
                ("draft", "in_progress") if self.picking_type_id.batch_auto_confirm else ("draft",),
            ),
            ("picking_type_id", "=", self.picking_type_id.id),
            ("company_id", "=", self.company_id.id if self.company_id else False),
        ]
        ...
        if self.picking_type_id.batch_group_by_destination:
            domain = expression.AND(
                [
                    domain,
                    [("picking_ids.partner_id.country_id", "=", self.partner_id.country_id.id)],
                ]
            )
        if self.picking_type_id.batch_group_by_src_loc:
            domain = expression.AND(
        ...
        if self.picking_type_id.batch_group_by_dest_loc:
            domain = expression.AND(
                [domain, [("picking_ids.location_dest_id", "=", self.location_dest_id.id)]]
            )
