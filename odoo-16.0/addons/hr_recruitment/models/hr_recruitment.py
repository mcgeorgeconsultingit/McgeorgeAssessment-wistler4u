...
AVAILABLE_PRIORITIES = [("0", "Normal"), ("1", "Good"), ("2", "Very Good"), ("3", "Excellent")]
...
class Applicant(models.Model):
    ...
    date_closed = fields.Datetime(
        "Hire Date", compute="_compute_date_closed", store=True, readonly=False, tracking=True
    )
    ...
    emp_id = fields.Many2one(
        "hr.employee", string="Employee", help="Employee linked to the applicant.", copy=False
    )
    ...
    attachment_ids = fields.One2many(
        "ir.attachment", "res_id", domain=[("res_model", "=", "hr.applicant")], string="Attachments"
    )
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        return [
            recipient for recipient in recipients if interviewer_group not in recipient["groups"]
        ]
    ...
    def name_get(self):
        if self.env.context.get("show_partner_name"):
            return [(applicant.id, applicant.partner_name or applicant.name) for applicant in self]
...
class ApplicantRefuseReason(models.Model):
    ...
    template_id = fields.Many2one(
        "mail.template", string="Email Template", domain="[('model', '=', 'hr.applicant')]"
    )
