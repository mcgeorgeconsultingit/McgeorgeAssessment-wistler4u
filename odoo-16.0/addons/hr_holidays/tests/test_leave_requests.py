...
class TestLeaveRequests(TestHrHolidaysCommon):
    def _check_holidays_status(self, holiday_status, ml, lt, rl, vrl):
        self.assertEqual(holiday_status.max_leaves, ml, "hr_holidays: wrong type days computation")
        self.assertEqual(
            holiday_status.leaves_taken, lt, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
            holiday_status.remaining_leaves, rl, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
            holiday_status.virtual_remaining_leaves, vrl, "hr_holidays: wrong type days computation"
        )
    ...
    def test_several_allocations_split(self):
        ...
        for unit in ["hour", "day"]:
            ...
            self.assertEqual(
                allocation_days[self.employee_emp_id][self.holidays_type_2][allocation_4days][
                    "leaves_taken"
                ],
            ...
            self.assertEqual(
                allocation_days[self.employee_emp_id][self.holidays_type_2][allocation_1day][
                    "leaves_taken"
                ],
