...
class AccountPaymentTerm(models.Model):
    ...
    def _default_line_ids(self):
        return [
            Command.create({"value": "balance", "value_amount": 0.0, "days": 0, "end_month": False})
        ]
    ...
    def _compute_terms(
        ...
        for line in self.line_ids.sorted(lambda line: line.value == "balance"):
            ...
            if line.value == "fixed":
                ...
                foreign_proportion = (
                    tax_amount_currency / untaxed_amount_currency if untaxed_amount_currency else 1
                )
                line_tax_amount = (
                    company_currency.round(line.value_amount * company_proportion) * sign
                )
            ...
            if line.discount_percentage:
                if company.early_pay_discount_computation in ("excluded", "mixed"):
                    ...
                    term_vals["discount_amount_currency"] = currency.round(
                        term_vals["foreign_amount"]
                        - line_untaxed_amount_currency * line.discount_percentage / 100.0
                    )
                else:
                    term_vals["discount_balance"] = company_currency.round(
            ...
            self.env["ir.property"].sudo().search(
                [
                    (
                        "value_reference",
                        "in",
                        ["account.payment.term,%s" % payment_term.id for payment_term in terms],
                    )
                ]
            ).unlink()
...
class AccountPaymentTermLine(models.Model):
    ...
    value_amount = fields.Float(
        string="Value", digits="Payment Terms", help="For percent enter a ratio between 0-100."
    )
    ...
    end_month = fields.Boolean(
        string="End of month", help="Switch to end of the month after having added months or days"
    )
    ...
    discount_days = fields.Integer(
        string="Discount Days", help="Number of days before the early payment proposition expires"
    )
    ...
    def _check_percent(self):
        for term_line in self:
            ...
            if term_line.discount_percentage and (
                term_line.discount_percentage < 0.0 or term_line.discount_percentage > 100.0
            ):
                raise ValidationError(
                    _("Discount percentages on the Payment Terms lines must be between 0 and 100.")
                )
    ...
    def _check_positive(self):
        for term_line in self:
            if term_line.months < 0 or term_line.days < 0:
                raise ValidationError(
                    _("The Months and Days of the Payment Terms lines must be positive.")
                )
