...
class GoogleService(models.AbstractModel):
    ...
    def _get_authorize_uri(
        self, service, scope, redirect_uri, state=None, approval_prompt=None, access_type=None
    ):
    ...
    def _do_request(
        ...
        _logger.debug(
            "Uri: %s - Type : %s - Headers: %s - Params : %s !", uri, method, headers, _log_params
        )
        ...
        try:
            if method.upper() in ("GET", "DELETE"):
                ...
                raise Exception(
                    _("Method not supported [%s] not in [GET, POST, PUT, PATCH or DELETE]!")
                    % (method)
                )
