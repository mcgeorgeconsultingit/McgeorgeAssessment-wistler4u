...
class WebsiteBlog(http.Controller):
    ...
    def _prepare_blog_values(
        ...
        if active_tag_ids:
            ...
            if fixed_tag_slug != tags:
                ...
                new_url = path.replace(
                    "/tag/%s" % tags, fixed_tag_slug and "/tag/%s" % fixed_tag_slug or "", 1
                )
        ...
        tag_category = tools.lazy(
            lambda: sorted(
                all_tags.mapped("category_id"), key=lambda category: category.name.upper()
            )
        )
        other_tags = tools.lazy(
            lambda: sorted(
                all_tags.filtered(lambda x: not x.category_id), key=lambda tag: tag.name.upper()
            )
        )
        ...
        return {
            ...
            "domain": domain,
            "state_info": state
            and {"state": state, "published": published_count, "unpublished": unpublished_count},
            "blogs": blogs,
            "blog": blog,
    ...
    def blog(self, blog=None, tag=None, page=1, search=None, **opt):
        ...
        blogs = tools.lazy(
            lambda: Blog.search(request.website.website_domain(), order="create_date asc, id asc")
        )
    ...
    def blog_feed(self, blog, limit="15", **kwargs):
        ...
        v["posts"] = request.env["blog.post"].search(
            [("blog_id", "=", blog.id)], limit=min(int(limit), 50), order="post_date DESC"
        )
        v["html2plaintext"] = html2plaintext
        r = request.render(
            "website_blog.blog_feed", v, headers=[("Content-Type", "application/atom+xml")]
        )
        return r
    ...
    def old_blog_post(self, blog, blog_post, tag_id=None, page=1, enable_editor=None, **post):
        ...
        return request.redirect(
            _build_url_w_params("/blog/%s/%s" % (slug(blog), slug(blog_post)), request.params),
            code=301,
        )
    ...
    def blog_post(self, blog, blog_post, tag_id=None, page=1, enable_editor=None, **post):
        ...
        next_post_id = (
            all_post_ids[(current_blog_post_index + 1) % nb_posts] if nb_posts > 1 else None
        )
