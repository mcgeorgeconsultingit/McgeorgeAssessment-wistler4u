...
class TestUnbuild(TestMrpCommon):
    ...
    def test_production_links_with_non_tracked_lots(self):
        ...
        self.assertEqual(
            sum(ml.mapped("qty_done")), 12.0, "Should have consumed 12 for the first lot"
        )
    ...
    def test_unbuild_with_routes(self):
        ...
        self.assertEqual(
            picking.location_id.id, unbuild_location.id, "Wrong source location in picking"
        )
        self.assertEqual(
    ...
    def test_unbuild_similar_tracked_components(self):
        ...
        self.assertEqual(
            uo.produce_line_ids.filtered(lambda sm: sm.product_id == compo).lot_ids, lot01 + lot02
        )
    ...
    def test_use_unbuilt_sn_in_mo(self):
        ...
        product_1_sn = self.env["stock.lot"].create(
            {"name": "sn1", "product_id": product_1.id, "company_id": self.env.company.id}
        )
