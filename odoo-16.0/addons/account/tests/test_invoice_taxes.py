...
class TestInvoiceTaxes(AccountTestInvoicingCommon):
        ...
        cls.group_tax = cls.env["account.tax"].create(
            {
                ...
                "amount_type": "group",
                "amount": 21,
                "children_tax_ids": [(4, cls.percent_tax_1_incl.id), (4, cls.percent_tax_2.id)],
                "sequence": 40,
            }
        )
    ...
    def test_tax_repartition(self):
        ...
        self.assertEqual(
            inv_base_line.tax_tag_ids, inv_base_tag, "Base line should have received base tag"
        )
        inv_tax_lines = invoice.line_ids.filtered(
            lambda x: x.tax_repartition_line_id.repartition_type == "tax"
        )
        self.assertEqual(
            len(inv_tax_lines), 2, "There should be two tax lines, one for each repartition line."
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ref_base_line.tax_tag_ids, ref_base_tag, "Base line should have received base tag"
        )
        ...
        self.assertEqual(
            abs(ref_tax_lines.filtered(lambda x: x.account_id == ref_base_line.account_id).balance),
            4.2,
    ...
    def test_tax_calculation_multi_currency(self):
        ...
        self.assertRecordValues(
            invoice.line_ids.filtered(lambda l: l.account_id.account_type == "asset_receivable"),
            [
                {
                    "balance": 686.54,
                }
