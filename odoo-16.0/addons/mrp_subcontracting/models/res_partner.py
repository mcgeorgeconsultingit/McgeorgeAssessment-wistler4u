...
class ResPartner(models.Model):
    ...
    def _compute_production_ids(self):
        results = self.env["mrp.production"].read_group(
            [("subcontractor_id.commercial_partner_id", "in", self.commercial_partner_id.ids)],
            ["ids:array_agg(id)"],
            ["subcontractor_id"],
        )
        for partner in self:
    ...
    def _search_is_subcontractor(self, operator, value):
        ...
        subcontractor_ids = (
            self.env["mrp.bom"].search([("type", "=", "subcontract")]).subcontractor_ids.ids
        )
    ...
    def _compute_is_subcontractor(self):
        ...
        for partner in self:
            partner.is_subcontractor = partner.user_has_groups("base.group_portal") and partner.env[
                "mrp.bom"
            ].search_count(
                [
                    ("type", "=", "subcontract"),
