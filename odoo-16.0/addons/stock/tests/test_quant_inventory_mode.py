...
class TestEditableQuant(TransactionCase):
    ...
    def test_create_quant_1(self):
        ...
        self.Quant.create(
            {"product_id": self.product.id, "location_id": self.stock.id, "inventory_quantity": 24}
    ...
    def test_edit_quant_3(self):
        ...
        quant = self.Quant.create(
            {"product_id": self.product.id, "location_id": self.room1.id, "quantity": 12}
        )
    ...
    def test_edit_quant_5(self):
        ...
        form_wizard = Form(
            self.env["stock.inventory.adjustment.name"].with_context(default_quant_ids=quant.ids)
        )
    ...
    def test_revert_inventory_adjustment(self):
        ...
        self.assertEqual(
            len(move_lines), 1, "One inventory adjustment move lines should have been created"
        )
        self.assertEqual(
            self.product.qty_available, 100, "Before revert inventory adjustment qty is 100"
        )
        move_lines.action_revert_inventory()
        self.assertEqual(
            self.product.qty_available, 0, "After revert inventory adjustment qty is not zero"
        )
    ...
    def test_multi_revert_inventory_adjustment(self):
        ...
        self.assertEqual(
            self.product.qty_available, 150, "Before revert multi inventory adjustment qty is 150"
        )
        self.assertEqual(
            len(move_lines), 2, "Two inventory adjustment move lines should have been created"
        )
        move_lines.action_revert_inventory()
        self.assertEqual(
            self.product.qty_available, 0, "After revert multi inventory adjustment qty is not zero"
        )
