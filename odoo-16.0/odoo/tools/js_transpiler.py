...
def convert_export_function_default(content):
    ...
    """
    repl = (
        r"""\g<space>__exports[Symbol.for("default")] = \g<identifier>; \g<type> \g<identifier>"""
    )
...
def convert_object_export(content):
    ...
    def repl(matchobj):
        object_process = (
            "{" + ", ".join([convert_as(val) for val in matchobj["object"][1:-1].split(",")]) + "}"
        )
...
{matchobj['space']}const {new_object} = {matchobj['default_export']}"""
        new_object = (
            f"""{{ [Symbol.for("default")]: {matchobj['default_export']},{new_object[1:]}"""
        )
        return f"{matchobj['space']}const {new_object} = require({matchobj['path']})"
