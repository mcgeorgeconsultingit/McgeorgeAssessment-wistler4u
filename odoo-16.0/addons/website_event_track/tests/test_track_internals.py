...
class TestTrackData(TestEventOnlineCommon):
    ...
    def test_track_partner_sync(self):
        ...
        self.assertIn(
            customer.name, new_track.partner_biography, "Low-level test: ensure correctly updated"
        )
        ...
        self.assertEqual(
            new_track.partner_email, customer.email, "Track customer should take over empty value"
        )
        self.assertEqual(
            new_track.partner_phone,
            test_phone,
...
class TestTrackSuggestions(TestEventOnlineCommon):
    def test_track_suggestion(self):
        ...
        with patch.object(
            WebsiteVisitor, "_get_visitor_from_request", lambda *args, **kwargs: emp_visitor
            ...
            self.assertEqual(
                track_suggestion, track_3, "Returned track should be the manually wishlisted one"
            )
            ...
            track_suggestion = current_track._get_track_suggestions(limit=1)
            self.assertEqual(
                track_suggestion, track_5, "Returned track should be the default wishlisted one"
            )
            ...
            track_suggestion = current_track._get_track_suggestions(limit=1)
            self.assertEqual(
                track_suggestion, track_4, "Returned track should the one with the most common tags"
            )
            ...
            track_suggestion = current_track._get_track_suggestions(limit=1)
            self.assertEqual(
                track_suggestion, track_6, "Returned track should the one with matching location"
            )
