...
class AssetsBundleMultiWebsite(AssetsBundle):
    ...
    def _get_assets_domain_for_already_processed_css(self, assets):
        res = super(AssetsBundleMultiWebsite, self)._get_assets_domain_for_already_processed_css(
            assets
        )
...
class IrQWeb(models.AbstractModel):
    ...
    def _prepare_frontend_environment(self, values):
        ...
        translatable = (
            editable
            and irQweb.env.context.get("lang") != irQweb.env["ir.http"]._get_default_lang().code
        )
    ...
    def _post_processing_att(self, tagName, atts):
        ...
        if isinstance(atts.get("style"), str) and "background-image" in atts["style"]:
            ...
            atts["style"] = re_background_image.sub(
                lambda m: "%s%s" % (m.group(1), website.get_cdn_url(m.group(2))), atts["style"]
            )
