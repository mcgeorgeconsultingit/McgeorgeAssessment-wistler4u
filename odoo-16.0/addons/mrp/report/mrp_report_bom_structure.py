...
class ReportBomStructure(models.AbstractModel):
        ...
        if bom_data["availability_state"] != "unavailable":
            if same_delay:
                ...
                res["leftover_date"] = format_date(
                    self.env, date_today + timedelta(days=bom_data["availability_delay"])
                )
    ...
    def _get_bom_data(
        ...
        if bom_line:
            current_quantity = (
                bom_line.product_uom_id._compute_quantity(line_qty, bom.product_uom_id) or 0
            )
            ...
            if product:
                ...
                prod_cost = (
                    bom.product_tmpl_id.uom_id._compute_price(
                        bom.product_tmpl_id.with_company(company).standard_price, bom.product_uom_id
                    )
                    * current_quantity
                )
                attachment_ids = (
        ...
        bom_report_line = {
            ...
            "link_id": (
                product.id if product.product_variant_count > 1 else product.product_tmpl_id.id
            )
        ...
        if not is_minimized:
            operations = self._get_operation_line(
                product,
                bom,
                float_round(current_quantity, precision_rounding=1, rounding_method="UP"),
                level + 1,
                index,
            )
        ...
        for component_index, line in enumerate(bom.bom_line_ids):
            ...
            if line.child_bom_id:
                component = self.with_context(parent_product_id=product.id)._get_bom_data(
                    line.child_bom_id,
                    warehouse,
                    line.product_id,
                    line_quantity,
                ...
                component = self.with_context(parent_product_id=product.id)._get_component_data(
                    bom,
                    warehouse,
                    line,
                    line_quantity,
        ...
        if not is_minimized:
            byproducts, byproduct_cost_portion = self._get_byproducts_lines(
                product, bom, current_quantity, level + 1, bom_report_line["bom_cost"], index
            )
    ...
    def _get_quantities_info(self, product, bom_uom, parent_bom, product_info):
        return {
            ...
            "on_hand_qty": product.uom_id._compute_quantity(product.qty_available, bom_uom)
            if product.detailed_type == "product"
            else False,
            "stock_loc": "in_stock",
        }
