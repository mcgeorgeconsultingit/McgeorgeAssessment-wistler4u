...
class StockMove(models.Model):
        ...
        for move in self:
            ...
            if forced_quantity:
                svl_vals["description"] = (
                    "Correction of %s (modification of past move)" % move.picking_id.name
                    or move.name
                )
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        valued_moves = {
            valued_type: self.env["stock.move"] for valued_type in self._get_valued_types()
        }
        for move in self:
    ...
    def product_price_update_before_done(self, forced_qty=None):
        ...
        for move in self.filtered(
            ...
            if float_is_zero(product_tot_qty_available, precision_rounding=rounding):
                ...
                new_std_price = (
                    (amount_unit * product_tot_qty_available) + (move._get_price_unit() * qty)
    ...
    def _prepare_analytic_lines(self):
        ...
        if self.state != "done":
            ...
            unit_amount = -sum(
                sum(vals["unit_amount"] for vals in lists) for lists in analytic_line_vals
            )
    ...
    def _get_partner_id_for_valuation_lines(self):
        return (
            ...
            and self.env["res.partner"]._find_accounting_partner(self.picking_id.partner_id).id
        ) or False
    ...
    def _account_analytic_entry_move(self):
        ...
        for move_id, analytic_line in zip(moves_to_link, analytic_lines):
            self.env["stock.move"].browse(move_id).analytic_account_line_id = analytic_line
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        company_from = (
            self._is_out() and self.mapped("move_line_ids.location_id.company_id") or False
        )
        company_to = (
            self._is_in() and self.mapped("move_line_ids.location_dest_id.company_id") or False
        )
