...
class MrpBom(models.Model):
    ...
    @api.constrains(
        "product_id", "product_tmpl_id", "bom_line_ids", "byproduct_ids", "operation_ids"
    )
    def _check_bom_lines(self):
        for bom in self:
            ...
            for byproduct in bom.byproduct_ids:
                ...
                if same_product:
                    raise ValidationError(
                        _("By-product %s should not be the same as BoM product.") % bom.display_name
                    )
    ...
    def onchange_product_tmpl_id(self):
        if self.product_tmpl_id:
            ...
            if (
                ...
                self.code = _(
                    "%s (new) %s", self.product_tmpl_id.name, number_of_bom_of_this_product
                )
            else:
                self.code = False
    ...
    def name_get(self):
        return [
            (
                bom.id,
                "%s%s" % (bom.code and "%s: " % bom.code or "", bom.product_tmpl_id.display_name),
            )
            for bom in self
        ]
