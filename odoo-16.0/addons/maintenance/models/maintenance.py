...
class MaintenanceEquipment(models.Model):
    ...
    def _name_search(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        if name and operator not in expression.NEGATIVE_TERM_OPERATORS and operator != "=":
            equipment_ids = self._search(
                [("name", "=", name)] + args, limit=limit, access_rights_uid=name_get_uid
            )
    ...
    @api.depends(
        "effective_date", "period", "maintenance_ids.request_date", "maintenance_ids.close_date"
    )
        ...
        for equipment in equipments:
            ...
            if next_maintenance_todo and last_maintenance_done:
                ...
                if (
                    ...
                    if (
                        last_maintenance_done.close_date + timedelta(days=equipment.period)
                        < date_now
                    ):
                        next_date = date_now
                    else:
    ...
    def write(self, vals):
        if vals.get("owner_user_id"):
            self.message_subscribe(
                partner_ids=self.env["res.users"].browse(vals["owner_user_id"]).partner_id.ids
            )
...
class MaintenanceRequest(models.Model):
    ...
    priority = fields.Selection(
        [("0", "Very Low"), ("1", "Low"), ("2", "Normal"), ("3", "High")], string="Priority"
    )
    color = fields.Integer("Color Index")
        ...
        for request in self.filtered(lambda request: request.schedule_date):
            ...
            if not updated:
                if request.equipment_id:
                    note = _("Request planned for %s", request.equipment_id._get_html_link())
                else:
                    note = False
...
class MaintenanceTeam(models.Model):
    ...
    todo_request_ids = fields.One2many(
        "maintenance.request", string="Requests", copy=False, compute="_compute_todo_requests"
    )
    ...
    def _compute_todo_requests(self):
        for team in self:
            ...
            team.todo_request_count_high_priority = self.env["maintenance.request"].search_count(
                [("maintenance_team_id", "=", team.id), ("priority", "=", "3")]
            )
            team.todo_request_count_block = self.env["maintenance.request"].search_count(
                [("maintenance_team_id", "=", team.id), ("kanban_state", "=", "blocked")]
            )
            team.todo_request_count_unscheduled = self.env["maintenance.request"].search_count(
                [("maintenance_team_id", "=", team.id), ("schedule_date", "=", False)]
            )
