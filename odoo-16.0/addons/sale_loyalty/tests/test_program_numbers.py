...
class TestSaleCouponProgramNumbers(TestSaleCouponNumbersCommon):
    def test_program_numbers_free_and_paid_product_qty(self):
        ...
        self.assertEqual(
            order.amount_total, 864, "Only paid product should have their price discounted"
        )
    ...
    def test_program_numbers_taxes_and_rules(self):
        ...
        self.assertEqual(
            order.amount_total, 2994.0, "The discount should be limited to $200 tax included"
        )
        self.assertEqual(
    ...
    def test_program_numbers_one_discount_line_per_tax(self):
        ...
        self.assertAlmostEqual(
            order.amount_total, 1901.11, 2, "The order total with programs should be 1901.11"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            order.amount_untaxed, 1594.95, "The order untaxed total with programs should be 1594.95"
        )
        self.assertEqual(
            len(order.order_line.ids),
            8,
        ...
        self.assertEqual(
            order.amount_untaxed, 1435.46, "The order untaxed total with programs should be 1435.46"
        )
        self.assertEqual(
            len(order.order_line.ids),
            12,
        ...
        self.assertEqual(
            order.amount_untaxed, 1363.46, "The order untaxed total with programs should be 1363.45"
        )
        self.assertEqual(
            len(order.order_line.ids),
            13,
        ...
        self.assertAlmostEqual(
            order.amount_total, 1358.2, 2, "The order total with programs should be 1358.20"
        )
        self.assertEqual(
            order.amount_untaxed, 1118, "The order untaxed total with programs should be 1118.00"
        )
        self.assertEqual(
            len(order.order_line.ids),
            10,
    ...
    def test_program_fixed_price(self):
        ...
        self.assertEqual(
            len(order.order_line.ids), 2, "There should be the product line and the reward line"
        )
    ...
    def test_program_next_order(self):
        ...
        self.assertEqual(
            len(order._get_reward_coupons()), 0, "No coupon should have been generated yet"
        )
        ...
        self.assertEqual(
            generated_coupon.points, 0, "The coupon should not have it's points already."
        )
    ...
    def test_coupon_and_program_discount_fixed_amount(self):
        ...
        self.assertEqual(
            order.amount_total, 65.0, "The promotion program should be correctly applied"
        )
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            order.amount_total,
        ...
        self._apply_promo_code(order, coupon.code)
        self.assertEqual(
            order.amount_total, 47.5, "The promotion program should now be correctly applied"
        )
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_excl(self):
        ...
        * Even applying the coupon in reverse order should yield same result
        ...
        self._apply_promo_code(order, "test_10pc")
        self.assertEqual(
            order.amount_total, 283.5, "The promotion program should be correctly applied"
        )
        ...
        self._apply_promo_code(order, coupon.code)
        self.assertEqual(
            order.amount_total, 26.5, "The promotion program should be correctly applied"
        )
        self._auto_rewards(order, self.all_programs)
        self._apply_promo_code(order, "test_10pc")
        self._auto_rewards(order, self.all_programs)
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_incl(self):
        ...
        * Even applying the coupon in reverse order should yield same result
        ...
        self._apply_promo_code(order, "test_10pc")
        self.assertEqual(
            order.amount_total, 270.0, "The promotion program should be correctly applied"
        )
        ...
        self._apply_promo_code(order, coupon.code)
        self.assertEqual(
            order.amount_total, 10.0, "The promotion program should be correctly applied"
        )
        self._apply_promo_code(order, "test_10pc")
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
    ...
    def test_program_discount_on_multiple_specific_products(self):
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 3, "Conference Chair + Drawer Black + 20% discount line"
        )
        # Name                 | Qty | price_unit |  Tax     |  HTVA   |   TVAC  |  TVA  |
        # --------------------------------------------------------------------------------
        ...
        self.assertEqual(
            order.amount_total, 99.50, "The 12.50 discount from the drawer black should be gone"
        )
        ...
        self.assertEqual(
            order.amount_untaxed, 78.34, "Total with taxes should be as per above comment"
        )
    ...
    def test_program_numbers_free_prod_with_min_amount_and_qty_on_same_prod(self):
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 2, "The other product should not affect the promotion"
        )
        self.assertEqual(
            order.amount_total,
            self.conferenceChair.lst_price * (sol1.product_uom_qty),
    ...
    def test_program_free_prods_with_min_qty_and_reward_qty_and_rule(self):
        ...
        self.assertEqual(
            order.amount_total, 700.0, "The price must be 500.0 since the coupon is not yet applied"
        )
    ...
    def test_fixed_amount_with_negative_cost(self):
        ...
        self.assertEqual(len(order.order_line), 3, "Promotion should add 1 line")
        self.assertEqual(
            order.amount_total, 5, "10$ discount should be applied on top of the 15$ original price"
        )
