...
class TestSafeEval(BaseCase):
    ...
    def test_02_literal_eval(self):
        ...
        self.assertEqual(
            actual, expected, "Simple python expressions are not working with literal_eval"
        )
...
class TestBase(TransactionCase):
    def _check_find_or_create(
        self, test_string, expected_name, expected_email, check_partner=False, should_create=False
    ):
        partner = self.env["res.partner"].find_or_create(test_string)
        if should_create and check_partner:
            self.assertTrue(
                partner.id > check_partner.id, "find_or_create failed - should have found existing"
            )
        elif check_partner:
            self.assertEqual(
                partner, check_partner, "find_or_create failed - should have found existing"
            )
    ...
    def test_10_res_partner_find_or_create(self):
        ...
        self._check_find_or_create(
            SAMPLES[0][0], SAMPLES[0][1], SAMPLES[0][2], check_partner=partner, should_create=False
        )
        ...
        new2 = self._check_find_or_create(
            SAMPLES[2][0], SAMPLES[2][1], SAMPLES[2][2], check_partner=new, should_create=True
        )

        new3 = self._check_find_or_create(
            SAMPLES[3][0], SAMPLES[3][1], SAMPLES[3][2], check_partner=new2, should_create=True
        )

        new4 = self._check_find_or_create(
            SAMPLES[4][0], SAMPLES[0][1], SAMPLES[0][2], check_partner=partner, should_create=False
        )

        new5 = self._check_find_or_create(
            SAMPLES[5][0], SAMPLES[5][1], SAMPLES[5][2], check_partner=new4, should_create=True
        )
    ...
    def test_15_res_partner_name_search(self):
        ...
        self.assertEqual(
            len(partners), 1, "Incorrect search number result for name_search with a limit"
        )
        self.assertEqual(
            partners[0][1],
    ...
    def test_20_res_partner_address_sync(self):
        ...
        p1 = res_partner.browse(
            res_partner.name_create("Denis Bladesmith <denis.bladesmith@ghoststep.com>")[0]
        )
        ...
        self.assertEqual(
            p1.street, p1street, "Address fields must not be synced after turning sync off"
        )
    ...
    def test_30_res_partner_first_contact_sync(self):
        ...
        self.assertEqual(
            p1.type, "contact", 'Default type must be "contact", not the copied parent type'
        )
    ...
    def test_40_res_partner_address_get(self):
        ...
        leaf10 = res_partner.create({"name": "Leaf 10", "parent_id": branch1.id, "type": "invoice"})
        ...
        leaf23 = res_partner.create({"name": "Leaf 23", "parent_id": branch2.id, "type": "contact"})
        ...
        self.assertEqual(
            ...
            {"delivery": leaf21.id, "invoice": leaf22.id, "contact": leaf22.id, "other": leaf22.id},
            "Invalid address resolution, should scan commercial entity ancestor and its descendants",
        )
        self.assertEqual(
            leaf23.address_get(["delivery", "invoice", "contact", "other"]),
            {"delivery": leaf21.id, "invoice": leaf23.id, "contact": leaf23.id, "other": leaf23.id},
    ...
    def test_commercial_partner_nullcompany(self):
        ...
        self.assertEqual(
            p0.commercial_partner_id, p0, "partner without a parent is their own commercial partner"
        )
        ...
        self.env.cr.execute(
            "update res_partner set is_company=null where id = any(%s)", [notcompanies.ids]
        )
    ...
    def test_50_res_partner_commercial_sync(self):
        res_partner = self.env["res.partner"]
        p0 = res_partner.create({"name": "Sigurd Sunknife", "email": "ssunknife@gmail.com"})
        sunhelm = res_partner.create(
            {
                ...
                "child_ids": [
                    Command.link(p0.id),
                    Command.create({"name": "Alrik Greenthorn", "email": "agr@sunhelm.com"}),
                ],
            }
        )
        p1 = res_partner.create(
        ...
        sunhelm.write(
            {
                "child_ids": [
                    Command.create({"name": "Ulrik Greenthorn", "email": "ugr@sunhelm.com"})
                ]
            }
        )
        ...
        for p in (p0, p1, p11, p2, p3):
            self.assertEqual(
                p.commercial_partner_id, sunhelm, "Incorrect commercial entity resolution"
            )
        ...
        for p in (p0, p1, p11, p2, p3):
            self.assertEqual(
                p.vat, sunhelmvat, "Commercial fields must be automatically and recursively synced"
            )
        ...
        p1.write({"parent_id": sunhelm.id, "is_company": True, "name": "Sunhelm Subsidiary"})
        self.assertEqual(
            p1.vat, p1vat, "Setting is_company should stop auto-sync of commercial fields"
        )
        self.assertEqual(
    ...
    def test_60_read_group(self):
        ...
        groups_data = res_users.read_group(
            domain, fields=["login"], groupby=["login"], orderby="login DESC", limit=3, offset=3
        )
        self.assertEqual(
            len(groups_data), 3, "Incorrect number of results when grouping on a field with limit"
        )
        self.assertEqual(
            [g["login"] for g in groups_data], ["bob", "alice2", "alice"], "Result mismatch"
        )
        ...
        for group_data in groups_data:
            ...
            self.assertEqual(
                group_data["color"], 3, "Incorrect sum for aggregated data for the column 'color'"
            )
        ...
        self.assertEqual(
            [g["color"] for g in groups_data], [-3, 0, 1, 2, 3, 6], "Incorrect ordering of the list"
        )

        # multi group, second level is int field, should still be summed in first level grouping
        groups_data = res_users.read_group(
            domain, fields=["name", "color"], groupby=["name", "color"], orderby="name DESC"
        )
        self.assertEqual(
            [g["name"] for g in groups_data],
            ["Nab", "Eve", "Bob", "Alice"],
        ...
        self.assertEqual(
            [g["color"] for g in groups_data], [3, 3, 2, 1], "Incorrect ordering of the list"
        )
        ...
        self.assertEqual(
            [g["name_count"] for g in groups_data], [1, 2, 1, 2], "Incorrect number of results"
        )
        ...
        self.assertEqual(
            [g["date_count"] for g in groups_data], [1, 1, 1, 3], "Incorrect number of results"
        )
        ...
        self.assertEqual(
            [g["date_count"] for g in groups_data], [2, 4], "Incorrect number of results"
        )

        # group on inherited date column (res_partner.date) -> Year-Month, custom order
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "date"], groupby=["date"], orderby="date DESC"
        )
        self.assertEqual(
            len(groups_data), 4, "Incorrect number of results when grouping on a field"
        )
        ...
        self.assertEqual(
            [g["date_count"] for g in groups_data], [3, 1, 1, 1], "Incorrect number of results"
        )
        ...
        self.assertEqual(
            [g["title_count"] for g in groups_data], [4, 2], "Incorrect number of results"
        )
        self.assertEqual(
            [g["color"] for g in groups_data], [10, -1], "Incorrect aggregation of int column"
        )

        # group on inherited many2one (res_partner.title), reversed natural order
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "title"], groupby=["title"], orderby="title desc"
        )
        self.assertEqual(
            len(groups_data), 2, "Incorrect number of results when grouping on a field"
        )
        ...
        self.assertEqual(
            [g["title_count"] for g in groups_data], [2, 4], "Incorrect number of results"
        )
        self.assertEqual(
            [g["color"] for g in groups_data], [-1, 10], "Incorrect aggregation of int column"
        )
        ...
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "title"], groupby=["title"], orderby="color"
        )
        self.assertEqual(
            len(groups_data), 2, "Incorrect number of results when grouping on a field"
        )
    ...
    def test_61_private_read_group(self):
        ...
        self.assertEqual(
            ["bob", "alice2", "alice"], [g["login"] for g in groups_data], "Result mismatch"
        )
...
class TestParentStore(TransactionCase):
    ...
    def test_duplicate_parent(self):
        ...
        self.assertEqual(
            len(new_struct), 4, "After duplication, the new object must have the childs records"
        )
...
class TestGroups(TransactionCase):
    def test_res_groups_fullname_search(self):
        ...
        groups = all_groups.search(
            [("full_name", "in", ["Administration / Access Rights", "Contact Creation"])]
        )
        self.assertTrue(
            groups,
