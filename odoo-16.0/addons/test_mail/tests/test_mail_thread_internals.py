...
class TestChatterTweaks(TestMailCommon, TestRecipients):
    ...
    def test_post_no_subscribe_author(self):
        ...
        ).message_post(body="Test Body", message_type="comment", subtype_xmlid="mail.mt_comment")
        self.assertEqual(
    ...
    def test_chatter_mail_notrack(self):
        ...
        self.flush_tracking()
        self.assertEqual(
            len(rec.message_ids), 1, "No new message should have been posted with mail_notrack key"
        )

        rec.with_context({"mail_notrack": False}).write({"user_id": self.user_employee.id})
        self.flush_tracking()
        self.assertEqual(len(rec.message_ids), 2, "A tracking message should have been posted")
        self.assertEqual(
    ...
    def test_chatter_tracking_disable(self):
        ...
        self.assertEqual(
            rec.sudo().mapped("message_ids.tracking_value_ids"), self.env["mail.tracking.value"]
        )
...
class TestDiscuss(TestMailCommon, TestRecipients):
    ...
    def test_mark_all_as_read(self):
        ...
        with patch.object(
            MailTestSimple, "check_access_rights", autospec=True, side_effect=_employee_crash
        ):
            ...
            self.assertEqual(
                needaction_accessible, 1, "a new message to a partner is readable to that partner"
            )
    ...
    def test_set_message_done_user(self):
        with self.assertSinglePostNotifications(
            [{"partner": self.partner_employee, "type": "inbox"}], message_info={"content": "Test"}
        ):
        ...
        self.assertMailNotifications(
            message,
            [{"notif": [{"partner": self.partner_employee, "type": "inbox", "is_read": True}]}],
        )
...
class TestMultiCompany(HttpCase):
    def test_redirect_to_records(self):
        ...
        response = self.url_open(
            ...
            % (self.multi_company_record._name, self.multi_company_record.id),
            timeout=15,
        )
