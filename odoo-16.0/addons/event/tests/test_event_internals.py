                ...
                "event_type_mail_ids": [
                    (
                        ...
                        {  # right at subscription
                            ...
                            % cls.env["ir.model.data"]._xmlid_to_res_id("event.event_subscription"),
                        },
                    ),
                    (
                        0,
                        ...
                        {  # 1 days before event
                            ...
                            "template_ref": "mail.template,%i"
                            % cls.env["ir.model.data"]._xmlid_to_res_id("event.event_reminder"),
                        },
                    ),
                ],
            }
...
class TestEventData(TestEventInternalsCommon):
    ...
    def test_event_date_computation(self):
        ...
        with freeze_time(self.reference_now):
            event.write(
                {
                    "registration_ids": [
                        (0, 0, {"partner_id": self.event_customer.id, "name": "test_reg"})
                    ],
    ...
    def test_event_configuration_mails_from_type(self):
        ...
        self.assertEqual(
            set(mail.mapped("mail_registration_ids.registration_id.id")), set([registration.id])
        )
        # start test scenario
    ...
    def test_event_mail_default_config(self):
        ...
        self.assertEqual(
            event.event_mail_ids[0].template_ref, self.env.ref("event.event_subscription")
        )
