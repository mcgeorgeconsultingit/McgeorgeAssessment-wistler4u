...
class MailRenderMixin(models.AbstractModel):
    ...
    def _valid_field_parameter(self, field, name):
        ...
        return name in ["render_engine", "render_options"] or super()._valid_field_parameter(
            field, name
        )
    ...
    def _render_encapsulate(self, layout_xmlid, html, add_context=None, context_record=None):
        template_ctx = {
            ...
            "model_description": self.env["ir.model"]._get(context_record._name).display_name
            if context_record
            else False,
        ...
        if not html:
            _logger.warning(
                "QWeb template %s not found when rendering encapsulation template." % (layout_xmlid)
            )
    ...
    def _render_lang(self, res_ids, engine="inline_template"):
        ...
        if not isinstance(res_ids, (list, tuple)):
            raise ValueError(
                _("Template rendering for language should be called with a list of IDs.")
            )
