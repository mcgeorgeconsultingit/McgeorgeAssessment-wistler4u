...
class TestProcRule(TransactionCase):
    ...
    def test_propagate_deadline_move(self):
        ...
        self.assertEqual(
            move_dest.date_deadline, new_deadline, msg="deadline date should be propagated"
        )
        move_orig._action_done()
        self.assertAlmostEqual(
            move_orig.date, datetime.now(), delta=timedelta(seconds=10), msg="date should be now"
        )
        self.assertEqual(
            move_orig.date_deadline, new_deadline, msg="deadline date should be unchanged"
        )
        self.assertEqual(
            move_dest.date_deadline, new_deadline, msg="deadline date should be unchanged"
        )
    ...
    def test_orderpoint_replenishment_view_1(self):
        ...
        product = self.env["product.product"].create(
            {"name": "Super Product", "type": "product", "route_ids": [route_2.id, route_3.id]}
        )
    ...
    def test_orderpoint_compute_warehouse_location(self):
        ...
        warehouse_b = self.env["stock.warehouse"].create({"name": "Test Warehouse", "code": "TWH"})
...
class TestProcRuleLoad(TransactionCase):
    ...
    def test_orderpoint_1(self):
        ...
        """
        warehouse = self.env["stock.warehouse"].create({"name": "Test Warehouse", "code": "TWH"})
