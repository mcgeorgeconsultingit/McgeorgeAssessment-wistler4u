...
class TestVariantsSearch(ProductVariantsCommon):
    def test_attribute_line_search(self):
        ...
        self.assertIn(
            self.product_template_shirt, search_attribute, "Shirt should be found searching Size"
        )

        search_value = self.env["product.template"].search([("attribute_line_ids", "=", "L")])
        self.assertIn(
            self.product_template_shirt, search_value, "Shirt should be found searching L"
        )
...
class TestVariants(ProductVariantsCommon):
    def test_variants_is_product_variant(self):
        ...
        self.assertFalse(template.is_product_variant, "Product template is not a variant")
        self.assertEqual(
            {True}, set(v.is_product_variant for v in variants), "Product variants are variants"
        )
    ...
    def test_standard_price(self):
        ...
        self.assertEqual(
            one_variant_product.with_company(company_b).cost_currency_id, company_b.currency_id
        )
        ...
        self.assertEqual(500.0, one_variant_template.with_company(company_b).standard_price)
    ...
    def test_archive_variant(self):
        ...
        self.assertEqual(len(template.product_variant_ids), 1)
        self.assertEqual(len(template.with_context(active_test=False).product_variant_ids), 2)
        variant_1.toggle_active()
...
class TestVariantsArchive(ProductVariantsCommon):
    ...
    def test_dynamic_attributes_archiving(self):
        ...
        # Reset archiving for the next assert
        template.write({"attribute_line_ids": [(3, template.attribute_line_ids[1].id, 0)]})
        self.assertTrue(product_white.active)
            ...
            Command.create(
                {
                    ...
                    "value_ids": [
                        Command.set([cls.color_attribute_red.id, cls.color_attribute_blue.id])
                    ],
                }
            ),
            Command.create(
    ...
    def _get_archived_variants(self):
        ...
        return (
            ...
            .search([("active", "=", False), ("product_tmpl_id", "=", self.template.id)])
        )
                    ...
                    Command.create(
                        {
                            "attribute_id": self.size_attribute.id,
                            "value_ids": [
                                (6, 0, (self.size_attribute_s + self.size_attribute_m).ids)
                            ],
                        }
                    )
                ],
...
class TestVariantsExclusion(ProductAttributesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env["product.template.attribute.line"].create(
            [
                ...
                },
                {
                    ...
                    "value_ids": [
                        (6, 0, [cls.storage_attr_value_128.id, cls.storage_attr_value_256.id])
                    ],
                },
            ]
        )
        ...
        def get_ptav(template, ptav):
            ...
            ).product_template_value_ids.filtered(lambda v: v.product_attribute_value_id == ptav)
    ...
    def test_variants_1_exclusion(self):
        ...
        # Delete exclusion
        self.smartphone_s.write({"exclude_for": [(2, self.smartphone_s.exclude_for.id, 0)]})
        self.assertEqual(
            len(self.smartphone.product_variant_ids),
            4,
    ...
    def test_variants_2_exclusions_different_lines(self):
        ...
        self.smartphone_s.write({"exclude_for": [(2, self.smartphone_s.exclude_for.ids[0], 0)]})
        self.assertEqual(
            len(self.smartphone.product_variant_ids),
            3,
