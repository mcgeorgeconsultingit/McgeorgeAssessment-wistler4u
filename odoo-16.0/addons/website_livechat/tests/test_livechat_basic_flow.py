...
class TestLivechatBasicFlowHttpCase(tests.HttpCase, TestLivechatCommon):
    def test_channel_created_on_user_interaction(self):
        ...
        channel = self.env["mail.channel"].search(
            [["livechat_active", "=", True], ["livechat_visitor_id", "=", self.visitor.id]]
        )
    ...
    def test_livechat_username(self):
        ...
        channel_1 = self.env["mail.channel"].search(
            [("livechat_visitor_id", "=", self.visitor.id), ("livechat_active", "=", True)], limit=1
        )

        # Check Channel naming
        self.assertEqual(
            channel_1.name, "%s %s" % (self.visitor.display_name, self.operator.livechat_username)
        )
        channel_1.unlink()
    ...
    def _common_basic_flow(self):
        ...
        self.assertEqual(
            self.visitor.display_name, "%s #%s" % (_("Website Visitor"), self.visitor.id)
        )
        self.assertEqual(
