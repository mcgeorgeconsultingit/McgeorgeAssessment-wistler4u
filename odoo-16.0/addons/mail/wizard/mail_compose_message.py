...
class MailComposer(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if result.get("composition_mode") == "comment" and (
            set(fields) & set(["model", "res_id", "partner_ids", "record_name", "subject"])
        ):
    ...
    def _process_recipient_values(self, mail_values_dict):
        ...
        for record_id, mail_values in mail_values_dict.items():
            ...
            recipients_info[record_id] = {
                "mail_to": mail_to,
                "mail_to_normalized": [
                    tools.email_normalize(mail) for mail in mail_to if tools.email_normalize(mail)
                ],
            }
    ...
    def _process_state(self, mail_values_dict):
        ...
        for record_id, mail_values in mail_values_dict.items():
            ...
            mail_to_normalized = (
                recipients["mail_to_normalized"][0] if recipients["mail_to_normalized"] else ""
            )
        ...
        if self.composition_mode == "mass_mail" and issubclass(
            ...
            if blacklist:
                ...
                blacklisted_rec_ids.update(
                    target["id"] for target in targets if target["email_normalized"] in blacklist
                )
