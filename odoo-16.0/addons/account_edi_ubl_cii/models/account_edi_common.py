...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _get_tax_unece_codes(self, invoice, tax):
        ...
        def create_dict(
            tax_category_code=None, tax_exemption_reason_code=None, tax_exemption_reason=None
        ):
            return {
        ...
        european_economic_area = self.env.ref("base.europe").country_ids.mapped("code") + [
            "NO",
            "IS",
            "LI",
        ]
        ...
        if customer.country_id.code == "NO":
            if tax.amount == 25:
                return create_dict(
                    tax_category_code="S", tax_exemption_reason=_("Output VAT, regular rate")
                )
            if tax.amount == 15:
                return create_dict(
                    tax_category_code="S",
            ...
            if tax.amount == 12:
                return create_dict(
                    tax_category_code="S", tax_exemption_reason=_("Output VAT, reduced rate, low")
                )
    ...
    def _import_invoice(self, journal, filename, tree, existing_invoice=None):
        ...
        if invoice:
            if logs:
                ...
                body = _(
                    "<strong>Format used to import the invoice: %s</strong>", str(self._description)
                )
        ...
        for allow_el in allowance_charge_nodes:
            ...
            if base_amount_node is not None:
                ...
                percent_node = allow_el.find(
                    "./{*}MultiplierFactorNumeric" if is_ubl else "./{*}CalculationPercent"
                )
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line, qty_factor):
        ...
        if (
            net_price_unit == 0
            and price_subtotal != net_price_unit * (billed_qty / basis_qty) - allow_charge_amount
        ):
