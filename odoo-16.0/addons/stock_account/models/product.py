    ...
    def write(self, vals):
        ...
        if "categ_id" in vals:
            ...
            for product_template in self:
                ...
                out_svl_vals_list, products_orig_quantity_svl, products = Product._svl_empty_stock(
...
class ProductProduct(models.Model):
    ...
    def _compute_value_svl(self):
        ...
        self.env["product.product"].browse([group["product_id"][0] for group in groups]).sudo(
            False
    ...
    def action_revaluation(self):
        self.ensure_one()
        ctx = dict(
            self._context, default_product_id=self.id, default_company_id=self.env.company.id
        )
        return {
        ...
        for candidate in candidates:
            ...
            if float_is_zero(qty_to_take_on_candidates, precision_rounding=self.uom_id.rounding):
                if float_is_zero(candidate.remaining_qty, precision_rounding=self.uom_id.rounding):
                    ...
                    new_standard_price = (
                        next_candidates and next_candidates[0].unit_cost or new_standard_price
                    )
                break
    ...
    def _run_fifo_vacuum(self, company=None):
        ...
        for svl_to_vacuum in svls_to_vacuum:
            ...
            vals = {
                ...
                "description": "Revaluation of %s (negative inventory)" % move.picking_id.name
                or move.name,
    ...
    def _svl_empty_stock(self, description, product_category=None, product_template=None):
        ...
        for product in impacted_products:
            ...
            if float_is_zero(product.quantity_svl, precision_rounding=product.uom_id.rounding):
                ...
                continue
            if (
                float_compare(product.quantity_svl, 0, precision_rounding=product.uom_id.rounding)
                > 0
            ):
        ...
        if "property_cost_method" in vals or "property_valuation" in vals:
            ...
            for product_category in self:
                ...
                if (
                    ...
                    for stock_property in property_stock_fields:
                        vals[stock_property] = (
                            vals.get(stock_property, False) or company_id[stock_property]
                        )
