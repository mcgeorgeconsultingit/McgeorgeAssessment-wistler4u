...
class TestOnchangeProductId(TransactionCase):
    ...
    def test_onchange_product_id(self):
        ...
        tax_include_id = self.tax_model.create(
            dict(name="Include tax", amount="21.00", price_include=True, type_tax_use="purchase")
        )
        ...
        fp_tax_id = self.fiscal_position_tax_model.create(
            dict(position_id=fp_id.id, tax_src_id=tax_include_id.id, tax_dest_id=tax_exclude_id.id)
        )
        po_vals = {
            "partner_id": partner_id.id,
