...
class OdooEdiProxyAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        ...
        message = "%s|%s|%s|%s|%s" % (
            ...
            self.id_client,
            json.dumps(
                werkzeug.urls.url_decode(parsed_url.query), sort_keys=True
            ),  # url query params sorted by key
            json.dumps(body, sort_keys=True),
        )  # http request body
        h = hmac.new(
            base64.b64decode(self.refresh_token), message.encode(), digestmod=hashlib.sha256
        )
