    ...
    def _populate_factories(self):
        ...
        address_factories_groups = [
            ...
            [  # BE, 2 records
                (
                    "street",
                    populate.iterate(["Rue des Bourlottes {counter}", "Rue Pinckaers {counter}"]),
                ),
        ...
        def _compute_contact_name(values=None, counter=0, **kwargs):
            ...
            mn = kwargs["random"].choices(
                [False] + tools._p_middlename_groups,
                weights=[1]
                + [1 / (len(tools._p_middlename_groups) or 1)] * len(tools._p_middlename_groups),
            )[0]
        ...
        return [
            (
                "partner_id",
                populate.iterate(
                    [False] + partner_ids, [2] + [1 / (len(partner_ids) or 1)] * len(partner_ids)
                ),
            ),
