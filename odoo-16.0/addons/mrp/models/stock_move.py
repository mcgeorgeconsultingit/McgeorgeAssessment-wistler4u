...
class StockMoveLine(models.Model):
    ...
    def _reservation_is_updatable(self, quantity, reserved_quant):
        ...
        if self.produce_line_ids.lot_id:
            ...
            if (
                float_compare(
                    ml_remaining_qty, quantity, precision_rounding=self.product_id.uom_id.rounding
                )
                < 0
            ):
                return False
    ...
    def write(self, vals):
        for move_line in self:
            production = (
                move_line.move_id.production_id or move_line.move_id.raw_material_production_id
            )
            if (
                production
                and move_line.state == "done"
                and any(field in vals for field in ("lot_id", "location_id", "qty_done"))
            ):
...
class StockMove(models.Model):
    ...
    product_qty_available = fields.Float(
        "Product On Hand Quantity", related="product_id.qty_available", depends=["product_id"]
    )
    ...
    def _compute_manual_consumption(self):
        for move in self:
            ...
            move.manual_consumption = (
                not move.raw_material_production_id.use_auto_consume_components_lots
                and (move.bom_line_id.manual_consumption or move.has_tracking != "none")
            )
    ...
    def _onchange_quantity_done(self):
        if (
            ...
            and not self.manual_consumption
            and self.should_consume_qty != self.quantity_done
        ):
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get("default_raw_material_production_id") or self.env.context.get(
    ...
    def action_explode(self):
        ...
        for move in self:
            ...
            bom = (
                self.env["mrp.bom"]
                .sudo()
                ._bom_find(move.product_id, company_id=move.company_id.id, bom_type="phantom")[
                    move.product_id
                ]
            )
            if not bom:
            ...
            if move.picking_id.immediate_transfer or float_is_zero(
                ...
                factor = (
                    move.product_uom._compute_quantity(move.quantity_done, bom.product_uom_id)
                    / bom.product_qty
                )
            else:
                factor = (
                    move.product_uom._compute_quantity(move.product_uom_qty, bom.product_uom_id)
                    / bom.product_qty
                )
    ...
    def _generate_move_phantom(self, bom_line, product_qty, quantity_done):
        ...
        if bom_line.product_id.type in ["product", "consu"]:
            vals = self.copy_data(
                default=self._prepare_phantom_move_values(bom_line, product_qty, quantity_done)
            )
    ...
    def _compute_kit_quantities(self, product_id, kit_qty, kit_bom, filters):
        ...
        for bom_line, bom_line_data in bom_sub_lines:
            ...
            if bom_line.product_id.type == "service":
                continue
            if float_is_zero(
                bom_line_data["qty"], precision_rounding=bom_line.product_uom_id.rounding
            ):
