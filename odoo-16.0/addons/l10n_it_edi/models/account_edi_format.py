...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_check_ordinary_invoice_configuration(self, invoice):
        ...
        if is_self_invoice and self._l10n_it_edi_services_or_goods(invoice) == "both":
            errors.append(
                _("Cannot apply Reverse Charge to a bill which contains both services and goods.")
            )
    ...
    def _l10n_it_edi_check_taxes_configuration(self, invoice):
        ...
        for invoice_line in invoice.invoice_line_ids.filtered(lambda x: not x.display_type):
            if len(invoice_line.tax_ids) != 1:
                errors.append(
                    _("In line %s, you must select one and only one tax.", invoice_line.name)
                )
        return errors
    ...
    def _l10n_it_edi_is_simplified(self, invoice):
        ...
        return all(
            [
                ...
                or (buyer.vat and (buyer.vat[:2].upper() == "IT" or buyer.vat[:2].isdecimal())),
    ...
    def _cron_receive_fattura_pa(self):
        ...
        proxy_users = self.env["account_edi_proxy_client.user"].search(
        ...
        for proxy_user in proxy_users:
            ...
            for id_transaction, fattura in res.items():
                ...
                invoice.unlink()
                invoice = self.env.ref("l10n_it_edi.edi_fatturaPA")._create_invoice_from_xml_tree(
                    fattura["filename"], tree
                )
            ...
            if elements:
                ...
                partner = elements and self.env["res.partner"].search(
                    AND(
                        [
                            OR(domains),
                            OR([[("company_id", "=", company.id)], [("company_id", "=", False)]]),
                        ]
                    ),
                    limit=1,
                )
    ...
    def _l10n_it_edi_search_tax_for_import(self, company, percentage, extra_domain=None):
        ...
        taxes = (
            ...
            .filtered(
                lambda tax: all(
                    [rep_line.factor_percent >= 0 for rep_line in tax.invoice_repartition_line_ids]
                )
            )
        )
        ...
        for body_tree in tree.xpath("//FatturaElettronicaBody"):
            ...
            if not company:
                ...
                if not company:
                    ...
                    _logger.warning(
                        "No company found with VAT or Codice Fiscale like %r.", elements[0].text
                    )
                    continue
            ...
            if not move_type:
                ...
                _logger.info(
                    "Document type not managed: %s. Invoice type is set by default.", document_type
                )

            # Setup the context for the Invoice Form
            invoice_ctx = invoice.with_company(company).with_context(default_move_type=move_type)
                ...
                if discount_elements:
                    ...
                    for discount_element in discount_elements:
                        ...
                        if discount_percentage:
                            discounted_amount *= (
                                1 - discount_sign * float(discount_percentage[0].text) / 100
                            )
            ...
            if elements:
                for element in elements:
                    ...
                    new_invoice.with_context(
                        no_new_invoice=True, default_res_id=new_invoice.id
                    ).message_post(
                        body=(_("Attachment from XML")), attachment_ids=[attachment_64.id]
                    )
            ...
            for element_code in elements_code:
                ...
                if partner:
                    product_supplier = self.env["product.supplierinfo"].search(
                        [("partner_id", "=", partner.id), ("product_code", "=", code.text)], limit=2
                    )
                    if (
                        product_supplier
    ...
    def _l10n_it_edi_export_invoice_as_xml(self, invoice):
        ...
        if not self._l10n_it_is_simplified_document_type(template_values["document_type"]):
            ...
            content = self.env["ir.qweb"]._render(
                "l10n_it_edi.account_invoice_it_simplified_FatturaPA_export", template_values
            )
            invoice.message_post(
                body=_(
    ...
    def _l10n_it_post_invoices_step_1(self, invoices):
        ...
        if proxy_user._get_demo_state() == "demo":
            responses = {
                i["data"]["filename"]: {"id_transaction": "demo"} for i in to_send.values()
            }
        else:
            try:
                ...
                return {
                    invoice: {"error": e.message, "blocking_level": "error"} for invoice in invoices
                }
    ...
    def _l10n_it_post_invoices_step_2(self, invoices):
        ...
        if proxy_user._get_demo_state() == "demo":
            ...
            return {
                invoice: {"attachment": invoice.l10n_it_edi_attachment_id} for invoice in invoices
            }
        else:
            try:
                responses = proxy_user._make_request(
                    proxy_user._get_server_url() + "/api/l10n_it_edi/1/in/TrasmissioneFatture",
        ...
        for id_transaction, response in responses.items():
            ...
            if state == "awaiting_outcome":
                ...
                to_return[invoice] = {
                    "error": _("You are not allowed to check the status of this invoice."),
                    "blocking_level": "error",
                }
                continue
                ...
                if "00404" in error_codes:
                    ...
                    to_return[invoice] = {
                        "error": self._format_error_message(
                            _("The invoice has been refused by the Exchange System"), errors
                        ),
                        "blocking_level": "error",
                    }
    ...
    def _get_proxy_identification(self, company):
        ...
        if not company.l10n_it_codice_fiscale:
            raise UserError(
                _("Please fill your codice fiscale to be able to receive invoices from FatturaPA")
            )
