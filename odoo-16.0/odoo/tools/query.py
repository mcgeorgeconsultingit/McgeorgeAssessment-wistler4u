...
class Query(object):
    ...
    def add_table(self, alias, table=None):
        ...
        assert alias not in self._tables and alias not in self._joins, "Alias %r already in %s" % (
            alias,
            str(self),
        )
        self._tables[alias] = table or alias
    ...
    def join(self, lhs_alias, lhs_column, rhs_table, rhs_column, link, extra=None, extra_params=()):
        ...
        """
        return self._join(
            "JOIN", lhs_alias, lhs_column, rhs_table, rhs_column, link, extra, extra_params
        )

    def left_join(
        self, lhs_alias, lhs_column, rhs_table, rhs_column, link, extra=None, extra_params=()
    ):
        ...
        """
        return self._join(
            "LEFT JOIN", lhs_alias, lhs_column, rhs_table, rhs_column, link, extra, extra_params
        )

    def _join(
        self, kind, lhs_alias, lhs_column, rhs_table, rhs_column, link, extra=None, extra_params=()
    ):
        assert lhs_alias in self._tables or lhs_alias in self._joins, "Alias %r not in %s" % (
            lhs_alias,
            str(self),
        )
    ...
    @property
    def tables(self):
        warnings.warn("deprecated Query.tables, use Query.get_sql() instead", DeprecationWarning)
    ...
    def add_join(self, connection, implicit=True, outer=False, extra=None, extra_params=()):
        ...
        rhs_alias = self._join(
            kind, lhs_alias, lhs_column, rhs_table, rhs_column, link, extra, extra_params
        )
