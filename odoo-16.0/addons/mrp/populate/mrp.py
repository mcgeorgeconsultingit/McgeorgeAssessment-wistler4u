...
class MrpBomLine(models.Model):
    ...
    def _populate_factories(self):
        ...
        def get_product_id(values, counter, random):
            ...
            last_product_bom = (
                bom.product_id if bom.product_id else bom.product_tmpl_id.product_variant_ids[-1]
            )
...
class MrpWorkcenter(models.Model):
    ...
    def _populate(self, size):
        ...
        for workcenter in res:
            nb_alternative = max(
                random.randint(0, 3), len(workcenters_by_company[workcenter.company_id.id]) - 1
            )
            if nb_alternative > 0:
    ...
    def _populate_factories(self):
        company_ids = self.env.registry.populated_models["res.company"][:COMPANY_NB_WITH_STOCK]
        resource_calendar_no_company = self.env.ref("resource.resource_calendar_std").copy(
            {"company_id": False}
        )
