...
class TestPortalAttachment(AccountTestInvoicingHttpCommon):
    ...
    def test_01_portal_attachment(self):
        ...
        res_binary = self.url_open(
            "/web/content/%d?access_token=%s" % (create_res["id"], create_res["access_token"])
        )
        ...
        res_image = self.url_open(
            "/web/image/%d?access_token=%s" % (create_res["id"], create_res["access_token"])
        )
        ...
        self.assertEqual(res.status_code, 200)
        self.assertIn(
            "You are not allowed to access 'Journal Entry' (account.move) records.", res.text
        )
