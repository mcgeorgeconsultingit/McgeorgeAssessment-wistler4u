...
class Slide(models.Model):
    ...
    source_type = fields.Selection(
        [("local_file", "Upload from Device"), ("external", "Retrieve from Google Drive")],
        default="local_file",
        required=True,
    )
    # generic
    ...
    slide_resource_ids = fields.One2many(
        "slide.slide.resource", "slide_id", string="Additional Resource for this slide", copy=True
    )
    slide_resource_downloadable = fields.Boolean(
        "Allow Download", default=False, help="Allow the user to download the content of the slide."
    )
    # google
    ...
    def _compute_like_info(self):
        ...
        rg_data_like = (
            ...
            .sudo()
            .read_group(
                [("slide_id", "in", self.ids), ("vote", "=", 1)], ["slide_id"], ["slide_id"]
            )
        )
        rg_data_dislike = (
            self.env["slide.slide.partner"]
        ...
        mapped_data_like = dict(
            (rg_data["slide_id"][0], rg_data["slide_id_count"]) for rg_data in rg_data_like
        )
        mapped_data_dislike = dict(
            (rg_data["slide_id"][0], rg_data["slide_id_count"]) for rg_data in rg_data_dislike
        )
    ...
    def _compute_slide_views(self):
        ...
        read_group_res = (
            self.env["slide.slide.partner"]
            .sudo()
            ._read_group([("slide_id", "in", self.ids)], ["slide_id"], groupby=["slide_id"])
        )
    ...
    def _compute_embed_counts(self):
        ...
        if self.ids:
            ...
            mapped_data = {res["slide_id"][0]: res.get("count_views", 0) for res in read_group_res}
    ...
    @api.depends(
        "slide_partner_ids.partner_id", "slide_partner_ids.vote", "slide_partner_ids.completed"
    )
    @api.depends("uid")
    ...
    def _compute_video_source_type(self):
        for slide in self:
            ...
            youtube_match = (
                re.match(self.YOUTUBE_VIDEO_ID_REGEX, slide.video_url) if slide.video_url else False
            )
            if (
                youtube_match
            ...
            vimeo_match = (
                re.search(self.VIMEO_VIDEO_ID_REGEX, slide.video_url) if slide.video_url else False
            )
    ...
    def _compute_google_drive_id(self):
        ...
        for slide in self:
            url = (
                slide.url or slide.document_google_url or slide.image_google_url or slide.video_url
            )
    ...
    def _on_change_url(self):
        ...
        if self.url or self.document_google_url or self.image_google_url or self.video_url:
            ...
            if slide_metadata:
                self.update({key: value for key, value in slide_metadata.items() if not self[key]})
    ...
    def create(self, vals_list):
        ...
        can_publish_channel_ids = (
            self.env["slide.channel"].browse(channel_ids).filtered(lambda c: c.can_publish).ids
        )
        ...
        for slide, vals in zip(slides, vals_list):
            ...
            if (
                any(
                    vals.get(url_param)
                    for url_param in ["url", "video_url", "document_google_url", "image_google_url"]
                )
    ...
    def _post_publication(self):
        for slide in self.filtered(
            lambda slide: slide.website_published and slide.channel_id.publish_template_id
        ):
            publish_template = slide.channel_id.publish_template_id
            html_body = publish_template.with_context(base_url=slide.get_base_url())._render_field(
                "body_html", slide.ids
            )[slide.id]
    ...
    def _action_vote(self, upvote=True):
        ...
        slide_partners = SlidePartnerSudo.search(
            [("slide_id", "in", self.ids), ("partner_id", "=", self.env.user.partner_id.id)]
        )
    ...
    def _action_mark_completed(self):
        ...
        SlidePartnerSudo = self.env["slide.slide.partner"].sudo()
        existing_sudo = SlidePartnerSudo.search(
            [("slide_id", "in", uncompleted_slides.ids), ("partner_id", "=", target_partner.id)]
        )
    ...
    def action_mark_uncompleted(self):
        if any(not slide.can_self_mark_uncompleted for slide in self):
            raise UserError(
                _("You cannot mark a slide as uncompleted if you are not among its members.")
            )
    ...
    def _compute_quiz_info(self, target_partner, quiz_done=False):
        ...
        slide_partners = (
            self.env["slide.slide.partner"]
            .sudo()
            .search([("slide_id", "in", self.ids), ("partner_id", "=", target_partner.id)])
        )
