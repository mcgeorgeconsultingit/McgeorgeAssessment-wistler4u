...
class FleetVehicle(models.Model):
    ...
    license_plate = fields.Char(
        tracking=True, help="License plate number of the vehicle (i = plate number for a car)"
    )
    vin_sn = fields.Char(
        "Chassis Number",
    ...
    driver_id = fields.Many2one(
        "res.partner", "Driver", tracking=True, help="Driver address of the vehicle", copy=False
    )
    ...
    color = fields.Char(
        help="Color of the vehicle", compute="_compute_model_fields", store=True, readonly=False
    )
    ...
    fuel_type = fields.Selection(
        FUEL_TYPES, "Fuel Type", compute="_compute_model_fields", store=True, readonly=False
    )
    horsepower = fields.Integer(compute="_compute_model_fields", store=True, readonly=False)
    horsepower_tax = fields.Float(
        "Horsepower Taxation", compute="_compute_model_fields", store=True, readonly=False
    )
    ...
    contract_renewal_total = fields.Text(
        compute="_compute_contract_reminder", string="Total of contracts due or overdue minus one"
    )
    ...
    frame_type = fields.Selection(
        [("diamant", "Diamant"), ("trapez", "Trapez"), ("wave", "Wave")], string="Bike Frame Type"
    )
    ...
    def act_show_log_cost(self):
        ...
        res.update(
            context=dict(
                copy_context, default_vehicle_id=self.id, search_default_parent_false=True
            ),
            domain=[("vehicle_id", "=", self.id)],
        )
        return res
