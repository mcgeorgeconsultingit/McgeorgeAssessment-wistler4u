...
class TestMailTemplate(MailCommon):
    ...
    def test_mail_compose_message_content_from_template_mass_mode(self):
        ...
        self.assertEqual(
            values[self.partner_employee.id]["subject"], "6", "We must trust mail template values"
        )
        self.assertIn(
            "13",
    ...
    def test_mail_template_acl(self):
        ...
        mail_template = (
            self.env["mail.template"].with_user(self.user_admin).create({"name": "Test template"})
        )
...
class TestMailTemplateReset(MailCommon):
    ...
    def test_mail_template_reset(self):
        ...
        self.assertEqual(
            mail_template.attachment_ids, self.env.ref("mail.mail_template_test_attachment")
        )
