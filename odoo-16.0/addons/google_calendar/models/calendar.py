...
class Meeting(models.Model):
    ...
    def _odoo_values(self, google_event, default_reminders=()):
        ...
        if google_event.start.get("dateTime"):
            ...
            start = (
                parse(google_event.start.get("dateTime")).astimezone(pytz.utc).replace(tzinfo=None)
            )
    ...
    def _odoo_attendee_commands(self, google_event):
        ...
        for attendee in zip(emails, partners, google_attendees):
            ...
            if email in attendees_by_emails:
                ...
                attendee_commands += [
                    (1, attendees_by_emails[email].id, {"state": attendee[2].get("responseStatus")})
                ]
            else:
                ...
                if attendee[2].get("self"):
                    ...
                    continue
                attendee_commands += [
                    (0, 0, {"state": attendee[2].get("responseStatus"), "partner_id": partner.id})
                ]
    ...
    def _odoo_reminders_commands(self, reminders=()):
        ...
        for reminder in reminders:
            ...
            alarm = self.env["calendar.alarm"].search(
                [("alarm_type", "=", alarm_type), ("duration_minutes", "=", minutes)], limit=1
            )
            if alarm:
                commands += [(4, alarm.id)]
            else:
