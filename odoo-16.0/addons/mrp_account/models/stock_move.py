...
class StockMove(models.Model):
    ...
    def _filter_anglo_saxon_moves(self, product):
        ...
        res += self.filtered(
            lambda m: m.bom_line_id.bom_id.product_tmpl_id.id == product.product_tmpl_id.id
        )
        return res
    ...
    def _get_src_account(self, accounts_data):
        if not self.unbuild_id:
            ...
            return (
                self.location_dest_id.valuation_out_account_id.id or accounts_data["stock_input"].id
            )
