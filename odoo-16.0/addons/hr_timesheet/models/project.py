...
class Project(models.Model):
    ...
    allow_timesheets = fields.Boolean(
        "Timesheets", compute="_compute_allow_timesheets", store=True, readonly=False, default=True
    )
    ...
    timesheet_count = fields.Integer(
        compute="_compute_timesheet_count", groups="hr_timesheet.group_hr_timesheet_user"
    )
    ...
    remaining_hours = fields.Float(
        compute="_compute_remaining_hours", string="Remaining Invoiced Time", compute_sudo=True
    )
    ...
    def _compute_encode_uom_in_days(self):
        self.encode_uom_in_days = self.env.company.timesheet_encode_uom_id == self.env.ref(
            "uom.product_uom_day"
        )
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type in [
            "tree",
            "form",
        ] and self.env.company.timesheet_encode_uom_id == self.env.ref("uom.product_uom_day"):
    ...
    def _compute_total_timesheet_time(self):
        ...
        for project in self:
            ...
            total_time = sum(
                [
                    unit_amount
                    * uoms_dict.get(product_uom_id, project.timesheet_encode_uom_id).factor_inv
...
class Task(models.Model):
    ...
    analytic_account_active = fields.Boolean(
        "Active Analytic Account", compute="_compute_analytic_account_active", compute_sudo=True
    )
    allow_timesheets = fields.Boolean(
        "Allow timesheets",
    ...
    remaining_hours_percentage = fields.Float(
        compute="_compute_remaining_hours_percentage", search="_search_remaining_hours_percentage"
    )
