...
class EventRegistration(models.Model):
    ...
    email = fields.Char(
        string="Email", compute="_compute_email", readonly=False, store=True, tracking=11
    )
    phone = fields.Char(
        string="Phone", compute="_compute_phone", readonly=False, store=True, tracking=12
    )
    mobile = fields.Char(
        string="Mobile", compute="_compute_mobile", readonly=False, store=True, tracking=13
    )
    # organization
    date_closed = fields.Datetime(
        string="Attended Date", compute="_compute_date_closed", readonly=False, store=True
    )
    ...
    def write(self, vals):
        ...
        to_confirm = (
            self.filtered(lambda registration: registration.state in {"draft", "cancel"})
            if confirming
            else None
        )
    ...
    def _message_get_default_recipients(self):
        ...
        return {r.id: {"partner_ids": [], "email_to": r.email, "email_cc": False} for r in self}
    ...
    def get_date_range_str(self, lang_code=False):
        ...
        if diff.days <= 0:
            ...
            return _(
                "on %(date)s",
                date=format_date(
                    self.env, self.event_begin_date, lang_code=lang_code, date_format="medium"
                ),
            )
