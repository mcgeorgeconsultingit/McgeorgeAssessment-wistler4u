...
class LunchOrder(models.Model):
    ...
    available_toppings_1 = fields.Boolean(
        help="Are extras available for this product", compute="_compute_available_toppings"
    )
    ...
    def _compute_display_reorder_button(self):
        ...
        for order in self:
            order.display_reorder_button = (
                show_button and order.state == "confirmed" and order.supplier_id.available_today
            )
    ...
    def _find_matching_lines(self, values):
        ...
        return self.search(domain).filtered(
            lambda line: (line.topping_ids_1 | line.topping_ids_2 | line.topping_ids_3).ids
            == toppings
        )
