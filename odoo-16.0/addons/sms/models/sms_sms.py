...
class SmsSms(models.Model):
    ...
    state = fields.Selection(
        [("outgoing", "In Queue"), ("sent", "Sent"), ("error", "Error"), ("canceled", "Canceled")],
        "SMS Status",
        readonly=True,
        copy=False,
        default="outgoing",
    ...
    def _split_batch(self):
        batch_size = int(
            self.env["ir.config_parameter"].sudo().get_param("sms.session.batch.size", 500)
        )
    ...
    def _send(self, unlink_failed=False, unlink_sent=True, raise_exception=False):
        ...
        try:
            ...
            _logger.info(
                "Sent batch %s SMS: %s: failed with exception %s", len(self.ids), self.ids, e
            )
            if raise_exception:
                raise
