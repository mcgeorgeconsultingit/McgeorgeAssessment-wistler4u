...
class Partner(models.Model):
    ...
    def _message_get_default_recipients(self):
        return {r.id: {"partner_ids": [r.id], "email_to": False, "email_cc": False} for r in self}
    ...
    def get_mention_suggestions(self, search, limit=8, channel_id=None):
        ...
        for domain in priority_conditions:
            ...
            partners |= self.search(
                expression.AND([[("id", "not in", partners.ids)], domain]), limit=remaining_limit
            )
        ...
        if channel_id:
            member_by_partner = {
                ...
                for member in self.env["mail.channel.member"].search(
                    [("channel_id", "=", channel_id), ("partner_id", "in", partners.ids)]
                )
            }
