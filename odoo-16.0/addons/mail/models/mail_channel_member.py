...
class ChannelMember(models.Model):
    ...
    guest_id = fields.Many2one(
        string="Guest", comodel_name="mail.guest", ondelete="cascade", readonly=True, index=True
    )
    partner_email = fields.Char("Email", related="partner_id.email", readonly=False)
    # channel
    channel_id = fields.Many2one(
        "mail.channel", string="Channel", ondelete="cascade", readonly=True, required=True
    )
    # state
    ...
    def _get_as_sudo_from_request(self, request, channel_id):
        ...
        if guest:
            return (
                ...
                .sudo()
                .search([("channel_id", "=", channel_id), ("guest_id", "=", guest.id)], limit=1)
            )
    ...
    def _notify_typing(self, is_typing):
        ...
        for member in self:
            ...
            notifications.append(
                [member.channel_id, "mail.channel.member/typing_status", formatted_member]
            )
            notifications.append(
                [member.channel_id.uuid, "mail.channel.member/typing_status", formatted_member]
    ...
    def _mail_channel_member_format(self, fields=None):
        ...
        for member in self:
            ...
            if "persona" in fields:
                ...
                if member.guest_id:
                    persona = {
                        "guest": member.guest_id._guest_format(
                            fields=fields.get("persona", {}).get("guest")
                        ).get(member.guest_id)
                    }
    ...
    def _rtc_join_call(self, check_rtc_session_ids=None):
        ...
        if len(self.channel_id.rtc_session_ids) == 1 and self.channel_id.channel_type in {
            "chat",
            "group",
        }:
