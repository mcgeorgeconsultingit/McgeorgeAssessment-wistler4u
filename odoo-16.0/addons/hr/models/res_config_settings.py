...
class ResConfigSettings(models.TransientModel):
    ...
    hr_presence_control_login = fields.Boolean(
        string="Based on user status in system", config_parameter="hr.hr_presence_control_login"
    )
    ...
    hr_presence_control_ip = fields.Boolean(
        string="Based on IP Address", config_parameter="hr_presence.hr_presence_control_ip"
    )
        ...
        for settings in self:
            if settings.module_hr_presence and not (
                ...
                raise ValidationError(
                    _("You should select at least one Advanced Presence Control option.")
                )
