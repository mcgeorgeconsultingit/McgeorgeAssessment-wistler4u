...
class TestMailRender(TestMailRenderCommon):
        ...
        for src, expected in zip(srces, results):
            for engine in ["inline_template"]:
                result = MailRenderMixin.with_context(**custom_ctx)._render_template(
                    src, partner._name, partner.ids, engine=engine, add_context=add_context
                )[partner.id]
    ...
    def test_render_field(self):
        ...
        for fname, expected in zip(["subject", "body_html"], self.base_rendered):
            rendered = template._render_field(fname, partner.ids, compute_lang=True)[partner.id]
...
class TestMailRenderSecurity(TestMailRenderCommon):
    ...
    def test_render_inline_template_impersonate(self):
        ...
        result = (
            ...
            .sudo()
            ._render_template_inline_template(src, partner._name, partner.ids)[partner.id]
        )
...
<p>return value</p>"""
        ...
        result = (
            ...
            ._render_template_inline_template(src, partner._name, partner.ids, add_context=context)[
                partner.id
            ]
        )
        self.assertEqual(expected, result)
