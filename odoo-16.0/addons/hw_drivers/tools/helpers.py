                ...
                if db_branch != local_branch:
                    with writable():
                        subprocess.check_call(
                            ["rm", "-rf", "/home/pi/odoo/addons/hw_drivers/iot_handlers/drivers/*"]
                        )
                        subprocess.check_call(
                            [
                                "rm",
...
def check_image():
    ...
    if valueActual == valueLastest:
        return False
    version = (
        checkFile.get(valueLastest, "Error").replace("iotboxv", "").replace(".zip", "").split("_")
    )
...
def get_ssid():
    ...
    if not ap:
        return (
            subprocess.check_output(["grep", "-oP", "(?<=ssid=).*", "/etc/hostapd/hostapd.conf"])
            .decode("utf-8")
            .rstrip()
        )
    ...
    return (
        subprocess.check_output(["sed", 's/.*"\\(.*\\)"/\\1/'], stdin=process_grep.stdout)
        .decode("utf-8")
        .rstrip()
    )
...
def get_wifi_essid():
    ...
    process_iwlist = subprocess.Popen(
        ["sudo", "iwlist", "wlan0", "scan"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    process_grep = subprocess.Popen(
...
def download_iot_handlers(auto=True):
    ...
    if server:
        ...
        try:
            resp = pm.request(
                "POST", server, fields={"mac": get_mac_address(), "auto": auto}, timeout=8
            )
            if resp.data:
                with writable():
