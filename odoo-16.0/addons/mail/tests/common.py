...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def mock_mail_gateway(self, mail_unlink_sent=False):
        ...
        with self.mock_smtplib_connection(), patch.object(
            ...
            MailMail, "create", autospec=True, wraps=MailMail, side_effect=_mail_mail_create
        ), patch.object(
            MailMail, "unlink", autospec=True, wraps=MailMail, side_effect=_mail_mail_unlink
        ):
    ...
    def gateway_mail_reply_wrecord(
        self, template, record, use_in_reply_to=True, target_model=None, target_field=None
    ):
    ...
    def gateway_mail_reply_wemail(
        self, template, email_to, use_in_reply_to=True, target_model=None, target_field=None
    ):
        """Simulate a reply through the mail gateway. Usage: giving a record,
        find an email sent to them and use its message-ID to simulate a reply.
    ...
    def assertMailMailWId(self, mail_id, status, author=None, content=None, fields_values=None):
    ...
    def assertSentEmail(self, author, recipients, **values):
        ...
        sent_mails = [
            mail
            for mail in self._mails
            if set(mail["email_to"]) == set(expected["email_to"])
            and mail["email_from"] == expected["email_from"]
        ]
        if len(sent_mails) > 1 and values.get("subject"):
            # try to better filter
            sent_mail = next(
                (mail for mail in sent_mails if mail["subject"] == values["subject"]), False
            )
        else:
        ...
        if "attachments_info" in expected:
            ...
            for attachment_info in expected["attachments_info"]:
                attachment = next(
                    attach for attach in attachments if attach[0] == attachment_info["name"]
                )
...
class MailCase(MockEmail):
    ...
    def mock_mail_app(self):
        ...
        with patch.object(
            Message, "create", autospec=True, wraps=Message, side_effect=_mail_message_create
    ...
    def _add_messages(cls, record, body_content, count=1, author=None, **kwargs):
        ...
        create_vals = [
            dict(values, body="%s/%02d" % (body_content, counter)) for counter in range(count)
        ]
    ...
    def assertMailNotifications(self, messages, recipients_info):
        ...
        for message_info in recipients_info:
            ...
            email_values = {"body_content": mbody, "references_content": message.message_id}
    ...
    def assertBusNotifications(self, channels, message_items=None, check_unique=True):
        ...
        self.assertEqual(
            set(bus_notifs.mapped("channel")), set([json_dump(channel) for channel in channels])
        )
    ...
    def assertNotified(self, message, recipients_info, is_complete=False):
        ...
        for rinfo in recipients_info:
            recipient_notif = next(
                (notif for notif in notifications if notif.res_partner_id == rinfo["partner"]),
                False,
            )
...
class MailCommon(common.TransactionCase, MailCase):
    ...
    def _activate_multi_lang(
        cls, lang_code="es_ES", layout_arch_db=None, test_record=False, test_template=False
    ):
...
<body>
    ...
    <p>Sent by <t t-esc="company.name"/></p>
        ...
        cls.env["ir.model.data"].create(
            {"model": "ir.ui.view", "module": "mail", "name": "test_layout", "res_id": view.id}
        )
