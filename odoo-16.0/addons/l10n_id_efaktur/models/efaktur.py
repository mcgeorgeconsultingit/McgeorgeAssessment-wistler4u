...
class Efaktur(models.Model):
        ...
        for record in self:
            ...
            if record.min[:-8] != record.max[:-8]:
                raise ValidationError(
                    _("First 5 digits should be same in Start Number and End Number.")
                )

            if int(record.min[-8:]) > int(record.max[-8:]):
                raise ValidationError(
                    _(
                        "Last 8 digits of End Number should be greater than the last 8 digit of Start Number"
                    )
                )

            if (int(record.max) - int(record.min)) > 10000:
                raise ValidationError(
                    _("The difference between the two numbers must not be greater than 10.000")
                )
    ...
    def _compute_default(self):
        for record in self:
            ...
            max_available = int(
                self.env["l10n_id_efaktur.efaktur.range"]
                .search([("company_id", "=", record.company_id.id)], order="max DESC", limit=1)
                .max
            )
