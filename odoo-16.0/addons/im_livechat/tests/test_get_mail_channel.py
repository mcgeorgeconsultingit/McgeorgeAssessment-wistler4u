...
class TestGetMailChannel(TransactionCase):
    def setUp(self):
        ...
        self.patch(
            type(self.env["im_livechat.channel"]), "_get_available_users", get_available_users
        )
    ...
    def test_channel_get_livechat_visitor_info(self):
        ...
        channel_info = self.livechat_channel.with_user(public_user)._open_livechat_mail_channel(
        ...
        channel_info = self.livechat_channel.with_user(test_user)._open_livechat_mail_channel(
        ...
        operator = self.operators[0]
        channel_info = self.livechat_channel.with_user(operator)._open_livechat_mail_channel(
            anonymous_name="whatever",
            previous_operator_id=operator.partner_id.id,
            user_id=operator.id,
        )
    ...
    def test_channel_not_pinned_for_operator_before_first_message(self):
        ...
        self.assertFalse(
            operator_channel_member.is_pinned, "channel should not be pinned for operator initially"
        )
    ...
    def test_operator_livechat_username(self):
        ...
        self.assertFalse(
            message_formats[0].get("email_from"), "should not send email_from to livechat user"
        )
    ...
    def test_read_channel_unpined_for_operator_after_one_day(self):
        ...
        self.assertFalse(
            member_of_operator.is_pinned, "read channel should be unpinned after one day"
        )
    ...
    def test_unread_channel_not_unpined_for_operator_after_autovacuum(self):
        ...
        with freeze_time(fields.Datetime.to_string(fields.datetime.now() + timedelta(days=1))):
            member_of_operator._gc_unpin_livechat_sessions()
        self.assertTrue(
            member_of_operator.is_pinned, "unread channel should not be unpinned after autovacuum"
        )
