...
class TestMrpByProduct(common.TransactionCase):
    ...
    def test_00_mrp_byproduct(self):
        ...
        self.assertEqual(
            mnf_product_a.state, "confirmed", "Production order should be in state confirmed"
        )
        ...
        self.assertEqual(
            len(mnf_product_a.move_raw_ids), 1, "Wrong consume move on production order."
        )
        ...
        self.assertEqual(
            by_product_move.product_uom_qty, 2, "Wrong produced quantity of sub product."
        )
