...
class SaleOrder(models.Model):
    ...
    @api.depends(
        "company_id.project_time_mode_id", "timesheet_ids", "company_id.timesheet_encode_uom_id"
    )
    def _compute_timesheet_total_duration(self):
        ...
        group_data = (
            self.env["account.analytic.line"]
            .sudo()
            ._read_group([("order_id", "in", self.ids)], ["order_id", "unit_amount"], ["order_id"])
        )
    ...
    def action_view_timesheet(self):
        ...
        action["context"] = {"search_default_billable_timesheet": True}  # erase default filters
        if self.order_line:
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        moves._link_timesheets_to_invoice(
            self.env.context.get("timesheet_start_date"), self.env.context.get("timesheet_end_date")
        )
        return moves
...
class SaleOrderLine(models.Model):
    ...
    remaining_hours = fields.Float(
        "Remaining Hours on SO", compute="_compute_remaining_hours", compute_sudo=True, store=True
    )
        ...
        if with_remaining_hours:
            ...
            for line in self:
                ...
                if line.remaining_hours_available:
                    ...
                    name = "{name}{remaining_time}".format(name=name, remaining_time=remaining_time)
