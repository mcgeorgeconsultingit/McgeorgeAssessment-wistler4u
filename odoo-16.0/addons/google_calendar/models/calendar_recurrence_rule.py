...
class RecurrenceRule(models.Model):
    ...
    def _apply_recurrence(
        self, specific_values_creation=None, no_send_edit=False, generic_values_creation=None
    ):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        for odoo_attendee_email in set(existing_attendees.mapped("email")):
            ...
            if email_normalize(odoo_attendee_email) not in emails:
                ...
                self.calendar_event_ids.write(
                    {
                        "need_sync": False,
                        "partner_ids": [Command.unlink(att.partner_id.id) for att in attendees],
                    }
                )
        ...
        if old_event_values and any(
            new_event_values[key] != old_event_values[key] for key in base_event_time_fields
        ):
            ...
            non_equal_values = [
                (
                    key,
                    old_event_values[key] and old_event_values[key].strftime("%m/%d/%Y, %H:%M:%S"),
                    "-->",
    ...
    def _create_from_google(self, gevents, vals_list):
        ...
        recurrence = super(RecurrenceRule, self.with_context(dont_notify=True))._create_from_google(
            gevents, vals_list
        )
