...
class TestHttp(http.Controller):
    ...
    def wsgi_environ(self):
        ...
        return request.make_response(json.dumps(environ, indent=4), headers=list(CT_JSON.items()))
    ...
    @http.route(
        "/test_http/echo-json-context", type="json", auth="user", methods=["POST"], csrf=False
    )
    def echo_json_context(self, **kwargs):
        return request.env.context

    @http.route(
        "/test_http/echo-json-over-http", type="http", auth="none", methods=["POST"], csrf=False
    )
    ...
    @http.route(
        "/test_http/cors_http_methods", type="http", auth="none", methods=["GET", "PUT"], cors="*"
    )
