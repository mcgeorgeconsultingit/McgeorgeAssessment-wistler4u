...
class HolidaysRequest(models.Model):
    ...
    def _populate_factories(self):
        ...
        employee_by_company = {
            k: list(v) for k, v in groupby(employee_records, key=lambda rec: rec["company_id"].id)
        }
        company_by_type = {
            rec.id: rec.company_id.id for rec in self.env["hr.leave.type"].browse(hr_leave_type_ids)
        }
        ...
        def compute_date_from(counter, **kwargs):
            date_from = datetime.datetime.now().replace(hour=0, minute=0, second=0) + relativedelta(
                days=int(3 * int(counter))
            )
            return date_from
