...
class IrRule(models.Model):
    ...
    def _get_failing(self, for_records, mode="read"):
        ...
        group_domains = expression.OR(
            [safe_eval(r.domain_force, eval_context) if r.domain_force else [] for r in group_rules]
        )
    ...
    def clear_cache(self):
        warnings.warn(
            "Deprecated IrRule.clear_cache(), use IrRule.clear_caches() instead", DeprecationWarning
        )
        self.clear_caches()
    ...
    def _make_access_error(self, operation, records):
        ...
        if not self.env.user.has_group("base.group_no_one") or not self.env.user.has_group(
            "base.group_user"
        ):
