...
class Field(MetaField("DummyField", (object,), {})):
    ...
    def _inverse_company_dependent(self, records):
        ...
        Property = records.env["ir.property"].sudo()
        values = {record.id: self.convert_to_write(record[self.name], record) for record in records}
    ...
    def is_editable(self):
        ...
        return (
            not self.readonly
            or self.states
            and any("readonly" in item for items in self.states.values() for item in items)
        )
    ...
    def update_db(self, model, columns):
        ...
        if (
            ...
            if join_field.type == "many2one" and join_field.store and not join_field.compute:
                ...
                if env.is_protected(self, record):
                    ...
                    try:
                        ...
                        if self.readonly and not self.store:
                            raise ValueError(
                                "Compute method failed to assign %s.%s" % (record, self.name)
                            )
...
class Float(Field):
    ...
    def column_type(self):
        ...
        return (
            ("numeric", "numeric") if self._digits is not None else ("float8", "double precision")
        )
...
class _String(Field):
    ...
    def write(self, records, value):
        ...
        if not dirty:
            cache.update_raw(
                records, self, [{lang: cache_value} for _id in records._ids], dirty=False
            )
            return records
        ...
        for record in records:
            ...
            new_translations = {
                l: self.translate(
                    lambda term: translation_dictionary.get(term, {l: None})[l], cache_value
                )
...
class Char(_String):
    ...
    def _setup_attrs(self, model_class, name):
        ...
        assert not (self.translate and self.size), "Translated field %s cannot have size %r" % (
            self,
            self.size,
        )
    ...
    def context_timestamp(record, timestamp):
        ...
        if tz_name:
            try:
                ...
                _logger.debug(
                    "failed to compute context/client-specific timestamp, " "using the UTC value",
                    exc_info=True,
                )
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record, values=None, validate=True):
        ...
        if value[:1] in magic_bytes:
            ...
            if guess_mimetype(decoded_value).startswith("image/svg") and not record.env.is_system():
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if isinstance(value, int) and (
            record._context.get("bin_size") or record._context.get("bin_size_" + self.name)
        ):
    ...
    def read(self, records):
        ...
        data = {att.res_id: att.datas for att in records.env["ir.attachment"].sudo().search(domain)}
    ...
    def create(self, record_values):
        ...
        with env.norecompute():
            env["ir.attachment"].sudo().with_context(binary_field_real_user=env.user,).create(
                [
                    {
                        "name": self.name,
    ...
    def _setup_attrs(self, model_class, name):
        ...
        for field in self._base_fields:
            ...
            if "selection" in field.args:
                if self.related:
                    _logger.warning(
                        "%s: selection attribute will be ignored as the field is related", self
                    )
            ...
            if "selection_add" in field.args:
                if self.related:
                    _logger.warning(
                        "%s: selection_add attribute will be ignored as the field is related", self
                    )
                selection_add = field.args["selection_add"]
                assert isinstance(selection_add, list), "%s: selection_add=%r must be a list" % (
                    self,
                    selection_add,
                )
                assert values is not None, "%s: selection_add=%r on non-list selection %r" % (
                    self,
                    selection_add,
                    self.selection,
                )
...
class Properties(Field):
    ...
    definition_record_field = (
        None  # field on the definition record which defined the Properties field definition
    )
        ...
        for record, record_values in zip(records, records_cached_values):
            ...
            for property_definition in definition:
                ...
                if type_ == "many2one":
                    ...
                    record_values[name] = [
                        id_ for id_ in property_value if id_ in existing_ids_per_model[comodel]
                    ]
    ...
    def write(self, records, value):
        ...
        if definition_changed:
            value = [definition for definition in value if not definition.get("definition_deleted")]
            for definition in value:
            ...
            for record in value:
                ...
                if not origin:
                    values = record._convert_to_write(
                        {name: record[name] for name in record._cache if name not in inv_names}
                    )
    ...
    def update_db(self, model, columns):
        if self.comodel_name in model.env:
            ...
            if self.inverse_name not in comodel._fields:
                raise UserError(
                    _("No inverse field %r found for %r") % (self.inverse_name, self.comodel_name)
                )
...
class Many2many(_RelationalMulti):
    ...
    def update_db(self, model, columns):
        ...
        if not self.manual:
            model.pool.post_init(
                model.env["ir.model.relation"]._reflect_relation, model, self.relation, self._module
            )
        comodel = model.env[self.comodel_name]
    ...
    def write_real(self, records_commands_list, create=False):
        ...
        return records.filtered(lambda record: new_relation[record.id] != old_relation[record.id])
