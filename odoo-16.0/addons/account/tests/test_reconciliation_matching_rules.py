...
class TestReconciliationMatchingRules(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.current_assets_account = cls.env["account.account"].search(
            [("account_type", "=", "asset_current"), ("company_id", "=", cls.company.id)], limit=1
        )
    ...
    def _create_invoice_line(
        ...
        invoice_form = Form(
            cls.env["account.move"].with_context(
                default_move_type=move_type, default_invoice_date=inv_date, default_date=inv_date
            )
        )
    ...
    def test_matching_fields_match_text_location(self):
        ...
        inv3 = self._create_invoice_line(
            1000, self.partner_a, "out_invoice", pay_reference="4444 tsoin 5555 tsoin 6666"
        )
    ...
    def test_zero_payment_tolerance_auto_reconcile(self):
        ...
        for inv_type, bsl_sign in (("out_invoice", 1), ("in_invoice", -1)):
            ...
            invl = self._create_invoice_line(
                1000.0, self.partner_a, inv_type, pay_reference="123456", inv_date="2019-01-01"
            )
    ...
    def test_payment_similar_communications(self):
        def create_payment_line(amount, memo, partner):
            ...
            return payment.line_ids.filtered(
                lambda x: x.account_id.account_type not in {"asset_receivable", "liability_payable"}
            )
