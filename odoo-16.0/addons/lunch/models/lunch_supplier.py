...
class LunchSupplier(models.Model):
    ...
    responsible_id = fields.Many2one(
        "res.users",
        string="Responsible",
        domain=lambda self: [("groups_id", "in", self.env.ref("lunch.group_lunch_manager").id)],
    ...
    delivery = fields.Selection(
        [("delivery", "Delivery"), ("no_delivery", "No Delivery")], default="no_delivery"
    )
    ...
    def _sync_cron(self):
        for supplier in self:
            ...
            if (
                ...
                not lc
                and sendat_tz <= fields.Datetime.context_timestamp(supplier, fields.Datetime.now())
            ):
