...
class TestPortalWizard(MailCommon):
    ...
    def test_portal_wizard_acl(self):
        portal_wizard = (
            self.env["portal.wizard"].with_context(active_ids=[self.partner.id]).create({})
        )

        with self.assertRaises(
            AccessError, msg="Standard users should not be able to open the portal wizard"
        ):
            self.env["portal.wizard"].with_context(active_ids=[self.partner.id]).with_user(
                self.user_employee
            ).create({})
    ...
    def test_portal_wizard_partner(self):
        ...
        self.assertEqual(
            self.partner.email, "first_email@example.com", "Must write on the email of the partner"
        )
        self.assertEqual(
            new_user.email, "first_email@example.com", "Must create the user with the right email"
        )
    ...
    def test_portal_wizard_public_user(self):
        ...
        portal_wizard = (
            self.env["portal.wizard"].with_context(active_ids=[public_partner.id]).create({})
        )
        ...
        self.assertFalse(
            self.public_user.has_group("base.group_public"), "Must remove the group public"
        )
        self.assertEqual(
            public_partner.email, "new_email@example.com", "Must change the email of the partner"
        )
        self.assertEqual(
            self.public_user.email, "new_email@example.com", "Must change the email of the user"
        )
        ...
        self.assertEqual(
            portal_user.user_id, self.public_user, "Must keep the user even if it is archived"
        )
        self.assertEqual(
            group_public,
    ...
    def test_portal_wizard_internal_user(self):
        ...
        portal_wizard = (
            self.env["portal.wizard"].with_context(active_ids=[internal_partner.id]).create({})
        )
    ...
    def test_portal_wizard_multi_company(self):
        ...
        portal_wizard = (
            self.env["portal.wizard"].with_context(active_ids=[partner_company_2.id]).create({})
        )
        portal_user = portal_wizard.user_ids
