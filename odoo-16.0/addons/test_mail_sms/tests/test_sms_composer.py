    ...
    * add test for sanitized / wrong numbers;
    ...
    def test_composer_comment_default(self):
        with self.with_user("employee"):
            composer = (
                self.env["sms.composer"]
                .with_context(active_model="mail.test.sms", active_id=self.test_record.id)
                .create(
                    {
                        "body": self._test_body,
                    }
    ...
    def test_composer_comment_field_1(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.test_record.customer_id, "number": self.test_numbers_san[0]}],
            self._test_body,
            messages,
        )
    ...
    def test_composer_default_recipient(self):
        ...
        self.assertEqual(
            composer.recipient_single_description, self.test_record.customer_id.display_name
        )
    ...
    def test_composer_comment_wo_partner_wo_value_update(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.env["res.partner"], "number": self.random_numbers_san[0]}],
            self._test_body,
            messages,
        )
...
class TestSMSComposerBatch(TestSMSCommon):
    ...
    def test_composer_batch_active_ids(self):
        ...
        for record, message in zip(self.records, messages):
            self.assertSMSNotification(
                [{"partner": record.customer_id}], "Hello %s zizisse an SMS." % record.name, message
            )
...
class TestSMSComposerMass(TestSMSCommon):
    ...
    def test_composer_mass_active_ids(self):
        ...
        for partner, record in zip(self.partners, self.records):
            self.assertSMSOutgoing(partner, None, content="Hello %s zizisse an SMS." % record.name)
    ...
    def test_composer_mass_active_ids_w_blacklist(self):
        ...
        for partner, record in zip(self.partners[5:], self.records[5:]):
            self.assertSMSOutgoing(
                partner, partner.phone_sanitized, content="Hello %s zizisse an SMS." % record.name
            )
        for partner, record in zip(self.partners[:5], self.records[:5]):
    ...
    def test_composer_template_context_action(self):
        ...
        with self.with_user("employee"):
            ...
            self.assertEqual(
                composer.body, "Hello %s ceci est en français." % test_record_2.display_name
            )
