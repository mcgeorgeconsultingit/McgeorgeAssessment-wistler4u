...
class TestHasGroup(TransactionCase):
    def setUp(self):
        ...
        self.test_user = self.env["res.users"].create(
            {
                "login": "testuser",
                "partner_id": self.env["res.partner"].create({"name": "Strawman Test User"}).id,
    ...
    def test_record(self):
        ...
        self.assertFalse(
            self.test_user.has_group(self.group1), "the test user shoudl not belong to group1"
        )
        ...
        portal_user = self.env["res.users"].create(
            {
                ...
                "name": "Portal test",
                "sel_groups_%s_%s_%s"
                % (self.grp_internal.id, self.grp_portal.id, grp_public.id): self.grp_portal.id,
                "sel_groups_%s_%s"
                % (grp_test_internal1.id, grp_test_internal2.id): grp_test_internal2.id,
            }
        )
    ...
    def test_implied_groups(self):
        ...
        user_a = U.create(
            {"name": "a", "login": "a", "groups_id": [Command.set([group_AA.id, group_user.id])]}
        )
        self.assertEqual(user_a.groups_id, (group_AA + group_A + group_user + group_no_one))

        user_b = U.create(
            {"name": "b", "login": "b", "groups_id": [Command.set([group_portal.id, group_AA.id])]}
        )
        ...
        self.assertEqual(
            user_a.groups_id, (group_AA + group_A + group_BB + group_B + group_user + group_no_one)
        )
    ...
    def test_has_group_cleared_cache_on_write(self):
        ...
        def populate_cache():
            ...
            self.assertTrue(
                self.registry._Registry__cache, "user.has_group cache must be populated"
            )
        ...
        self.assertFalse(
            self.registry._Registry__cache, "Writing on group must invalidate user.has_group cache"
        )
