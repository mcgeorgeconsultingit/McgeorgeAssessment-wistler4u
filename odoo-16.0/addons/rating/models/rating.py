...
class Rating(models.Model):
    ...
    res_model = fields.Char(
        string="Document Model", related="res_model_id.model", store=True, index=True, readonly=True
    )
    ...
    is_internal = fields.Boolean(
        "Visible Internally Only", readonly=False, related="message_id.is_internal", store=True
    )
    ...
    def _compute_parent_res_name(self):
        for rating in self:
            ...
            if rating.parent_res_model and rating.parent_res_id:
                name = (
                    self.env[rating.parent_res_model].sudo().browse(rating.parent_res_id).name_get()
                )
                name = (
                    name
                    and name[0][1]
    ...
    def _compute_rating_image(self):
        ...
        for rating in self:
            try:
                ...
                rating.rating_image = (
                    base64.b64encode(open(image_path, "rb").read()) if image_path else False
                )
