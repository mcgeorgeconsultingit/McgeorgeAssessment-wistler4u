...
class SlideQuestion(models.Model):
    ...
    attempts_count = fields.Integer(
        compute="_compute_statistics", groups="website_slides.group_website_slides_officer"
    )
    ...
    def _compute_statistics(self):
        slide_partners = (
            self.env["slide.slide.partner"].sudo().search([("slide_id", "in", self.slide_id.ids)])
        )
        slide_stats = dict(
            (s.slide_id.id, dict({"attempts_count": 0, "attempts_unique": 0, "done_count": 0}))
        ...
        for question in self:
            ...
            question.attempts_avg = (
                stats.get("attempts_count", 0) / stats.get("attempts_unique", 1) if stats else 0
            )
