...
class SaleOrder(models.Model):
    ...
    is_abandoned_cart = fields.Boolean(
        "Abandoned Cart", compute="_compute_abandoned_cart", search="_search_abandoned_cart"
    )
    ...
    def _cart_update(self, product_id, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if product.lst_price == 0 and product.website_id.prevent_zero_price_sale:
            raise UserError(
                _("The given product does not have a price therefore it cannot be added to cart.")
            )
        ...
        **kwargs,
        ...
        if no_variant_attribute_values:
            values["product_no_variant_attribute_value_ids"] = [
                fields.Command.set(
                    [int(attribute["value"]) for attribute in no_variant_attribute_values]
                )
            ]
        ...
        for line in self.website_order_line.filtered("product_id"):
            ...
            if accessory_products:
                ...
                all_accessory_products |= accessory_products.filtered(
                    ...
                    and (not product.company_id or product.company_id == line.company_id)
    ...
    def _filter_can_send_abandoned_cart_mail(self):
        ...
        return self.filtered(
            ...
            and not any(
                transaction.state == "error" for transaction in abandoned_sale_order.transaction_ids
            )
            and any(
