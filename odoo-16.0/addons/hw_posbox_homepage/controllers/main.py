...
class IoTboxHomepage(Home):
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        subprocess.check_call(
            [
                get_resource_path("point_of_sale", "tools/posbox/configuration/connect_to_wifi.sh"),
                essid,
                password,
                persistent,
            ]
        ...
        if server:
            res_payload["server"] = {"url": server, "message": "Redirect to Odoo Server"}
        else:
            res_payload["server"] = {
    ...
    def connect_to_server(self, token, iotname):
        ...
        if iotname and platform.system() == "Linux":
            subprocess.check_call(
                [
                    get_resource_path("point_of_sale", "tools/posbox/configuration/rename_iot.sh"),
                    iotname,
                ]
            )
        else:
    ...
    def step_by_step_configure(self, token, iotname, essid, password, persistent=False):
        ...
        subprocess.check_call(
            [
                get_resource_path(
                    "point_of_sale", "tools/posbox/configuration/connect_to_server_wifi.sh"
                ),
                url,
                iotname,
                token,
    ...
    def enable_ngrok(self, auth_token):
        if subprocess.call(["pgrep", "ngrok"]) == 1:
            subprocess.Popen(
                ["ngrok", "tcp", "-authtoken", auth_token, "-log", "/tmp/ngrok.log", "22"]
            )
    ...
    def upgrade(self):
        commit = (
            subprocess.check_output(
                ["git", "--work-tree=/home/pi/odoo/", "--git-dir=/home/pi/odoo/.git", "log", "-1"]
            )
            .decode("utf-8")
            .replace("\n", "<br/>")
        )
