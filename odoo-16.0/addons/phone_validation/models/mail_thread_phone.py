...
class PhoneMixin(models.AbstractModel):
    ...
    def _search_phone_mobile_search(self, operator, value):
        ...
        if value.startswith("+") or value.startswith("00"):
            ...
            if operator not in ("=", "!="):  # for like operators
                term = f"{term}%"
            self._cr.execute(query, (pattern, "00" + term, pattern, "+" + term) * len(phone_fields))
        else:
            if operator in expression.NEGATIVE_TERM_OPERATORS:
                where_str = " AND ".join(
    ...
    def phone_get_sanitized_numbers(self, number_fname="mobile", force_format="E164"):
        ...
        for record in self:
            ...
            res[record.id] = phone_validation.phone_sanitize_numbers_w_record(
                [number], record, record_country_fname=country_fname, force_format=force_format
            )[number]["sanitized"]
        return res
    ...
    def phone_get_sanitized_number(self, number_fname="mobile", force_format="E164"):
        ...
        return phone_validation.phone_sanitize_numbers_w_record(
            [number], self, record_country_fname=country_fname, force_format=force_format
        )[number]["sanitized"]
