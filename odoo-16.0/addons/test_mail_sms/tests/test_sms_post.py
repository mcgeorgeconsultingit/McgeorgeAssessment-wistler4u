    ...
    * add tests for new mail.message and mail.thread fields;
    ...
    def test_message_sms_internals_resend_existingd(self):
        ...
        with self.with_user("employee"), self.mockSMSGateway():
            ...
            test_record._notify_thread_by_sms(
                messages, [{"id": self.partner_1.id, "notif": "sms"}], resend_existing=True
            )
    ...
    def test_message_sms_internals_sms_numbers(self):
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=self.partner_1.ids, sms_numbers=self.random_numbers
            )
    ...
    def test_message_sms_model_partner(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1}, {"partner": self.partner_2}], self._test_body, messages
        )
    ...
    def test_message_sms_on_numbers(self):
        ...
        self.assertSMSNotification(
            [{"number": self.random_numbers_san[0]}, {"number": self.random_numbers_san[1]}],
            self._test_body,
            messages,
        )
    ...
    def test_message_sms_on_partner_ids_default(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.test_record.customer_id, "number": self.test_numbers_san[1]}],
            self._test_body,
            messages,
        )
    ...
    def test_message_sms_on_partner_ids_w_numbers(self):
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=self.partner_1.ids, sms_numbers=self.random_numbers[:1]
            )
...
class TestSMSPostException(TestSMSCommon, TestSMSRecipients):
    ...
    def test_message_sms_crash_credit_single(self):
        with self.with_user("employee"), self.mockSMSGateway(
            ...
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=(self.partner_1 | self.partner_2 | self.partner_3).ids
            )
    ...
    def test_message_sms_crash_wrong_number_single(self):
        with self.with_user("employee"), self.mockSMSGateway(
            nbr_t_error={self.partner_2.phone_get_sanitized_number(): "wrong_number_format"}
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=(self.partner_1 | self.partner_2 | self.partner_3).ids
...
class TestSMSApi(TestSMSCommon):
        ...
        with self.with_user("employee"):
            with self.mockSMSGateway():
                self.env["mail.test.sms"].browse(self.records.ids)._message_sms_schedule_mass(
                    body=self._test_body, mass_keep_log=False
                )
    ...
    def test_message_schedule_sms_w_log(self):
        with self.with_user("employee"):
            with self.mockSMSGateway():
                self.env["mail.test.sms"].browse(self.records.ids)._message_sms_schedule_mass(
                    body=self._test_body, mass_keep_log=True
                )
    ...
    def test_message_schedule_sms_w_template(self):
        ...
        for record in self.records:
            self.assertSMSOutgoing(
                record.customer_id, None, content="Dear %s this is an SMS." % record.display_name
            )
