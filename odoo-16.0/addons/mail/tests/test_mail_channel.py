...
class TestChannelAccessRights(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.public_channel = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_create(name="Public Channel", group_id=None)["id"]
        )
        # Group
        ...
        cls.chat_user_employee_1 = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_get(cls.user_employee_1.partner_id.ids)["id"]
        )
    ...
    @mute_logger(
        "odoo.addons.base.models.ir_rule", "odoo.addons.base.models.ir_model", "odoo.models"
    )
    @users("user_public")
    ...
    def test_access_employee(self):
        ...
        new_channel = self.env["mail.channel"].create({"name": "Test", "channel_type": "channel"})
        new_group = self.env["mail.channel"].create({"name": "Test", "channel_type": "group"})
        new_chat = self.env["mail.channel"].create({"name": "Test", "channel_type": "chat"})
...
class TestChannelInternals(MailCommon):
    ...
    def test_channel_members(self):
        ...
        self.env["mail.channel.member"].sudo().search(
            [("partner_id", "in", self.test_partner.ids), ("channel_id", "in", channel.ids)]
        ).unlink()
        self.assertEqual(channel.message_partner_ids, self.env["res.partner"])
        self.assertEqual(channel.channel_partner_ids, self.env["res.partner"])
    ...
    def test_channel_info_get_should_update_last_interest_dt(self):
        ...
        self.assertEqual(
            channel_info["last_interest_dt"], retrieve_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        )
    ...
    def test_channel_unsubscribe_auto(self):
        ...
        self.assertEqual(
            self.test_channel.channel_partner_ids, self.user_employee.partner_id | test_partner
        )
        self.assertEqual(
            private_group.channel_partner_ids, self.user_employee.partner_id | test_partner
        )
    ...
    def test_channel_private_unfollow(self):
        ...
        public_channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(name="Channel for Everyone", group_id=None)[
                "id"
            ]
        )
    ...
    def test_channel_should_generate_correct_default_avatar(self):
        ...
        private_group = self.env["mail.channel"].browse(
            self.env["mail.channel"].create_group(partners_to=self.user_employee.partner_id.ids)[
                "id"
            ]
        )
    ...
    def test_mail_message_starred_group(self):
        ...
        test_group_own_message = test_group.with_user(self.user_employee.id).message_post(
            body="TestingMessage"
        )
        test_group_own_message.write({"starred_partner_ids": [(6, 0, self.partner_employee.ids)]})
        self.assertEqual(
            self.user_employee.with_user(self.user_employee)._init_messaging()["starred_counter"], 1
        )
        ...
        self.assertEqual(
            self.user_employee.with_user(self.user_employee)._init_messaging()["starred_counter"], 2
        )
    ...
    def test_create_chat_channel_should_only_pin_the_channel_for_the_current_user(self):
        ...
        member_of_current_user = self.env["mail.channel.member"].search(
            [("channel_id", "=", chat["id"]), ("partner_id", "=", self.env.user.partner_id.id)]
        )
