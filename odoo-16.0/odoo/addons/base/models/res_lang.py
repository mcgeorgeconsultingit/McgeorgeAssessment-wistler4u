...
class Lang(models.Model):
    ...
    code = fields.Char(
        string="Locale Code", required=True, help="This field is used to set/get locales for user"
    )
    ...
    def _compute_field_flag_image_url(self):
        for lang in self:
            if lang.flag_image:
                ...
                lang.flag_image_url = (
                    f"/base/static/img/country_flags/{lang.code.lower().rsplit('_')[-1]}.png"
                )
    ...
    def _lang_get_code(self, url_code):
        return (
            self.with_context(active_test=True).search([("url_code", "=", url_code)]).code
            or url_code
        )
