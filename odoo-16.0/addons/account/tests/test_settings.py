...
class TestSettings(AccountTestInvoicingCommon):
    ...
    def switch_tax_settings(self, config):
        ...
        config.execute()
        self.assertEqual(
            self.env.user.has_group("account.group_show_line_subtotals_tax_excluded"), True
        )
        self.assertEqual(
            self.env.user.has_group("account.group_show_line_subtotals_tax_included"), False
        )
        ...
        config.execute()
        self.assertEqual(
            self.env.user.has_group("account.group_show_line_subtotals_tax_excluded"), False
        )
        self.assertEqual(
            self.env.user.has_group("account.group_show_line_subtotals_tax_included"), True
        )
