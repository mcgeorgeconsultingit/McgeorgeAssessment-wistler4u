...
class LunchAlert(models.Model):
    ...
    available_today = fields.Boolean(
        "Is Displayed Today", compute="_compute_available_today", search="_search_available_today"
    )
    ...
    def _sync_cron(self):
        ...
        for alert in self:
            ...
            lc = cron.lastcall
            if (lc and sendat_tz.date() <= fields.Datetime.context_timestamp(alert, lc).date()) or (
                not lc
                and sendat_tz <= fields.Datetime.context_timestamp(alert, fields.Datetime.now())
            ):
