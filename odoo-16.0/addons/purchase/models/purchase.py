...
class PurchaseOrder(models.Model):
    ...
    receipt_reminder_email = fields.Boolean(
        "Receipt Reminder Email", related="partner_id.receipt_reminder_email", readonly=False
    )
    reminder_date_before_receipt = fields.Integer(
        "Days Before Receipt", related="partner_id.reminder_date_before_receipt", readonly=False
    )
    ...
    def _compute_date_calendar_start(self):
        for order in self:
            order.date_calendar_start = (
                order.date_approve if (order.state in ["purchase", "done"]) else order.date_order
            )
    ...
    @api.depends(
        "order_line.taxes_id", "order_line.price_subtotal", "amount_total", "amount_untaxed"
    )
    ...
    def onchange_partner_id(self):
        ...
        if not self.partner_id:
            ...
            self.currency_id = (
                self.partner_id.property_purchase_currency_id.id or self.env.company.currency_id.id
            )
        return {}
    ...
    def _add_supplier_to_product(self):
        ...
        for line in self.order_line:
            ...
            partner = (
                self.partner_id if not self.partner_id.parent_id else self.partner_id.parent_id
            )
            if (
                line.product_id
    ...
    def retrieve_dashboard(self):
        ...
        self.env.cr.execute(
            query, (one_week_ago, self.env.ref("purchase.mt_rfq_sent").id, self.env.company.id)
        )
    ...
    def _send_reminder_mail(self, send_single=False):
        ...
        if template:
            ...
            for order in orders:
                ...
                if date and (
                    send_single
                    or (date - relativedelta(days=order.reminder_date_before_receipt)).date()
                    == datetime.today().date()
                ):
                    if send_single:
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ["purchase", "done"] and not order.mail_reception_confirmed:
                ...
                order.message_post(
                    body=_("The order receipt has been acknowledged by %s.", order.partner_id.name)
                )
    ...
    def _create_update_date_activity(self, updated_dates):
        note = Markup("<p>%s</p>\n") % _(
            "%s modified receipt dates for the following products:", self.partner_id.name
        )
        for line, date in updated_dates:
            note += Markup("<p> - %s</p>\n") % _(
        ...
        activity = self.activity_schedule(
            "mail.mail_activity_data_warning", summary=_("Date Updated"), user_id=self.user_id.id
        )
...
class PurchaseOrderLine(models.Model):
    ...
    taxes_id = fields.Many2many(
        "account.tax", string="Taxes", domain=["|", ("active", "=", False), ("active", "=", True)]
    )
    ...
    order_id = fields.Many2one(
        "purchase.order", string="Order Reference", index=True, required=True, ondelete="cascade"
    )
    ...
    invoice_lines = fields.One2many(
        "account.move.line", "purchase_line_id", string="Bill Lines", readonly=True, copy=False
    )
    ...
    def _onchange_product_packaging_id(self):
        if self.product_packaging_id and self.product_qty:
            ...
            if (
                float_compare(
                    newqty, self.product_qty, precision_rounding=self.product_uom.rounding
                )
                != 0
            ):
                return {
    ...
    def _compute_product_qty(self):
        for line in self:
            if line.product_packaging_id:
                ...
                if (
                    float_compare(
                        product_qty, line.product_qty, precision_rounding=line.product_uom.rounding
                    )
                    != 0
                ):
