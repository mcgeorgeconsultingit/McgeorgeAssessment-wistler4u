    ...
    def _get_calendars_validity_within_period(self, start, end, default_company=None):
        ...
        if resource_without_contract:
            calendars_within_period_per_resource.update(
                ...
                )._get_calendars_validity_within_period(start, end, default_company=default_company)
            )
        ...
        for contract in contracts:
            ...
            calendars_within_period_per_resource[contract.employee_id.resource_id.id][
                ...
                [
                    (
                        tz.localize(datetime.combine(contract.date_start, datetime.min.time()))
                        if contract.date_start > start.astimezone(tz).date()
                        else start,
                        tz.localize(datetime.combine(contract.date_end, datetime.max.time()))
