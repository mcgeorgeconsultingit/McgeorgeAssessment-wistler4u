...
class HRLeaveType(models.Model):
            ...
            if leave_type.overtime_deductible and leave_type.requires_allocation == "no":
                ...
                if employee_id.total_overtime > 0:
                    ...
                    name = "%(name)s (%(count)s)" % {
                        ...
                        "count": _(
                            "%s hours available", format_duration(employee_id.total_overtime)
                        ),
                    }
    ...
    def get_employees_days(self, employee_ids, date=None):
        ...
        deductible_time_off_type_ids = (
            ...
            .search([("overtime_deductible", "=", True), ("requires_allocation", "=", "no")])
            .ids
        )
            ...
            for allocation_id in allocations:
                if allocation_id in deductible_time_off_type_ids:
                    res[employee_id][allocation_id]["virtual_remaining_leaves"] = (
                        self.env["hr.employee"].sudo().browse(employee_id).total_overtime
                    )
