...
class StockRule(models.Model):
    ...
    action = fields.Selection(
        selection_add=[("manufacture", "Manufacture")], ondelete={"manufacture": "cascade"}
    )
    ...
    def _get_message_dict(self):
        ...
        if self.location_src_id:
            manufacture_message += _(" <br/><br/> The components will be taken from <b>%s</b>.") % (
                source
            )
    ...
    def _run_manufacture(self, procurements):
        ...
        for procurement, rule in procurements:
            if (
                float_compare(
                    procurement.product_qty, 0, precision_rounding=procurement.product_uom.rounding
                )
                <= 0
            ):
            ...
            if rule.picking_type_id == warehouse_id.sam_type_id:
                if (
                    ...
                    procurement.values["group_id"] = (
                        procurement.values["group_id"]
                        .stock_move_ids.filtered(lambda m: m.state not in ["done", "cancel"])
                ...
                if group:
                    ...
                    procurement.values["group_id"] = self.env["procurement.group"].create(
                        {"name": name}
                    )
    ...
    def _prepare_mo_vals(
        ...
        mo_values = {
            ...
            "product_qty": product_uom._compute_quantity(product_qty, bom.product_uom_id)
            if bom
            else product_qty,
