...
class MailTestActivity(models.Model):
    ...
    def action_start(self, action_summary):
        return self.activity_schedule("test_mail.mail_act_test_todo", summary=action_summary)
...
class MailTestTicketEL(models.Model):
    ...
    email_from = fields.Char("Email", compute="_compute_email_from", readonly=False, store=True)
...
class MailTestComposerMixin(models.Model):
    ...
    description = fields.Html(
        "Description", render_engine="qweb", render_options={"post_process": True}, sanitize=False
    )
