...
class ResCompany(models.Model):
    ...
    def _autorise_lock_date_changes(self, vals):
        ...
        * You cannot delete a tax lock date, lock a period that is not finished yet or the tax lock date must be set after
        ...
        fiscalyear_lock_date = vals.get("fiscalyear_lock_date") and fields.Date.from_string(
            vals["fiscalyear_lock_date"]
        )
        ...
        for company in self:
            ...
            if (
                ...
                raise UserError(
                    _("The lock date for accountants is irreversible and can't be removed.")
                )
