            ...
            for prod in products:
                ...
                if product_interim_account.reconcile:
                    ...
                    product_stock_moves = stock_moves.filtered(
                        lambda stock_move: stock_move.product_id == prod
                    )
                    ...
                    # Reconcile.
                    if any(
                        aml.amount_currency and not aml.balance for aml in product_account_moves
                    ):
...
class AccountMoveLine(models.Model):
    ...
    def _create_in_invoice_svl(self):
        ...
        for line in self:
            ...
            price_unit = line.currency_id._convert(
                price_unit, line.company_id.currency_id, line.company_id, line.date, round=False
            )
    ...
    def _get_stock_layer_price_difference(self, layers, layers_price_unit, price_unit):
        ...
        for layer in layers:
            if (
                float_compare(
                    total_to_correct, 0, precision_rounding=self.product_id.uom_id.rounding
                )
                <= 0
            ):
                break
            ...
            po_pu_curr = po_line.currency_id._convert(
                po_line.price_unit, self.currency_id, self.company_id, self.date, round=False
            )
