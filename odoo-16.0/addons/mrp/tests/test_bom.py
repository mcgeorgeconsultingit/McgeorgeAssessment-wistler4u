...
class TestBoM(TestMrpCommon):
    def test_01_explode(self):
        ...
        self.assertEqual(
            ...
            set(
                ...
                .mapped("bom_line_ids")
                .filtered(lambda line: not line.child_bom_id or line.child_bom_id.type != "phantom")
                .ids
            ),
        )
    ...
    def test_13_bom_kit_qty(self):
        ...
        kit_product_qty, _, _ = (self.product_7_3 + self.product_2 + self.product_3).mapped(
            "qty_available"
        )
    ...
    def test_20_bom_report(self):
        ...
        workcenter = self.env["mrp.workcenter"].create({"costs_hour": 10, "name": "Deserts Table"})
        ...
        self.assertEqual(
            float_compare(report_values["lines"]["operations_cost"], 23.84, precision_digits=2),
            0,
        ...
        for component_line in report_values["lines"]["components"]:
            ...
            if component_line["product"].id == butter.id:
                ...
                self.assertEqual(
                    float_compare(component_line["bom_cost"], (7.01 * 5), precision_digits=2), 0
                )
            if component_line["product"].id == biscuit.id:
                # 6 kg of biscuits at 1.50$ for 11kg of crumble -> 9$
                self.assertEqual(
                    float_compare(component_line["bom_cost"], (1.5 * 6), precision_digits=2), 0
                )
        # total price = 35.05 + 9 + operation_cost(23.84) = 67.89
        self.assertEqual(
            float_compare(report_values["lines"]["bom_cost"], 67.89, precision_digits=2),
            0,
            "Product Bom Price is not correct",
        )
        self.assertEqual(
            float_compare(report_values["lines"]["bom_cost"] / 11.0, 6.17, precision_digits=2),
            0,
        ...
        self.assertEqual(
            float_compare(
                report_values["lines"]["operations_cost"], operation_cost, precision_digits=2
            ),
            0,
        ...
        for component_line in report_values["lines"]["components"]:
            ...
            if component_line["product"].id == butter.id:
                ...
                self.assertEqual(
                    float_compare(
                        component_line["bom_cost"], (7.01 * 5) * (1 / 11), precision_digits=2
                    ),
                    0,
                )
            if component_line["product"].id == biscuit.id:
                ...
                self.assertEqual(
                    float_compare(
                        component_line["bom_cost"], (1.5 * 6) * (1 / 11), precision_digits=2
                    ),
                    0,
                )
        ...
        self.assertEqual(
            float_compare(report_values_22["lines"]["operations_cost"], 47.66, precision_digits=2),
            0,
        ...
        self.assertEqual(
            float_compare(report_values["lines"]["operations_cost"], 208.33, precision_digits=2), 0
        )
        ...
        for component_line in report_values["lines"]["components"]:
            ...
            if component_line["product"].id == cream.id:
                ...
                self.assertEqual(
                    float_compare(component_line["bom_cost"], (3 * 5.17), precision_digits=2), 0
                )
            if component_line["product"].id == crumble.id:
                # 5.4 kg of crumble at the cost of a batch.
                crumble_cost = self.env["report.mrp.report_bom_structure"]._get_report_data(
                    bom_id=bom_crumble.id, searchQty=5.4, searchVariant=False
                )["lines"]["bom_cost"]
                self.assertEqual(
                    float_compare(component_line["bom_cost"], crumble_cost, precision_digits=2), 0
                )
        # total price = Cream (15.51€) + crumble_cost (34.63 €) + operation_cost(208,33) = 258.47€
        self.assertEqual(
            float_compare(report_values["lines"]["bom_cost"], 258.47, precision_digits=2),
            0,
            "Product Bom Price is not correct",
        )
