...
class TranslationToolsTestCase(BaseCase):
    ...
    def test_translate_xml_inline1(self):
        ...
        self.assertItemsEqual(
            terms, ["Form stuff", "Blah <i>blah</i> blah", "Put some <b>more text</b> here"]
        )
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch(
            "odoo.addons.base.models.ir_module.Module._load_module_terms", _load_module_terms
        ):
            wizard.lang_install()
...
class TestTranslation(TransactionCase):
    ...
    def test_103_duplicate_record_fr(self):
        ...
        category_no = category.with_context({})
        self.assertEqual(
            category_no.name, "Clients (copie)", "Duplication should set untranslated value"
        )

        category_fr = category.with_context({"lang": "fr_FR"})
        self.assertEqual(
            category_fr.name, "Clients (copie)", "Did not used default value for translated value"
        )
    ...
    def test_108_search_en(self):
        ...
        self.assertEqual(
            category_equal.id, self.customers.id, "Search with '=' doesn't work for English"
        )
        category_ilike = CategoryEn.search([("name", "ilike", "stoMer")])
        self.assertIn(
            self.customers, category_ilike, "Search with 'ilike' doesn't work for English"
        )
        category_eq_ilike = CategoryEn.search([("name", "=ilike", "CustoMers")])
        self.assertIn(
            self.customers, category_eq_ilike, "Search with '=ilike' doesn't work for English"
        )
    ...
    def test_109_search_fr(self):
        ...
        self.assertEqual(
            category_equal.id, self.customers.id, "Search with '=' doesn't work for non English"
        )
        category_ilike = CategoryFr.search([("name", "ilike", "lIen")])
        self.assertIn(
            self.customers, category_ilike, "Search with 'ilike' doesn't work for non English"
        )
        category_eq_ilike = CategoryFr.search([("name", "=ilike", "clieNts")])
        self.assertIn(
            self.customers, category_eq_ilike, "Search with '=ilike' doesn't work for non English"
        )
...
class TestTranslationWrite(TransactionCase):
    ...
    def test_00(self):
        ...
        category = (
            self.env["res.partner.category"].with_context(lang="en_US").create({"name": "English"})
        )
        ...
        category2 = (
            self.env["res.partner.category"].with_context(lang="fr_FR").create({"name": "French"})
        )
    ...
    def test_03_fr_single(self):
        ...
        langs = self.env["res.lang"].get_installed()
        self.assertEqual(
            [("fr_FR", "French / Français")], langs, "Test did not started with expected languages"
        )
    ...
    def test_field_selection(self):
        ...
        self.assertEqual(fg["state"]["selection"], [("manual", "Custo"), ("base", "Pas touche!")])
...
class TestXMLTranslation(TransactionCase):
    ...
    <div class="s_table_of_content_main" data-name="Content">
        ...
        self.assertEqual(view.with_env(env_en).arch_db, archf % terms_en)
        self.assertEqual(
            view.with_env(env_fr).arch_db, archf % (terms_fr[0], terms_en[1], terms_en[2])
        )
        self.assertEqual(
            view.with_env(env_nl).arch_db, archf % (terms_nl[0], terms_en[1], terms_en[2])
        )
    ...
    def test_update_field_translations(self):
        ...
        view.update_field_translations(
            "arch_db", {"en_US": {"Fork": "Fork2"}, "fr_FR": {"Fourchette": "Fourchette2"}}
        )
