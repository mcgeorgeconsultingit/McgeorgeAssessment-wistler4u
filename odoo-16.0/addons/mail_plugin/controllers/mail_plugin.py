...
class MailPluginController(http.Controller):
    @http.route(
        "/mail_client_extension/modules/get", type="json", auth="outlook", csrf=False, cors="*"
    )
    ...
    @http.route(
        "/mail_plugin/partner/enrich_and_create_company", type="json", auth="outlook", cors="*"
    )
    ...
    @http.route(
        "/mail_plugin/partner/enrich_and_update_company", type="json", auth="outlook", cors="*"
    )
    ...
    def res_partner_get(self, email=None, name=None, partner_id=None, **kwargs):
        ...
        if not (partner_id or (name and email)):
            return {
                "error": _("You need to specify at least the partner_id or the name and the email")
            }
    ...
    def _get_partner_data(self, partner):
        ...
        if not partner_values["name"]:
            ...
            name, email = request.env["res.partner"]._parse_partner_name(partner_values["email"])
