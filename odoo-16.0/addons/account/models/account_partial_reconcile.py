...
class AccountPartialReconcile(models.Model):
    ...
    def _check_required_computed_currencies(self):
        bad_partials = self.filtered(
            lambda partial: not partial.debit_currency_id or not partial.credit_currency_id
        )
        if bad_partials:
            raise ValidationError(
                _("Missing foreign currencies on partials having ids: %s", bad_partials.ids)
            )
    ...
    def _compute_max_date(self):
        for partial in self:
            partial.max_date = max(partial.debit_move_id.date, partial.credit_move_id.date)
            ...
            for partial_values in move_values["partials"]:
                ...
                for caba_treatment, line in move_values["to_process_lines"]:
                    ...
                    if caba_treatment == "tax":
                        ...
                        grouping_key = self._get_cash_basis_base_line_grouping_key_from_vals(
                            cb_line_vals
                        )
                    ...
                    if grouping_key in partial_lines_to_create:
                        ...
                        if caba_treatment == "tax":
                            aggregated_vals.update(
                                {
                                    "tax_base_amount": aggregated_vals["tax_base_amount"]
                ...
                for grouping_key, aggregated_vals in partial_lines_to_create.items():
                    ...
                    if "tax_repartition_line_id" in line_vals:
                        ...
                        counterpart_line_vals = self._prepare_cash_basis_counterpart_tax_line_vals(
                            tax_line, line_vals
                        )
                        ...
                        if tax_line.account_id.reconcile:
                            ...
                            to_reconcile_after.append(
                                (tax_line, move_index, counterpart_line_vals["sequence"])
                            )

                    else:
                        # Base line.

                        counterpart_line_vals = self._prepare_cash_basis_counterpart_base_line_vals(
                            line_vals
                        )
                        counterpart_line_vals["sequence"] = sequence + 1
