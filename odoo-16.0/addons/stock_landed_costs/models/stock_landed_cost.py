...
class StockLandedCost(models.Model):
    ...
    picking_ids = fields.Many2many(
        "stock.picking", string="Transfers", copy=False, states={"done": [("readonly", True)]}
    )
        ...
        for cost in self:
            ...
            if (
                ...
                for product in cost.cost_lines.product_id:
                    ...
                    all_amls.filtered(
                        lambda aml: aml.account_id == input_account and not aml.reconciled
                    ).reconcile()
...
class StockLandedCostLine(models.Model):
    ...
    def onchange_product_id(self):
        ...
        self.split_method = (
            self.product_id.product_tmpl_id.split_method_landed_cost or self.split_method or "equal"
        )
...
class AdjustmentLines(models.Model):
    ...
    def _create_accounting_entries(self, move, qty_out):
        ...
        if not credit_account_id:
            raise UserError(
                _("Please configure Stock Expense Account for product: %s.") % (cost_product.name)
            )
