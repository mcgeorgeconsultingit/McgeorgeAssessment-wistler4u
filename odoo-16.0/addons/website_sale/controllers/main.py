...
class Website(main.Website):
    ...
    def theme_customize_data(self, is_view_data, enable=None, disable=None, reset_view_arch=False):
        ...
        if any(
            ...
            for key in ["website_sale.products_list_view", "website_sale.add_grid_or_list_option"]
        ):
...
class WebsiteSale(http.Controller):
    ...
    def shop(
        ...
        if (
            not pricelist or request.session.get("website_sale_pricelist_time", 0) < now - 60 * 60
    ...
    def resequence_product_image(self, image_res_model, image_res_id, move):
        ...
        if (
            ...
            raise ValidationError(
                _("Can not resequence embedded image/video with a non compatible image.")
            )
    ...
    def cart_update_json(
        ...
        **kw,
        ...
        values["minor_amount"] = (
            payment_utils.to_minor_currency_units(order.amount_total, order.currency_id),
        )
                ...
                if mode[1] == "billing":
                    ...
                    if not kw.get("use_same"):
                        kw["callback"] = kw.get("callback") or (
                            ...
                            and (mode[0] == "edit" and "/shop/checkout" or "/shop/address")
                        )
    ...
    def _create_or_edit_partner(self, partner_details, edit=False, **custom_values):
        ...
        sanitized_values = {k: v for k, v in values.items() if k in self.WRITABLE_PARTNER_FIELDS}
    ...
    @http.route(
        "/shop/payment/get_status/<int:sale_order_id>", type="json", auth="public", website=True
    )
