...
class TestAssetsGenerateTimeCommon(odoo.tests.TransactionCase):
    def generate_bundles(self):
        ...
        bundles = {
            key for module in installed_module_names for key in get_manifest(module)["assets"]
        }
            ...
            with mute_logger("odoo.addons.base.models.assetsbundle"):
                for assets_type in "css", "js":
                    try:
                        ...
                        _logger.info(
                            "Error detected while generating bundle %r %s", bundle, assets_type
                        )
...
class TestAssetsGenerateTime(TestAssetsGenerateTimeCommon):
    ...
    def test_assets_generate_time(self):
        ...
        for bundle, duration in self.generate_bundles():
            ...
            self.assertLess(
                duration, threshold, "Bundle %r took more than %s sec" % (bundle, threshold)
            )
...
class TestLoad(HttpCase):
    def test_assets_already_exists(self):
        ...
        with patch(
            "odoo.addons.base.models.assetsbundle.AssetsBundle.save_attachment", save_attachment
        ):
