...
class LuxTaxGenerator:
    ...
    def tax_codes_to_csv(self):
        writer = pycompat.csv_writer(open("account.tax.code.template-%s.csv" % self.suffix, "wb"))
    ...
    def taxes_to_csv(self):
        writer = pycompat.csv_writer(open("account.tax.template-%s.csv" % self.suffix, "wb"))
        taxes_iterator = self.iter_taxes()
        ...
        for row in sorted(taxes_iterator, key=lambda r: r["description"]):
            ...
            if row["parent_id:id"]:
                ...
                cur_seq = seq
            writer.writerow([pycompat.to_text(v) for v in list(row.values())[3:]] + [cur_seq])
