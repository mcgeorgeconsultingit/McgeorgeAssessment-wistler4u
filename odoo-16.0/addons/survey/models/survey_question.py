...
class SurveyQuestion(models.Model):
    ...
    background_image = fields.Image(
        "Background Image", compute="_compute_background_image", store=True, readonly=False
    )
    ...
    answer_datetime = fields.Datetime(
        "Correct datetime answer", help="Correct date and time answer for this question."
    )
    ...
    is_time_limited = fields.Boolean(
        "The question is limited in time", help="Currently only supported for live sessions."
    )
    ...
    validation_required = fields.Boolean(
        "Validate entry", compute="_compute_validation_required", readonly=False, store=True
    )
    ...
    @api.depends(
        "survey_id.access_token", "background_image", "page_id", "survey_id.background_image_url"
    )
    ...
    @api.depends(
        "survey_id.question_and_page_ids.is_page", "survey_id.question_and_page_ids.sequence"
    )
    ...
    @api.depends(
        "question_type", "scoring_type", "answer_date", "answer_datetime", "answer_numerical_box"
    )
    ...
    def _validate_char_box(self, answer):
        ...
        if self.validation_required:
            if not (self.validation_length_min <= len(answer) <= self.validation_length_max):
                return {
                    self.id: self.validation_error_msg or _("The answer you entered is not valid.")
                }
        return {}
        ...
        for question in self:
            ...
            if question.question_type in ["simple_choice", "multiple_choice", "matrix"]:
                answer_lines = all_lines.filtered(
                    ...
                    or (line.answer_type == "char_box" and question.comment_count_as_answer)
                )
        ...
        table_data = [
            {
                "row": row,
                "columns": [
                    {"suggested_answer": sug_answer, "count": count_data[(row, sug_answer)]}
                    for sug_answer in suggested_answers
                ],
            }
    ...
    def _get_stats_summary_data_choice(self, user_input_lines):
        ...
        if self.question_type == "multiple_choice":
            ...
            right_inputs = user_input_lines.filtered(lambda line: line.answer_is_correct).mapped(
                "user_input_id"
            )
        return {
