    ...
    def test_01_fuzzy_names(self):
        ...
        for search, expected in typos.items():
            ...
            if not fuzzy_guess or (
                fuzzy_guess not in expected and fuzzy_guess not in [exp[:-1] for exp in expected]
            ):
                mismatch_count += 1
                _logger.info(
                    "'%s' fuzzy matched to '%s' instead of %s", search, fuzzy_guess, expected
                )

        ratio = 100.0 * mismatch_count / len(typos)
        _logger.info(
            "%s wrong guesses over %s tested typos (%.2f%%)", mismatch_count, len(typos), ratio
        )
        typos.clear()
        words.clear()
...
class TestAutoComplete(TransactionCase):
    ...
    def _check_highlight(self, term, value):
        ...
        self.assertTrue(
            f'<span class="text-primary">{term}</span>' in value.lower(), "Term must be highlighted"
        )
    ...
    def test_04_fuzzy_results(self):
        ...
        self.assertEqual(
            1, suggestions["results_count"], "Text data contains one page with 'approximately'"
        )
    ...
    def test_05_long_url(self):
        ...
        self.assertEqual(
            1, suggestions["results_count"], "Text data contains one page with 'long url'"
        )
        self.assertEqual(1, len(suggestions["results"]), "Single result must be present")
    ...
    def test_09_hyphen(self):
        ...
        self.assertEqual(
            1, suggestions["results_count"], "Text data contains one page with 'weekend'"
        )
