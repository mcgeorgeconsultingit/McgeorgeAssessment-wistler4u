...
class StockWarehouse(models.Model):
    ...
    pbm_mto_pull_id = fields.Many2one("stock.rule", "Picking Before Manufacturing MTO Rule")
    ...
    pbm_type_id = fields.Many2one(
        "stock.picking.type", "Picking Before Manufacturing Operation Type", check_company=True
    )
    sam_type_id = fields.Many2one(
        "stock.picking.type", "Stock After Manufacturing Operation Type", check_company=True
    )
    ...
    def _get_global_route_rules_values(self):
        ...
        location_src = (
            self.manufacture_steps == "mrp_one_step" and self.lot_stock_id or self.pbm_loc_id
        )
    ...
    def _get_sequence_values(self, name=False, code=False):
        ...
        values.update(
            {
                "pbm_type_id": {
                    "name": self.name + " " + _("Sequence picking before manufacturing"),
                    "prefix": self.code + "/PC/",
                    "padding": 5,
                    "company_id": self.company_id.id,
                },
        ...
        data.update(
            {
                ...
                "manu_type_id": {
                    ...
                    "default_location_src_id": self.manufacture_steps in ("pbm", "pbm_sam")
    ...
    def check_product_is_not_kit(self):
        if self.env["mrp.bom"].search(
            ...
            raise ValidationError(
                _("A product with a kit-type bill of materials can not have a reordering rule.")
            )

    def _get_orderpoint_products(self):
        non_kit_ids = []
        for products in split_every(
            2000, super()._get_orderpoint_products().ids, self.env["product.product"].browse
        ):
            kit_ids = set(
                k.id for k in self.env["mrp.bom"]._bom_find(products, bom_type="phantom").keys()
            )
