...
class PortalWizard(models.TransientModel):
    ...
    def _default_partner_ids(self):
        partner_ids = self.env.context.get("default_partner_ids", []) or self.env.context.get(
            "active_ids", []
        )
        contact_ids = set()
        for partner in self.env["res.partner"].sudo().browse(partner_ids):
            contact_partners = (
                partner.child_ids.filtered(lambda p: p.type in ("contact", "other")) | partner
            )
...
class PortalWizardUser(models.TransientModel):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Contact", required=True, readonly=True, ondelete="cascade"
    )
    ...
    def _compute_email_state(self):
        ...
        normalized_emails = [
            email_normalize(portal_user.email) for portal_user in portal_users_with_email
        ]
        existing_users = (
            self.env["res.users"]
    ...
    def action_grant_access(self):
        ...
        if self.is_portal or self.is_internal:
            raise UserError(
                _('The partner "%s" already has the portal access.', self.partner_id.name)
            )
        ...
        if not user_sudo.active or not self.is_portal:
            user_sudo.write(
                {"active": True, "groups_id": [(4, group_portal.id), (3, group_public.id)]}
            )
    ...
    def action_revoke_access(self):
        ...
        if user_sudo and user_sudo.has_group("base.group_portal"):
            ...
            if len(user_sudo.groups_id) <= 1:
                user_sudo.write(
                    {"groups_id": [(3, group_portal.id), (4, group_public.id)], "active": False}
                )
            else:
    ...
    def _assert_user_email_uniqueness(self):
        ...
        if self.email_state == "exist":
            raise UserError(
                _('The contact "%s" has the same email as an existing user', self.partner_id.name)
            )
