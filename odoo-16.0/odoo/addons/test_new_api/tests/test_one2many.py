...
class One2manyCase(TransactionCase):
    def setUp(self):
        ...
        for name, editions in movies_data:
            ...
            for edition in editions:
                self.Edition.create(
                    {"res_model_id": movie_model_id, "name": edition, "res_id": movie_id}
                )

    def operations(self):
        """Run operations on o2m fields to check all works fine."""
        # Check the lines first
        self.assertItemsEqual(self.multi.lines.mapped("name"), [str(i) for i in range(10)])
    ...
    def test_cache_invalidation(self):
        ...
        self.env.flush_all()
        with self.assertQueryCount(0):
            self.assertEqual(attachment.name, record0.display_name, "field should be computed")
        ...
        self.env.flush_all()
        with self.assertQueryCount(0):
            self.assertEqual(attachment.name, record1.display_name, "field should be recomputed")
        with self.assertQueryCount(1):
