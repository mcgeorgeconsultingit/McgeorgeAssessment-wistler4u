...
class AccountMove(models.Model):
    ...
    def _build_spreadsheet_formula_domain(self, formula_params):
        ...
        if formula_params["include_unposted"]:
            base_domain = expression.AND([base_domain, [("move_id.state", "!=", "cancel")]])
        else:
            base_domain = expression.AND([base_domain, [("move_id.state", "=", "posted")]])
    ...
    def spreadsheet_fetch_debit_credit(self, args_list):
        ...
        for args in args_list:
            ...
            query.order = None
            query_str, params = query.select("SUM(debit) AS debit", "SUM(credit) AS credit")
