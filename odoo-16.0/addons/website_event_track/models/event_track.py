...
class Track(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", "Contact", help="Contact of the track, may be different from speaker."
    )
    # speaker information
    ...
    partner_biography = fields.Html(
        string="Biography", compute="_compute_partner_biography", readonly=False, store=True
    )
    ...
    partner_company_name = fields.Char(
        "Company Name", compute="_compute_partner_company_name", readonly=False, store=True
    )
    ...
    is_track_today = fields.Boolean("Is Track Today", compute="_compute_track_time_data")
    is_track_upcoming = fields.Boolean("Is Track Upcoming", compute="_compute_track_time_data")
    ...
    website_image_url = fields.Char(
        string="Image URL", compute="_compute_website_image_url", compute_sudo=True, store=False
    )
    # wishlist / visitors management
    event_track_visitor_ids = fields.One2many(
        "event.track.visitor", "track_id", string="Track Visitors", groups="event.group_event_user"
    )
    ...
    def _compute_partner_tag_line(self):
        for track in self:
            ...
            if track.partner_function:
                ...
                tag_line = _(
                    "%(name)s from %(company)s", name=tag_line, company=track.partner_company_name
                )
    ...
    def _compute_track_time_data(self):
        ...
        for track in self:
            if not track.date:
                track.is_track_live = (
                    track.is_track_soon
                ) = track.is_track_today = track.is_track_upcoming = track.is_track_done = False
    ...
    def create(self, vals_list):
        ...
        for track in tracks:
            email_values = (
                {} if self.env.user.email else {"email_from": self.env.company.catchall_formatted}
            )
