...
class ProductProduct(models.Model):
    ...
    all_product_tag_ids = fields.Many2many(
        "product.tag", compute="_compute_all_product_tag_ids", search="_search_all_product_tag_ids"
    )
    ...
    def _compute_can_image_variant_1024_be_zoomed(self):
        for record in self:
            record.can_image_variant_1024_be_zoomed = (
                ...
                and tools.is_image_size_above(record.image_variant_1920, record.image_variant_1024)
            )
    ...
    @api.depends(
        "create_date", "write_date", "product_tmpl_id.create_date", "product_tmpl_id.write_date"
    )
    ...
    def _compute_partner_ref(self):
        for product in self:
            for supplier_info in product.seller_ids:
                if supplier_info.partner_id.id == product._context.get("partner_id"):
                    product_name = (
                        supplier_info.product_name or product.default_code or product.name
                    )
    ...
    def name_get(self):
        ...
        for product in self.sudo():
            ...
            sellers = (
                self.env["product.supplierinfo"].sudo().browse(self.env.context.get("seller_id"))
                or []
            )
    ...
    def _select_seller(self, partner_id=False, quantity=0.0, date=None, uom_id=False, params=False):
        ...
        for seller in sellers:
            ...
            if (
                ...
                and float_compare(quantity_uom_seller, seller.min_qty, precision_digits=precision)
                == -1
            ):
                continue
    ...
    def toggle_active(self):
        ...
        tmpl_to_deactivate = self.filtered(
            lambda product: (
                product.product_tmpl_id.active and not product.product_tmpl_id.product_variant_ids
            )
        ).mapped("product_tmpl_id")
        # We activate product templates which are inactive with active variants.
        tmpl_to_activate = self.filtered(
            lambda product: (
                not product.product_tmpl_id.active and product.product_tmpl_id.product_variant_ids
            )
        ).mapped("product_tmpl_id")
