...
class FileTouchable(AddonManifestPatched):
    ...
    def _touch(self, filepath, touch_time=None):
        ...
        return patch(
            "os.path.getmtime", lambda filename: self.touches.get(filename) or GETMTINE(filename)
        )
...
class TestJavascriptAssetsBundle(FileTouchable):
    ...
    def test_03_date_invalidation(self):
        ...
        with self._touch(path):
            ...
            self.assertNotEqual(
                version0, version1, "the version must should because the bundle has changed."
            )
...
class TestAssetsManifest(AddonManifestPatched):
    ...
    def test_06_replace(self):
        ...
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.manifest1")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_06_2_replace(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.manifest4")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_07_remove(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.manifest5")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_09_remove_wholeglob(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.manifest2.js")], order="create_date DESC", limit=1
        )
    ...
    def test_12_include2(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.manifest6")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_14_other_module(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_other.mockmanifest1")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_19_css_specific_attrs_in_tcallassets(self):
        ...
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.irasset2")], order="create_date DESC", limit=1
        )
    ...
    def test_21_js_before_css(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.bundle4")], order="create_date DESC", limit=1
        )
        content = attach.raw.decode()
        self.assertStringEqual(
            content,
    ...
    def test_27_mixing_after_before_js_css_in_irasset(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.bundle4")], order="create_date DESC", limit=2
        )
        attach_css = None
        attach_js = None
        for a in attach:
    ...
    def test_31(self):
        ...
        self.env["ir.qweb"]._render(view.id)
        attach = self.env["ir.attachment"].search(
            [("name", "ilike", "test_assetsbundle.irassetsec")], order="create_date DESC", limit=1
        )
    ...
    def test_33(self):
        ...
        self.assertTrue(
            "Unallowed to fetch files from addon notinstalled_module" in str(cm.exception)
        )
