...
class Websocket:
        ...
        while self.state is not ConnectionState.CLOSED:
            try:
                readables = {
                    ...
                    for selector_key in self._selector.select(type(self).INACTIVITY_TIMEOUT)
                }
    ...
    def trigger_notification_dispatching(self):
        ...
        if self.state is not ConnectionState.OPEN:
            return
        readables = {selector_key[0].fileobj for selector_key in self._selector.select(0)}
...
class WebsocketRequest:
    ...
    def serve_websocket_message(self, message):
        try:
            ...
            raise InvalidWebsocketRequest(f"Key {exc.args[0]!r} is missing from request") from exc
        except ValueError as exc:
...
class WebsocketConnectionHandler:
    ...
    def _assert_handshake_validity(cls, headers):
        ...
        missing_or_empty_headers = {
            header for header in cls._REQUIRED_HANDSHAKE_HEADERS if header not in headers
        }
