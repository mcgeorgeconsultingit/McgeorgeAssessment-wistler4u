...
class TestORM(TransactionCase):
    ...
    def test_access_deleted_records(self):
        ...
        self.assertEqual(
            [{"id": c2.id, "name": "Y"}], cs.read(["name"]), "read() should skip deleted records"
        )
    ...
    def test_groupby_date(self):
        ...
        res = partners.read_group(
            [("id", "in", partner_ids)], ["date"], ["date:month", "date:day"], lazy=False
        )
        ...
        res = partners.read_group(
            [("id", "in", partner_ids)], ["date"], groupby=["date:month"], orderby="date:month DESC"
        )
        self.assertEqual([item["date:month"] for item in res], months)

        # order by date should reorder by date:month
        res = partners.read_group(
            [("id", "in", partner_ids)], ["date"], groupby=["date:month"], orderby="date DESC"
        )
        self.assertEqual([item["date:month"] for item in res], months)
