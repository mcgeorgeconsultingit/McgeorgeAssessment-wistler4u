...
class Project(models.Model):
    ...
    def action_open_project_purchase_orders(self):
        query = self.env["purchase.order.line"]._search([])
        query.add_where(
            "purchase_order_line.analytic_distribution ? %s", [str(self.analytic_account_id.id)]
        )
            ...
            if purchase_order_line_read:
                ...
                for pol_read in purchase_order_line_read:
                    ...
                    analytic_contribution = (
                        pol_read["analytic_distribution"][str(self.analytic_account_id.id)] / 100.0
                    )
                    amount_invoiced -= (
                    ...
                    if pol_read["qty_to_invoice"] > 0:
                        amount_to_invoice -= (
                            price_unit * pol_read["qty_to_invoice"] * analytic_contribution
                        )
                    else:
                        amount_to_invoice -= (
                            price_unit
