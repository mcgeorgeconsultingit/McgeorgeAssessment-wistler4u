...
class PurchaseRequisitionCreateAlternative(models.TransientModel):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Vendor", required=True, help="Choose a vendor for alternative PO"
    )
    ...
    def action_create_alternative(self):
        ...
        alt_po = (
            self.env["purchase.order"].with_context(origin_po_id=self.origin_po_id.id).create(vals)
        )
        return {
    ...
    def _get_alternative_values(self):
        ...
        if self.copy_products and self.origin_po_id:
            vals["order_line"] = [
                Command.create({"product_id": line.product_id.id, "product_qty": line.product_qty})
                for line in self.origin_po_id.order_line
            ]
        return vals
