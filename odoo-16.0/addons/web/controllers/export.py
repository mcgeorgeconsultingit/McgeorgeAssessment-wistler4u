...
class GroupExportXlsxWriter(ExportXlsxWriter):
    ...
    def write_group(self, row, column, group_name, group, group_depth=0):
        group_name = (
            group_name[1] if isinstance(group_name, tuple) and len(group_name) > 1 else group_name
        )
...
class Export(http.Controller):
    ...
    def get_fields(
        ...
        fields_sequence = sorted(
            fields.items(), key=lambda field: odoo.tools.ustr(field[1].get("string", "").lower())
        )
    ...
    def fields_info(self, model, export_fields):
        ...
        for (base, length), subfields in itertools.groupby(
            sorted(export_fields), lambda field: (field.split("/", 1)[0], len(field.split("/", 1)))
        ):
    ...
    def base(self, data):
        ...
        if not import_compat and groupby:
            ...
            groups_data = Model.read_group(
                domain, [x if x != ".id" else "id" for x in field_names], groupby, lazy=False
            )
...
class CSVExport(ExportFormat, http.Controller):
    ...
    def index(self, data):
        try:
            ...
            payload = json.dumps(
                {"code": 200, "message": "Odoo Server Error", "data": http.serialize_exception(exc)}
            )
