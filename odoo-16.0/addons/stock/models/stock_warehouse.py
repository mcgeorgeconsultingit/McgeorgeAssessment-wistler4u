...
class Warehouse(models.Model):
    ...
    code = fields.Char(
        "Short Name", required=True, size=5, help="Short name used to identify your warehouse"
    )
    ...
    sequence = fields.Integer(
        default=10, help="Gives the sequence of this line when displaying the warehouses."
    )
    _sql_constraints = [
        (
            "warehouse_name_uniq",
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            for field_name, values in sub_locations.items():
                ...
                vals[field_name] = (
                    self.env["stock.location"].with_context(active_test=False).create(values).id
                )
        ...
        for warehouse, vals in zip(warehouses, vals_list):
            ...
            (view_location_id | view_location_id.with_context(active_test=False).child_ids).write(
    ...
    def write(self, vals):
        ...
        for warehouse in warehouses:
            ...
            depends = [
                depend
                for depends in [
                    value.get("depends", []) for value in warehouse._get_routes_values().values()
                ]
            ...
            depends = [
                depend
                for depends in [value.get("depends", []) for value in global_rules.values()]
                for depend in depends
            ]
            ...
            if "active" in vals:
                ...
                if move_ids:
                    raise UserError(
                        _("You still have ongoing operations for picking types %s in warehouse %s")
                ...
                location_ids = (
                    ...
                    .with_context(active_test=False)
                    .search([("location_id", "child_of", warehouse.view_location_id.id)])
                )
    ...
    def _check_multiwarehouse_group(self):
        cnt_by_company = (
            self.env["stock.warehouse"]
            .sudo()
            ._read_group([("active", "=", True)], ["company_id"], groupby=["company_id"])
        )
        if cnt_by_company:
    ...
    def _create_or_update_route(self):
        ...
        for route_field, route_data in self._get_routes_values().items():
            ...
            if route_data["route_create_values"].get("warehouse_selectable", False) or route_data[
                "route_update_values"
            ].get("warehouse_selectable", False):
    ...
    def get_rules_dict(self):
        ...
        return {
            warehouse.id: {
                ...
                "two_steps": [
                    self.Routing(
                        supplier_loc, warehouse.wh_input_stock_loc_id, warehouse.in_type_id, "pull"
                    ),
                    self.Routing(
                ...
                "ship_only": [
                    self.Routing(
                        warehouse.lot_stock_id, customer_loc, warehouse.out_type_id, "pull"
                    )
                ],
                "pick_ship": [
                    self.Routing(
    ...
    def _get_receive_rules_dict(self):
        ...
        return {
            ...
            "two_steps": [
                self.Routing(
                    self.wh_input_stock_loc_id, self.lot_stock_id, self.int_type_id, "pull_push"
                )
            ],
            "three_steps": [
                self.Routing(
                ...
                self.Routing(
                    self.wh_qc_stock_loc_id, self.lot_stock_id, self.int_type_id, "pull_push"
                ),
            ],
        }
    ...
    def _update_reception_delivery_resupply(self, reception_new, delivery_new):
        ...
        for warehouse in self:
            ...
            if (
                ...
                and (warehouse.reception_steps == "one_step" or reception_new == "one_step")
            ):
                warehouse._check_reception_resupply(input_loc)
            if (
                delivery_new
                and warehouse.delivery_steps != delivery_new
                and (warehouse.delivery_steps == "ship_only" or delivery_new == "ship_only")
            ):
    ...
    def action_view_all_routes(self):
        ...
        return {
            ...
            "limit": 20,
            "context": dict(
                self._context, default_warehouse_selectable=True, default_warehouse_ids=self.ids
            ),
        }
