...
class TestAccountBankStatementLine(AccountTestInvoicingCommon):
    ...
    def test_constraints(self):
        ...
        with self.assertRaises(UserError), self.cr.savepoint():
            st_line.move_id.write({"line_ids": [(0, 0, vals) for vals in addition_lines_to_create]})
    ...
    def test_statement_split(self):
        ...
        statement1 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line7.id}).create({})
        )
        self.assertRecordValues(
            statement1,
            [
        ...
        statement2 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line6.id}).create({})
        )
        self.assertRecordValues(
            statement1,
            [
        ...
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        statement3 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line3.id}).create({})
        )
        self.assertRecordValues(
            self.env["account.bank.statement.line"].search(
                [("company_id", "=", self.env.company.id)]
        ...
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        statement5 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line2.id}).create({})
        )
        self.assertRecordValues(
            self.env["account.bank.statement.line"].search(
                [("company_id", "=", self.env.company.id)]
        ...
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        statement7 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line4.id}).create({})
        )
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        self.assertRecordValues(
            self.env["account.bank.statement.line"].search(
        ...
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        statement9 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line8.id}).create({})
        )
        self.assertRecordValues(
            self.env["account.bank.statement.line"].search(
                [("company_id", "=", self.env.company.id)]
        ...
        self.env["account.bank.statement.line"].invalidate_model(["running_balance"])
        statement10 = (
            self.env["account.bank.statement"].with_context({"split_line_id": line1.id}).create({})
        )
        self.assertRecordValues(
            self.env["account.bank.statement.line"].search(
                [("company_id", "=", self.env.company.id)]
