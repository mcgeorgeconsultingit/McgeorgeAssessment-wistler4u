...
class TestSaleMrpLeadTime(TestStockCommon):
    ...
    def test_00_product_company_level_delays(self):
        ...
        mo_scheduled = (
            out_date
            - timedelta(days=self.product_1.produce_delay)
            - timedelta(days=company.manufacturing_lead)
        )
        self.assertAlmostEqual(
            fields.Datetime.from_string(manufacturing_order.date_planned_start),
            mo_scheduled,
            delta=timedelta(seconds=1),
    ...
    def test_01_product_route_level_delays(self):
        ...
        mo_scheduled = (
            out_date
            - timedelta(days=self.product_1.produce_delay)
            - timedelta(days=self.warehouse_1.delivery_route_id.rule_ids[0].delay)
            - timedelta(days=self.env.ref("base.main_company").manufacturing_lead)
        )
        self.assertAlmostEqual(
            fields.Datetime.from_string(manufacturing_order.date_planned_start),
            mo_scheduled,
