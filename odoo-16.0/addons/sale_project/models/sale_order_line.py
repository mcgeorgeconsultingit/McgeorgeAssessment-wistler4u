...
class SaleOrderLine(models.Model):
                ...
                if len(sols) > 1 and all(sols.mapped("is_service")):
                    result += [
                        (
                            ...
                            % (
                                names.get(line.id),
                                format_amount(self.env, line.price_unit, line.currency_id),
                            ),
                        )
                        for line in sols
                    ]
    ...
    @api.depends(
        "qty_delivered_method", "product_uom_qty", "reached_milestones_ids.quantity_percentage"
    )
        ...
        for line in lines:
            if line.state == "sale" and not line.is_expense:
                ...
                if line.task_id and not has_task:
                    msg_body = _(
                        "Task Created (%s): %s", line.product_id.name, line.task_id._get_html_link()
                    )
    ...
    def _timesheet_create_project(self):
        ...
        if self.product_id.project_template_id:
            ...
            project = self.product_id.project_template_id.with_context(no_create_folder=True).copy(
                values
            )
            project.tasks.write(
                {
            ...
            if project_only_sol_count == 1:
                values["name"] = (
                    "%s - [%s] %s"
                    % (values["name"], self.product_id.default_code, self.product_id.name)
