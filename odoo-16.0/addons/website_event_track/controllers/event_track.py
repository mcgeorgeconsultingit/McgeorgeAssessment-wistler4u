...
class EventTrackController(http.Controller):
    ...
    def _event_tracks_get_values(self, event, tag=None, **searches):
        ...
        if searches.get("search"):
            search_domain = expression.AND([search_domain, [("name", "ilike", searches["search"])]])
        ...
        date_begin_tz_all = list(
            set(dt.date() for dt in self._get_dt_in_event_tz(tracks_wdate.mapped("date"), event))
        )
        ...
        for tracks_group in tracks_by_day:
            ...
            tracks_group["default_collapsed"] = (today_tz != tracks_group["date"]) and all(
                track.is_track_done and not track.is_track_live for track in tracks_group["tracks"]
            )
    ...
    def _prepare_calendar_values(self, event):
        ...
        track_time_slots = set().union(
            *(
                time_slot.keys()
                for time_slot in [time_slots for time_slots in time_slots_by_tracks.values()]
            )
        )
        ...
        for day, time_slots in time_slots_by_day.items():
            ...
            for i in range(0, time_slots_count + 1):
                global_time_slots_by_day[day][current_time_slot] = tracks_by_rounded_times.get(
                    current_time_slot, {}
                )
    ...
    def time_slot_rounder(self, time, rounded_minutes):
        ...
        """
        return time.replace(second=0, microsecond=0, minute=0, hour=time.hour) + timedelta(
            minutes=rounded_minutes * (time.minute // rounded_minutes)
        )
    ...
    def _split_track_by_days(self, track, local_tz):
        ...
        """
        start_date = (
            fields.Datetime.from_string(track.date).replace(tzinfo=pytz.utc).astimezone(local_tz)
        )
    ...
    def track_reminder_toggle(self, track_id, set_reminder_on):
        ...
        if not track.wishlisted_by_default:
            if (
                not event_track_partner or event_track_partner.is_wishlisted == set_reminder_on
            ):  # ignore if new state = old state
                return {"error": "ignored"}
            event_track_partner.is_wishlisted = set_reminder_on
        else:
            if (
                not event_track_partner or event_track_partner.is_blacklisted != set_reminder_on
            ):  # ignore if new state = old state
                return {"error": "ignored"}
    ...
    def event_track_proposal(self, event, **post):
        return request.render(
            "website_event_track.event_track_proposal", {"event": event, "main_object": event}
        )
    ...
    def _get_dt_in_event_tz(self, datetimes, event):
        ...
        return [utc.localize(dt, is_dst=False).astimezone(timezone(tz_name)) for dt in datetimes]
