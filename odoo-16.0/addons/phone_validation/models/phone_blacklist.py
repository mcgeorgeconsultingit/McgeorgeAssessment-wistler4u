...
class PhoneBlackList(models.Model):
    ...
    number = fields.Char(
        string="Phone Number", required=True, tracking=True, help="Number should be E164 formatted"
    )
    ...
    def create(self, values):
        ...
        for value in values:
            ...
            if not sanitized:
                raise UserError(
                    sanitized_values["msg"] + _(" Please correct the number and try again.")
                )
    ...
    def add(self, number):
        sanitized = phone_validation.phone_sanitize_numbers_w_record([number], self.env.user)[
            number
        ]["sanitized"]
