...
class SaleOrderLine(models.Model):
        ...
        for line in self:
            ...
            relevant_bom = boms.filtered(
                ...
                and (
                    ...
                    or (b.product_tmpl_id == line.product_id.product_tmpl_id and not b.product_id)
                )
            )
            if relevant_bom:
    ...
    def _compute_qty_delivered(self):
        ...
        for order_line in self:
            if order_line.qty_delivered_method == "stock_move":
                boms = order_line.move_ids.filtered(lambda m: m.state != "cancel").mapped(
                    "bom_line_id.bom_id"
                )
                ...
                if relevant_bom:
                    ...
                    if dropship:
                        ...
                        if (
                            any(
                                (m.location_dest_id.usage == "customer" and m.state != "done")
                                or (
                    ...
                    order_line.qty_delivered += relevant_bom.product_uom_id._compute_quantity(
    ...
    def _get_qty_procurement(self, previous_product_uom_qty=False):
        ...
        if bom:
            if not self.product_uom_qty or (
                previous_product_uom_qty and not previous_product_uom_qty.get(self.id, 0.0)
            ):
                return (
                    previous_product_uom_qty and previous_product_uom_qty.get(self.id, 0.0)
                ) or self.qty_delivered
