...
class TestPurchaseRequisition(TestPurchaseRequisitionCommon):
    ...
    def test_01_cancel_purchase_requisition(self):
        ...
        self.assertEqual(
            self.bo_requisition.state, "cancel", "Requisition should be in cancelled state."
        )
    ...
    def test_02_purchase_requisition(self):
        ...
        self.assertEqual(
            supplierinfo09.partner_id, seller_partner1, "The supplierinfo is not correct"
        )
        ...
        self.assertEqual(
            supplierinfo13.partner_id, seller_partner1, "The supplierinfo is not correct"
        )
    ...
    def test_07_alternative_purchases_wizards(self):
        ...
        alt_po_wiz = Form(
            self.env["purchase.requisition.create.alternative"].with_context(**action["context"])
        )
        ...
        self.assertEqual(
            best_date_pol.order_id.id, orig_po.id, "Best date PO line was not correctly calculated"
        )
        self.assertEqual(
    ...
    def test_08_purchases_multi_linkages(self):
        ...
        self.assertEqual(
            len(po_1.alternative_po_ids), 2, "PO1 and PO2 should only be linked to each other"
        )
        self.assertEqual(
            len(po_3.alternative_po_ids), 2, "PO3 and PO4 should only be linked to each other"
        )
        ...
        groups = self.env["purchase.order.group"].search([("order_ids", "in", pos.ids)])
        self.assertEqual(
            len(po_1.alternative_po_ids), 5, "All 5 POs should be linked to each other now"
        )
        self.assertEqual(
            len(groups),
            1,
