...
class WebsiteProfile(http.Controller):
    ...
    def _prepare_badges_domain(self, **kwargs):
        ...
        if "badge_category" in kwargs:
            domain = expression.AND(
                [[("challenge_ids.challenge_category", "=", kwargs.get("badge_category"))], domain]
            )
        return domain
    ...
    def view_all_users_page(self, page=1, **kwargs):
        ...
        if user_count:
            ...
            users = User.sudo().search(
                dom, limit=self._users_per_page, offset=pager["offset"], order="karma DESC"
            )
            ...
            max_position = max(
                [user_data["karma_position"] for user_data in position_map.values()], default=1
            )
            ...
            if my_user.website_published and my_user.karma and my_user.id not in users.ids:
                ...
                if current_user:
                    ...
                    user_data = self._get_position_map(position_domain, current_user, group_by).get(
                        current_user.id, {}
                    )
    ...
    def _get_user_tracking_karma_gain_position(self, domain, user_ids, group_by):
        ...
        results = (
            ...
            .browse(user_ids)
            ._get_tracking_karma_gain_position(domain, from_date=from_date, to_date=to_date)
        )
