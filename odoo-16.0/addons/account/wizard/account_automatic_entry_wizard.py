...
class AutomaticEntryWizard(models.TransientModel):
    ...
    action = fields.Selection(
        [("change_period", "Change Period"), ("change_account", "Change Account")], required=True
    )
    ...
    account_type = fields.Selection(
        [("income", "Revenue"), ("expense", "Expense")], compute="_compute_account_type", store=True
    )
    ...
    def _compute_total_amount(self):
        for record in self:
            record.total_amount = (
                (record.percentage or 100) * sum(record.move_line_ids.mapped("balance")) / 100
            )
    ...
    def _compute_account_type(self):
        for record in self:
            record.account_type = (
                "income" if sum(record.move_line_ids.mapped("balance")) < 0 else "expense"
            )
                ...
                for move in wizard.move_line_ids.move_id:
                    if move._get_violated_lock_dates(move.date, False):
                        raise ValidationError(
                            _("The date of some related entries is protected by a lock date")
                        )
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("active_model") != "account.move.line" or not self.env.context.get(
            "active_ids"
        ):
        ...
        if any(move_line.reconciled for move_line in move_line_ids):
            raise UserError(
                _("You can only change the period/account for items that are not yet reconciled.")
            )
        if any(line.company_id != move_line_ids[0].company_id for line in move_line_ids):
            raise UserError(
                _("You cannot use this wizard on journal entries belonging to different companies.")
            )
    ...
    def _get_move_dict_vals_change_account(self):
        ...
        for line in self.move_line_ids.filtered(
            ...
            if (
                ...
                and self.destination_account_id.currency_id != self.company_id.currency_id
            ):
        ...
        return [
            {
                ...
                and _("Transfer entry to %s", self.destination_account_id.display_name or ""),
    ...
    def _get_move_dict_vals_change_period(self):
        ...
        move_data = {
            "new_date": {
                ...
                "ref": self._format_strings(
                    _("{label}: Adjusting Entry of {new_date}"), self.move_line_ids[0].move_id
                ),
                "date": fields.Date.to_string(self.date),
                "journal_id": self.journal_id.id,
            }
        ...
        for date, grouped_lines in groupby(self.move_line_ids, lambda m: m.move_id.date):
            ...
            move_data[date] = {
                ...
                "line_ids": [],
                "ref": self._format_strings(
                    _("{label}: Adjusting Entry of {date}"), grouped_lines[0].move_id, amount
                ),
    ...
    def _compute_move_data(self):
        for record in self:
            if record.action == "change_period":
                if any(
                    line.account_id.account_type != record.move_line_ids[0].account_id.account_type
        ...
        for move in self.move_line_ids.move_id:
            ...
            if (
                ...
                accrual_move_lines = accrual_move.mapped("line_ids").filtered(
                    lambda line: line.account_id == accrual_account
                )[accrual_move_offsets[accrual_move] : accrual_move_offsets[accrual_move] + 2]
            ...
            destination_messages += [
                self._format_strings(
                    _("Adjusting Entry {link}: {percent:.2f}% of {amount} recognized from {date}"),
                    move,
                    amount,
                )
            ]
