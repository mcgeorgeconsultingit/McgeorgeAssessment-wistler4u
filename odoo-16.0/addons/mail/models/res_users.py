...
class Users(models.Model):
    ...
    def create(self, vals_list):
        ...
        log_portal_access = not self._context.get("mail_create_nolog") and not self._context.get(
            "mail_notrack"
        )
    ...
    def write(self, vals):
        ...
        if vals.get("groups_id"):
            ...
            user_group_ids += [
                id for command in vals["groups_id"] if command[0] == 6 for id in command[2]
            ]
            self.env["mail.channel"].search(
    ...
    def _deactivate_portal_user(self, **post):
        ...
        if post.get("request_blacklist"):
            users_to_blacklist = self.filtered(lambda user: tools.email_normalize(user.email))
        else:
            users_to_blacklist = []
