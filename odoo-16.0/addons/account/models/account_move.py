...
class AccountMove(models.Model):
    ...
    attachment_ids = fields.One2many(
        "ir.attachment", "res_id", domain=[("res_model", "=", "account.move")], string="Attachments"
    )

    # === Hash Fields === #
    restrict_mode_hash_table = fields.Boolean(related="journal_id.restrict_mode_hash_table")
    secure_sequence_number = fields.Integer(
        string="Inalteralbility No Gap Sequence #", readonly=True, copy=False, index=True
    )
    ...
    def _compute_always_tax_exigible(self):
        for record in self:
            ...
            record.always_tax_exigible = (
                not record.is_invoice(True) and not record._collect_tax_cash_basis_values()
            )
        ...
        for invoice in self:
            ...
            if payment_state_matters:
                reconciliation_vals = [
                    ...
                    if x["source_line_account_type"] in ("asset_receivable", "liability_payable")
                ]
            ...
            if move.state == "posted" and move.is_invoice(include_receipts=True):
                ...
                for reconciled_partial in reconciled_partials:
                    ...
                    if (
                        ...
                        and counterpart_line.currency_id != counterpart_line.company_id.currency_id
                    ):
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if rounding_line:
                    ...
                    move.tax_totals["formatted_amount_total_rounded"] = (
                        formatLang(self.env, amount_total_rounded, currency_obj=move.currency_id)
                        or ""
                    )
            else:
    ...
    def _compute_show_payment_term_details(self):
        ...
        for invoice in self:
            if (
                invoice.move_type in ("out_invoice", "out_receipt", "in_invoice", "in_receipt")
            ...
            if not vendor_display_name:
                if move.invoice_source_email:
                    ...
                    vendor_display_name = _(
                        "#Created by: %s", move.sudo().create_uid.name or self.env.user.name
                    )
    ...
    def _build_credit_warning_message(self, record, updated_credit):
        ...
        msg = _(
            ...
            partner_id.name,
            formatLang(
                self.env, partner_id.credit_limit, currency_obj=record.company_id.currency_id
            ),
        )
    ...
    def _fetch_duplicate_supplier_reference(self, only_posted=False):
        ...
        if not moves.ids:
            ...
            place_holders = {
                "id": 0,
                **{
                    field_name: moves._fields[field_name].convert_to_write(moves[field_name], moves)
                    or None
        ...
        self.env.cr.execute(
            ...
            SELECT
             ...
             GROUP BY move.id
        """,
            {"only_posted": only_posted, "moves": tuple(moves.ids or [0]), **place_holders},
        )
        return {
                ...
                for amount_by_group_list in invoice_totals["groups_by_subtotal"].values():
                    for amount_by_group in amount_by_group_list:
                        tax_lines = move.line_ids.filtered(
                            lambda line: line.tax_group_id.id == amount_by_group["tax_group_id"]
                        )
                        ...
                        if tax_lines:
                            ...
                            delta_amount = (
                                tax_group_old_amount * sign - amount_by_group["tax_group_amount"]
                            )
    ...
    def _validate_taxes_country(self):
        ...
        for record in self:
            ...
            impacted_countries = (
                amls.tax_ids.country_id | amls.tax_line_id.country_id | amls.tax_tag_ids.country_id
            )
    ...
    def _recompute_cash_rounding_lines(self):
        ...
        if self.invoice_cash_rounding_id and existing_cash_rounding_line:
            ...
            old_strategy = (
                "biggest_tax" if existing_cash_rounding_line.tax_line_id else "add_invoice_line"
            )
    ...
    def _sync_dynamic_line(
        self, existing_key_fname, needed_vals_fname, needed_dirty_fname, line_type, container
    ):
        def existing():
            return {
        ...
        to_delete = sorted(
            ...
            | {line.id for key, line in existing_after.items() if key not in needed_after}
        )
        to_create = {
            key: values for key, values in needed_after.items() if key not in existing_after
        }
        to_write = {
        ...
        if to_create:
            self.env["account.move.line"].create(
                [{**key, **values, "display_type": line_type} for key, values in to_create.items()]
            )
        if to_write:
    ...
    def _sync_invoice(self, container):
        ...
        for move in after:
            if changed("payment_reference"):
                move.line_ids.filtered(lambda l: l.display_type == "payment_term").name = after[
                    move
                ]["payment_reference"]
        ...
        with self._disable_recursion(container, "skip_invoice_sync") as disabled:
            ...
            def update_containers():
                ...
                misc_container["records"] = container["records"].filtered(
                    lambda m: m.move_type == "entry" and not m.tax_cash_basis_origin_move_id
                )
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if not relaxed:
            ...
            if sequence_number_reset == "year":
                ...
                param["anti_regex"] = (
                    re.sub(r"\?P<\w+>", "?:", self._sequence_monthly_regex.split("(?P<seq>")[0])
                    + "$"
                )
                ...
                param["anti_regex"] = (
                    re.sub(r"\?P<\w+>", "?:", self._sequence_yearly_regex.split("(?P<seq>")[0])
                    + "$"
                )
    ...
    def _compute_string_to_hash(self):
        ...
        for move in self:
            ...
            move.string_to_hash = dumps(
                values, sort_keys=True, ensure_ascii=True, indent=None, separators=(",", ":")
            )
    ...
    def _copy_recurring_entries(self):
        ...
        for record in self:
            ...
            if (
                ...
                record.copy(
                    default=record._get_fields_to_copy_recurring_entries({"date": next_date})
                )
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
            ...
            values.update(
                {
                    "invoice_date": self._apply_delta_recurring_entries(
                        self.invoice_date, self.auto_post_origin_id.invoice_date, self.auto_post
                    )
                }
            )
        if not self.invoice_payment_term_id and self.invoice_date_due:
            ...
            values.update(
                {"invoice_date_due": values["date"] + (self.invoice_date_due - self.date)}
            )
        return values
    ...
    def _get_invoice_counterpart_amls_for_early_payment_discount_per_payment_term_line(self):
        ...
        for aml in self.line_ids.filtered(lambda x: x.display_type == "payment_term"):
            ...
            if early_pay_discount_computation == "included" and product_lines.tax_ids:
                ...
                last_tax_line = (
                    list(res["tax_lines"][aml].values()) or list(res["base_lines"][aml].values())
                )[-1]
    ...
    def _post(self, soft=True):
        ...
        for invoice in self.filtered(lambda move: move.is_invoice(include_receipts=True)):
            if (
                ...
                raise UserError(
                    _(
                        ...
                        formatLang(
                            self.env, invoice.amount_total, currency_obj=invoice.currency_id
                        ),
                        formatLang(
                            self.env,
            ...
            if (
                float_compare(
                    invoice.amount_total, 0.0, precision_rounding=invoice.currency_id.rounding
                )
                < 0
            ):
                raise UserError(
        ...
        for move in to_post:
            ...
            if move.line_ids.account_id.filtered(lambda account: account.deprecated):
                raise UserError(
                    _("A line of this move is using a deprecated account, you cannot post it.")
                )
        ...
        for invoice in to_post:
            ...
            if (
                ...
                invoice.activity_schedule(
                    date_deadline=min(
                        (date for date in invoice.line_ids.mapped("date_maturity") if date),
    ...
    def _find_and_set_purchase_orders(
        self, po_references, partner_id, amount_total, prefer_purchase_line=False, timeout=10
    ):
    ...
    def _move_dict_to_preview_vals(self, move_vals, currency_id=None):
        preview_vals = {
            "group_name": "%s, %s"
            % (format_date(self.env, move_vals["date"]) or _("[Not set]"), move_vals["ref"]),
        ...
        for line in preview_vals["items_vals"]:
            if "partner_id" in line[2]:
                ...
                line[2]["partner_id"] = (
                    self.env["res.partner"].browse(line[2]["partner_id"]).sudo().display_name
                )
        ...
        if qr_code_method:
            ...
            for candidate_method, _candidate_name in self.env[
                ...
                if self.partner_bank_id._eligible_for_qr_code(
                    candidate_method, self.partner_id, self.currency_id, raises_error=False
                ):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        followers = [
            partner
            for partner in self._mail_find_partner_from_emails(cc_mail_addresses, extra_domain)
            if partner
        ]
        ...
        senders = partners = [
            partner
            for partner in self._mail_find_partner_from_emails(from_mail_addresses, extra_domain)
            if partner
        ]
        ...
        if partners:
            ...
            if is_internal_partner(partners[0]):
                ...
                partners = [
                    ...
                    and partner.company_id.id in (False, company_id)
                ]
