...
class TestServerActions(TestServerActionsBase):
    ...
    def test_10_code(self):
        ...
        self.assertFalse(
            run_res, "ir_actions_server: code server action correctly finished should return False"
        )

        partners = self.test_partner.search([("name", "ilike", "TestingPartner_code")])
        self.assertEqual(
            len(partners), 1, "ir_actions_server: 1 new partner should have been created"
        )
        ...
        self.action.write(
            {
                ...
                "fields_lines": [
                    Command.create({"col1": self.res_partner_name_field.id, "value": _name}),
        ...
        action2 = self.action.create(
            {
                ...
                "fields_lines": [
                    Command.create(
                        {"col1": self.res_partner_name_field.id, "value": "RaoulettePoiluchette"}
                    ),
                    Command.create(
    ...
    def test_50_groups(self):
        ...
        self.assertItemsEqual(
            bindings.get("action"), self.action.read(["name", "sequence", "binding_view_types"])
        )

        self.action.with_context(self.context).run()
        self.assertEqual(
            self.test_country.vat_label, "VatFromTest", "vat label should be changed to VatFromTest"
        )
    ...
    def test_60_sort(self):
        ...
        bindings = Actions.get_bindings("res.country")
        self.assertEqual(
            [vals.get("name") for vals in bindings["action"]], ["TestAction2", "TestAction"]
        )
    ...
    def test_70_copy_action(self):
        ...
        self.assertEqual(r.copy().state, "open", "by default state should be reset by copy")

        # then check that on server action we've changed that
        self.assertEqual(
            self.action.copy().state, "code", "copying a server action should not reset the state"
        )
