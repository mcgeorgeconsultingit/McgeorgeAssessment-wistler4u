...
class AccountAnalyticLine(models.Model):
                ...
                if timesheet.so_line and timesheet.so_line.product_id.type == "service":
                    if timesheet.so_line.product_id.invoice_policy == "delivery":
                        if timesheet.so_line.product_id.service_type == "timesheet":
                            invoice_type = (
                                "timesheet_revenues" if timesheet.amount > 0 else "billable_time"
                            )
                        else:
    ...
    def _compute_so_line(self):
        for timesheet in self.filtered(
            ...
            timesheet.so_line = (
                timesheet.project_id.allow_billable and timesheet._timesheet_determine_sale_line()
            )
    ...
    def _get_timesheets_to_merge(self):
        ...
        return res.filtered(
            lambda l: not l.timesheet_invoice_id or l.timesheet_invoice_id.state != "posted"
        )
