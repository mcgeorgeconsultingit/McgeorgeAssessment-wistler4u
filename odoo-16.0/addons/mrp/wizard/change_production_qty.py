    ...
    def change_prod_qty(self):
        ...
        for wizard in self:
            ...
            self._update_finished_moves(
                production, new_production_qty - qty_produced, old_production_qty - qty_produced
            )
            ...
            for wo in production.workorder_ids:
                ...
                if production.product_id.tracking == "serial":
                    quantity = (
                        1.0 if not float_is_zero(quantity, precision_digits=precision) else 0.0
                    )
                else:
                    quantity = (
                        quantity
                        if (
                            quantity > 0 and not float_is_zero(quantity, precision_digits=precision)
                        )
                        else 0
                    )
