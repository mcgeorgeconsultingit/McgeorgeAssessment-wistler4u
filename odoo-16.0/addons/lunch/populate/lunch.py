...
class LunchProduct(models.Model):
    ...
    def _populate_factories(self):
        ...
        category_by_company = {
            k: list(v) for k, v in groupby(category_records, key=lambda rec: rec["company_id"].id)
        }

        supplier_ids = self.env.registry.populated_models["lunch.supplier"]
        company_by_supplier = {
            rec.id: rec.company_id.id for rec in self.env["lunch.supplier"].browse(supplier_ids)
        }
    ...
    def _populate_factories(self):
        ...
        return [
            ...
            (
                ...
                populate.iterate(
                    [[], [(6, 0, location_ids)]], then=populate.compute(get_location_ids)
                ),
            ),
        ...
        return [
            ...
            (
                ...
                populate.randdatetime(
                    relative_before=relativedelta(months=-3), relative_after=relativedelta(months=3)
                ),
            ),
        ]
...
class LunchAlert(models.Model):
    ...
    def _populate_factories(self):
        ...
        return [
            ...
            (
                "recipients",
                populate.cartesian(["everyone", "last_week", "last_month", "last_year"]),
            ),
            ...
            (
                "until",
                populate.randdatetime(
                    relative_before=relativedelta(years=-2), relative_after=relativedelta(years=2)
                ),
            ),
