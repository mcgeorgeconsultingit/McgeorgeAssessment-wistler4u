...
class ProjectCustomerPortal(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "project_count" in counters:
            values["project_count"] = (
                ...
                if request.env["project.project"].check_access_rights("read", raise_exception=False)
                else 0
            )
        if "task_count" in counters:
            values["task_count"] = (
                request.env["project.task"].search_count([])
                if request.env["project.task"].check_access_rights("read", raise_exception=False)
                else 0
            )
        return values
    ...
    @http.route(
        ["/my/projects", "/my/projects/page/<int:page>"], type="http", auth="user", website=True
    )
    ...
    @http.route(
        "/my/projects/<int:project_id>/project_sharing", type="http", auth="user", methods=["GET"]
    )
    ...
    @http.route(
        "/my/projects/<int:project_id>/task/<int:task_id>", type="http", auth="public", website=True
    )
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        if project:
            ...
            try:
                project_accessible = bool(
                    task.project_id.id
                    and self._document_check_access("project.project", task.project_id.id)
                )
            except (AccessError, MissingError):
        ...
        if project:
            ...
            values["next_record"] = (
                current_task_index < total_task - 1 and task_url % history[current_task_index + 1]
            )
    ...
    def _prepare_tasks_values(
        ...
        def get_grouped_tasks(pager_offset):
            ...
            request.session[
                "my_project_tasks_history" if url.startswith("/my/projects") else "my_tasks_history"
            ] = tasks.ids[:100]
            ...
            if group:
                if group == "milestone_id":
                    grouped_tasks = [
                        Task_sudo.concat(*g)
                        for k, g in groupbyelem(tasks_project_allow_milestone, itemgetter(group))
                    ]
                    ...
                    grouped_tasks = [
                        Task_sudo.concat(*g) for k, g in groupbyelem(tasks, itemgetter(group))
                    ]
            else:
                grouped_tasks = [tasks]
