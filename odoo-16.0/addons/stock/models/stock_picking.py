...
class PickingType(models.Model):
    ...
    show_entire_packs = fields.Boolean(
        "Move Entire Packages", help="If ticked, you will be able to select entire packages to move"
    )
    ...
    company_id = fields.Many2one(
        "res.company", "Company", required=True, default=lambda s: s.env.company.id, index=True
    )
...
class Picking(models.Model):
    ...
    def _compute_show_qty_button(self):
        ...
        for picking in self:
            ...
            if any(
                ...
                not float_is_zero(m.quantity_done, precision_rounding=m.product_uom.rounding)
                and float_compare(
                    m.quantity_done,
    ...
    def _compute_has_deadline_issue(self):
        for picking in self:
            picking.has_deadline_issue = (
                picking.date_deadline and picking.date_deadline < picking.scheduled_date or False
            )
    ...
    def _compute_delay_alert_date(self):
        ...
        delay_alert_date_data = {
            data["picking_id"][0]: data["delay_alert_date"] for data in delay_alert_date_data
        }
        for picking in self:
    ...
    def _compute_state(self):
        ...
        for move in self.env["stock.move"].search([("picking_id", "in", self.ids)]):
            ...
            picking_moves_state_map[picking_id.id].update(
                {
                    "any_draft": picking_moves_state_map[picking_id.id].get("any_draft", False)
                    or move_state == "draft",
                    "all_cancel": picking_moves_state_map[picking_id.id].get("all_cancel", True)
                    ...
                    "any_cancel_and_not_scrapped": picking_moves_state_map[picking_id.id].get(
                        "any_cancel_and_not_scrapped", False
                    )
        ...
        for picking in self:
            ...
            if not picking_moves_state_map[picking_id]:
                ...
                if (
                    ...
                    and picking_moves_state_map[picking_id]["any_cancel_and_not_scrapped"]
                ):
                    picking.state = "cancel"
                else:
                    picking.state = "done"
    ...
    def _set_scheduled_date(self):
        for picking in self:
            if picking.state in ("done", "cancel"):
                raise UserError(
                    _("You cannot change the Scheduled Date on a done or cancelled transfer.")
                )
    ...
    def _get_show_allocation(self, picking_type_id):
        ...
        if lines:
            ...
            wh_location_ids = self.env["stock.location"]._search(
                [
                    ("id", "child_of", picking_type_id.warehouse_id.view_location_id.id),
                    ("usage", "!=", "supplier"),
                ]
            )
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(
            self.move_line_ids.filtered(lambda ml: ml.package_id == package), "reserved_qty"
        )
    ...
    def _get_lot_move_lines_for_sanity_check(self, none_done_picking_ids, separate_pickings=True):
        ...
        def get_line_with_done_qty_ids(move_lines):
            ...
            return move_lines.filtered(
                ...
                and float_compare(ml.qty_done, 0, precision_rounding=ml.product_uom_id.rounding)
            ).ids
        ...
        if separate_pickings:
            ...
            lines_to_check_ids = [
                line_id
                for picking in self
                for line_id in get_relevant_move_line_ids(none_done_picking_ids, picking)
            ]
        else:
    ...
    def _sanity_check(self, separate_pickings=True):
        ...
        for picking in self:
            ...
            if all(
                float_is_zero(
                    move_line.reserved_qty, precision_rounding=move_line.product_uom_id.rounding
                )
    ...
    def action_set_quantities_to_reservation(self):
        self.move_ids.filtered(
            lambda m: float_is_zero(m.quantity_done, precision_rounding=m.product_uom.rounding)
        )._set_quantities_to_reservation()

    def action_clear_quantities_to_zero(self):
        self.move_ids.filtered(
            lambda m: float_compare(
                m.quantity_done, m.reserved_availability, precision_rounding=m.product_uom.rounding
            )
            == 0
        ...
        for picking in self:
            ...
            if moves_to_backorder:
                backorder_picking = picking.copy(
                    {"name": "/", "move_ids": [], "move_line_ids": [], "backorder_id": picking.id}
                )
    ...
    def action_put_in_pack(self):
        ...
        if self.state not in ("done", "cancel"):
            ...
            if not move_line_ids:
                move_line_ids = picking_move_lines.filtered(
                    lambda ml: float_compare(
                        ml.reserved_uom_qty, 0.0, precision_rounding=ml.product_uom_id.rounding
                    )
                    > 0
                    and float_compare(
                        ml.qty_done, 0.0, precision_rounding=ml.product_uom_id.rounding
            ...
            if move_line_ids:
                ...
                raise UserError(
                    _("Please add 'Done' quantities to the picking to create a new pack.")
                )
