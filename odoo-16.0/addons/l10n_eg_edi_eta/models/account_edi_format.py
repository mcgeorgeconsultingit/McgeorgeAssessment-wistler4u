...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_get_einvoice_status(self, invoice):
        ...
        if access_data.get("error"):
            return access_data
        request_url = (
            f"/api/v1.0/documentsubmissions/{url_quote(invoice.l10n_eg_submission_number)}"
        )
        request_data = {
            "body": None,
            "header": {
    ...
    def _l10n_eg_validate_info_address(self, partner_id, issuer=False, invoice=False):
        ...
        if (
            invoice and invoice.amount_total >= invoice.company_id.l10n_eg_invoicing_threshold
            ...
            lines.append(
                {
                    ...
                    "taxableItems": [
                        ...
                        for tax_details in line_tax_details.get("tax_details", {}).values()
    ...
    def _check_move_configuration(self, invoice):
        ...
        if not all(
            ...
            errors.append(
                _("Please make sure the invoice lines UoM codes are all set up correctly")
            )
        if not all(
            ...
            errors.append(
                _("Please make sure the invoice lines taxes all have the correct ETA tax code")
            )
        if not all(
            ...
            errors.append(
                _("Please make sure the EGS/GS1 Barcode is set correctly on all products")
            )
        return errors
