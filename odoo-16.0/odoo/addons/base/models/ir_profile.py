    ...
    def set_profiling(self, profile=None, collectors=None, params=None):
        ...
        if profile:
            ...
            if not limit:
                ...
                raise UserError(
                    _("Profiling is not enabled on this database. Please contact an administrator.")
                )
...
class EnableProfilingWizard(models.TransientModel):
    ...
    expiration = fields.Datetime(
        "Enable profiling until", compute="_compute_expiration", store=True, readonly=False
    )
