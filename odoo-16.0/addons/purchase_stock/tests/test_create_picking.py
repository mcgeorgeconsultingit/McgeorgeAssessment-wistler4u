...
class TestCreatePicking(common.TestProductCommon):
    ...
    def test_02_check_mto_chain(self):
        ...
        self.assertEqual(
            customer_move.procure_method, "make_to_stock", "Customer move should be passed to mts."
        )
    ...
    def test_04_mto_multiple_po(self):
        ...
        self.assertTrue(
            customer_move_2.exists(), "The second customer move should not be merged in the first."
        )
    ...
    def test_08_check_update_qty_mto_chain(self):
        ...
        self.assertEqual(
            len(purchase_orders), 1, "No RFQ should have been created for a negative demand"
        )
        ...
        purchase_orders = self.env["purchase.order"].search([("partner_id", "=", partner.id)])
        self.assertEqual(
            len(purchase_orders), 2, "A new RFQ should have been created for missing demand."
        )
