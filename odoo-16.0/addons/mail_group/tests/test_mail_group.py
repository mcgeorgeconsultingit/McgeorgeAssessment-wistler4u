...
class TestMailGroup(TestMailListCommon):
    ...
    def test_constraint_valid_email(self):
        mail_group = (
            self.env["mail.group"].with_user(self.user_employee).browse(self.test_group.ids)
        )
    ...
    def test_find_member_for_alias(self):
        ...
        self.assertFalse(
            self.test_group._alias_get_error_message({}, msg_dict, self.test_group.alias_id)
        )
    ...
    def test_mail_group_access_mode_groups(self):
        ...
        with self.assertRaises(
            AccessError, msg="Only moderator / responsible and admin can write on the group"
        ):
