...
class StockPutawayRule(models.Model):
    ...
    def _default_product_id(self):
        if self.env.context.get("active_model") == "product.template" and self.env.context.get(
            "active_id"
        ):
    ...
    def _domain_category_id(self):
        ...
        if active_model in ("product.template", "product.product") and self.env.context.get(
            "active_id"
        ):
    ...
    storage_category_id = fields.Many2one(
        "stock.storage.category", "Storage Category", ondelete="cascade", check_company=True
    )
            ...
            for location in child_locations:
                ...
                if package_type:
                    if location.quant_ids.filtered(
                        lambda q: q.package_id and q.package_id.package_type_id == package_type
                    ):
                    ...
                    float_compare(
                        qty_by_location[location.id], 0, precision_rounding=product.uom_id.rounding
                    )
                    > 0
                ):
                    if location._check_can_be_used(
