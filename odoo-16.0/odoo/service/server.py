...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def server_bind(self):
        ...
        if os.environ.get("LISTEN_FDS") == "1" and os.environ.get("LISTEN_PID") == str(os.getpid()):
            ...
            _logger.info(
                "HTTP service (werkzeug) running on %s:%s", self.server_name, self.server_port
            )
            ...
            while self.quit_signals_received == 0:
                ...
                if self.limit_reached_time:
                    ...
                    if (
                        ...
                        dumpstacks(
                            thread_idents=[thread.ident for thread in self.limits_reached_threads]
                        )
                        self.reload()
...
class Worker(object):
    ...
    def _runloop(self):
        ...
        try:
            ...
            _logger.exception(
                "Worker %s (%s) Exception occurred, exiting...", self.__class__.__name__, self.pid
            )
            sys.exit(1)
...
def load_test_file_py(registry, test_file):
    ...
    try:
        ...
        for mod in [
            m for m in get_modules() if "%s%s%s" % (os.path.sep, m, os.path.sep) in test_file
        ]:
            for mod_mod in loader.get_test_modules(mod):
                mod_path, _ = os.path.splitext(getattr(mod_mod, "__file__", ""))
                if test_path == config._normalize(mod_path):
                    tests = loader.unwrap_suite(unittest.TestLoader().loadTestsFromModule(mod_mod))
