...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_mail_composer_form_attachments(self):
        ...
        with self.assertQueryCount(__system__=12, employee=12):
            composer_form = Form(
        ...
        with self.assertQueryCount(__system__=46, employee=54):  # tm+com 39/47
            composer._action_send_mail()
    ...
    def test_mail_composer_w_template_form(self):
        ...
        customer = self.env["res.partner"].browse(self.customer.ids)
        with self.assertQueryCount(__system__=34, employee=36):  # tm 24/26 / com 33/35
            composer_form = Form(
                self.env["mail.compose.message"].with_context(
                    {
                        "default_composition_mode": "comment",
        ...
        self.assertFalse(message.attachment_ids)
        self.assertEqual(
            message.notified_partner_ids, customer + self.user_test.partner_id + new_partner
        )
    ...
    def test_mail_composer_w_template_form_attachments(self):
        ...
        customer = self.env["res.partner"].browse(self.customer.ids)
        with self.assertQueryCount(__system__=35, employee=37):  # tm 25/27 / com 34/36
            composer_form = Form(
                self.env["mail.compose.message"].with_context(
                    {
                        "default_composition_mode": "comment",
    ...
    def test_message_log_batch(self):
        ...
        with self.assertQueryCount(__system__=1, employee=1):
            records._message_log_batch(
                bodies=dict((record.id, "<p>Test _message_log</p>") for record in records),
    ...
    def test_message_subscribe_subtypes(self):
        record = self.env["mail.test.simple"].create({"name": "Test"})
        subtype_ids = (
            self.env.ref("test_mail.st_mail_test_simple_external") | self.env.ref("mail.mt_comment")
        ).ids
    ...
    def test_notification_reply_to_batch(self):
        ...
        with self.assertQueryCount(__system__=1, employee=1):
            ...
            reply_to = test_records._notify_get_reply_to(default=self.env.user.email_formatted)
...
class TestMailComplexPerformance(BaseMailPerformance):
    ...
    def test_complex_mail_mail_send_batch_complete(self):
        ...
        mails[-2].write(
            {"email_cc": False, "email_to": "strange@example¢¡.com", "recipient_ids": [(5, 0)]}
        )
        ...
        with (
            self.assertQueryCount(__system__=44, employee=44),
    ...
    def test_complex_message_post(self):
        ...
        with self.assertQueryCount(__system__=53, employee=54):
            record.message_post(
        ...
        self.assertEqual(
            record.message_ids[0].notified_partner_ids, self.partners | self.user_portal.partner_id
        )
    ...
    def test_complex_message_post_template(self):
        ...
        # about 20 (19 ?) queries per additional customer group
        with self.assertQueryCount(__system__=61, employee=62):
    ...
    def test_complex_message_subscribe(self):
        ...
        self.assertEqual(
            rec1.message_partner_ids, self.env.user.partner_id | self.user_portal.partner_id
        )
    ...
    def test_complex_tracking_subscription_create(self):
        ...
        with self.assertQueryCount(__system__=91, employee=92):  # tm 91/92
            rec = self.env["mail.test.ticket"].create(
                {
                    "name": "Test",
        ...
        self.assertEqual(
            rec1.message_ids[0].notified_partner_ids, self.partners | self.user_portal.partner_id
        )
    ...
    def test_complex_tracking_subscription_subtype(self):
        ...
        rec1 = rec.with_context(active_test=False)  # to see inactive records
        self.assertEqual(
            rec1.message_partner_ids, self.user_portal.partner_id | self.env.user.partner_id
        )
        self.assertEqual(len(rec1.message_ids), 1)
    ...
    def test_complex_tracking_template(self):
        ...
        self.assertEqual(
            rec1.message_ids[2].notified_partner_ids, self.partners | self.user_portal.partner_id
        )
