...
class TestItEdiReverseCharge(TestItEdi):
    ...
    def setUpClass(cls):
        ...
        def repartition_lines(*lines):
            ...
            return [(5, 0, 0)] + [
                (0, 0, {**line._asdict(), "tag_ids": get_tag_ids(line[2])}) for line in lines
            ]
        ...
        cls.purchase_tax_4p_already_in_italy = (
            cls.env["account.tax"].with_company(cls.company).create(tax_data_4p_already_in_italy)
        )
        ...
        cls.reverse_charge_bill_san_marino = (
            cls.env["account.move"].with_company(cls.company).create(bill_data_san_marino)
        )
