...
class TestDataRecycle(TransactionCase):
        ...
        cls.recycle_model = cls.env["data_recycle.model"].create(
            {
                ...
                .search([("name", "=", "date"), ("model_id", "=", cls.partner_model.id)], limit=1)
                .id,
                "time_field_delta": 1,
    ...
    def test_recycle_flow(self):
        ...
        self.assertEqual(
            set(self.recycle_model.recycle_record_ids.mapped("res_id")), set(self.old_partners.ids)
        )
    ...
    def test_recycle_domain(self):
        ...
        self.assertTrue(
            self.old_partners[0].id not in self.recycle_model.recycle_record_ids.mapped("res_id")
        )
