...
class TestPartner(MailCommon):
    ...
    def test_res_partner_find_or_create(self):
        ...
        self._check_find_or_create(
            SAMPLES[8][0], SAMPLES[8][1], SAMPLES[8][2], check_partner=new6, should_create=True
        )
    ...
    def test_res_partner_get_mention_suggestions_priority(self):
        ...
        self.assertEqual(
            list(
                map(
                    lambda p: isinstance(p["user"], dict) and p["user"]["isInternalUser"],
                    partners_format,
                )
            ),
    ...
    def test_res_partner_merge_wizards(self):
        ...
        self.assertEqual(
            len(p2.message_ids), len(all_msg) + 1, "Should have original messages + a log"
        )
