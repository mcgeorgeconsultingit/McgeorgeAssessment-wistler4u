        ...
        for campaign in self:
            campaign.ab_testing_total_pc = sum(
                [
                    ...
                    for mailing in campaign.mailing_mail_ids.filtered("ab_testing_enabled")
                ]
            )
        ...
        if self.ids:
            ...
            for data in mailing_data:
                if data["ab_testing_enabled"]:
                    ab_testing_mapped_data.setdefault(data["campaign_id"][0], []).append(
                        data["__count"]
                    )
        ...
        for campaign in self:
            if not campaign.ab_testing_completed and campaign.ab_testing_total_pc >= 100:
                raise ValidationError(
                    _("The total percentage for an A/B testing campaign should be less than 100%")
                )
