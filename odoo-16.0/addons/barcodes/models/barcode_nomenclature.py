...
class BarcodeNomenclature(models.Model):
    ...
    rule_ids = fields.One2many(
        "barcode.rule", "barcode_nomenclature_id", string="Rules", help="The list of barcode rules"
    )
    ...
    def match_pattern(self, barcode, pattern):
        ...
        if numerical_content:  # the pattern encodes a numerical content
            ...
            decimal_part = (
                "0." + value_string[decimal_part_match.start() : decimal_part_match.end() - 1]
            ...
            match["base_code"] = (
                barcode[:num_start] + (num_end - num_start - 2) * "0" + barcode[num_end - 2 :]
            ...
            pattern = (
                pattern[:num_start] + (num_end - num_start - 2) * "0" + pattern[num_end:]
