...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_refund(self):
        ...
        order_payment = self.PosMakePayment.with_context(**payment_context).create(
            {"amount": order.amount_total, "payment_method_id": self.cash_payment_method.id}
        )
        ...
        self.assertEqual(
            current_session.state, "closed", msg="State of current session should be closed."
        )
    ...
    def test_order_to_picking(self):
        ...
        self.assertEqual(
            self.pos_order_pos1.picking_ids[0].state, "done", "Picking should be in done state."
        )
        self.assertEqual(
        ...
        # One picking attached and having all the positive move lines in the correct state
        self.assertEqual(
            self.pos_order_pos2.picking_ids[0].state, "done", "Picking should be in done state."
        )
        self.assertEqual(
        ...
        # One picking attached and having all the positive move lines in the correct state
        self.assertEqual(
            self.pos_order_pos3.picking_ids[0].state, "done", "Picking should be in done state."
        )
        self.assertEqual(
    ...
    def test_create_from_ui(self):
        ...
        self.assertEqual(
            num_starting_orders + 1, len(current_session.order_ids), "Submitted order not encoded"
        )
        ...
        self.assertFalse(
            self.pos_config.current_session_id, "Current session not properly recomputed"
        )
        ...
        self.assertEqual(
            len(rescue_session.order_ids), 2, "Rescue session does not contain both orders"
        )
    ...
    def test_order_to_payment_currency(self):
        ...
        self.assertTrue(
            current_session.move_id, "Journal entry should have been attached to the session."
        )
    ...
    def test_order_to_invoice_no_tax(self):
        ...
        self.assertEqual(self.pos_order_pos1.state, "paid", "Order should be in paid state.")
        self.assertFalse(
            self.pos_order_pos1.account_move, "Invoice should not be attached to order yet."
        )
        ...
        with freeze_time("2020-01-03"):
            ...
            for closing_entry_line, misc_reversal_entry_line in zip(
                ...
                if closing_entry_line.balance < 0:
                    ...
                    self.assertEqual(
                        closing_entry_line.account_id, misc_reversal_entry_line.account_id
                    )
            ...
            self.assertEqual(
                invoice_receivable_line.matching_number, payment_receivable_line.matching_number
            )
            ...
            payment_receivable_pos_line = payment.line_ids.filtered(
                ...
                == self.company_data["company"].account_default_pos_receivable_account_id
            )
