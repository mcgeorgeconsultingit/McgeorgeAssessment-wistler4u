...
def rpm_sign(args, file_name):
    ...
    rpmsign = pexpect.spawn(
        "/bin/bash", ["-c", "rpm --resign %s" % file_name], cwd=os.path.join(args.pub, "rpm")
    )
...
class Docker:
    ...
    def build_image(self):
        ...
        run_cmd(
            ["docker", "build", "--rm=True", "-t", self.tag, "."], chdir=docker_dir, timeout=1200
        ).check_returncode()
...
class KVM(object):
    ...
    def rsync(
        self,
        rsync_args,
        options=["--delete", "--exclude", ".git", "--exclude", ".tx", "--exclude", "__pycache__"],
    ):
        cmd = [
            "rsync",
            "-a",
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.version"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.python"), "w", encoding="utf-8"
        ) as f:
        ...
        self.rsync(
            ["%s/" % self.args.build_dir, "%s@127.0.0.1:%s" % (self.login, remote_build_dir)]
        )
...
def parse_args():
    ...
    ap.add_argument(
        "-b", "--build-dir", default=build_dir, help="build directory (%(default)s)", metavar="DIR"
    )
    ...
    # Windows VM
    ap.add_argument(
        "--vm-winxp-image", default="/home/odoo/vm/win1036/win10_winpy36.qcow2", help="%(default)s"
    )
    ap.add_argument(
