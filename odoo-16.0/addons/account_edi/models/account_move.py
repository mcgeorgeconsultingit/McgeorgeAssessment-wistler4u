...
class AccountMove(models.Model):
    ...
    edi_web_services_to_process = fields.Text(compute="_compute_edi_web_services_to_process")
    ...
    def _prepare_edi_tax_details(
        self, filter_to_apply=None, filter_invl_to_apply=None, grouping_key_generator=None
    ):
    ...
    def _prepare_edi_vals_to_export(self):
        ...
        for index, line in enumerate(
            self.invoice_line_ids.filtered(lambda line: line.display_type == "product"), start=1
        ):
...
class AccountMoveLine(models.Model):
    ...
    def _prepare_edi_vals_to_export(self):
        ...
        res = {
            ...
            "price_subtotal_unit": self.currency_id.round(self.price_subtotal / self.quantity)
            if self.quantity
            else 0.0,
            ...
            "price_discount_unit": (gross_price_subtotal - self.price_subtotal) / self.quantity
            if self.quantity
            else 0.0,
            "gross_price_total_unit": self.currency_id.round(gross_price_subtotal / self.quantity)
            if self.quantity
            else 0.0,
    ...
    def reconcile(self):
        ...
        all_lines = (
            self + self.matched_debit_ids.debit_move_id + self.matched_credit_ids.credit_move_id
        )
