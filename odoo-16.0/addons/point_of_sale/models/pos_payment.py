...
class PosPayment(models.Model):
    ...
    payment_date = fields.Datetime(
        string="Date", required=True, readonly=True, default=lambda self: fields.Datetime.now()
    )
    ...
    session_id = fields.Many2one(
        "pos.session", string="Session", related="pos_order_id.session_id", store=True, index=True
    )
            ...
            if payment.name:
                res.append(
                    (
                        ...
                        % (
                            payment.name,
                            formatLang(self.env, payment.amount, currency_obj=payment.currency_id),
                        ),
                    )
                )
            else:
