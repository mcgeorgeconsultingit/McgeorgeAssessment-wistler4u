...
class StockMoveLine(models.Model):
    ...
    product_category_name = fields.Char(
        related="product_id.categ_id.complete_name", store=True, string="Product Category"
    )
    ...
    consume_line_ids = fields.Many2many(
        "stock.move.line", "stock_move_line_consume_rel", "consume_line_id", "produce_line_id"
    )
    produce_line_ids = fields.Many2many(
        "stock.move.line", "stock_move_line_consume_rel", "produce_line_id", "consume_line_id"
    )
    ...
    def _onchange_qty_done(self):
        ...
        if self.qty_done and self.product_id.tracking == "serial":
            ...
            if (
                float_compare(qty_done, 1.0, precision_rounding=self.product_id.uom_id.rounding)
                != 0
            ):
                message = _(
        ...
        for package, smls in groupby(self, lambda sml: sml.result_package_id):
            ...
            if package.package_type_id:
                ...
                for sml in smls:
                    ...
                    )._get_putaway_strategy(sml.product_id, quantity=sml.reserved_uom_qty)
                    excluded_smls -= sml
    ...
    def write(self, vals):
        ...
        if "product_id" in vals and any(
            vals.get("state", ml.state) != "draft" and vals["product_id"] != ml.product_id.id
            for ml in self
        ):
        ...
        if updates or "qty_done" in vals:
            ...
            mls = self.filtered(
                lambda ml: ml.move_id.state == "done" and ml.product_id.type == "product"
            )
        ...
        for ml in mls_todo:
            if ml.product_id.type == "product":
                ...
                if (
                    ...
                    and float_compare(ml.qty_done, ml.reserved_uom_qty, precision_rounding=rounding)
                    > 0
                ):
    ...
    def _log_message(self, record, move, template, vals):
        ...
        if "package_result_id" in vals and vals["package_result_id"] != move.package_result_id.id:
            data["result_package_name"] = (
                self.env["stock.quant.package"].browse(vals.get("result_package_id")).name
            )
    ...
    def _free_reservation(
        ...
        if quantity > available_quantity:
            ...
            for candidate in outdated_candidates:
                if (
                    float_compare(candidate.reserved_qty, quantity, precision_rounding=rounding)
                    <= 0
                ):
                    ...
                    candidate.reserved_uom_qty = self.product_id.uom_id._compute_quantity(
                        quantity_split, candidate.product_uom_id, rounding_method="HALF-UP"
                    )
                    move_to_recompute_state |= candidate.move_id
                    break
    ...
    def _get_aggregated_product_quantities(self, **kwargs):
        ...
        for move_line in self:
            ...
            if line_key not in aggregated_move_lines:
                ...
                if backorders and not kwargs.get("strict"):
                    ...
                    qty_ordered -= sum(
                        map(
                            lambda m: m.product_uom_id._compute_quantity(m.qty_done, uom),
                            previous_move_lines,
                        )
                    )
        ...
        for empty_move in pickings.move_ids:
            if not (
                ...
                and float_is_zero(
                    empty_move.quantity_done, precision_rounding=empty_move.product_uom.rounding
                )
            ):
                continue
