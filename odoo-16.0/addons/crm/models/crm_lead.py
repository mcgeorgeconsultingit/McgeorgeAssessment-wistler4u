...
PLS_COMPUTE_BATCH_STEP = (
    50000  # odoo.models.PREFETCH_MAX = 1000 but larger cluster can speed up global computation
)
...
class Lead(models.Model):
    ...
    company_currency = fields.Many2one(
        "res.currency", string="Currency", compute="_compute_company_currency", compute_sudo=True
    )
    # Dates
    ...
    contact_name = fields.Char(
        "Contact Name", tracking=30, compute="_compute_contact_name", readonly=False, store=True
    )
    ...
    title = fields.Many2one(
        "res.partner.title", string="Title", compute="_compute_title", readonly=False, store=True
    )
    ...
    lang_id = fields.Many2one(
        "res.lang", string="Language", compute="_compute_lang_id", readonly=False, store=True
    )
    ...
    automated_probability = fields.Float(
        "Automated Probability", compute="_compute_probabilities", readonly=True, store=True
    )
    ...
    lost_reason_id = fields.Many2one(
        "crm.lost.reason", string="Lost Reason", index=True, ondelete="restrict", tracking=True
    )
    # Statistics
    ...
    duplicate_lead_count = fields.Integer(
        compute="_compute_potential_lead_duplicates", string="Potential Duplicate Lead Count"
    )
    # UX
    ...
    def _compute_phone_state(self):
        for lead in self:
            ...
            if lead.phone:
                country_code = (
                    lead.country_id.code if lead.country_id and lead.country_id.code else None
                )
                try:
    ...
    def _compute_is_automated_probability(self):
        ...
        for lead in self:
            lead.is_automated_probability = (
                tools.float_compare(lead.probability, lead.automated_probability, 2) == 0
            )
    ...
    def _compute_recurring_revenue_monthly_prorated(self):
        for lead in self:
            lead.recurring_revenue_monthly_prorated = (
                (lead.recurring_revenue_monthly or 0.0) * (lead.probability or 0) / 100.0
            )
    ...
    def _compute_potential_lead_duplicates(self):
        ...
        for lead in self:
            ...
            if lead.partner_name and len(lead.partner_name) >= MIN_NAME_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead", common_lead_domain + [("partner_name", "ilike", lead.partner_name)]
                )
            if lead.contact_name and len(lead.contact_name) >= MIN_NAME_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead", common_lead_domain + [("contact_name", "ilike", lead.contact_name)]
                )
            ...
            if lead.phone and len(lead.phone) >= MIN_PHONE_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead", common_lead_domain + [("phone_mobile_search", "ilike", lead.phone)]
                )
            if lead.mobile and len(lead.mobile) >= MIN_PHONE_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead", common_lead_domain + [("phone_mobile_search", "ilike", lead.mobile)]
                )
    ...
    def _onchange_phone_validation(self):
        if self.phone:
            self.phone = (
                self.phone_get_sanitized_number(number_fname="phone", force_format="INTERNATIONAL")
                or self.phone
            )
    ...
    def _onchange_mobile_validation(self):
        if self.mobile:
            self.mobile = (
                self.phone_get_sanitized_number(number_fname="mobile", force_format="INTERNATIONAL")
                or self.mobile
            )
    ...
    def _get_partner_email_update(self):
        ...
        if self.partner_id and self.email_from != self.partner_id.email:
            ...
            partner_email_normalized = (
                tools.email_normalize(self.partner_id.email) or self.partner_id.email or False
            )
    ...
    def _get_partner_phone_update(self):
        ...
        if self.partner_id and self.phone != self.partner_id.phone:
            lead_phone_formatted = (
                self.phone_get_sanitized_number(number_fname="phone") or self.phone or False
            )
    ...
    def _auto_init(self):
        ...
        tools.create_index(
            self._cr, "crm_lead_create_date_team_id_idx", self._table, ["create_date", "team_id"]
        )
        return res
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        order_items = [
            order_item.strip().lower() for order_item in (order or self._order).split(",")
        ]
        ...
        my_lead_ids_ordered = sorted(
            search_res.ids, key=lambda lead_id: my_lead_mapping[lead_id], reverse=not activity_asc
        )
    ...
    def _get_rainbowman_message(self):
        ...
        """
        self.env.cr.execute(query, {"user_id": self.user_id.id, "team_id": self.team_id.id})
    ...
    def _merge_get_fields_address(self):
        ...
        source_lead = max(
            self,
            key=lambda lead: len(
                list(lead[field] for field in PARTNER_ADDRESS_FIELDS_TO_SYNC if lead[field])
            ),
        )
    ...
    def _format_properties(self):
        ...
        for definition in properties:
            ...
            if property_type == "boolean":
                ...
                options = {
                    option[0]: option[1:] for option in (definition.get(property_type) or [])
                }
    ...
    def _create_customer(self):
        ...
        if not contact_name:
            contact_name = (
                Partner._parse_partner_name(self.email_from)[0] if self.email_from else False
            )
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        result = dict(
            (team_id, dict((field, dict(won_total=0, lost_total=0)) for field in leads_fields))
    ...
    def _pls_increment_frequencies(self, from_state=None, to_state=None):
        ...
        ) = self._pls_prepare_update_frequency_table(target_state=from_state or to_state)
    ...
    def _pls_get_lead_pls_values(self, domain=[]):
        ...
        if domain:
            ...
            from_clause, where_clause, where_params = (
                self.env["crm.lead"].with_context(active_test=False)._where_calc(domain).get_sql()
            )
            ...
            for lead in self:
                ...
                for field in pls_fields:
                    if (
                        ...
                        continue
                    value = (
                        lead[field].id if isinstance(lead[field], models.BaseModel) else lead[field]
                    )
                    if value or field in ("email_state", "phone_state"):
                        lead_values.append((field, value))
                if use_tags:
