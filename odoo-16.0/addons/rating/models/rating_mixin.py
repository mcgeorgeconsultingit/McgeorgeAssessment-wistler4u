...
class RatingMixin(models.AbstractModel):
    ...
    def _compute_rating_stats(self):
        ...
        mapping = {
            item["res_id"]: {"rating_count": item["__count"], "rating_avg": item["rating"]}
            ...
            for record in self:
                ...
                if record._rating_get_parent_field_name() in values:
                    record.rating_ids.sudo().write(
                        {"parent_res_id": record[record._rating_get_parent_field_name()].id}
                    )
    ...
    def rating_apply(
        ...
        if rate < 0 or rate > 5:
            raise ValueError(
                "Wrong rating value. A rate should be between 0 and 5 (received %d)." % rate
            )
        if token:
        ...
        if issubclass(type(self), self.env.registry["mail.thread"]):
            ...
            scheduled_datetime = (
                fields.Datetime.now() + datetime.timedelta(hours=2) if notify_delay_send else None
            )
            ...
            if rating.message_id:
                self._message_update_content(
                    rating.message_id, rating_body, scheduled_date=scheduled_datetime, strict=False
                )
            else:
                self.message_post(
