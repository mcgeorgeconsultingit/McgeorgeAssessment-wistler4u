    ...
    def __init__(self, name, bases, attrs):
        ...
        if not self._abstract and self._name not in self._inherit:
            ...
            if attrs.get("_log_access", self._auto):
                ...
                add_default(
                    "write_date",
                    fields.Datetime(string="Last Updated on", automatic=True, readonly=True),
                )
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def load(self, fields, data):
        ...
        def flush(*, xml_id=None, model=None):
            ...
            try:
                ...
                if not any(message["type"] == "error" for message in messages):
                    ...
                    messages.append(
                        dict(info, type="error", message=_("Unknown database error: '%s'", e))
                    )
                return
            except Exception:
                pass
            ...
            for i, rec_data in enumerate(data_list, 1):
                try:
                    ...
                    info = rec_data["info"]
                    messages.append(
                        dict(info, type="error", **PGERROR_TO_OE[e.pgcode](self, fg, info, e))
                    )
        ...
        for id, xid, record, info in converted:
            if self.env.context.get("import_file") and self.env.context.get("import_skip_records"):
                if any(
                    [record.get(field) is None for field in self.env.context["import_skip_records"]]
                ):
                    continue
            if xid:
    ...
    def _extract_records(self, fields_, data, log=lambda a: None, limit=float("inf")):
        ...
        * ".id" is the Database ID for the record
        ...
        get_o2m_values = itemgetter_tuple(
            [index for index, fnames in enumerate(fields_) if fields[fnames[0]].type == "one2many"]
        )
    ...
    def _validate_fields(self, field_names, excluded_names=()):
        ...
        for check in self._constraint_methods:
            if not field_names.isdisjoint(check._constrains) and excluded_names.isdisjoint(
                check._constrains
            ):
                check(self)
    ...
    def _name_search(self, name="", args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        # optimize out the default criterion of ``like ''`` that matches everything
            ...
            aggregator = (
                expression.AND if operator in expression.NEGATIVE_TERM_OPERATORS else expression.OR
            )
    ...
    def _add_missing_default_values(self, values):
        ...
        missing_defaults = {
            name for name, field in self._fields.items() if name not in values if not avoid(field)
        }
    ...
    def _read_group_fill_temporal(
        ...
        empty_item.update(
            {key: False for key in [group["groupby"] for group in annotated_groupbys[1:]]}
        )
    ...
    def _read_group_prepare(self, orderby, aggregated_fields, annotated_groupbys, query):
        ...
        groupby_fields = {
            gb[key]: gb["groupby"] for gb in annotated_groupbys for key in ("field", "groupby")
        }
            ...
            if order_field == "id" or order_field in groupby_fields:
                ...
                if self._fields[order_field_name].type == "many2one" and not is_many2one_id:
                    ...
                    if order_clause:
                        ...
                        groupby_terms += [
                            order_term.split()[0] for order_term in order_clause.split(",")
                        ]
                else:
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        groupby = (
            [groupby] if isinstance(groupby, str) else groupby[:1] if lazy else OrderedSet(groupby)
        )
        groupby_dates = [
            groupby_description
    ...
    def _read_group_raw(
        ...
        for fspec in fields:
            ...
            if func:
                ...
                if not (
                    field.base_field.store and field.base_field.column_type and field.group_operator
                ):
                    continue
        ...
        result = [
            self._read_group_format_result(d, annotated_groupbys, groupby, domain) for d in data
        ]
    ...
    def _read_group_resolve_many2x_fields(self, data, fields):
        many2xfields = {
            field["field"] for field in fields if field["type"] in ["many2one", "many2many"]
        }
    ...
    def _check_removed_columns(self, log=False):
        ...
        # another module)
        cr = self._cr
        cols = [name for name, field in self._fields.items() if field.store and field.column_type]
        cr.execute(
    ...
    def _update_field_translations(self, field_name, translations, digest=None):
        ...
        if field.translate is True:
            ...
            if any(
                ...
                raise UserError(
                    _("Translations for model translated fields only accept falsy values and str")
                )
    ...
    def get_field_translations(self, field_name, langs=None):
        ...
        if not callable(field.translate):
            translations = [
                {"lang": lang, "source": val_en, "value": self.with_context(lang=lang)[field_name]}
                for lang in langs
            ]
        else:
    ...
    def _filter_access_rules(self, operation):
        ...
        return self.browse(
            [it for it in self._ids if not (it or it.origin) or (it or it.origin) in valid_ids]
        )
    ...
    def write(self, vals):
        ...
        * Other non-relational fields use a string for value
        ...
        to_compute = [field.name for field in protected if field.compute and field.name not in vals]
        if to_compute:
    ...
    def _add_precomputed_values(self, vals_list):
        ...
        """
        precomputable = {fname: field for fname, field in self._fields.items() if field.precompute}
        if not precomputable:
            return

        # determine which vals must be completed
        vals_list_todo = [
            vals for vals in vals_list if any(fname not in vals for fname in precomputable)
        ]
    ...
    def _create(self, data_list):
        ...
        for data, record in zip(data_list, records):
            ...
            vals = dict(
                {k: v for d in data["inherited"].values() for k, v in d.items()}, **data["stored"]
            )
    ...
    def _load_records(self, data_list, update=False):
        ...
        existing = {("%s.%s" % row[1:3]): row for row in imd._lookup_xmlids(xml_ids, self)}
    ...
    @api.model
    def _search(
        self, domain, offset=0, limit=None, order=None, count=False, access_rights_uid=None
    ):
        """
    ...
    def _check_m2m_recursion(self, field_name):
        ...
        field = self._fields.get(field_name)
        if not (
            field and field.type == "many2many" and field.comodel_name == self._name and field.store
        ):
        ...
        for model_name, fields_ in model_fields.items():
            ...
            if any(field in dirty_fields for field in fields_):
                ...
                context_none = dict.fromkeys(
                    key for field in fields_ for key in self.pool.field_depends_context[field]
                )
    ...
    def _has_onchange(self, field, other_fields):
        ...
        return (field.name in self._onchange_methods) or any(
            dep in other_fields for dep in self.pool.get_dependent_fields(field.base_field)
        )
    ...
    def _onchange_eval(self, field_name, onchange, result):
        ...
        def process(res):
            ...
            if res.get("domain"):
                _logger.warning(
                    "onchange method %s returned a domain, this is deprecated", method.__qualname__
                )
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        for name, subnames in nametree.items():
            if subnames and values.get(name):
                ...
                fnames = [
                    subname for subname in subnames if lines._fields[subname].base_field.store
                ]
                lines._read(fnames)
        ...
        while todo:
            ...
            todo = [name for name in nametree if name not in done and snapshot0.has_changed(name)]
...
def convert_pgerror_not_null(model, fields, info, e):
    if e.diag.table_name != model._table:
        return {"message": _("Missing required value for the field '%s'", e.diag.column_name)}

    field_name = e.diag.column_name
    field = fields[field_name]
    message = _("Missing required value for the field '%s' (%s)", field["string"], field_name)
    return {
        "message": message,
        "field": field_name,
    }
