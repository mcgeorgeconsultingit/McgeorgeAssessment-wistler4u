...
class MailActivityMixin(models.AbstractModel):
    ...
    def _compute_activity_exception_type(self):
        ...
        for record in self:
            ...
            record.activity_exception_decoration = (
                exception_activity_type_id and exception_activity_type_id.decoration_type
            )
    ...
    def _read_progress_bar(self, domain, group_by, progress_bar):
        ...
        annotated_groupbys = [
            self._read_group_process_groupby(gb, query) for gb in [group_by, "activity_state"]
        ]
        ...
        select_terms = [
            '%s as "%s"' % (gb["qualified_field"], gb["groupby"]) for gb in annotated_groupbys
        ]
        ...
        data = [
            {key: self._read_group_prepare_data(key, val, groupby_dict) for key, val in row.items()}
    ...
    def toggle_active(self):
        ...
        if record_to_deactivate:
            ...
            self.env["mail.activity"].sudo().search(
                [("res_model", "=", self._name), ("res_id", "in", record_to_deactivate.ids)]
            ).unlink()
    ...
    def activity_search(self, act_type_xmlids="", user_id=None, additional_domain=None):
        ...
        activity_types_ids = [
            type_id
            for type_id in (
                Data._xmlid_to_res_id(xmlid, raise_if_not_found=False) for xmlid in act_type_xmlids
            )
            if type_id
        ]
        ...
        **act_values
        ...
        for record in self:
            ...
            note = self.env["ir.qweb"]._render(
                view_ref, render_context, minimal_qcontext=True, raise_if_not_found=False
            )
