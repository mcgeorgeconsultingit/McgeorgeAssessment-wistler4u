...
class ProjectTaskRecurrence(models.Model):
    ...
    def _check_repeat_until_month(self):
        if self.filtered(
            ...
            and monthrange(r.repeat_until.year, r.repeat_until.month)[1] != r.repeat_until.day
        ):
            raise ValidationError(
                _("The end date should be after the day of the month or the last day of the month")
            )
    ...
    def _get_next_recurring_dates(
        ...
        **kwargs
        ...
        if repeat_unit == "day":
            ...
            month = (
                list(MONTHS.keys()).index(repeat_month) + 1 if repeat_month else date_start.month
            )
    ...
    def _new_task_values(self, task):
        ...
        create_values["stage_id"] = (
            task.project_id.type_ids[0].id if task.project_id.type_ids else task.stage_id.id
        )
