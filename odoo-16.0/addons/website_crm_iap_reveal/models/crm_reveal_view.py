...
class CRMRevealView(models.Model):
    ...
    def init(self):
        self._cr.execute(
            "SELECT indexname FROM pg_indexes WHERE indexname = %s", ("crm_reveal_view_ip_rule_id",)
        )
    ...
    def _clean_reveal_views(self):
        ...
        domain.append(
            (
                "create_date",
                "<",
                fields.Datetime.to_string(datetime.date.today() - relativedelta(weeks=weeks_valid)),
            )
        )
