...
class Project(models.Model):
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "form" and self.env.company.timesheet_encode_uom_id == self.env.ref(
            "uom.product_uom_day"
        ):
    ...
    def _search_pricing_type(self, operator, value):
        ...
        if not (
            ...
            or (isinstance(value, str) and value in ("task_rate", "fixed_rate", "employee_rate"))
        ):
    ...
    def _get_profitability_items_from_aal(self, profitability_items, with_action=True):
        ...
        def convert_dict_into_profitability_data(d, cost=True):
            ...
            for invoice_type, vals in d.items():
                ...
                if record_ids:
                    if (
                        invoice_type not in ["other_costs", "other_revenues"] and can_see_timesheets
        ...
        for revenue in profitability_items["revenues"]["data"]:
            ...
            if can_see_timesheets and record_ids:
                ...
                action["context"] = json.dumps(
                    {"search_default_groupby_invoice": 1 if revenue_id == "billable_time" else 0}
                )
...
class ProjectTask(models.Model):
    ...
    so_analytic_account_id = fields.Many2one(
        related="sale_order_id.analytic_account_id", string="Sale Order Analytic Account"
    )
