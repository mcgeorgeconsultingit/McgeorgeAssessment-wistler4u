...
class TestWiseOperator(TransactionCase):
    def test_wise_operator(self):
        ...
        pick1_wise.move_line_ids[0].write(
            {"result_package_id": package1.id, "qty_done": 4, "location_dest_id": self.shelf1.id}
        )
        ...
        self.assertEqual(
            set(pack_ids2.mapped("location_id.id")), set([self.shelf1.id, self.shelf2.id])
        )
        ...
        self.assertEqual(
            set(new_move_lines.mapped("location_id.id")), set([self.shelf1.id, self.shelf2.id])
        )
        ...
        self.assertEqual(
            delivery_order_wise2.state, "assigned", "Delivery order 2 should be back in ready state"
        )
