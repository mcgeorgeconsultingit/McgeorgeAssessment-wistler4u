...
class HrContract(models.Model):
    ...
    def _get_interval_leave_work_entry_type(self, interval, leaves, bypassing_codes):
        ...
        including_rcleaves = [
            l[2]
            for l in leaves
            if l[2] and interval_start >= l[2].date_from and interval_stop <= l[2].date_to
        ]
        ...
        if bypassing_codes:
            bypassing_rc_leave = [
                ...
                if l.holiday_id.holiday_status_id.work_entry_type_id.code in bypassing_codes
            ]
        else:
            bypassing_rc_leave = []
        ...
        for contract in self:
            ...
            for leave in leaves:
                ...
                for overlapping_contract in overlapping_contracts:
                    ...
                    new_date_from = max(
                        leave.date_from,
                        datetime.combine(overlapping_contract.date_start, datetime.min.time()),
                    )
                    new_date_to = min(
                        leave.date_to,
                        datetime.combine(
                            overlapping_contract.date_end or date.max, datetime.max.time()
                        ),
                    )
