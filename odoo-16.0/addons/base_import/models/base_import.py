...
EXTENSIONS = {"." + ext: handler for mime, (ext, handler, req) in FILE_TYPE_DICT.items()}
...
class Import(models.TransientModel):
    ...
    file = fields.Binary(
        "File", help="File to check and/or import, raw binary (not base64)", attachment=False
    )
    ...
    def _read_file(self, options):
        ...
        if req:
            raise ImportError(
                _('Unable to load "{extension}" file: requires Python module "{modname}"').format(
                    extension=file_extension, modname=req
                )
            )
        raise ValueError(
            _('Unsupported file format "{}", import only supports CSV, ODS, XLS and XLSX').format(
                self.file_type
            )
        )
    ...
    def _read_ods(self, options):
        ...
        content = [row for row in doc.getSheet(sheet) if any(x for x in row if x.strip())]
        ...
        if not separator:
            ...
            for candidate in (",", ";", "\t", " ", "|", unicodedata.lookup("unit separator")):
                ...
                it = pycompat.csv_reader(
                    io.BytesIO(csv_data), quotechar=options["quoting"], delimiter=candidate
                )
                w = None
                for row in it:
                    width = len(row)
    ...
    def _try_match_date_time(self, preview_values, options):
        ...
        datetime_patterns.extend("%s %s" % (d, t) for d in date_patterns for t in TIME_PATTERNS)
        ...
        try:
            ...
            if batch_cutoff:
                if count > batch_cutoff:
                    ...
                    batch = bool(next(itertools.islice(rows, batch_cutoff - count, None), None))
    ...
    def _parse_float_from_data(self, data, index, name, options):
        for line in data:
            ...
            line[index] = (
                line[index].replace(thousand_separator, "").replace(decimal_separator, ".")
            )
            old_value = line[index]
            line[index] = self._remove_currency_symbol(line[index])
            if line[index] is False:
                raise ImportValidationError(
                    _("Column %s contains incorrect values (value: %s)", name, old_value),
                    field=name,
                )
    ...
    def _parse_import_data_recursive(self, model, prefix, data, import_fields, options):
        ...
        for name, field in all_fields.items():
            ...
            # Also verify that the field name exactly match the import_field at the correct level.
                ...
                name + "/" in import_field and name == import_field.split("/")[prefix.count("/")]
                ...
                with requests.Session() as session:
                    ...
                    for num, line in enumerate(data):
                        if re.match(
                            config.get("import_image_regex", DEFAULT_IMAGE_REGEX), line[index]
                        ):
    ...
    def _import_image_by_url(self, url, session, field, line_number):
        ...
        try:
            response = session.get(
                url, timeout=int(config.get("import_image_timeout", DEFAULT_IMAGE_TIMEOUT))
            )
            ...
            if (
                ...
                raise ImportValidationError(
                    _("File size exceeds configured maximum (%s bytes)", maxsize), field=field
                )
    ...
    def _handle_fallback_values(self, import_field, input_file_data, fallback_values):
        ...
        for field_string in fallback_values:
            ...
            selection_values = [
                value.lower()
                for (key, value) in target_model.fields_get([target_field])[target_field][
                    "selection"
                ]
            ]
