...
class PosOrder(models.Model):
    ...
    customer_count = fields.Integer(
        string="Guests", help="The amount of customers that have been served by this order."
    )
    ...
    def _get_order_lines(self, orders):
        ...
        for order_id, order_lines in groupby(extended_order_lines, key=lambda x: x[2]["order_id"]):
            next(order for order in orders if order["id"] == order_id[0])["lines"] = list(
                order_lines
            )
    ...
    def _get_payment_lines(self, orders):
        ...
        for order_id, payment_lines in groupby(
            ...
            next(order for order in orders if order["id"] == order_id[0])["statement_ids"] = list(
                payment_lines
            )
