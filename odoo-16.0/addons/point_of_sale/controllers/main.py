...
class PosController(PortalAccount):
        ...
        if kwargs and request.httprequest.method == "POST":
            ...
            if not error:
                return self._get_invoice(
                    partner_values, invoice_values, pos_order, additional_invoice_fields, kwargs
                )
            else:
    ...
    def _get_invoice(
        self, partner_values, invoice_values, pos_order, additional_invoice_fields, kwargs
    ):
        ...
        if request.env.user._is_public():
            ...
            partner_values.update(
                {key: kwargs[key] for key in self.OPTIONAL_BILLING_FIELDS if key in kwargs}
            )
