...
class AlarmManager(models.AbstractModel):
    ...
    def _get_next_potential_limit_alarm(self, alarm_type, seconds=None, partners=None):
        ...
        result = {key: result[key] for key in set(events._filter_access_rules("read").ids)}
        return result
    ...
    def _get_events_by_alarm_to_notify(self, alarm_type):
        ...
        already.
        """
        lastcall = self.env.context.get("lastcall", False) or fields.date.today() - relativedelta(
            weeks=1
        )
        self.env.cr.execute(
            """
    ...
    def _send_reminder(self):
        ...
        event_ids = list(
            set(event_id for event_ids in events_by_alarm.values() for event_id in event_ids)
        )
