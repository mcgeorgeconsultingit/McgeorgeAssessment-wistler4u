...
class LinkTracker(models.Model):
    ...
    def _compute_redirected_url(self):
        ...
        *only* for URLs that redirect to the local website (base URL).
        ...
        no_external_tracking = (
            self.env["ir.config_parameter"].sudo().get_param("link_tracker.no_external_tracking")
        )
    ...
    def _get_title_from_url(self, url):
        try:
            ...
            ) > URL_MAX_SIZE or "text/html" not in head.headers.get("Content-Type", "text/html"):
                return url
...
class LinkTrackerClick(models.Model):
    ...
    link_id = fields.Many2one("link.tracker", "Link", index=True, required=True, ondelete="cascade")
    ip = fields.Char(string="Internet Protocol")
    country_id = fields.Many2one("res.country", "Country")

    def _prepare_click_values_from_route(self, **route_values):
        click_values = dict(
            (fname, route_values[fname]) for fname in self._fields if fname in route_values
        )
