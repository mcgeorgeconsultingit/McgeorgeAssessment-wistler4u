    ...
    def _create_resource_leave(self):
        ...
        for leave in self.filtered(lambda l: l.employee_id):
            ...
            for contract in contracts:
                if (
                    contract
                    and contract.resource_calendar_id != leave.employee_id.resource_calendar_id
                ):
                    resource_leave_values += [
                        {
    ...
    def _check_contracts(self):
        ...
        for holiday in self.filtered("employee_id"):
            ...
            if len(contracts.resource_calendar_id) > 1:
                state_labels = {
                    e[0]: e[1] for e in contracts._fields["state"]._description_selection(self.env)
                }
...
%s""",
    ...
    def write(self, vals):
        ...
        start = min(
            ...
            + [fields.Datetime.from_string(vals.get("date_from", False)) or datetime.max]
        )
        stop = max(
            self.mapped("date_to")
    ...
    def _compute_can_cancel(self):
        ...
        work_entries = (
            ...
            .sudo()
            .search([("state", "=", "validated"), ("leave_id", "in", cancellable_leaves.ids)])
        )
