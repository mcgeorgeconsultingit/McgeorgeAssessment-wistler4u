...
class BaseFollowersTest(TestMailCommon):
    ...
    def test_followers_subtypes_specified(self):
        test_record = self.test_record.with_user(self.user_employee)
        test_record.message_subscribe(
            partner_ids=[self.user_employee.partner_id.id], subtype_ids=[self.mt_mg_nodef.id]
        )
        self.assertEqual(test_record.message_partner_ids, self.user_employee.partner_id)
        follower = self.env["mail.followers"].search(
            [
    ...
    def test_followers_multiple_subscription_force(self):
        ...
        test_record.message_subscribe(
            partner_ids=[self.user_admin.partner_id.id], subtype_ids=[self.mt_mg_nodef.id]
        )
        ...
        self.assertEqual(test_record.message_partner_ids, self.user_admin.partner_id)
        self.assertEqual(
            test_record.message_follower_ids.subtype_ids, self.mt_mg_nodef | self.mt_al_nodef
        )
    ...
    def test_followers_private_address(self):
        ...
        self.assertEqual(
            document.message_follower_ids.partner_id, self.partner_portal | private_address
        )
...
class AdvancedFollowersTest(TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.umb_nodef = Subtype.create(
            {"name": "Container NoDefault", "default": False, "res_model": "mail.test.container"}
        )
        cls.umb_def = Subtype.create(
            {"name": "Container Default", "default": True, "res_model": "mail.test.container"}
        )
    ...
    def test_auto_subscribe_inactive(self):
        ...
        self.assertEqual(self.test_track.message_partner_ids, self.user_employee.partner_id)
        self.assertEqual(
            self.test_track.message_follower_ids.partner_id, self.user_employee.partner_id
        )
    ...
    def test_auto_subscribe_responsible(self):
        ...
        self.assertEqual(
            sub.message_partner_ids, (self.user_employee.partner_id | self.user_admin.partner_id)
        )
...
class AdvancedResponsibleNotifiedTest(TestMailCommon):
    ...
    def test_auto_subscribe_notify_email(self):
        ...
        self.assertEqual(
            sub.message_partner_ids, (self.user_employee.partner_id | notified_user.partner_id)
        )
...
class RecipientsNotificationTest(TestMailCommon):
        ...
        for record, record_id in zip(records, record_ids):
            ...
            for partner in partners:
                ...
                if partner_to_users and partner_to_users.get(
                    ...
                    user = next(
                        (user for user in partner.user_ids if not user.share), self.env["res.users"]
                    )
                    if not user:
                ...
                if record:
                    self.assertEqual(
                        partner_data["is_follower"], partner in record.message_partner_ids
                    )
                else:
    ...
    def test_notification_nodupe(self):
        ...
        recipients_data = self.env["mail.followers"]._get_recipient_data(
            test, "comment", self.env.ref("mail.mt_comment").id, pids=self.common_partner.ids
        )
    ...
    def test_notification_user_choice(self):
        ...
        with self.assertSinglePostNotifications(
            [{"group": "customer", "partner": shared_partner, "status": "sent", "type": "inbox"}],
    ...
    def test_recipients_fetch(self):
        ...
        self.assertNotIn(
            ...
            test_records[0]
            .message_follower_ids.filtered(lambda fol: fol.partner_id == self.partner_portal)
            .subtype_ids,
        ...
        self.assertRecipientsData(
            recipients_data, test_records[0], self.env.user.partner_id + self.partner_portal
        )
        ...
        recipients_data = self.env["mail.followers"]._get_recipient_data(
            test_records[0], "comment", False, pids=(self.common_partner + self.partner_admin).ids
        )
        self.assertRecipientsData(
        ...
        recipients_data = self.env["mail.followers"]._get_recipient_data(
            test_records, "comment", self.env.ref("mail.mt_comment").id, pids=self.partner_admin.ids
        )
        ...
        self.assertRecipientsData(
            recipients_data_3, test_records[4:], self.env.user.partner_id + self.partner_admin
        )

        # multi mode, pids only
        recipients_data = self.env["mail.followers"]._get_recipient_data(
            test_records, "comment", False, pids=(self.env.user.partner_id + self.partner_admin).ids
        )
        self.assertRecipientsData(
