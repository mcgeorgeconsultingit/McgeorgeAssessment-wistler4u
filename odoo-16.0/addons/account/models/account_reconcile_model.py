...
class AccountReconcileModelPartnerMapping(models.Model):
    ...
    model_id = fields.Many2one(
        comodel_name="account.reconcile.model", readonly=True, required=True, ondelete="cascade"
    )
        ...
        for record in self:
            if not (record.narration_regex or record.payment_ref_regex):
                raise ValidationError(
                    _("Please set at least one of the match texts to create a partner mapping.")
                )
            try:
                ...
                raise ValidationError(
                    _("The following regular expression is invalid to create a partner mapping: %s")
                    % current_regex
                )
...
class AccountReconcileModel(models.Model):
    ...
    number_entries = fields.Integer(
        string="Number of entries related to this model", compute="_compute_number_entries"
    )
        ...
        for record in self:
            if record.allow_payment_tolerance:
                if (
                    ...
                    raise ValidationError(
                        _("A payment tolerance defined as an amount should always be higher than 0")
                    )
    ...
    def _get_write_off_move_lines_dict(self, residual_balance, partner_id):
        ...
        for line in self.line_ids:
            ...
            if line.tax_ids:
                taxes = line.tax_ids
                detected_fiscal_position = self.env["account.fiscal.position"]._get_fiscal_position(
    ...
    def _is_applicable_for(self, st_line, partner):
        ...
        if (
            (self.match_journal_ids and st_line.move_id.journal_id not in self.match_journal_ids)
        ...
        for record, rule_field, record_field in [
            ...
            if (
                (self["match_" + rule_field] == "contains" and rule_term not in record_term)
