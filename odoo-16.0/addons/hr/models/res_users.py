...
class User(models.Model):
    ...
    private_city = fields.Char(
        related="address_home_id.city", string="Private City", readonly=False, related_sudo=False
    )
    ...
    private_zip = fields.Char(
        related="address_home_id.zip", readonly=False, string="Private Zip", related_sudo=False
    )
    ...
    is_address_home_a_company = fields.Boolean(
        related="employee_id.is_address_home_a_company", readonly=False, related_sudo=False
    )
    ...
    employee_resource_calendar_id = fields.Many2one(
        related="employee_id.resource_calendar_id", string="Employee's Working Hours", readonly=True
    )
    ...
    def create(self, vals_list):
        ...
        if employee_create_vals:
            self.env["hr.employee"].with_context(clean_context(self.env.context)).create(
                employee_create_vals
            )
        return res
