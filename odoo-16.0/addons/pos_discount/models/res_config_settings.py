...
class ResConfigSettings(models.TransientModel):
    ...
    pos_discount_product_id = fields.Many2one(
        "product.product", compute="_compute_pos_discount_product_id", store=True, readonly=False
    )

    @api.depends("company_id", "pos_module_pos_discount", "pos_config_id")
    def _compute_pos_discount_product_id(self):
        default_product = (
            self.env.ref("point_of_sale.product_product_consumable", raise_if_not_found=False)
