...
class TestPoSBasicConfig(TestPoSCommon):
        ...
        self._run_test(
            {
                ...
                "orders": [
                    {
                        "pos_order_lines_ui_args": [(self.product1, 10), (self.product2, 5)],
                        "uid": "00100-010-0001",
                    },
                    {
                        "pos_order_lines_ui_args": [(self.product2, 7), (self.product3, 1)],
                        "uid": "00100-010-0002",
                    },
                    {
    ...
    def test_return_order(self):
        ...
        def _before_closing_cb():
            ...
            make_payment.check()
            self.assertEqual(
                refund_order.state, "paid", "Payment is registered, order should be paid."
            )
            self.assertAlmostEqual(
        ...
        self._run_test(
            {
                ...
                "orders": [
                    {
                        "pos_order_lines_ui_args": [(self.product1, 1), (self.product2, 5)],
    ...
    def test_rounding_method(self):
        ...
        self.config.rounding_method = self.env["account.cash.rounding"].create(
            {
                ...
                "profit_account_id": self.company["default_cash_difference_income_account_id"]
                .copy()
                .id,
                "loss_account_id": self.company["default_cash_difference_expense_account_id"]
                .copy()
                .id,
        ...
        orders.append(
            self.create_ui_order_data(
                [(self.product4, 3), (self.product2, 20)], payments=[(self.bank_pm1, 429.90)]
            )
        )

        orders.append(
            self.create_ui_order_data(
                [(self.product1, 6), (self.product4, 4)], payments=[(self.bank_pm1, 99.85)]
            )
        )
    ...
    def test_invoice_past_order(self):
        ...
        with freeze_time(fields.Datetime.now() + relativedelta(days=2)):
            ...
            self.assertTrue(
                order_to_invoice.account_move.invoice_date != order_to_invoice.date_order.date(),
