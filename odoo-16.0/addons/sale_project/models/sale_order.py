...
class SaleOrder(models.Model):
    ...
    tasks_ids = fields.Many2many(
        "project.task", compute="_compute_tasks_ids", string="Tasks associated to this sale"
    )
    tasks_count = fields.Integer(
        string="Tasks", compute="_compute_tasks_ids", groups="project.group_project_user"
    )
    ...
    def _compute_visible_project(self):
        ...
        for order in self:
            order.visible_project = any(
                ...
                for service_tracking in order.order_line.mapped("product_id.service_tracking")
            )
