...
class TestAccountMoveReconcile(AccountTestInvoicingCommon):
    ...
    def assertFullReconcileAccount(self, full_reconcile, account):
        self.assertFullReconcile(
            full_reconcile, self.env["account.move.line"].search([("account_id", "=", account.id)])
        )
    ...
    def test_reconcile_lines_corner_case_3_zero_balance_one_foreign_currency(self):
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id, [{"date": fields.Date.from_string("2017-01-01")}]
        )
        self.assertRecordValues(
    ...
    def test_reconcile_special_mexican_workflow_1(self):
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id, [{"date": fields.Date.from_string("2019-06-24")}]
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id, [{"date": fields.Date.from_string("2019-06-28")}]
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id, [{"date": fields.Date.from_string("2019-09-24")}]
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
    ...
    def test_reconcile_special_mexican_workflow_2(self):
        ...
        self.assertRecordValues(
            refund1_rec_line + inv1_rec_line + inv2_rec_line + pay1_rec_line + pay2_rec_line,
            [
    ...
    def test_reconcile_cash_basis_revert(self):
        ...
        ).line_ids.filtered(lambda line: line.account_id == self.cash_basis_transfer_account)
    ...
    def test_caba_mix_reconciliation(self):
        ...
        caba_inv = self.init_invoice(
            "in_invoice", amounts=[300], post=True, taxes=self.cash_basis_tax_a_third_amount
        )
    ...
    def test_caba_double_tax(self):
        ...
        move_form = Form(
            self.env["account.move"].with_context(
                default_move_type="in_invoice", account_predictive_bills_disable_prediction=True
            )
        )
    ...
    def test_caba_double_tax_negative_line(self):
        ...
        invoice = self.init_invoice(
            "in_invoice", amounts=[300, -60], post=True, taxes=self.cash_basis_tax_a_third_amount
        )
    ...
    def test_caba_dest_acc_reconciliation_partial_pmt(self):
        ...
        caba_inv = self.init_invoice(
            "in_invoice", amounts=[900], post=True, taxes=self.cash_basis_tax_a_third_amount
        )
    ...
    def test_caba_rounding_adjustment_multicurrency(self):
        ...
        currency_id = self.setup_multi_currency_data({"name": "Minovsky Dollar", "rounding": 0.01})[
            "currency"
        ].id
    ...
    def test_cash_basis_taxline_without_account(self):
        ...
        expected_values = [
            {"account_id": self.cash_basis_base_account.id, "debit": 0.0, "credit": 800.0},
            {"account_id": self.cash_basis_base_account.id, "debit": 800.0, "credit": 0.0},
            {"account_id": self.cash_basis_transfer_account.id, "debit": 0.0, "credit": 64.0},
            {"account_id": self.tax_account_1.id, "debit": 64.0, "credit": 0.0},
            {"account_id": self.cash_basis_transfer_account.id, "debit": 0.0, "credit": 96.0},
            {"account_id": self.cash_basis_base_account.id, "debit": 96.0, "credit": 0.0},
        ]
