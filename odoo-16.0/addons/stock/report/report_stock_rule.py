...
class ReportStockRule(models.AbstractModel):
    ...
    def _get_routes(self, data):
        ...
        return (
            product.route_ids | product.categ_id.total_route_ids | warehouse_ids.mapped("route_ids")
        )
    ...
    def _sort_locations(self, rules_and_loc, warehouses):
        ...
        for warehouse_id in warehouses:
            ...
            locations |= self._sort_locations_by_warehouse(
                rules_and_loc, used_rules, start_locations, ordered_locations, warehouse_id
            )
            if any(location not in locations for location in all_warehouse_locations):
                ...
                locations |= self._sort_locations_by_warehouse(
                    rules_and_loc, used_rules, remaining_locations, ordered_locations, warehouse_id
                )
    ...
    def _sort_locations_by_warehouse(
        self, rules_and_loc, used_rules, start_locations, ordered_locations, warehouse_id
    ):
        ...
        if rules_start:
            ...
            remaining_rules = (
                self.env["stock.rule"].concat(*([r["rule"] for r in rules_and_loc])) - used_rules
            )
            ...
            start_locations = (
                rules_start_dest_locations - ordered_locations - remaining_rules_location
            )
