...
class Team(models.Model):
    ...
    def _compute_assignment_enabled(self):
        assign_enabled = (
            self.env["ir.config_parameter"].sudo().get_param("crm.lead.auto.assignment", False)
        )
    ...
    def unlink(self):
        ...
        if frequencies:
            existing_noteam = (
                ...
                .sudo()
                .search(
                    [("team_id", "=", False), ("variable", "in", frequencies.mapped("variable"))]
                )
            )
            for frequency in frequencies:
                ...
                if match:
                    ...
                    add_won_count = float_round(
                        frequency.won_count, precision_digits=0, rounding_method="HALF-UP"
                    )
                    add_lost_count = float_round(
                        frequency.lost_count, precision_digits=0, rounding_method="HALF-UP"
                    )
                    new_won_count = exist_won_count + add_won_count
                    new_lost_count = exist_lost_count + add_lost_count
                    match.won_count = (
                        new_won_count if float_compare(new_won_count, 0.1, 2) == 1 else 0.1
                    )
                    match.lost_count = (
                        new_lost_count if float_compare(new_lost_count, 0.1, 2) == 1 else 0.1
                    )
                else:
                    existing_noteam += (
                        self.env["crm.lead.scoring.frequency"]
    ...
    def _action_assign_leads_logs(self, teams_data, members_data):
        ...
        if duplicates:
            message_parts.append(
                _("%(duplicates)s duplicates leads have been merged.", duplicates=duplicates)
            )
        ...
        if not assigned and members_assigned:
            if len(self) == 1:
                ...
                message_parts.append(
                    _("No new lead allocated to the teams because no lead match their domains.")
                )
        elif assigned:
            if len(self) == 1:
                message_parts.append(
    ...
    def _allocate_leads(self, work_days=1):
        ...
        # some final log
        _logger.info(
            "## Assigned %s leads", (len(global_data["assigned"]) + len(global_data["merged"]))
        )
