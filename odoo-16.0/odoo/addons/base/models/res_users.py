    ...
    def _search_full_name(self, operator, operand):
        ...
        for group in operand:
            ...
            category_ids = (
                ...
                .sudo()
                ._search([("name", operator, [category_name] if lst else category_name)])
            )
...
class Users(models.Model):
    ...
    signature = fields.Html(
        string="Email Signature", compute="_compute_signature", readonly=False, store=True
    )
    ...
    groups_id = fields.Many2many(
        "res.groups", "res_groups_users_rel", "uid", "gid", string="Groups", default=_default_groups
    )
    ...
    def _check_credentials(self, password, env):
        ...
        * (re)raise AccessDenied if the credentials are still invalid
        ...
        assert password
        self.env.cr.execute(
            "SELECT COALESCE(password, '') FROM res_users WHERE id=%s", [self.env.user.id]
        )
        ...
        for user in self.sudo():
            if user.action_id.type == "ir.actions.client":
                ...
                if action.tag == "reload":
                    raise ValidationError(
                        _('The "%s" action cannot be selected as home action.', action.name)
                    )
        ...
        try:
            with cls.pool.cursor() as cr:
                ...
                with self._assert_can_auth(user=login):
                    user = self.search(
                        self._get_login_domain(login), order=self._get_login_order(), limit=1
                    )
                    if not user:
                        raise AccessDenied()
...
class UsersView(models.Model):
    ...
    def _prepare_warning_for_group_inheritance(self, user):
        ...
        for group in current_groups:
            current_groups_by_category[group.category_id] |= group.trans_implied_ids.filtered(
                lambda grp: grp.category_id == group.category_id
            )

        missing_groups = {}
        # We don't want to show warning for "Technical" and "Extra Rights" groups
        categories_to_ignore = self.env.ref("base.module_category_hidden") + self.env.ref(
            "base.module_category_usability"
        )
        for group in current_groups:
            ...
            missing_implied_groups = missing_implied_groups.filtered(
                lambda g: g.category_id not in (group.category_id | categories_to_ignore)
    ...
    def _remove_reified_groups(self, values):
        ...
        if "groups_id" not in values and (add or rem):
            ...
            values1["groups_id"] = list(itertools.chain(zip(repeat(3), rem), zip(repeat(4), add)))
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        if any(is_reified_group(field) for field in names):
            field_name = ["groups_id"] + [field for field in names if not is_reified_group(field)]
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        missing = (
            set(self.SELF_WRITEABLE_FIELDS).union(self.SELF_READABLE_FIELDS).difference(res.keys())
        )
        if allfields:
        ...
        if missing:
            res.update(
                {
                    key: dict(
                        values, readonly=key not in self.SELF_WRITEABLE_FIELDS, searchable=False
                    )
    ...
    def change_password_button(self):
        for line in self:
            if not line.new_passwd:
                raise UserError(
                    _("Before clicking on 'Change Password', you have to write a new password.")
                )
...
class APIKeys(models.Model):
    ...
    def _remove(self):
        ...
        if self.env.is_system() or self.mapped("user_id") == self.env.user:
            ...
            return {"type": "ir.actions.act_window_close"}
        raise AccessError(
            _("You can not remove API keys unless they're yours or you are a system user")
        )
