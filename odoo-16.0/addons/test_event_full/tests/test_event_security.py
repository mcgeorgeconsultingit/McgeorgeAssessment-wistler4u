...
class TestEventSecurity(TestEventFullCommon):
    ...
    def test_event_access_event_registration(self):
        ...
        self.assertEqual(
            registration.event_id.name, event.name, "Registration users should be able to read"
        )
    ...
    def test_event_access_event_manager(self):
        ...
        with self.assertRaises(AccessError):
            event_config = (
                self.env["res.config.settings"].with_user(self.user_eventmanager).create({})
            )
    ...
    def test_multi_companies(self):
        ...
        self.assertIn(
            event_company_1, result, "You must be able to read the events in your company"
        )
        self.assertNotIn(
            event_company_2, result, "You must not be able to read events outside of your company"
        )
