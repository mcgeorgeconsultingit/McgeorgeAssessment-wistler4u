...
class TestComposerForm(TestMailComposer):
    ...
    def test_mail_composer_comment_attachments(self):
        ...
        report_attachments = [
            att for att in composer_form.attachment_ids if att not in template_1_attachments
        ]
    ...
    def test_mail_composer_mass(self):
        ...
        self.assertFalse(
            composer_form.record_name, "MailComposer: mass mode should have void record name"
        )
        self.assertFalse(composer_form.reply_to)
        self.assertFalse(composer_form.reply_to_force_new)
        self.assertEqual(
    ...
    def test_mail_composer_mass_wtpl(self):
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    self.test_records, add_web=True, default_template_id=self.template.id
                )
            )
        )
        # self.assertTrue(composer_form.auto_delete)
...
class TestComposerInternals(TestMailComposer):
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                # values coming from template: attachment_ids only (report is dynamic)
                    ...
                    self.assertEqual(sorted(composer.attachment_ids.ids), sorted(attachs.ids))
    ...
    def test_mail_composer_author(self):
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                test_records = self.test_records if batch else self.test_record
                ctx = self._get_web_context(
                    test_records, add_web=False, default_composition_mode=composition_mode
                )
                ...
                composer.write(
                    {"template_id": self.template.id, "author_id": self.env.user.partner_id}
                )
                # currently onchange necessary
                composer._onchange_template_id_wrapper()
                self.assertEqual(
    ...
    def test_mail_composer_content(self):
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                self.assertEqual(
                    composer.body, '<p>Back to my amazing body <t t-out="record.name>/></p>'
                )
                self.assertEqual(composer.mail_server_id, self.mail_server_global)
    ...
    def test_mail_composer_recipients(self):
        ...
        self.template.write({"email_cc": "test.cc.1@test.example.com, test.cc.2@test.example.com"})
        template_void = self.template.copy(
            default={
                "email_cc": False,
                "email_to": False,
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                if composition_mode == "comment":
                    ...
                    self.assertEqual(
                        composer.reply_to, "info@test.example.com", "Template was rendered"
                    )
                    self.assertFalse(
                    ...
                    self.assertEqual(
                        composer.partner_ids, base_recipients, "Mass mode: kept original values"
                    )
                    self.assertEqual(
                        composer.reply_to, self.template.reply_to, "Mass mode: raw template value"
                    )
                    self.assertFalse(
                        composer.reply_to_force_new
    ...
    def test_mail_composer_rights_attachments(self):
        ...
        self.assertEqual(
            self.test_record.message_ids[0].subject, f"TemplateSubject {self.test_record.name}"
        )
        self.assertEqual(
    ...
    def test_mail_composer_rights_portal(self):
        ...
        with patch.object(MailTestTicket, "check_access_rights", return_value=True):
            ...
            ).create({"subject": "Subject", "body": "<p>Body text 2</p>"})._action_send_mail()
    ...
    def test_mail_composer_save_template(self):
        ...
        template = self.env["mail.template"].search(
            [("model", "=", self.test_record._name), ("subject", "=", "Template Subject")], limit=1
        )
        self.assertEqual(
            template.name,
            "%s: %s" % (self.env["ir.model"]._get(self.test_record._name).name, "Template Subject"),
        )
        self.assertEqual(
            template.body_html, "<p>Template Body</p>", "email_template incorrect body_html"
        )
...
class TestComposerResultsComment(TestMailComposer):
    ...
    def test_mail_composer_notifications_delete(self):
        ...
        self.assertEqual(
            len(self._new_mails.exists()), 2, "Should not have deleted mail.mail records"
        )
    ...
    def test_mail_composer_post_parameters(self):
        ...
        self.assertEqual(
            message.subtype_id, self.env.ref("mail.mt_comment", "Mail: default subtype is comment")
        )
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        self.assertMailMail(
            ...
            email_values={
                ...
                "attachments_info": [
                    ...
                    {
                        "name": "TestReport for %s.html" % self.test_record.name,
                        "type": "text/plain",
                    },
                ],
            },
        ...
        self.assertMailMail(
            self.test_record.customer_id + new_partners,
            "sent",
            mail_message=message,
            author=self.partner_employee,  # author != email_from (template sets only email_from)
            email_values={
        ...
        self.assertEqual(message.subtype_id, self.env.ref("mail.mt_comment"))
        self.assertNotified(
            message, [{"partner": self.partner_admin, "is_read": False, "type": "inbox"}]
        )
        ...
        self.assertEqual(
            ...
            set(
                [
                    "AttFileName_00.txt",
                    "AttFileName_01.txt",
                    "TestReport for %s.html" % self.test_record.name,
                ]
            ),
        )
        self.assertEqual(
            set(message.attachment_ids.mapped("res_model")), set([self.test_record._name])
        )
...
class TestComposerResultsCommentStatus(TestMailComposer):
    ...
    def test_comment_blacklist(self):
        ...
        self.assertEqual(
            len(self._mails), 2, "Should have sent 2 emails, skipping the exclusion list"
        )
...
class TestComposerResultsMass(TestMailComposer):
    ...
    def test_mail_composer_delete(self):
        ...
        self.assertEqual(
            self._new_msgs.exists(), self._new_msgs, "Should not have deleted mail.message records"
        )
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        _mails_records = [
            [
                ...
                if "%s-%s" % (record.id, record._name) in mail["message_id"]
            ]
            for record in self.test_records
        ]
        ...
        for record in self.test_records:
            self.assertMailMail(
                ...
                email_values={
                    "body_content": "TemplateBody %s" % record.name,
                    "email_from": self.partner_employee_2.email_formatted,
                    "reply_to": formataddr((f"{record.name}", "dynamic.reply.to@test.com")),
                    "subject": "TemplateSubject %s" % record.name,
                },
                fields_values={
                    "email_from": self.partner_employee_2.email_formatted,
...
class TestComposerResultsMassStatus(TestMailComposer):
    ...
    def test_mailing_blacklist_mixin(self):
        ...
        self.assertEqual(
            len(self._mails), 1, "Should have sent 1 email, and skipped an excluded email."
        )
    ...
    def test_mailing_duplicates_document_based(self):
        ...
        self.assertEqual(
            len(self._mails), 6, "Should have sent 6 emails, and skipped an excluded email"
        )
