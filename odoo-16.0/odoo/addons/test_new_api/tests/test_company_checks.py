...
class TestCompanyCheck(common.TransactionCase):
    ...
    def test_company_environment(self):
        ...
        self.assertEqual(
            user.sudo().with_context(allowed_company_ids=companies.ids).env.companies, companies
        )

        self.assertEqual(
            user.sudo().with_context(allowed_company_ids=[self.company_b.id, "abc"]).env.company,
            self.company_b,
        )
        """
