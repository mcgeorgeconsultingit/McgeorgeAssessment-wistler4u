...
class EventBoothCategory(models.Model):
    ...
    price_reduce_taxinc = fields.Float(
        string="Price Reduce Tax inc", compute="_compute_price_reduce_taxinc", compute_sudo=True
    )
    ...
    def _compute_image_1920(self):
        for category in self:
            category.image_1920 = (
                category.image_1920 if category.image_1920 else category.product_id.image_1920
            )
    ...
    def _compute_price_reduce(self):
        for category in self:
            ...
            lst_price = product.currency_id._convert(
                product.lst_price, pricelist.currency_id, self.env.company, fields.Datetime.now()
            )
            discount = (
                (lst_price - product._get_contextual_price()) / lst_price if lst_price else 0.0
            )
    ...
    def _compute_price_reduce_taxinc(self):
        for category in self:
            ...
            taxes = tax_ids.compute_all(
                category.price_reduce, category.currency_id, 1.0, product=category.product_id
            )
