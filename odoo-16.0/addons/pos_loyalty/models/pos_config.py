...
class PosConfig(models.Model):
        ...
        if gift_card_programs:
            for gc_program in gift_card_programs:
                ...
                if rule.reward_point_amount != 1 or rule.reward_point_mode != "money":
                    raise UserError(
                        _("Invalid gift card program rule. Use 1 point per currency spent.")
                    )
                ...
                if (
                    ...
                    raise UserError(
                        _("Invalid gift card program reward. Use 1 currency per point discount.")
                    )
    ...
    def use_coupon_code(self, code, creation_date, partner_id):
        ...
        if not coupon.program_id.reward_ids or not any(
            reward.required_points <= coupon.points for reward in coupon.program_id.reward_ids
        ):
            return {
                "successful": False,
                "payload": {
