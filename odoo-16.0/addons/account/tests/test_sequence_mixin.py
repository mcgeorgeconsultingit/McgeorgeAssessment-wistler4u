...
class TestSequenceMixin(TestSequenceMixinCommon):
    ...
    def test_journal_sequence_ordering(self):
        ...
        copies = reduce(
            (lambda x, y: x + y), [self.create_move(date=self.test_move.date) for i in range(6)]
        )
        ...
        wizard = Form(
            self.env["account.resequence.wizard"].with_context(
                active_ids=set(copies.ids) - set(copies[4].ids), active_model="account.move"
            ),
        )
...
class TestSequenceMixinConcurrency(TransactionCase):
    ...
    def test_sequence_concurency(self):
        ...
        self.assertEqual(
            moves.mapped("name"), ["CT/2016/01/0001", "CT/2016/01/0002", "CT/2016/01/0003"]
        )
