...
class TestHttpStaticCommon(TestHttpBase):
    ...
    def assertDownload(self, url, headers, assert_status_code, assert_headers, assert_content=None):
...
class TestHttpStatic(TestHttpStaticCommon):
    ...
    def test_static09_binary_field_inline(self):
        self.assertDownloadGizeh(
            "/web/content/test_http.earth?field=glyph_inline", assert_filename="Earth.png"
        )
...
class TestHttpStaticLogo(TestHttpStaticCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_of_company_of_superuser, cls.user_company2 = [
            new_test_user(
                cls.env, f"user_{company.id}", company_id=company.id, password=cls.password
            )
