...
class TestImage(TransactionCase):
    ...
    def setUp(self):
        ...
        draw.rectangle(
            xy=[(offset, 0), (image.size[0] - offset, image.size[1])], fill=self.fill_color
        )
        ...
        draw = ImageDraw.Draw(image)
        draw.rectangle(
            xy=[(0, offset), (image.size[0], image.size[1] - offset)], fill=self.fill_color
        )
    ...
    def test_10_image_process_source(self):
        ...
        self.assertEqual(
            tools.image_process(self.img_svg), self.img_svg, "return source if format is SVG"
        )
    ...
    def test_14_image_process_crop(self):
        ...
        for test in tests:
            ...
            px = (half_width, top)
            self.assertEqual(
                image.getpixel(px), test[4][0], "%s - color top (%s, %s)" % (test[5], px[0], px[1])
            )
            # verify bottom
            ...
            px = (left, half_height)
            self.assertEqual(
                image.getpixel(px), test[4][2], "%s - color left (%s, %s)" % (test[5], px[0], px[1])
            )
            # verify right
    ...
    def test_ptype_image_to_jpeg(self):
        ...
        self.assertEqual(
            tools.image.image_apply_opt(image1, "JPEG"), tools.image.image_apply_opt(image2, "JPEG")
        )
