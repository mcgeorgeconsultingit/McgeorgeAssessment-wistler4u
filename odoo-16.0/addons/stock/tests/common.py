...
class TestStockCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.uom_kg = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_kgm), ("uom_type", "=", "reference")], limit=1
        )
        ...
        cls.uom_unit = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_unit), ("uom_type", "=", "reference")], limit=1
        )
        ...
        cls.kgB = cls.ProductObj.create(
            {"name": "kg-B", "type": "product", "uom_id": cls.uom_kg.id, "uom_po_id": cls.uom_kg.id}
        )
        cls.gB = cls.ProductObj.create(
            {"name": "g-B", "type": "product", "uom_id": cls.uom_gm.id, "uom_po_id": cls.uom_gm.id}
        )
