...
def try_report(cr, uid, rname, ids, data=None, context=None, our_module=None, report_type=None):
    ...
    if res_format == "pdf":
        ...
        try:
            ...
            proc = Popen(
                ["pdftotext", "-enc", "UTF-8", "-nopgbrk", rfname, "-"], shell=False, stdout=PIPE
            )
...
def try_report_action(
    ...
    if isinstance(action_id, str):
        if "." in action_id:
            ...
            if not our_module:
                raise ValueError(
                    'You cannot only specify action_id "%s" without a module name' % action_id
                )
            act_xmlid = action_id
    ...
    def _exec_action(action, datas, env):
        ...
        if action["type"] in ["ir.actions.act_window", "ir.actions.submenu"]:
            ...
            log_test("will emulate a %s view: %s#%s", view_type, datas["res_model"], view_id or "?")
            ...
            try:
                ...
                _logger.warning(
                    "Cannot resolve the view arch and locate the buttons!", exc_info=True
                )
            ...
            while buttons and not res:
                b = buttons.pop()
                log_test(
                    'in the "%s" form, I will press the "%s" button.', action_name, b["string"]
                )
                if not b["type"]:
            ...
            if "ids" in datas:
                del datas["ids"]
            res = try_report(
                cr, uid, action["report_name"], ids, datas, context, our_module=our_module
            )
            return res
        else:
