...
class TestUi(HttpCaseWithUserDemo):
    ...
    def test_website_event_pages_seo(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url(url), "website_event_pages_seo", login="admin"
        )
...
class TestWebsiteAccess(HttpCaseWithUserDemo, OnlineEventCase):
    ...
    def test_website_access_event_manager(self):
        ...
        self.assertTrue(
            published_events[0].name in resp.text, "Managers must see the unpublished events."
        )
        self.assertTrue(
            unpublished_events[0].name in resp.text, "Managers must see the published events."
        )
    ...
    def test_website_access_event_uer(self):
        ...
        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text, "Event user must see the unpublished events."
        )
        self.assertTrue(
            unpublished_events[0].name in resp.text, "Event user must see the published events."
        )
    ...
    def test_website_access_portal(self):
        ...
        resp = self.url_open("/event/%i" % unpublished_events[0].id)
        self.assertEqual(
            resp.status_code, 403, "Portal user must not have access to unpublished event."
        )

        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text, "Portal must see the published events."
        )
        self.assertFalse(
            unpublished_events[0].name in resp.text, "Portal should not see the unpublished events."
        )
    ...
    def test_website_access_public(self):
        ...
        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text, "Public must see the published events."
        )
        self.assertFalse(
            unpublished_events[0].name in resp.text, "Public should not see the unpublished events."
        )

    def test_sitemap(self):
        resp = self.url_open("/sitemap.xml")
        self.assertTrue(
            "/event/event-0" in resp.text, "Published events must be present in the sitemap"
        )
        self.assertTrue(
            "/event/event-1" in resp.text, "Published events must be present in the sitemap"
        )
        self.assertFalse(
            "/event/event-2" in resp.text, "Unpublished events must not be present in the sitemap"
        )
