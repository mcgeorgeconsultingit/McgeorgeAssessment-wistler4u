...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        self.env.cr.execute(
            """SELECT id FROM res_users WHERE id = %s FOR NO KEY UPDATE SKIP LOCKED""", [self.id]
        )
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_odoo2google(
            calendar_service
        )
    ...
    def _sync_all_google_calendar(self):
        ...
        for user in users:
            ...
            try:
                ...
                _logger.exception(
                    "[%s] Calendar Synchro - Exception : %s !", user, exception_to_unicode(e)
                )
