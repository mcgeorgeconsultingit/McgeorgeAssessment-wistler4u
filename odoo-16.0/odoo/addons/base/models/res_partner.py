...
_tzs = [
    (tz, tz)
    for tz in sorted(pytz.all_timezones, key=lambda tz: tz if not tz.startswith("Etc/") else "_")
]
...
class Partner(models.Model):
    ...
    barcode = fields.Char(
        help="Use a barcode to identify this contact.", copy=False, company_dependent=True
    )
    ...
    def _compute_tz_offset(self):
        for partner in self:
            partner.tz_offset = datetime.datetime.now(pytz.timezone(partner.tz or "GMT")).strftime(
                "%z"
            )
    ...
    def _compute_same_vat_partner_id(self):
        for partner in self:
            ...
            partner.same_vat_partner_id = (
                bool(partner.vat) and not partner.parent_id and Partner.search(domain, limit=1)
            )
    ...
    def _check_barcode_unicity(self):
        if (
            self.barcode
            and self.env["res.partner"].search_count([("barcode", "=", self.barcode)]) > 1
        ):
            ...
            while current_partner:
                ...
                while to_scan:
                    ...
                    if len(result) == len(adr_pref):
                        return result
                    to_scan = [
                        c for c in record.child_ids if c not in visited if not c.is_company
                    ] + to_scan
        ...
        for vals in vals_list:
            if vals.get("state_id"):
                ...
                if state.country_id.id != country_id:
                    state_domain = [("code", "=", state.code), ("country_id", "=", country_id)]
