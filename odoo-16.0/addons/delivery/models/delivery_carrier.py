...
class DeliveryCarrier(models.Model):
    ...
    prod_environment = fields.Boolean(
        "Environment", help="Set to True if your credentials are certified for production."
    )
    debug_logging = fields.Boolean("Debug logging", help="Log requests in order to ease debugging")
    company_id = fields.Many2one(
        "res.company", string="Company", related="product_id.company_id", store=True, readonly=False
    )
    ...
    country_ids = fields.Many2many(
        "res.country", "delivery_carrier_country_rel", "carrier_id", "country_id", "Countries"
    )
    state_ids = fields.Many2many(
        "res.country.state", "delivery_carrier_state_rel", "carrier_id", "state_id", "States"
    )
    ...
    def _product_price_to_company_currency(self, quantity, product, company):
        return company.currency_id._convert(
            quantity * product.standard_price, product.currency_id, company, fields.Date.today()
        )
