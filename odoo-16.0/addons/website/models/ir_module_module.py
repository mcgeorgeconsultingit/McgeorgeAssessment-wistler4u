...
class IrModuleModule(models.Model):
    ...
    def write(self, vals):
        ...
        for module in self:
            if module.name.startswith("theme_") and vals.get("state") == "installed":
                _logger.info(
                    "Module %s has been loaded as theme template (%s)" % (module.name, module.state)
                )
                ...
                if module.state in ["to install", "to upgrade"]:
                    ...
                    if module.state == "to upgrade" and request:
                        ...
                        websites_to_update = (
                            current_website if current_website in websites_to_update else Website
                        )
    ...
    def _post_copy(self, old_rec, new_rec):
        ...
        for (src_field, dst_field) in translated_fields:
            ...
            if not callable(old_field.translate):
                ...
                old_translation_lang = old_translations.get(cur_lang) or old_translations.get(
                    "en_US"
                )
    ...
    def _theme_load(self, website):
        ...
        for module in self:
            _logger.info(
                "Load theme %s for website %s from template." % (module.mapped("name"), website.id)
            )
    ...
    def _theme_unload(self, website):
        ...
        for module in self:
            _logger.info(
                "Unload theme %s for website %s from template." % (self.mapped("name"), website.id)
            )

            for model_name in self._theme_model_names:
                template = self._get_module_data(model_name)
                models = (
                    template.with_context(**{"active_test": False, MODULE_UNINSTALL_FLAG: True})
                    .mapped("copy_ids")
                    .filtered(lambda m: m.website_id == website)
                )
                models.unlink()
        ...
        for generic_arch_db, specific_arch_db, specific_id in self.env.cr.fetchall():
            ...
            for lang in langs_update:
                specific_arch_db[lang] = field.translate(
                    lambda term: specific_translation_dictionary.get(term, {lang: None})[lang],
                    specific_arch_db_en,
                )
