...
class TestAccessRights(TestCommonSalePurchaseNoChart):
    ...
    def test_access_saleperson(self):
        ...
        with self.assertRaises(AccessError):
            purchase_orders = (
                self.env["purchase.order"].with_user(self.user_salesperson).browse(action["res_id"])
            )
            purchase_orders.read()

        # try to access PO as purchase person
        purchase_orders = (
            self.env["purchase.order"].with_user(self.user_purchaseperson).browse(action["res_id"])
        )
        purchase_orders.read()
