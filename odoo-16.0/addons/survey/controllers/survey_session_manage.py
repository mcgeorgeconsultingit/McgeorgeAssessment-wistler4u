...
class UserInputSession(http.Controller):
    ...
    @http.route(
        "/survey/session/manage/<string:survey_token>", type="http", auth="user", website=True
    )
    def survey_session_manage(self, survey_token, **kwargs):
        ...
        return request.render(
            "survey.user_input_session_manage", self._prepare_manage_session_values(survey)
        )
    ...
    def survey_session_next_question(self, survey_token, go_back=False, **kwargs):
        ...
        if next_question:
            ...
            request.env["bus.bus"]._sendone(
                survey.access_token, "next_question", {"question_start": now.timestamp()}
            )
    ...
    @http.route(
        "/survey/session/results/<string:survey_token>", type="json", auth="user", website=True
    )
    ...
    @http.route(
        "/survey/session/leaderboard/<string:survey_token>", type="json", auth="user", website=True
    )
    ...
    def survey_start_short(self, session_code):
        ...
        if survey_error:
            return request.render(
                "survey.survey_session_code", dict(**survey_error, session_code=session_code)
            )
        return request.redirect(survey.get_start_url())

    @http.route(
        "/survey/check_session_code/<string:session_code>", type="json", auth="public", website=True
    )
    ...
    def _prepare_manage_session_values(self, survey):
        ...
        values.update(
            self._prepare_question_results_values(survey, request.env["survey.user_input.line"])
        )
