...
def parse_args():
    ...
    parser.add_argument(
        "--data-dir", "-D", dest="data_dir", type=str, help="Directory where to store Odoo data"
    )
    parser.add_argument(
        "--skip",
        "-s",
    ...
    parser.add_argument(
        "--uninstall", "-U", type=str, help="Comma-separated list of modules to uninstall/reinstall"
    )
    parser.add_argument(
        "--standalone",
        type=str,
...
def test_scripts(args):
    ...
    funcs = list(
        unique(func for tag in args.standalone.split(",") for func in standalone_tests[tag])
    )
    ...
    for index, func in enumerate(funcs, start=1):
        with odoo.registry(args.database).cursor() as cr:
            env = odoo.api.Environment(cr, odoo.SUPERUSER_ID, {})
            _logger.info(
                "Executing standalone script: %s (%d / %d)", func.__name__, index, len(funcs)
            )
            try:
                func(env)
            except Exception:
