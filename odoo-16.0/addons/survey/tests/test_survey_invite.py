...
class TestSurveyInvite(common.TestSurveyCommon):
            ...
            self.env["survey.survey"].create(
                {
                    ...
                    "question_and_page_ids": [
                        (
                            0,
                            0,
                            {"is_page": True, "question_type": False, "title": "P0", "sequence": 1},
                        )
                    ],
                }
            ),
    ...
    def test_survey_invite(self):
        ...
        self.assertEqual(
            Answer.search([("survey_id", "=", self.survey.id)]), self.env["survey.user_input"]
        )
    ...
    def test_survey_invite_authentication_nosignup(self):
        ...
        self.assertEqual(
            set(answers.mapped("email")), set([self.user_emp.email, self.user_portal.email])
        )
        self.assertEqual(
