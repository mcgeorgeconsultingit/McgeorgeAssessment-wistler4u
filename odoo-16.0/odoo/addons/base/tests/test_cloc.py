...
for rec in records:
    ...
    def test_several_xml_id(self):
        ...
        cl.count_customization(self.env)
        self.assertEqual(
            cl.code.get("odoo/studio", 0), 2, "Count Should count SA with a non standard xml_id"
        )
        self.create_xml_id("__import__", "third", sa)
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(
            cl.code.get("odoo/studio", 0), 2, "SA with several xml_id should be counted only once"
        )
    ...
    def test_cloc_exclude_xml_id(self):
        ...
        cl.count_customization(self.env)
        self.assertEqual(
            cl.code.get("odoo/studio", 0), 0, "Should not count SA with cloc_exclude xml_id"
        )
        ...
        cl.count_customization(self.env)
        self.assertEqual(
            cl.code.get("odoo/studio", 0), 0, "Should not count Field with cloc_exclude xml_id"
        )
