...
class WebsiteCustomer(http.Controller):
    ...
    def sitemap_industry(env, rule, qs):
        ...
        countries = (
            env["res.partner"].sudo().read_group(dom, ["id", "country_id"], groupby="country_id")
        )
    ...
    def customers(self, country=None, industry=None, page=0, **post):
        ...
        industries = Partner.sudo().read_group(
            domain, ["id", "industry_id"], groupby="industry_id", orderby="industry_id"
        )
        ...
        if industry:
            ...
            if industry.id not in (x["industry_id"][0] for x in industries if x["industry_id"]):
                if industry.exists():
                    industries.append(
                        {"industry_id_count": 0, "industry_id": (industry.id, industry.name)}
                    )

        industries.sort(key=lambda d: (d.get("industry_id") or (0, ""))[1])

        industries.insert(
            0, {"industry_id_count": partners_count, "industry_id": (0, _("All Industries"))}
        )

        # group by country, based on customers found with the search(domain)
        countries = Partner.sudo().read_group(
            domain, ["id", "country_id"], groupby="country_id", orderby="country_id"
        )
        ...
        if country:
            ...
            if country.id not in (x["country_id"][0] for x in countries if x["country_id"]):
                if country.exists():
                    countries.append(
                        {"country_id_count": 0, "country_id": (country.id, country.name)}
                    )
                    countries.sort(key=lambda d: (d["country_id"] or (0, ""))[1])

        countries.insert(
            0, {"country_id_count": country_count, "country_id": (0, _("All Countries"))}
        )
