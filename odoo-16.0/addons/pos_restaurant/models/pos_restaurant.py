...
class RestaurantTable(models.Model):
    ...
    shape = fields.Selection(
        [("square", "Square"), ("round", "Round")], string="Shape", required=True, default="square"
    )
    ...
    color = fields.Char(
        "Color", help="The table's color, expressed as a valid 'background' CSS property value"
    )
    ...
    def create_from_ui(self, table):
        ...
        sanitized_table = dict(
            [(key, val) for key, val in table.items() if key in self._fields and val is not None]
        )
...
class RestaurantPrinter(models.Model):
    ...
    proxy_ip = fields.Char(
        "Proxy IP Address", help="The IP Address or hostname of the Printer's hardware proxy"
    )
