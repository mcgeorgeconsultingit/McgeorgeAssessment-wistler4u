...
class StockRule(models.Model):
    ...
    action = fields.Selection(selection_add=[("buy", "Buy")], ondelete={"buy": "cascade"})
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            if not po:
                positive_values = [
                    ...
                    if float_compare(p.product_qty, 0.0, precision_rounding=p.product_uom.rounding)
                    >= 0
                ]
                if positive_values:
    ...
    def _get_procurements_to_merge(self, procurements):
        ...
        return [
            pro_g
            for __, pro_g in groupby(procurements, key=self._get_procurements_to_merge_groupby)
        ]
    ...
    def _update_purchase_order_line(
        ...
        price_unit = (
            self.env["account.tax"]._fix_tax_included_price_company(
                seller.price, line.product_id.supplier_taxes_id, line.taxes_id, company_id
            )
            if seller
            else 0.0
        )
    ...
    def _prepare_purchase_order(self, company_id, origins, values):
        ...
        return {
            ...
            "currency_id": partner.with_company(company_id).property_purchase_currency_id.id
    ...
    def _make_po_get_domain(self, company_id, values, partner):
        ...
        if values.get("orderpoint_id"):
            ...
            delta_days = int(
                self.env["ir.config_parameter"].sudo().get_param("purchase_stock.delta_days_merge")
                or 0
            )
            domain += (
                (
                    ...
                    "<=",
                    datetime.combine(
                        procurement_date + relativedelta(days=delta_days), datetime.max.time()
                    ),
                ),
                (
                    "date_order",
                    ">=",
                    datetime.combine(
                        procurement_date - relativedelta(days=delta_days), datetime.min.time()
                    ),
                ),
            )
        if group:
