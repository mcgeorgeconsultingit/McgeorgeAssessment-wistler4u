...
class SaleOrderLine(models.Model):
    ...
    def _check_event_booth_registration_ids(self):
        if len(self.event_booth_registration_ids.event_booth_id.event_id) > 1:
            raise ValidationError(
                _("Registrations from the same Order Line must belong to a single event.")
            )
    ...
    def _onchange_product_id_booth(self):
        ...
        if self.event_id and (
            not self.product_id or self.product_id not in self.event_booth_pending_ids.product_id
        ):
            self.event_id = None
    ...
    def _get_display_price(self):
        if self.event_booth_pending_ids and self.event_id:
            ...
            if pricelist.discount_policy == "with_discount":
                total_price = sum(
                    [booth.booth_category_id.price_reduce for booth in self.event_booth_pending_ids]
                )
            else:
