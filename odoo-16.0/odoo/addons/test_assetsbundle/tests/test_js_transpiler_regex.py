...
class TestJsTranspiler(TransactionCase):
    def test_correct_ODOO_MODULE_RE(self):
        ...
        for case in cases:
            ...
            if "alias" in case:
                assert ODOO_MODULE_RE.match(case).groupdict().get("alias"), (
                    "URL_RE is failing for alias... >%s<" % case
                )
                assert ODOO_MODULE_RE.match(case).groupdict().get("alias") == "web.test", (
                    "URL_RE does not get the right alias for ... >%s<" % case
                )
            if "default" in case:
                assert ODOO_MODULE_RE.match(case).groupdict().get("default"), (
                    "URL_RE is failing for default... >%s<" % case
                )
                assert ODOO_MODULE_RE.match(case).groupdict().get("default") == "false", (
                    "URL_RE does not get the right default for ... >%s<" % case
                )
    ...
    def test_incorrect_ODOO_MODULE_RE(self):
        ...
        for case in cases:
            assert not ODOO_MODULE_RE.match(case).groupdict().get("alias"), (
                "URL_RE should fail because of too much spaces but didn't... >%s<" % case
            )
        ...
        for case in cases:
            if "alias" in case and "default" in case:
                assert not ODOO_MODULE_RE.match(case).groupdict().get(
                    "alias"
                ) or not ODOO_MODULE_RE.match(case).groupdict().get("default"), (
                    "URL_RE should fail for alias and default... >%s<" % case
                )
            elif "alias" in case:
                assert not ODOO_MODULE_RE.match(case).groupdict().get("alias"), (
                    "URL_RE should fail for alias... >%s<" % case
                )
