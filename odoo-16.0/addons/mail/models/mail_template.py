...
class MailTemplate(models.Model):
    ...
    model = fields.Char(
        "Related Document Model", related="model_id.model", index=True, store=True, readonly=True
    )
    subject = fields.Char(
        "Subject", translate=True, prefetch=True, help="Subject (placeholders may be used here)"
    )
    ...
    email_to = fields.Char(
        "To (Emails)", help="Comma-separated recipient addresses (placeholders may be used here)"
    )
    ...
    def generate_recipients(self, results, res_ids):
        ...
        if (
            ...
            records_company = {
                rec["id"]: (rec["company_id"][0] if rec["company_id"] else None) for rec in records
            }

        for res_id, values in results.items():
            partner_ids = values.get("partner_ids", list())
            if self._context.get("tpl_partners_only"):
                mails = tools.email_split(values.pop("email_to", "")) + tools.email_split(
                    values.pop("email_cc", "")
                )
    ...
    def send_mail(
        ...
        if email_layout_xmlid and values["body_html"]:
            ...
            body = model.env["ir.qweb"]._render(
                email_layout_xmlid, template_ctx, minimal_qcontext=True, raise_if_not_found=False
            )
            if not body:
                _logger.warning(
