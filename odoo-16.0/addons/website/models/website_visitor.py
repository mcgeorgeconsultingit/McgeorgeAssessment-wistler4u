...
class WebsiteVisitor(models.Model):
    ...
    last_visited_page_id = fields.Many2one(
        "website.page", string="Last Visited Page", compute="_compute_last_visited_page_id"
    )

    # Time fields
    create_date = fields.Datetime("First Connection", readonly=True)
    last_connection_datetime = fields.Datetime(
        "Last Connection", default=fields.Datetime.now, help="Last page view date", readonly=True
    )
    ...
    def name_get(self):
        res = []
        for record in self:
            res.append((record.id, record.partner_id.name or _("Website Visitor #%s", record.id)))
        return res
    ...
    def _compute_page_statistics(self):
        ...
        for visitor in self:
            visitor_info = mapped_data.get(
                visitor.id, {"page_count": 0, "visitor_page_count": 0, "page_ids": set()}
            )
    ...
    def _compute_last_visited_page_id(self):
        ...
        mapped_data = {
            result["visitor_id"][0]: result["page_id"][0] for result in results if result["page_id"]
        }
        for visitor in self:
    ...
    def _compute_time_statistics(self):
        for visitor in self:
            ...
            visitor.is_connected = (datetime.now() - visitor.last_connection_datetime) < timedelta(
                minutes=5
            )
    ...
    def _inactive_visitors_domain(self):
        ...
        delay_days = int(
            self.env["ir.config_parameter"].sudo().get_param("website.visitor.live.days", 60)
        )
