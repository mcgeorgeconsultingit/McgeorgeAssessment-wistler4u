...
class MailGroup(models.Model):
    ...
    def _compute_mail_group_message_last_month_count(self):
        ...
        messages_data = {
            message["mail_group_id"][0]: message["mail_group_id_count"] for message in messages_data
        }
    ...
    def _compute_mail_group_message_count(self):
        ...
        result_per_group = {
            result["mail_group_id"][0]: result["mail_group_id_count"] for result in results
        }
        for group in self:
    ...
    def _search_member_partner_ids(self, operator, operand):
        return [
            (
                "member_ids",
                "in",
                self.env["mail.group.member"].sudo()._search([("partner_id", operator, operand)]),
            )
        ]
    ...
    def _check_moderation_guidelines(self):
        if any(
            group.moderation_guidelines and not group.moderation_guidelines_msg for group in self
        ):
    ...
    def _notify_members(self, message):
        ...
        member_emails = {email_normalize(member.email): member.email for member in self.member_ids}
        ...
        for batch_email_member in tools.split_every(batch_size, member_emails.items()):
            ...
            for email_member_normalized, email_member in batch_email_member:
                ...
                footer = self.env["ir.qweb"]._render(
                    "mail_group.mail_group_footer", template_values, minimal_qcontext=True
                )
    ...
    def _notify_moderators(self):
        ...
        for group in groups:
            ...
            for moderator in moderators_to_notify:
                ...
                email_from = (
                    moderator.company_id.catchall_formatted or moderator.company_id.email_formatted
                )
    ...
    def action_join(self):
        ...
        _logger.info(
            '"%s" (#%s) joined mail.group "%s" (#%s)', partner.name, partner.id, self.name, self.id
        )
    ...
    def action_leave(self):
        ...
        _logger.info(
            '"%s" (#%s) leaved mail.group "%s" (#%s)', partner.name, partner.id, self.name, self.id
        )
