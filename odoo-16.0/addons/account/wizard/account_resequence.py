...
class ReSequenceWizard(models.TransientModel):
    ...
    def _compute_preview_moves(self):
        ...
        for record in self:
            new_values = sorted(
                json.loads(record.new_values).values(), key=lambda x: x["server-date"], reverse=True
            )
            changeLines = []
            in_elipsis = 0
            previous_line = None
    ...
    def _compute_new_values(self):
        ...
        for record in self.filtered("first_name"):
            ...
            for j, period_recs in enumerate(moves_by_period.values()):
                    ...
                    seq_format.format(
                        **{
                            **format_values,
                            ...
                            "seq": i
                            + (format_values["seq"] if j == (len(moves_by_period) - 1) else 1),
                        }
                    )
                ...
                for move, new_name in zip(
                    period_recs.sorted(lambda m: (m.date, m.name or "", m.id)), new_name_list
                ):
