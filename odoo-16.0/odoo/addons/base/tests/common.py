    ...
    def _use_currency(cls, currency_code):
        ...
        if not cls.env.company.currency_id == currency:
            cls.env.transaction.cache.set(
                cls.env.company, type(cls.env.company).currency_id, currency.id, dirty=True
            )
...
class MockSmtplibCase:
    ...
    def mock_smtplib_connection(self):
        ...
        with patch(
            "smtplib.SMTP_SSL", side_effect=lambda *args, **kwargs: self.testing_smtp_session
        ), patch(
            "smtplib.SMTP", side_effect=lambda *args, **kwargs: self.testing_smtp_session
        ), patch.object(
            type(IrMailServer), "_is_test_mode", lambda self: False
        ), patch.object(
            type(IrMailServer), "connect", wraps=IrMailServer, side_effect=connect_origin
    ...
    def assert_email_sent_smtp(
        self, smtp_from=None, smtp_to_list=None, message_from=None, from_filter=None, emails_count=1
    ):
