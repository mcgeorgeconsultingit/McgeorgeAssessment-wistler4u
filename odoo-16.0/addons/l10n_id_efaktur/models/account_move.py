...
def _csv_row(data, delimiter=",", quote='"'):
    return (
        quote
        + (quote + delimiter + quote).join([str(x).replace(quote, "\\" + quote) for x in data])
        + quote
        + "\n"
    )
...
class AccountMove(models.Model):
    ...
    def _onchange_l10n_id_tax_number(self):
        for record in self:
            if record.l10n_id_tax_number and record.move_type not in self.get_purchase_types():
                raise UserError(
                    _("You can only change the number manually for a Vendor Bills and Credit Notes")
                )
    ...
    def reset_efaktur(self):
        ...
        for move in self:
            ...
            move.message_post(
                body="e-Faktur Reset: %s " % (move.l10n_id_tax_number), subject="Reset Efaktur"
            )
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == "posted"):
            ...
            eTax["UANG_MUKA_PPN"] = int(
                abs(sum(lines.mapped(lambda l: float_round(l.price_total - l.price_subtotal, 0))))
            )
            ...
            for line in move.line_ids.filtered(lambda l: l.display_type == "product"):
                ...
                if line.price_subtotal < 0:
                    ...
                    line_dict.update(
                        {
                            "DISKON": int(
                                float_round(invoice_line_total_price - line.price_subtotal, 0)
                            ),
