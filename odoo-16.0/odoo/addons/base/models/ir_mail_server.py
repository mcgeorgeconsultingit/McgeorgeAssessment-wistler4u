...
class IrMailServer(models.Model):
    ...
    def write(self, vals):
        ...
        error_usage_details = "\n".join(
            line for server in servers_ordered_by_name for line in usage_details_per_server[server]
        )
        if is_multiple_server_usage:
            raise UserError(
                _(
    ...
    def _get_test_email_addresses(self):
        ...
        if self.from_filter:
            ...
            default_from = (
                self.env["ir.config_parameter"].sudo().get_param("mail.default.from", "odoo")
            )
    ...
    def test_smtp_connection(self):
        for server in self:
            ...
            try:
                ...
                if code != 354:
                    raise UserError(
                        _("The server refused the test connection " "with error %(repl)s")
                        % locals()
                    )
                ...
                raise UserError(
                    _("No response received. Check server address and port number.\n %s", ustr(e))
                )
                ...
                raise UserError(
                    _("An SSL exception occurred. Check connection security type.\n %s", ustr(e))
                )
        ...
        if mail_server:
            ...
            if (
                ...
                try:
                    ...
                    raise UserError(
                        _("The private key or the certificate is not a valid file. \n%s", str(e))
                    )
                except SSLError as e:
                    raise UserError(
            ...
            if smtp_ssl_certificate_filename and smtp_ssl_private_key_filename:
                try:
                    ...
                    ssl_context.load_cert_chain(
                        smtp_ssl_certificate_filename, keyfile=smtp_ssl_private_key_filename
                    )
                    # Check that the private key match the certificate
                    ssl_context._ctx.check_privatekey()
                except SSLCryptoError as e:
    ...
    def build_email(
        ...
        if attachments:
            for (fname, fcontent, mime) in attachments:
                maintype, subtype = (
                    mime.split("/") if mime and "/" in mime else ("application", "octet-stream")
                )
    ...
    def _prepare_email_message(self, message, smtp_session):
        ...
        bounce_address = (
            message["Return-Path"] or self._get_default_bounce_address() or message["From"]
        )
