...
class Event(models.Model):
    ...
    menu_register_cta = fields.Boolean(
        "Extra Register Button", compute="_compute_menu_register_cta", readonly=False, store=True
    )
    # sub-menus management
    introduction_menu = fields.Boolean(
        "Introduction Menu", compute="_compute_website_menu_data", readonly=False, store=True
    )
    ...
    location_menu = fields.Boolean(
        "Location Menu", compute="_compute_website_menu_data", readonly=False, store=True
    )
    ...
    register_menu = fields.Boolean(
        "Register Menu", compute="_compute_website_menu_data", readonly=False, store=True
    )
        ...
        * logged as visitor: check partner or visitor are linked to a
        ...
        if self.env.user._is_public() and not current_visitor:
            ...
            events = (
                ...
                .search(
                    expression.AND(
                        [domain, ["&", ("event_id", "in", self.ids), ("state", "!=", "cancel")]]
                    )
                )
                .event_id
            )
    ...
    def _compute_menu_register_cta(self):
        ...
        for event in self:
            if event.event_type_id and event.event_type_id != event._origin.event_type_id:
                ...
                event.website_menu != event._origin.website_menu or not event.menu_register_cta
            ):
    ...
    def _update_website_menus(self, menus_update_by_field=None):
        ...
        for event in self:
            ...
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("community_menu")
            ):
                event._update_website_menu_entry(
                    "community_menu", "community_menu_ids", "community"
                )
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("introduction_menu")
            ):
                event._update_website_menu_entry(
                    "introduction_menu", "introduction_menu_ids", "introduction"
                )
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("location_menu")
            ):
                event._update_website_menu_entry("location_menu", "location_menu_ids", "location")
            if event.menu_id and (
                not menus_update_by_field or event in menus_update_by_field.get("register_menu")
            ):
    ...
    def _create_menu(self, sequence, name, url, xml_id, menu_type):
        ...
        if not url:
            ...
            page_result = (
                ...
                .sudo()
                .new_page(
                    name=name + " " + self.name, template=xml_id, add_menu=False, ispage=False
                )
            )
            ...
            return [
                ...
                [
                    ...
                    (
                        "date_begin",
                        "<",
                        sd(first_day_of_the_month + relativedelta(months=months_delta + 1)),
                    ),
                ],
                0,
            ]
