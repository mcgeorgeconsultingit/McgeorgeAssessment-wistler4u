...
class StockMove(models.Model):
    ...
    def _default_group_id(self):
        if self.env.context.get("default_picking_id"):
            return (
                self.env["stock.picking"].browse(self.env.context["default_picking_id"]).group_id.id
            )
        return False
    ...
    priority = fields.Selection(
        PROCUREMENT_PRIORITIES, "Priority", default="0", compute="_compute_priority", store=True
    )
    ...
    orderpoint_id = fields.Many2one(
        "stock.warehouse.orderpoint", "Original Reordering Rule", check_company=True, index=True
    )
    ...
    forecast_expected_date = fields.Datetime(
        "Forecasted Expected date", compute="_compute_forecast_information", compute_sudo=True
    )
    ...
    @api.depends(
        "move_line_ids.qty_done", "move_line_ids.product_uom_id", "move_line_nosuggest_ids.qty_done"
    )
    ...
    def _quantity_done_set(self):
        def _process_decrease(move, quantity):
            ...
            for ml in res_mls:
                ...
                if move.picking_id.immediate_transfer:
                    ...
                    float_compare(qty_to_unreserve, 0, precision_rounding=move.product_uom.rounding)
                    > 0
                ):
        ...
        for move in self:
            ...
            qty = float_round(
                move.quantity_done, precision_digits=precision_digits, rounding_method="HALF-UP"
            )
...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    def _multi_line_quantity_done_set(self, quantity_done):
        ...
        if (
            float_compare(
                quantity_done, ml_quantity_done, precision_rounding=self.product_uom.rounding
            )
            != 0
        ):
            raise UserError(
        ...
        if not any(self._ids):
            ...
            result = {
                ...
                for data in self.env["stock.move.line"]._read_group(
                    [("move_id", "in", self.ids)], ["move_id", "reserved_qty"], ["move_id"]
                )
            }
            for move in self:
                move.reserved_availability = move.product_id.uom_id._compute_quantity(
    ...
    def _compute_forecast_information(self):
        ...
        for move in product_moves:
            if move._is_consuming() and move.state == "draft":
                ...
                prefetch_virtual_available[key_virtual_available(move, incoming=True)].add(
                    move.product_id.id
                )
            ...
            if move._is_consuming():
                if move.state == "assigned":
                    ...
                    move.forecast_availability = (
                        virtual_available_dict[key_virtual_available(move)][move.product_id.id]
                        - move.product_qty
                    )
                elif move.state in ("waiting", "confirmed", "partially_available"):
                    outgoing_unreserved_moves_per_warehouse[move.location_id.warehouse_id].add(
                        move.id
                    )
            elif move.picking_type_id.code == "incoming":
    ...
    def _set_date_deadline(self, new_deadline):
        ...
        already_propagate_ids = self.env.context.get("date_deadline_propagate_ids", set()) | set(
            self.ids
        )
    ...
    def _compute_lot_ids(self):
        ...
        for domain in [domain_nosuggest, domain_suggest]:
            ...
            lots_by_move_id_list.append(
                {by_move["move_id"][0]: by_move["lot_ids"] for by_move in lots_by_move_id}
            )
        for move in self:
            move.lot_ids = lots_by_move_id_list[0 if move.picking_type_id.show_reserved else 1].get(
                move._origin.id, []
            )
    ...
    def write(self, vals):
        ...
        if "quantity_done" in vals and any(move.state == "cancel" for move in self):
            raise UserError(
                _("You cannot change a cancelled stock move, create a new line instead.")
            )
        if "product_uom" in vals and any(move.state == "done" for move in self):
            raise UserError(
                _("You cannot change the UoM for a stock move that has been set to 'Done'.")
            )
        ...
        for move in self:
            ...
            if move.location_dest_id.company_id == self.env.company:
                rule = self.env["procurement.group"]._search_rule(
                    move.route_ids, move.product_packaging_id, move.product_id, warehouse_id, domain
                )
            else:
                rule = (
                    self.sudo()
            ...
            if rule and (
                ...
                or move.origin_returned_move_id.location_dest_id.id != rule.location_dest_id.id
            ):
    ...
    def _merge_moves(self, merge_into=False):
        ...
        neg_qty_moves = self.filtered(
            lambda m: float_compare(m.product_qty, 0.0, precision_rounding=m.product_uom.rounding)
            < 0
        )
        ...
        for candidate_moves in candidate_moves_list:
            ...
            candidate_moves = (
                candidate_moves.filtered(lambda m: m.state not in ("done", "cancel", "draft"))
                - neg_qty_moves
            )
        ...
        for neg_move in neg_qty_moves:
            ...
            for pos_move in moves_by_neg_key.get(neg_key(neg_move), []):
                ...
                if (
                    float_compare(
                        pos_move.price_unit, neg_move.price_unit, precision_digits=rounding
                    )
                    == 0
                ):
                    new_total_value = (
                    ...
                    neg_move.price_unit = float_round(
                        new_total_value / neg_move.product_qty, precision_digits=price_unit_prec
                    )
    ...
    def _onchange_lot_ids(self):
        quantity_done = sum(
            ...
            for ml in self.move_line_ids.filtered(lambda ml: not ml.lot_id and ml.lot_name)
        )
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        self.env["procurement.group"].run(
            procurement_requests, raise_user_error=not self.env.context.get("from_orderpoint")
        )
        ...
        for move in neg_r_moves:
            ...
            for m in move.move_orig_ids | move.move_dest_ids:
                ...
                if (
                    float_compare(m.product_uom_qty, 0, precision_rounding=m.product_uom.rounding)
                    < 0
                ):
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        if quantity:
            ...
            if (
                float_compare(quantity, uom_quantity_back_to_product_uom, precision_digits=rounding)
                == 0
            ):
                vals = dict(vals, reserved_uom_qty=uom_quantity)
            else:
                vals = dict(
                    vals, reserved_uom_qty=quantity, product_uom_id=self.product_id.uom_id.id
                )
        package = None
        if reserved_quant:
    ...
    def _update_reserved_quantity(
        ...
        if (
            self.product_packaging_id
            and self.product_id.product_tmpl_id.categ_id.packaging_reserve_method == "full"
        ):
        ...
        for reserved_quant, quantity in quants:
            ...
            if (
                to_update
                and float_compare(
                    quantity, uom_quantity_back_to_product_uom, precision_digits=rounding
                )
                == 0
            ):
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        for move in moves:
            if move.quantity_done <= 0 and not move.is_inventory:
                if (
                    float_compare(
                        move.product_uom_qty, 0.0, precision_rounding=move.product_uom.rounding
                    )
                    == 0
                    or cancel_backorder
                ):
        ...
        for move in moves_todo:
            ...
            rounding = self.env["decimal.precision"].precision_get("Product Unit of Measure")
            if (
                float_compare(move.quantity_done, move.product_uom_qty, precision_digits=rounding)
                < 0
            ):
        ...
        for result_package in moves_todo.mapped("move_line_ids.result_package_id").filtered(
    ...
    def _prepare_move_split_vals(self, qty):
        vals = {
            ...
            "procure_method": "make_to_stock",
            "move_dest_ids": [
                (4, x.id) for x in self.move_dest_ids if x.state not in ("done", "cancel")
            ],
    ...
    def _recompute_state(self):
        ...
        for move in self:
            ...
            if move.state in ("cancel", "done", "draft"):
                ...
                float_compare(
                    move.reserved_availability, move.product_uom_qty, precision_rounding=rounding
                )
                >= 0
            ):
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        for ml in self.move_line_ids:
            if float_is_zero(
                ml.reserved_uom_qty, precision_rounding=ml.product_uom_id.rounding
            ) and float_is_zero(ml.qty_done, precision_rounding=ml.product_uom_id.rounding):
                res.append((2, ml.id))
    ...
    def _adjust_procure_method(self):
        ...
        for location, product_ids in mtso_products_by_locations.items():
            products = (
                self.env["product.product"].browse(product_ids).with_context(location=location.id)
            )
        ...
        for move in mtso_moves:
            ...
            if (
                float_compare(
                    needed_qty, forecasted_qty, precision_rounding=move.product_uom.rounding
                )
                <= 0
            ):
                move.procure_method = "make_to_stock"
    ...
    def _trigger_scheduler(self):
        ...
        for move in self:
            ...
            if orderpoint and move.product_qty > orderpoint.product_min_qty and move.origin:
                ...
                orderpoints_context_by_company[orderpoint.company_id][orderpoint.id].append(
                    move.origin
                )
    ...
    def _get_forecast_availability_outgoing(self, warehouse):
        ...
        for product in product_ids:
            ...
            for demand, out in unreconciled_outs:
                ...
                if (
                    not float_is_zero(remaining, precision_rounding=out.product_id.uom_id.rounding)
