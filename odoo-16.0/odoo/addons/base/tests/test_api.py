...
class TestAPI(SavepointCaseWithUserDemo):
    ...
    def test_50_environment(self):
        ...
        self.assertTrue(
            demo_partner.company_id, "This partner is supposed to be linked to a company"
        )
    ...
    def test_60_reversed(self):
        ...
        reversed_ids = list(
            unique(record.state_id.id for record in reversed(records) if record.state_id)
        )
        ...
        reversed_ids = list(
            unique(child.id for record in reversed(records) for child in record.child_ids)
        )
    ...
    def test_80_filter(self):
        ...
        self.assertEqual(
            ps.filtered(lambda p: p.parent_id.employee), ps.filtered("parent_id.employee")
        )
    ...
    def test_80_map(self):
        ...
        self.assertEqual(ps.mapped(lambda p: p.parent_id.name), [p.parent_id.name for p in ps])
