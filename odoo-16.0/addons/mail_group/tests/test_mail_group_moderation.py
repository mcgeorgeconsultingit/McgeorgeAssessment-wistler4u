...
class TestMailGroupModeration(TestMailListCommon):
    ...
    def test_moderation_rule_api(self):
        ...
        self.assertEqual(
            len(mail_group.moderation_rule_ids), 4, "Should have created only one moderation rule"
        )
        self.assertEqual(
            set(mail_group.moderation_rule_ids.mapped("email")),
        ...
        self.assertEqual(
            moderation_2.status, "allow", "Should have write on the existing moderation rule"
        )
        self.assertEqual(
            moderation_3.status, "ban", "Should not have changed moderation of the other group"
        )
...
class TestModeration(TestMailListCommon):
    ...
    @mute_logger(
        "odoo.addons.mail.models.mail_thread", "odoo.addons.mail_group.models.mail_group_message"
    )
    @users("employee")
    def test_moderation_flow_accept(self):
        ...
        self.assertEqual(
            new_email_message.moderation_status, "accepted", "Should have accepted the message"
        )
        self.assertEqual(
    ...
    def test_moderation_flow_allow(self):
        ...
        mail_group_2_as2 = (
            self.env["mail.group"].with_user(self.user_employee_2).browse(self.test_group_2.ids)
        )
        self.assertEqual(len(mail_group.mail_group_message_ids), 3)
    ...
    def test_moderation_flow_reject(self):
        ...
        self.assertEqual(
            new_email_message.moderation_status, "rejected", "Should have rejected the message"
        )
        self.assertEqual(
            old_email_message.moderation_status,
            "pending_moderation",
