...
class TestDiscussFullPerformance(TransactionCase):
    ...
    def test_init_messaging(self):
        ...
        self.channel_channel_public_1 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(name="public channel 1", group_id=None)["id"]
        )
        self.channel_channel_public_1.add_members(
            (
                self.users[0] + self.users[2] + self.users[3] + self.users[4] + self.users[8]
            ).partner_id.ids
        )
        self.channel_channel_public_2 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(name="public channel 2", group_id=None)["id"]
        )
        self.channel_channel_public_2.add_members(
            (
                self.users[0] + self.users[2] + self.users[4] + self.users[7] + self.users[9]
            ).partner_id.ids
        )
        ...
        self.channel_channel_group_1.add_members(
            (
                self.users[0] + self.users[2] + self.users[3] + self.users[6] + self.users[12]
            ).partner_id.ids
        )
        ...
        self.channel_channel_group_2.add_members(
            (
                self.users[0] + self.users[2] + self.users[6] + self.users[7] + self.users[13]
            ).partner_id.ids
        )
        # create chats
        ...
        self.channel_group_1 = self.env["mail.channel"].browse(
            self.env["mail.channel"].create_group((self.users[0] + self.users[12]).partner_id.ids)[
                "id"
            ]
        )
        # create livechats
        ...
        self.channel_livechat_2.with_user(self.env.ref("base.public_user")).sudo().message_post(
            body="test"
        )
        # add needaction
                    ...
                    "seen_partners_info": [
                        {
                            ...
                            "id": self.channel_livechat_2.channel_member_ids.filtered(
                                lambda m: m.partner_id == self.env.ref("base.public_partner")
                            ).id,
