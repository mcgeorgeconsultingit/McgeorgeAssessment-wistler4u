...
class PropertiesCase(TransactionCase):
    ...
    def test_properties_field_create_batch(self):
        ...
        self.assertEqual(
            properties_values_2[0]["value"], "draft", msg="Should have taken the default value"
        )
    ...
    def test_properties_field_default(self):
        ...
        self.assertEqual(
            message.attributes[0]["value"], "draft", msg="Should have taken the default value"
        )

        message.attributes = [{"name": "state", "value": None}]
        self.assertFalse(
            message.attributes[0]["value"], msg="Writing None should not reset to the default value"
        )
        ...
        with patch.object(
            self.env["test_new_api.message"]._fields["discussion"], "default", default_discussion
        ):
            message = self.env["test_new_api.message"].create(
                {
                    "name": "Test Message",
    ...
    def test_properties_field_many2one_basic(self):
        ...
        self.assertEqual(
            sql_values, {"moderator_partner_id": self.partner_2.id, "discussion_color_code": False}
        )
    ...
    def test_properties_field_tags(self):
        ...
        self.assertEqual(
            message.attributes[0]["tags"], [["be", "BE", 1], ["fr", "FR", 2], ["it", "IT", 1]]
        )
    ...
    def test_properties_field_many2many_filtering(self):
        ...
        self.assertEqual(
            values, [(tag.id, None if i >= 5 else tag.name) for i, tag in enumerate(tags.sudo())]
        )
    ...
    def test_properties_field_change_definition(self):
        ...
        value = self._get_sql_properties(self.message_1)
        self.assertFalse(
            value.get("state"), msg="After updating an other property, the value must be cleaned"
        )
    ...
    def test_properties_field_onchange(self):
        ...
        message = message_form.save()
        self.assertEqual(
            self.discussion_2.attributes_definition, [{"name": "new_property", "type": "char"}]
        )
        self.assertEqual(
            message.attributes, [{"name": "new_property", "type": "char", "value": "test value"}]
        )
