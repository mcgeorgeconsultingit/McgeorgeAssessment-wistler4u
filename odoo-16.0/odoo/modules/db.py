...
def initialize(cr):
    ...
    while True:
        ...
        if not to_auto_install:
            break
        cr.execute(
            """UPDATE ir_module_module SET state='to install' WHERE name in %s""",
...
def create_categories(cr, categories):
    ...
    while categories:
        ...
        xml_id = "module_category_" + ("_".join(x.lower() for x in category)).replace(
            "&", "and"
        ).replace(" ", "_")
        ...
        if not c_id:
            cr.execute(
                "INSERT INTO ir_module_category \
                    (name, parent_id) \
                    VALUES (%s, %s) RETURNING id",
                (Json({"en_US": categories[0]}), p_id),
            )
            c_id = cr.fetchone()[0]
            cr.execute(
                "INSERT INTO ir_model_data (module, name, res_id, model, noupdate) \
                       VALUES (%s, %s, %s, %s, %s)",
                ("base", xml_id, c_id, "ir.module.category", True),
            )
        else:
            c_id = c_id[0]
        p_id = c_id
