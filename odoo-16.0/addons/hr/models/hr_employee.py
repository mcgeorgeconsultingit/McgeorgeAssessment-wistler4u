...
class HrEmployeePrivate(models.Model):
    ...
    private_email = fields.Char(
        related="address_home_id.email", string="Private Email", groups="hr.group_hr_user"
    )
    lang = fields.Selection(
        related="address_home_id.lang", string="Lang", groups="hr.group_hr_user", readonly=False
    )
    ...
    country_of_birth = fields.Many2one(
        "res.country", string="Country of Birth", groups="hr.group_hr_user", tracking=True
    )
    birthday = fields.Date("Date of Birth", groups="hr.group_hr_user", tracking=True)
    ssnid = fields.Char(
        "SSN No", help="Social Security Number", groups="hr.group_hr_user", tracking=True
    )
    sinid = fields.Char(
        "SIN No", help="Social Insurance Number", groups="hr.group_hr_user", tracking=True
    )
    ...
    departure_description = fields.Html(
        string="Additional Information", groups="hr.group_hr_user", copy=False, tracking=True
    )
    ...
    def write(self, vals):
        ...
        if vals.get("user_id"):
            ...
            vals.update(
                self._sync_user(
                    self.env["res.users"].browse(vals["user_id"]), (bool(self.image_1920))
                )
            )
        ...
        if vals.get("department_id") or vals.get("user_id"):
            department_id = (
                vals["department_id"] if vals.get("department_id") else self[:1].department_id.id
            )
    ...
    def toggle_active(self):
        ...
        unarchived_employees.write(
            {"departure_reason_id": False, "departure_description": False, "departure_date": False}
        )
        ...
        if archived_employees:
            ...
            employee_domain = [
                [(field, "in", archived_employees.ids)] for field in employee_fields_to_empty
            ]
            user_domain = [
                [(field, "in", archived_employees.user_id.ids) for field in user_fields_to_empty]
            ]
