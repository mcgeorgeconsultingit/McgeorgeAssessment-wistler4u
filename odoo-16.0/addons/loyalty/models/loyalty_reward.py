...
class LoyaltyReward(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "program_type" in self.env.context:
            ...
            if (
                ...
                and isinstance(program_default_values[program_type]["reward_ids"][1][2], dict)
            ):
                result.update(
                    {
                        k: v
                        for k, v in program_default_values[program_type]["reward_ids"][1][2].items()
                        if k in fields_list
                    }
                )
        return result
    ...
    def name_get(self):
        return [
            (reward.id, "%s - %s" % (reward.program_id.name, reward.description)) for reward in self
        ]
    ...
    discount_applicability = fields.Selection(
        [("order", "Order"), ("cheapest", "Cheapest Product"), ("specific", "Specific Products")],
        default="order",
    )
    ...
    def _get_discount_product_domain(self):
        ...
        if self.discount_product_category_id:
            domain = expression.OR(
                [domain, [("categ_id", "child_of", self.discount_product_category_id.id)]]
            )
        if self.discount_product_tag_id:
            domain = expression.OR(
                [domain, [("all_product_tag_ids", "in", self.discount_product_tag_id.id)]]
            )
