...
def ad_paths():
    warnings.warn(
        '"odoo.modules.module.ad_paths" is a deprecated proxy to ' '"odoo.addons.__path__".',
        DeprecationWarning,
        stacklevel=2,
    )
...
class OdooHook(object):
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r"^openerp\b", name):
            warnings.warn(
                "openerp is a deprecated alias to odoo.", DeprecationWarning, stacklevel=2
            )
            return self
...
def initialize_sys_path():
    ...
    base_path = os.path.normcase(
        os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), "addons"))
    )
...
def get_module_filetree(module, dir="."):
    warnings.warn(
        "Since 16.0: use os.walk or a recursive glob or something", DeprecationWarning, stacklevel=2
    )
...
def load_manifest(module, mod_path=None):
    ...
    if not manifest["description"]:
        readme_path = [opj(mod_path, x) for x in README if os.path.isfile(opj(mod_path, x))]
        if readme_path:
    ...
    if isinstance(manifest["auto_install"], collections.abc.Iterable):
        ...
        assert not non_dependencies, (
            ...
            "non-dependencies [%s] for module %s" % (", ".join(non_dependencies), module)
        )
