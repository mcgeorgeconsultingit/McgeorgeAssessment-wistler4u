...
class AccountAnalyticLine(models.Model):
    ...
    ancestor_task_id = fields.Many2one(
        "project.task", related="task_id.ancestor_id", store=True, index="btree_not_null"
    )
    ...
    def write(self, values):
        ...
        if not (
            self.user_has_groups("hr_timesheet.group_hr_timesheet_approver") or self.env.su
    ...
    def _apply_time_label(self, view_node, related_model):
        ...
        encoding_uom_name = (
            self.env.company.timesheet_encode_uom_id.with_context(prefetch_fields=False).sudo().name
        )
        for node in doc.xpath(
