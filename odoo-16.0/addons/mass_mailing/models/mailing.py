...
class MassMailing(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if (
            ...
            vals.update({"schedule_type": "scheduled", "schedule_date": default_calendar_date})
    ...
    def _check_mailing_filter_model(self):
        ...
        for mailing in self:
            if (
                ...
                and mailing.mailing_model_id != mailing.mailing_filter_id.mailing_model_id
            ):
    ...
    def _compute_mailing_filter_count(self):
        ...
        mapped_data = {
            data["mailing_model_id"][0]: data["mailing_model_id_count"] for data in filter_data
        }
        for mailing in self:
    ...
    def _compute_mail_server_available(self):
        self.mail_server_available = (
            self.env["ir.config_parameter"].sudo().get_param("mass_mailing.outgoing_mail_server")
        )
    ...
    def _compute_mailing_type_description(self):
        for mailing in self:
            mailing.mailing_type_description = dict(self._fields.get("mailing_type").selection).get(
    ...
    def write(self, values):
        ...
        if any(self.mapped("ab_testing_schedule_datetime")):
            schedule_date = min(
                m.ab_testing_schedule_datetime for m in self if m.ab_testing_schedule_datetime
            )
            ab_testing_cron = self.env.ref("mass_mailing.ir_cron_mass_mailing_ab_testing").sudo()
    ...
    def action_put_in_queue(self):
        ...
        cron._trigger(
            schedule_date or fields.Datetime.now() for schedule_date in self.mapped("schedule_date")
        )
    ...
    def action_fetch_favorites(self, extra_domain=None):
        ...
        values_list = [
            values for values in values_list if not tools.is_html_empty(values["body_arch"])
        ]

        # You see first the mailings without responsible, then your mailings and then the others
        values_list.sort(
            key=lambda values: values["user_id"][0] != self.env.user.id if values["user_id"] else -1
        )
    ...
    def update_opt_out(self, email, list_ids, value):
        if len(list_ids) > 0:
            ...
            message = (
                ...
                if value
                else _("The recipient <strong>subscribed to %s</strong> mailing list(s)")
            )
    ...
    def action_send_winner_mailing(self):
        ...
        if len(self.campaign_id) != 1:
            raise ValueError(
                _("To send the winner mailing the same campaign should be used by the mailings")
            )
        if any(mailing.ab_testing_completed for mailing in self):
            raise ValueError(
                _("To send the winner mailing the campaign should not have been completed.")
            )
        ...
        if sorted_by != "manual":
            ...
            selected_mailings = ab_testing_mailings.filtered(lambda m: m.state == "done").sorted(
                sorted_by, reverse=True
            )
    ...
    def _get_default_ab_testing_campaign_values(self, values=None):
        ...
        return {
            ...
            "name": _(
                "A/B Test: %s", values.get("subject") or self.subject or fields.Datetime.now()
            ),
    ...
    def _get_opt_out_list(self):
        ...
        if hasattr(self.env[self.mailing_model_name], "_mailing_get_opt_out_list"):
            ...
            _logger.info(
                "Mass-mailing %s targets %s, blacklist: %s emails", self, target._name, len(opt_out)
            )
        else:
            _logger.info(
                "Mass-mailing %s targets %s, no opt out list available", self, target._name
            )
        return opt_out
    ...
    def _get_remaining_recipients(self):
        ...
        if self.ab_testing_enabled and self.ab_testing_pc == 100:
            trace_domain = expression.AND(
                [
                    trace_domain,
                    [("mass_mailing_id", "=", self._get_ab_testing_siblings_mailings().ids)],
                ]
            )
        else:
            trace_domain = expression.AND(
    ...
    def _action_send_statistics(self):
        ...
        for mailing in self:
            ...
            rendered_body = self.env["ir.qweb"]._render("digest.digest_mail_main", rendering_data)
    ...
    def _convert_inline_images_to_urls(self, body_html):
        ...
        def _image_to_url(b64image: bytes):
            ...
            return "/web/image/%s?access_token=%s" % (attachment.id, attachment.access_token)
