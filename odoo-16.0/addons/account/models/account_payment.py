...
class AccountPayment(models.Model):
    ...
    reconciled_invoices_count = fields.Integer(
        string="# Reconciled Invoices", compute="_compute_stat_buttons_from_reconciliation"
    )
        ...
        for payment in self:
            if payment.journal_id.type == "cash":
                ...
                payment.show_partner_bank_account = (
                    payment.payment_method_code in self._get_method_codes_using_bank_account()
                )
            payment.require_partner_bank_account = (
                payment.state == "draft"
                and payment.payment_method_code in self._get_method_codes_needing_bank_account()
            )
    ...
    @api.depends(
        "partner_id", "company_id", "payment_type", "destination_journal_id", "is_internal_transfer"
    )
    ...
    @api.depends(
        "journal_id", "partner_id", "partner_type", "is_internal_transfer", "destination_journal_id"
    )
    ...
    def name_get(self):
        return [
            (payment.id, payment.move_id.name != "/" and payment.move_id.name or _("Draft Payment"))
            for payment in self
        ]
    ...
    def _synchronize_to_moves(self, changed_fields):
        ...
        if not any(
            field_name in changed_fields for field_name in self._get_trigger_fields_to_synchronize()
        ):
            return
