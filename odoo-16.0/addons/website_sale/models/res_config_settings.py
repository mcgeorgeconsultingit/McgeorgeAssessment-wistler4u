...
class ResConfigSettings(models.TransientModel):
    ...
    salesteam_id = fields.Many2one(
        "crm.team", related="website_id.salesteam_id", string="Sales Team", readonly=False
    )
    ...
    send_abandoned_cart_email = fields.Boolean(
        "Abandoned Email", related="website_id.send_abandoned_cart_email", readonly=False
    )
    add_to_cart_action = fields.Selection(related="website_id.add_to_cart_action", readonly=False)
    terms_url = fields.Char(
        compute="_compute_terms_url", string="URL", help="A preview will be available at this URL."
    )

    module_delivery = fields.Boolean(compute="_compute_module_delivery", store=True, readonly=False)
