...
class TestSaleMrpKitBom(TransactionCase):
    ...
    def test_sale_mrp_kit_cost(self):
        ...
        cat = self.env["product.category"].create({"name": "fifo", "property_cost_method": "fifo"})
        ...
        so.action_confirm()
        line = so.order_line
        purchase_price = line.product_id.with_company(line.company_id)._compute_average_price(
        ...
        kit_2 = self.env["product.template"].create(
            {
                ...
                "attribute_line_ids": [
                    (
                        0,
                        0,
                        {"attribute_id": prod_attr.id, "value_ids": [(6, 0, prod_attr_values.ids)]},
                    )
                ],
            }
        )
