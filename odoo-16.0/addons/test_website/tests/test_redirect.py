...
class TestRedirect(HttpCase):
    ...
    def test_01_redirect_308_model_converter(self):
        ...
        self.assertTrue(
            country_ad.name in r.text, "Ensure the controller returned the expected value"
        )
        self.assertTrue(
            redirect_url in r.text, "Ensure the url_for has replaced the href URL in the DOM"
        )
        ...
        self.assertTrue(
            country_ad.name in r.text, "Ensure the controller returned the expected value (2)"
        )
        self.assertTrue(
            redirect_url in r.text, "Ensure the url_for has replaced the href URL in the DOM"
        )
    ...
    def test_02_redirect_308_RequestUID(self):
        ...
        with patch.object(WebsiteHttp, "_get_error_html", _get_error_html):
            ...
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"), f"{self.base_url()}/test_website/308/name-100"
            )
            ...
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"), f"{self.base_url()}/test_website/308/xx-100"
            )
