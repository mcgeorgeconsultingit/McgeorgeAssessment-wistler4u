...
class StockPickingBatch(models.Model):
    ...
    show_validate = fields.Boolean(compute="_compute_show_validate", string="Show Validate Button")
    ...
    move_ids = fields.One2many("stock.move", string="Stock moves", compute="_compute_move_ids")
    ...
    def _compute_scheduled_date(self):
        for rec in self:
            rec.scheduled_date = min(
                rec.picking_ids.filtered("scheduled_date").mapped("scheduled_date"), default=False
            )
    ...
    def action_done(self):
        def has_no_qty_done(picking):
            return all(
                float_is_zero(line.qty_done, precision_rounding=line.product_uom_id.rounding)
