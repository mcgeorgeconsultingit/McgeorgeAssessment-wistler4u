...
class HolidaysRequest(models.Model):
    ...
    validation_type = fields.Selection(
        string="Validation Type", related="holiday_status_id.leave_validation_type", readonly=False
    )
    # HR data
    ...
    request_date_from_period = fields.Selection(
        [("am", "Morning"), ("pm", "Afternoon")], string="Date Period Start", default="am"
    )
    # request type
    ...
    request_unit_hours = fields.Boolean(
        "Custom Hours", compute="_compute_request_unit_hours", store=True, readonly=False
    )
    # view
    ...
    def _auto_init(self):
        ...
        tools.create_index(
            self._cr, "hr_leave_date_to_date_from_index", self._table, ["date_to", "date_from"]
        )
        return res
        ...
        for holiday in self:
            ...
            if not holiday.request_date_from:
                ...
                attendance_from, attendance_to = holiday._get_attendances(
                    holiday.employee_id, holiday.request_date_from, holiday.request_date_to
                )
                ...
                holiday.date_from = self._get_start_or_end_from_attendance(
                    hour_from, compensated_request_date_from, holiday.employee_id or holiday
                )
    ...
    def _compute_from_holiday_type(self):
        ...
        if self._context.get("active_model") == "hr.leave.allocation" and self._context.get(
            "active_id"
        ):
    ...
    def _compute_has_stress_day(self):
        ...
        if date_from and date_to:
            stress_days = self.employee_id._get_stress_days(date_from.date(), date_to.date())
            ...
            for leave in self:
                ...
                leave.has_stress_day = (
                    leave.date_from and leave.date_to and stress_days.filtered_domain(domain)
                )
        else:
        ...
        for holiday in self:
            ...
            if holiday.date_from and holiday.date_to:
                ...
                if holiday.state == "validate":
                    ...
                    intervals = (
                        calendar._attendance_intervals_batch(start_dt, end_dt, resource)[
                            resource.id
                        ]
                        - calendar._leave_intervals_batch(start_dt, end_dt, None)[False]
                    )  # Substract Global Leaves
                    number_of_hours = sum(
                        (stop - start).total_seconds() / 3600 for start, stop, dummy in intervals
                    )
                else:
    ...
    def _check_date(self):
        ...
        for holiday in self:
            ...
            if conflicting_holidays:
                ...
                holiday_states = dict(
                    conflicting_holidays.fields_get(allfields=["state"])["state"]["selection"]
                )
    ...
    def _check_holidays(self):
        for holiday in self:
            mapped_days = self.holiday_status_id.get_employees_days(
                (holiday.employee_id | holiday.employee_ids).ids, holiday.date_from.date()
            )
            if (
            ...
            if holiday.employee_id:
                ...
                if (
                    ...
                    or float_compare(leave_days["virtual_remaining_leaves"], 0, precision_digits=2)
                    == -1
                ):
                    raise ValidationError(
                        _(
                ...
                for employee in holiday.employee_ids:
                    ...
                    if (
                        float_compare(
                            leave_days["remaining_leaves"], self.number_of_days, precision_digits=2
                        )
                        == -1
                        or float_compare(
    ...
    def create(self, vals_list):
        ...
        if self._context.get("leave_compute_date_from_to") and employees:
            ...
            for (date_from, date_to), employee_ids in leave_date_employees.items():
                employee_leave_date_duration[(date_from, date_to)] = self._get_number_of_days_batch(
                    date_from, date_to, employee_ids
                )
            for values in vals_list:
                ...
                if employee_id and values.get("date_from") and values.get("date_to"):
                    ...
                    values["number_of_days"] = employee_leave_date_duration[(date_from, date_to)][
                        values["employee_id"]
                    ]["days"]
        ...
        if not self._context.get("leave_fast_create"):
            ...
            mapped_validation_type = {
                leave_type.id: leave_type.leave_validation_type for leave_type in leave_types
            }
        ...
        holidays = super(HolidaysRequest, self.with_context(mail_create_nosubscribe=True)).create(
            vals_list
        )
        ...
        for holiday in holidays:
            if not self._context.get("leave_fast_create"):
                ...
                if holiday.validation_type == "no_validation":
                    ...
                    holiday_sudo.message_subscribe(
                        partner_ids=[holiday._get_responsible_for_approval().partner_id.id]
                    )
    ...
    def write(self, values):
        ...
        is_officer = (
            self.env.user.has_group("hr_holidays.group_hr_holidays_user") or self.env.is_superuser()
        )
    ...
    def _unlink_if_correct_states(self):
        ...
        state_description_values = {
            elem[0]: elem[1] for elem in self._fields["state"]._description_selection(self.env)
        }
        now = fields.Datetime.now()
    ...
    def action_confirm(self):
        if self.filtered(lambda holiday: holiday.state != "draft"):
            raise UserError(
                _('Time off request must be in Draft state ("To Submit") in order to confirm it.')
            )
    ...
    def action_refuse(self):
        current_employee = self.env.user.employee_id
        if any(
            holiday.state not in ["draft", "confirm", "validate", "validate1"] for holiday in self
        ):
            raise UserError(
    ...
    def _force_cancel(self, reason, msg_subtype="mail.mt_comment"):
        for leave in self:
            leave.message_post(
                body=_("The time off has been canceled: %s", reason), subtype_xmlid=msg_subtype
            )
    ...
    def split_leave(self, time_domain_dict):
        ...
        for global_time_off in sorted(
            filter(
                lambda r: r["date_to"] > self.date_from and r["date_from"] < self.date_to,
                time_domain_dict,
            ),
    ...
    def _get_start_or_end_from_attendance(self, hour, date, employee):
        ...
        return (
            holiday_tz.localize(datetime.combine(date, hour)).astimezone(UTC).replace(tzinfo=None)
        )
