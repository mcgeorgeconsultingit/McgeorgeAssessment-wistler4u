    ...
    def test_early_payment_end_date(self):
        inv_1200_10_percents_discount_no_tax = self.env["account.move"].create(
            {
                ...
                "date": "2019-01-01",
                "invoice_line_ids": [
                    Command.create({"name": "line", "price_unit": 1200.0, "tax_ids": []})
                ],
        ...
        for line in inv_1200_10_percents_discount_no_tax.line_ids:
            if line.display_type == "payment_term":
                self.assertEqual(line.discount_date, fields.Date.from_string("2019-01-11") or False)
        ...
        out_invoice_1 = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    Command.create(
                        {"product_id": self.product_a.id, "price_unit": 1000.0, "tax_ids": []}
                    )
                ],
                "invoice_payment_term_id": self.early_pay_10_percents_10_days.id,
            }
        ...
        out_invoice_2 = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    Command.create(
                        {"product_id": self.product_a.id, "price_unit": 2000.0, "tax_ids": []}
                    )
                ],
                "invoice_payment_term_id": self.early_pay_10_percents_10_days.id,
            }
