...
class TestSlidesManagement(slides_common.SlidesCase):
    ...
    def test_archive(self):
        ...
        for slide in self.channel.slide_ids:
            ...
            self.assertTrue(
                slide.is_published, "All slide should be unpublished when a channel is archived"
            )
    ...
    def test_mail_completed(self):
        ...
        for user in self.user_officer | self.user_emp:
            self.assertTrue(
                any(
                    mail.model == "slide.channel.partner" and user.partner_id in mail.recipient_ids
