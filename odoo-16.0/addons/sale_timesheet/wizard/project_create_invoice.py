...
class ProjectCreateInvoice(models.TransientModel):
    ...
    project_id = fields.Many2one(
        "project.project", "Project", help="Project to make billable", required=True
    )
    ...
    amount_to_invoice = fields.Monetary(
        ...
        help="Total amount to invoice on the sales order, including all items (services, storables, expenses, ...)",
    )
    ...
    def _compute_candidate_orders(self):
        for p in self:
            p._candidate_orders = p.project_id.mapped("tasks.sale_line_id.order_id").filtered(
