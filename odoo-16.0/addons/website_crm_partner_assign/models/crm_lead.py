        ...
        for lead in self:
            if lead.active and lead.probability < 100:
                if lead.partner_assigned_id and lead.partner_assigned_id.user_id != lead.user_id:
                    salesmans_leads.setdefault(lead.partner_assigned_id.user_id.id, []).append(
                        lead.id
                    )
        ...
        for lead in self:
            ...
            if not partner_id:
                tag_to_add = self.env.ref(
                    "website_crm_partner_assign.tag_portal_lead_partner_unavailable", False
                )
                if tag_to_add:
        ...
        for lead in self:
            ...
            if lead.country_id:
                result = self.env["res.partner"]._geo_localize(
                    lead.street, lead.zip, lead.city, lead.state_id.name, lead.country_id.name
                )
                if result:
