...
ALLOWED_KEYWORD = frozenset(
    ["False", "None", "True", "and", "as", "elif", "else", "for", "if", "in", "is", "not", "or"]
    + list(_BUILTINS)
)
...
class IrQWeb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    @tools.conditional(
        ...
        tools.ormcache(
            "template", "tuple(self.env.context.get(k) for k in self._get_template_cache_keys())"
        ),
    )
    ...
    def _compile(self, template):
        ...
        if ref:
            base_key_cache = self._get_cache_key(
                tuple([ref] + [self.env.context.get(k) for k in self._get_template_cache_keys()])
            )
        ...
        def generate_functions():
            ...
            try:
                ...
                raise QWebException(
                    "Error when compiling xml template", self, template, code=code, ref=ref
                ) from e
    ...
    def _generate_code(self, template):
        ...
        compile_context["ref_name"] = element.attrib.pop(
            "t-name", template if isinstance(template, str) and "<" not in template else None
        )
    ...
    def _load(self, ref):
        ...
        if isinstance(ref, int):
            ...
            model_data = (
                self.env["ir.model.data"].sudo().search_read(domain, ["module", "name"], limit=1)
            )
            if model_data:
            ...
            if t.exact_type in [token.LPAR, token.LSQB, token.LBRACE]:
                ...
                if bracket_depth == 0:
                    ...
                    tokens[open_bracket_index : index + 1] = [
                        tokenize.TokenInfo(
                            token.QWEB, code, tokens[open_bracket_index].start, t.end, ""
                        )
                    ]
        ...
        while index < len(tokens):
            ...
            if t.exact_type == token.NAME:
                if string == "lambda":  # lambda => allowed values
                    ...
                    and tokens[index + 1].exact_type in [token.DOT, token.LPAR, token.LSQB, "qweb"]
                ):
    ...
    def _compile_directives(self, el, compile_context, level):
        ...
        for att in el.attrib:
            if (
                ...
                and getattr(self, f"_compile_directive_{att[2:].replace('-', '_')}", None)
            ):
                code.extend(self._compile_directive(el, compile_context, directive, level))
    ...
    def _compile_directive_options(self, el, compile_context, level):
        ...
        if t_options and dict_options:
            ...
            code.append(
                indent_code(f"values['__qweb_options__'] = {{{', '.join(dict_options)}}}", level)
            )
        elif t_options:
            code.append(
                indent_code(f"values['__qweb_options__'] = {self._compile_expr(t_options)}", level)
            )
        else:
    ...
    def _compile_directive_att(self, el, compile_context, level):
        ...
        if any(not name.startswith("t-") for name in el.attrib):
            nsprefixmap = {
                v: k for k, v in chain(compile_context["nsmap"].items(), el.nsmap.items())
            }
            for key in list(el.attrib):
        ...
        for key in list(el.attrib):
            if key.startswith("t-attf-"):
                value = el.attrib.pop(key)
                code.append(
                    indent_code(f"attrs[{key[7:]!r}] = {self._compile_format(value)}", level)
                )
        ...
        * whose value is the content of the tag (being Markup safe).
        ...
        if "t-set" in el.attrib:
            ...
            if "t-value" in el.attrib:
                ...
                code.append(
                    indent_code(f"values[{varname!r}] = {self._compile_format(exprf)}", level)
                )
            elif varname[0] == "{":
    ...
    def _compile_directive_out(self, el, compile_context, level):
        ...
        if expr == T_CALL_SLOT and code_options != "True":
            ...
            if expr == T_CALL_SLOT:
                code.append(
                    indent_code(f"content = Markup(''.join(values.get({T_CALL_SLOT}, [])))", level)
                )
            else:
    ...
    def _compile_directive_field(self, el, compile_context, level):
        ...
        assert tagName != "t", "t-field can not be used on a t element, provide an actual HTML node"
        assert "." in el.get("t-field"), "t-field must have at least a dot like 'record.field_name'"
    ...
    def _compile_directive_nocache(self, el, compile_context, level):
        ...
        code.append(
            indent_code(
                f"yield ({compile_context['template']!r}, {def_name!r}, cached_values)", level + 1
            )
        )
    ...
    def _generate_asset_nodes(
        ...
        files, remains = self._get_asset_content(
            bundle, defer_load=defer_load, lazy_load=lazy_load, media=css and media or None
        )
        asset = self._get_asset_bundle(bundle, files, env=self.env, css=css, js=js)
        remains = [
            node for node in remains if (css and node[0] == "link") or (js and node[0] == "script")
        ]
