...
def migrate_set_tags_and_taxes_updatable(cr, registry, module):
    ...
    if xml_record_ids:
        cr.execute(
            "update ir_model_data set noupdate = 'f' where id in %s", (tuple(xml_record_ids),)
        )
...
def preserve_existing_tags_on_taxes(cr, registry, module):
    ...
    if xml_records:
        cr.execute(
            "update ir_model_data set noupdate = 't' where id in %s", [tuple(xml_records.ids)]
        )
...
def update_taxes_from_templates(cr, chart_template_xmlid):
    ...
    def _cleanup_tags(tags):
        ...
        for tag in tags:
            ...
            aml_using_tag = (
                env["account.move.line"].sudo().search([("tax_tag_ids", "in", tag.id)], limit=1)
            )
                    ...
                    tax_template_vals.append(
                        (
                            position_tax,
                            {
                                ...
                                "tax_dest_id": position_tax.tax_dest_id
        ...
        for account in self:
            if not re.match(ACCOUNT_CODE_REGEX, account.code):
                raise ValidationError(
                    _("The account code can only contain alphanumeric characters and dots.")
                )
...
class AccountChartTemplate(models.Model):
    ...
    code_digits = fields.Integer(
        string="# of Digits", required=True, default=6, help="No. of Digits to use for account code"
    )
    ...
    account_ids = fields.One2many(
        "account.account.template", "chart_template_id", string="Associated Account Templates"
    )
    ...
    def _load(self, company):
        ...
        if existing_accounts:
            ...
            prop_values = [
                "account.account,%s" % (account_id,) for account_id in existing_accounts.ids
            ]
            ...
            if existing_journals:
                prop_values.extend(
                    ["account.journal,%s" % (journal_id,) for journal_id in existing_journals.ids]
                )
        ...
        if not company.account_journal_suspense_account_id:
            company.account_journal_suspense_account_id = (
                self._create_liquidity_journal_suspense_account(company, self.code_digits)
            )
            ...
            company.account_journal_payment_debit_account_id = self.env["account.account"].create(
                {
                    ...
                    "code": self.env["account.account"]._search_new_account_code(
                        company, self.code_digits, company.bank_account_code_prefix or ""
                    ),
                    "reconcile": True,
                    "account_type": "asset_current",
                    "company_id": company.id,
        ...
        company.account_sale_tax_id = (
            ...
            .search(
                [("type_tax_use", "in", ("sale", "all")), ("company_id", "=", company.id)], limit=1
            )
            .id
        )
    ...
    def existing_accounting(self, company_id):
        ...
        if (
            ...
            .sudo()
            .search([("company_id", "=", company_id.id), ("state", "!=", "draft")], limit=1)
        ):
            return True
        return False
    ...
    def _load_template(self, company, code_digits=None, account_ref=None, taxes_ref=None):
        ...
        generated_tax_res = self.with_context(active_test=False).tax_template_ids._generate_tax(
            company
        )
...
class AccountTaxTemplate(models.Model):
    ...
    def _try_instantiating_foreign_taxes(self, country, company):
        ...
        def create_foreign_tax_account(existing_account, additional_label):
            new_code = self.env["account.account"]._search_new_account_code(
                existing_account.company_id, len(existing_account.code), existing_account.code[:-2]
            )
            return self.env["account.account"].create(
                {
        ...
        def get_existing_tax_account(foreign_tax_rep_line, force_tax=None):
            ...
            return (
                self.env["account.tax.repartition.line"].search(search_domain, limit=1).account_id
            )
    ...
    def _get_tax_vals(self, company, tax_template_to_tax):
        ...
        if self.invoice_repartition_line_ids:
            ...
            ] = self.invoice_repartition_line_ids.get_repartition_line_create_vals(company)
        if self.refund_repartition_line_ids:
            val[
                "refund_repartition_line_ids"
            ] = self.refund_repartition_line_ids.get_repartition_line_create_vals(company)
            ...
            for template in templates:
                if all(child in tax_template_to_tax for child in template.children_tax_ids):
                    ...
                    if self.chart_template_id.country_id:
                        ...
                        raise UserError(
                            _("Please first define a fiscal country for company %s.", company.name)
                        )
...
class AccountTaxRepartitionLineTemplate(models.Model):
                ...
                if vals.get("account_id"):
                    ...
                    if account_type:
                        vals["use_in_tax_closing"] = not (
                            account_type.startswith("income") or account_type.startswith("expense")
                        )
    ...
    def _validate_report_expressions(self):
        for record in self:
            all_engines = set(
                (record.plus_report_expression_ids + record.minus_report_expression_ids).mapped(
                    "engine"
                )
            )
...
class AccountFiscalPositionTemplate(models.Model):
    ...
    account_ids = fields.One2many(
        "account.fiscal.position.account.template", "position_id", string="Account Mapping"
    )
...
class AccountReconcileModelLineTemplate(models.Model):
    ...
    force_tax_included = fields.Boolean(
        string="Tax Included in Price", help="Force the tax to be managed as a price included tax."
    )
