...
class TestPoSSetup(TestPoSCommon):
    ...
    def test_taxes(self):
        ...
        self.assertEqual(
            tax7.invoice_repartition_line_ids.mapped("account_id").id, self.tax_received_account.id
        )
        ...
        self.assertEqual(
            tax10.invoice_repartition_line_ids.mapped("account_id").id, self.tax_received_account.id
        )
