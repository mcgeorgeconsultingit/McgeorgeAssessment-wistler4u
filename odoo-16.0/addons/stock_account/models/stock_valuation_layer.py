...
class StockValuationLayer(models.Model):
    ...
    account_move_id = fields.Many2one(
        "account.move", "Journal Entry", readonly=True, check_company=True, index="btree_not_null"
    )
    ...
    def init(self):
        tools.create_index(
            ...
            self._table,
            ["product_id", "remaining_qty", "stock_move_id", "company_id", "create_date"],
        )
    ...
    def _consume_specific_qty(self, qty_valued, qty_to_value):
        ...
        for candidate in self:
            ...
            tmp_value += qty_taken_on_candidate * (
                (candidate.value + sum(candidate.stock_valuation_layer_ids.mapped("value")))
                / candidate.quantity
            )
