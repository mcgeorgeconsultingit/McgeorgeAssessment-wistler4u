...
class TestIsMultiLang(odoo.tests.HttpCase):
    def test_01_is_multilang_url(self):
        ...
        for data in [None, {"post": True}]:  # GET / POST
            ...
            self.assertEqual(
                "/get_post_nomultilang", body.find('./a[@id="get_post_nomultilang"]').get("href")
            )
    ...
    def test_02_url_lang_code_underscore(self):
        ...
        country1.update_field_translations(
            "name", {it.code: country1.name + " Italia", be.code: country1.name + " Belgium"}
        )
    ...
    def test_03_head_alternate_href(self):
        ...
        self.assertRegex(
            r.text, r'<link rel="alternate" hreflang="en" href="http://[^"]+/contactus"/>'
        )
