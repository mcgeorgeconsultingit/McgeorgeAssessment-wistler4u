...
class PosOrder(models.Model):
                    ...
                    (
                        ...
                        None,
                        {
                            "name": _(
                                ...
                                float_repr(
                                    line.product_id.lst_price, self.currency_id.decimal_places
                                ),
    ...
    account_move = fields.Many2one(
        "account.move", string="Invoice", readonly=True, copy=False, index="btree_not_null"
    )
        ...
        for order in self:
            if order.is_total_cost_computed:
                ...
                order.margin_percent = (
                    not float_is_zero(amount_untaxed, order.currency_id.rounding)
    ...
    def _compute_batch_amount_all(self):
        ...
        """
        amounts = {
            order_id: {"paid": 0, "return": 0, "taxed": 0, "taxes": 0} for order_id in self.ids
        }
    ...
    def _prepare_invoice_vals(self):
        ...
        invoice_date = (
            fields.Datetime.now() if self.session_id.state == "closed" else self.date_order
        )
        vals = {
    ...
    def _apply_invoice_payments(self):
        ...
        payment_moves = (
            self.payment_ids.sudo().with_company(self.company_id)._create_payment_moves()
        )
    ...
    def _prepare_mail_values(self, name, client, ticket):
        message = _("<p>Dear %s,<br/>Here is your electronic ticket for the %s. </p>") % (
            client["name"],
            name,
        )
...
class PosOrderLine(models.Model):
    ...
    def _order_line_fields(self, line, session_id=None):
        ...
        line = [
            line[0],
            line[1],
            {k: v for k, v in line[2].items() if k in self.env["pos.order.line"]._fields},
        ]
        return line
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            price = self.order_id.pricelist_id._get_product_price(self.product_id, self.qty or 1.0)
    ...
    def _compute_margin(self):
        for line in self:
            ...
            line.margin_percent = (
                not float_is_zero(line.price_subtotal, line.currency_id.rounding)
