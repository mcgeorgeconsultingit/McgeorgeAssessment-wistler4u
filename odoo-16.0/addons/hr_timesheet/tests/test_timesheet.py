...
class TestTimesheet(TestCommonTimesheet):
    def setUp(self):
        ...
        # but there is an ir.rule about it, crashing during its evaluation
        rule = self.env.ref(
            "timesheet_grid.timesheet_line_rule_user_update-unlink", raise_if_not_found=False
        )
        if rule:
            rule.active = False
    ...
    def test_transfert_project(self):
        ...
        self.assertEqual(
            len(task_child.timesheet_ids), 1, "The timesheet should be linked to task_child"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(self.task1.timesheet_ids), 1, "The timesheet still should be linked to task1"
        )
        self.assertEqual(
            len(task_child.timesheet_ids), 1, "The timesheet still should be linked to task_child"
        )
        self.assertEqual(
            len(task_grandchild.timesheet_ids),
            1,
    ...
    def test_task_with_timesheet_project_change(self):
        ...
        task_1 = self.env["project.task"].create(
            {"name": "First task", "user_ids": self.user_employee2, "project_id": project.id}
        )
    ...
    def test_ensure_product_uom_set_in_timesheet(self):
        self.assertFalse(
            self.project_customer.timesheet_ids, "No timesheet should be recorded in this project"
        )
        self.assertFalse(
