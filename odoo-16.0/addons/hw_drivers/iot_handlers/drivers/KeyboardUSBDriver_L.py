...
class KeyboardUSBDriver(Driver):
    ...
    def send_layouts_list(cls):
        ...
        if server:
            ...
            try:
                pm.request(
                    "POST", server, fields={"available_layouts": json.dumps(cls.available_layouts)}
                )
    ...
    def load_layouts_list(cls):
        tree = etree.parse(
            "/usr/share/X11/xkb/rules/base.xml", etree.XMLParser(ns_clean=True, recover=True)
        )
    ...
    def _change_keyboard_layout(self, new_layout):
        ...
        subprocess.call(
            ["setxkbmap", "-display", ":0.0", ",".join(KeyboardUSBDriver.keyboard_layout_groups)]
        )
    ...
    def _is_scanner(self):
        ...
        is_scanner = (
            any(x in device_name for x in scanner_name) or self.dev.interface_protocol == "0"
        )
    ...
    def _scancode_to_char(self, scancode):
        ...
        keysym = ctypes.c_int(
            xlib.XkbKeycodeToKeysym(KeyboardUSBDriver.display, scancode + 8, group, modifiers)
        )
    ...
    def _get_active_modifiers(self, scancode):
        ...
        modifiers = 0
        uppercase = (
            self._tracked_modifiers["right_shift"] or self._tracked_modifiers["left_shift"]
