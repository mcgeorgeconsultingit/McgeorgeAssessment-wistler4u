...
class MrpWorkcenter(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", "Currency", related="company_id.currency_id", readonly=True, required=True
    )
    ...
    oee = fields.Float(
        compute="_compute_oee", help="Overall Equipment Effectiveness, based on the last month"
    )
    oee_target = fields.Float(
        string="OEE Target", help="Overall Effective Efficiency Target in percentage", default=90
    )
    performance = fields.Integer(
        "Performance", compute="_compute_performance", help="Performance over the last month"
    )
        ...
        for workcenter in self:
            if workcenter in workcenter.alternative_workcenter_ids:
                raise ValidationError(
                    _("Workcenter %s cannot be an alternative of itself.", workcenter.name)
                )
    ...
    def _compute_working_state(self):
        for workcenter in self:
            ...
            time_log = self.env["mrp.workcenter.productivity"].search(
                [("workcenter_id", "=", workcenter.id), ("date_end", "=", False)], limit=1
            )
            if not time_log:
    ...
    def _compute_oee(self):
        for order in self:
            if order.productive_time:
                order.oee = round(
                    order.productive_time * 100.0 / (order.productive_time + order.blocked_time), 2
                )
            else:
                order.oee = 0.0
        ...
        for workcenter in self:
            if duration.get(workcenter.id):
                workcenter.performance = (
                    100 * duration_expected.get(workcenter.id, 0.0) / duration[workcenter.id]
                )
            else:
    ...
    def create(self, vals_list):
        ...
        records = super(MrpWorkcenter, self.with_context(default_resource_type="material")).create(
            vals_list
        )
        return records
                    ...
                    if (
                        Intervals(
                            [
                                (
                                    start_interval,
                                    start + timedelta(minutes=min(remaining, interval_minutes)),
                                    dummy,
                                )
                            ]
                        )
...
class MrpWorkcenterProductivityLoss(models.Model):
    ...
    loss_type = fields.Selection(
        string="Effectiveness Category", related="loss_id.loss_type", store=True, readonly=False
    )
...
class MrpWorkcenterProductivity(models.Model):
    ...
    loss_id = fields.Many2one(
        "mrp.workcenter.productivity.loss", "Loss Reason", ondelete="restrict", required=True
    )
    loss_type = fields.Selection(
