...
class AccountingTestTemplConsistency(TransactionCase):
    ...
    def check_fields_consistency(self, model_from, model_to, exceptions=None):
        ...
        from_fields = self.get_model_fields(model_from, extra_domain=extra_domain).filtered_domain(
            [("modules", "=", "account")]
        )

        to_fields_set = set([f.name for f in self.get_model_fields(model_to)])
        for field in from_fields:
            assert field.name in to_fields_set, 'Missing field "%s" from "%s" in model "%s".' % (
                field.name,
                model_from,
                model_to,
            )
    ...
    def test_fiscal_position_fields(self):
        ...
        # accounts
        self.check_fields_consistency(
            "account.fiscal.position.account.template", "account.fiscal.position.account"
        )
        self.check_fields_consistency(
            "account.fiscal.position.account", "account.fiscal.position.account.template"
        )
    ...
    def test_account_group_fields(self):
        ...
        self.check_fields_consistency(
            "account.group", "account.group.template", exceptions=["company_id", "parent_path"]
        )
        self.check_fields_consistency(
