...
class BaseImportCase(TransactionCase):
    def assertEqualFields(self, fields1, fields2):
        ...
        assert f1 == f2, "\n".join(
            difflib.unified_diff(pprint.pformat(f1).splitlines(), pprint.pformat(f2).splitlines())
        )
...
class TestBasicFields(BaseImportCase):
    ...
    def test_readonly_states(self):
        ...
        self.assertEqualFields(
            ...
            make_field(field_type="char", model_name="base_import.tests.models.char.states"),
        )
...
class TestMatchHeadersSingle(TransactionCase):
    ...
    def test_distant_match(self):
        ...
        match = Import._get_mapping_suggestion(
            header, [{"name": "bob", "string": field_string, "type": "char"}], ["char"], {}
        )
...
class test_convert_import_data(TransactionCase):
    ...
    def test_parse_relational_fields(self):
        ...
        data, import_fields = import_wizard._convert_import_data(
            ["name", "parent_id/.id", "parent_id/date", "parent_id/partner_latitude"], options
        )
    ...
    def test_filtered(self):
        ...
        data, fields = import_wizard._convert_import_data(
            ["name", False, "othervalue"], {"quoting": '"', "separator": ",", "has_headers": True}
        )
    ...
    def test_empty_rows(self):
        ...
        data, fields = import_wizard._convert_import_data(
            ["name", "somevalue"], {"quoting": '"', "separator": ",", "has_headers": True}
        )
...
foo2,US,persons\n""",
        ...
        self.assertEqual(
            partners_now[0].company_type, "person", "Foo partner's country should be person"
        )

        self.assertEqual(
            partners_now[1].country_id.id, False, "foo1 partner's country should be False"
        )

        self.assertEqual(
            partners_now[2].company_type, False, "foo2 partner's country should be False"
        )
        # if results empty, no errors
        self.assertItemsEqual(results["messages"], [])
...
class TestBatching(TransactionCase):
    ...
    def test_limit_on_lines(self):
        ...
        * if we have one line per record, we probably want to import <limit>
        ...
        results = import_wizard.execute_import(["name", "value/value"], [], {**opts, "limit": 5})
        ...
        results = import_wizard.execute_import(["name", "value/value"], [], {**opts, "limit": 15})
        self.assertFalse(results["messages"])
        self.assertEqual(
            len(results["ids"]),
            2,
...
class test_failures(TransactionCase):
    def test_big_attachments(self):
        ...
        writer.writerows(
            [["name", "db_datas"], ["foo", base64.b64encode(im.tobytes()).decode("ascii")]]
        )
        ...
        results = import_wizard.execute_import(
            ["name", "db_datas"], [], {"has_headers": True, "separator": ",", "quoting": '"'}
        )
