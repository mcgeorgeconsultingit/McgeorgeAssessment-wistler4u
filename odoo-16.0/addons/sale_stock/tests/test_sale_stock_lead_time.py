...
class TestSaleStockLeadTime(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def test_01_product_route_level_delays(self):
        ...
        out = order.picking_ids.filtered(
            lambda r: r.picking_type_id == self.company_data["default_warehouse"].out_type_id
        )
        ...
        pack = order.picking_ids.filtered(
            lambda r: r.picking_type_id == self.company_data["default_warehouse"].pack_type_id
        )
        ...
        pick = order.picking_ids.filtered(
            lambda r: r.picking_type_id == self.company_data["default_warehouse"].pick_type_id
        )
