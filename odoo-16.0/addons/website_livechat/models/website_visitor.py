...
class WebsiteVisitor(models.Model):
    ...
    mail_channel_ids = fields.One2many(
        "mail.channel", "livechat_visitor_id", string="Visitor's livechat channels", readonly=True
    )
    ...
    def _compute_livechat_operator_id(self):
        ...
        visitor_operator_map = {
            int(result["livechat_visitor_id"][0]): int(result["livechat_operator_id"][0])
        ...
        if mail_channel_vals_list:
            ...
            for mail_channel_info in mail_channels_info:
                notifications.append(
                    [operator.partner_id, "website_livechat.send_chat_request", mail_channel_info]
                )
    ...
    def _upsert_visitor(self, access_token, force_track_values=None):
        ...
        if upsert == "inserted":
            ...
            if mail_channel_uuid:
                mail_channel = (
                    request.env["mail.channel"].sudo().search([("uuid", "=", mail_channel_uuid)])
                )
                mail_channel.write(
                    {
