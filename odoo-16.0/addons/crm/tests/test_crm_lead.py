...
class TestCRMLead(TestCrmCommon):
    ...
    def test_crm_lead_contact_fields_mixed(self):
        ...
        self.assertLeadAddress(
            lead, False, False, False, False, self.env["res.country.state"], self.country_ref
        )
        # other contact fields
        ...
        self.assertEqual(
            lead.function, "Parmesan Rappeur", "User input should take over partner value"
        )
    ...
    def test_crm_lead_creation_partner_address(self):
        ...
        self.assertEqual(
            lead.street2, empty_partner.street2, "Street 2 should be sync from the Partner"
        )
        ...
        self.assertEqual(
            lead.state_id, empty_partner.state_id, "State should be sync from the Partner"
        )
        self.assertEqual(
            lead.country_id, empty_partner.country_id, "Country should be sync from the Partner"
        )
    ...
    def test_crm_lead_creation_partner_no_address(self):
        ...
        self.assertEqual(
            lead.email_from, lead_data["email_from"], "Email From should keep its initial value"
        )
        self.assertEqual(
            lead.partner_name,
            empty_partner.name,
        ...
        self.assertEqual(
            lead.street2, False, "Street2 should remain since partner has no address field set"
        )
        self.assertEqual(
            lead.country_id,
            self.country_ref,
        ...
        self.assertEqual(
            lead.city, False, "City should remain since partner has no address field set"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lead.mobile, empty_partner.mobile, "Mobile from partner should be set on the lead"
        )
        self.assertEqual(
            lead.title, empty_partner.title, "Title from partner should be set on the lead"
        )
        self.assertEqual(
            lead.function, empty_partner.function, "Function from partner should be set on the lead"
        )
        self.assertEqual(
    ...
    def test_crm_lead_currency_sync(self):
        lead = self.env["crm.lead"].create({"name": "Lead 1", "company_id": self.company_main.id})
    ...
    def test_crm_lead_lost_date_closed(self):
        ...
        self.assertEqual(
            lead.date_closed, datetime.now(), "Closed date is updated after marking lead as lost"
        )
    ...
    def test_crm_lead_partner_sync_email_phone(self):
        ...
        with self.debug_mode():
            ...
            self.assertEqual(
                lead_form.phone, partner_phone_formatted, "Lead: form automatically formats numbers"
            )
            self.assertEqual(
                lead_form.mobile,
            ...
            lead_form.save()
            self.assertEqual(
                partner.phone, partner_phone, "Lead / Partner: partner values sent to lead"
            )
            self.assertEqual(
                lead.email_from, partner_email, "Lead / Partner: partner values sent to lead"
            )
            self.assertEqual(
    ...
    def test_crm_lead_partner_sync_email_phone_corner_cases(self):
        ...
        with self.debug_mode():
            ...
            self.assertFalse(
                lead_form.partner_email_update, "Formatting-only change should not trigger write"
            )
            self.assertFalse(
                lead_form.partner_phone_update, "Formatting-only change should not trigger write"
            )
    ...
    def test_phone_mobile_search_format(self):
        ...
        self.assertEqual(
            leads, self.env["crm.lead"].search([("phone_mobile_search", "like", "0499223311")])
        )
        self.assertEqual(
            leads, self.env["crm.lead"].search([("phone_mobile_search", "like", "0499/223311")])
        )
        self.assertEqual(
            leads, self.env["crm.lead"].search([("phone_mobile_search", "like", "0499/22.33.11")])
        )
        self.assertEqual(
            leads, self.env["crm.lead"].search([("phone_mobile_search", "like", "0499/22 33 11")])
        )
        self.assertEqual(
            leads, self.env["crm.lead"].search([("phone_mobile_search", "like", "0499/223 311")])
        )
