    ...
    def test_populate_factories(self):
        for model in self.env.values():
            ...
            factories_fields = set(
                [field_name for field_name, factory in factories if not field_name.startswith("_")]
            )
            ...
            if not factories:
                ...
                def is_electable(field):
                    return (
                        ...
                        and field.store
                        and field.name
                        not in ("create_uid", "write_uid", "write_date", "create_date", "id")
                ...
                if no_factory_fields:
                    _logger.info(
                        "Model %s has some undefined field: %s", model._name, no_factory_fields
                    )
