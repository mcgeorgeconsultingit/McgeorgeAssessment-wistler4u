...
class TestFields(TransactionCaseWithUserDemo):
    ...
    def test_17_compute_depends_on_many2many(self):
        ...
        group.write(
            {"user_ids": [Command.link(user1.id), Command.link(user2.id), Command.link(user3.id)]}
        )
    ...
    def test_21_date_datetime_helpers(self):
        ...
        self.assertEqual(
            end_of(_datetime, "year"), datetime.combine(_date.replace(month=12, day=31), time.max)
        )
        ...
        self.assertEqual(end_of(_datetime, "month"), datetime.combine(date(2077, 10, 31), time.max))

        # week
        self.assertEqual(end_of(_date, "week"), date(2077, 10, 24))
        self.assertEqual(
            end_of(_datetime, "week"), datetime.combine(datetime(2077, 10, 24), time.max)
        )
    ...
    def test_85_binary_guess_zip(self):
        ...
        self.env["test_new_api.binary_svg"].with_user(self.env.ref("base.user_demo"),).create(
            {
...
class TestX2many(common.TransactionCase):
    ...
    def test_search_one2many(self):
        ...
        # test 'in'
        result = recs.search(
            [("id", "in", recs.ids), ("lines", "in", (line1 + line2 + line3 + line4).ids)]
        )
...
%endif"""
    ...
    def test_01_sanitize_groups(self):
        ...
        bypass_user = self.env["res.users"].create(
            {
                ...
                "groups_id": [
                    (6, 0, [self.ref("base.group_user"), self.ref("base.group_sanitize_override")])
                ],
            }
        )
        record = self.env["test_new_api.mixed"].create({})
        ...
        normalized_val = (
            '<blockquote data-o-mail-quote-node="1" data-o-mail-quote="1">Something</blockquote>'
        )
        write_vals = {"comment5": val}

        record.with_user(internal_user).write(write_vals)
        self.assertEqual(record.comment5, normalized_val, "should be normalized (not in groups)")
...
class TestParentStore(common.TransactionCase):
    def setUp(self):
        ...
        self._cats = Cat.concat(cat0, cat1, cat2, cat3, cat4, cat5, cat6, cat7, cat8, cat9)
...
class TestSelectionOndeleteAdvanced(common.TransactionCase):
    ...
    def setUp(self):
        ...
        for model_name in (self.MODEL_BASE, self.MODEL_REQUIRED):
            ...
            self.addCleanup(
                setattr, Model, "_BaseModel__base_classes", Model._BaseModel__base_classes
            )
...
def select(model, *fnames):
    ...
    table = model._table
    terms = ", ".join(f'"{table}"."{fname}" AS "{fname}"' for fname in ["id"] + list(fnames))
