...
class Repair(models.Model):
    ...
    name = fields.Char(
        "Repair Reference", default="New", index="trigram", copy=False, required=True, readonly=True
    )
    ...
    invoice_method = fields.Selection(
        [("none", "No Invoice"), ("b4repair", "Before Repair"), ("after_repair", "After Repair")],
    ...
    move_id = fields.Many2one(
        "stock.move", "Inventory Move", copy=False, readonly=True, tracking=True, check_company=True
    )
    fees_lines = fields.One2many("repair.fee", "repair_id", "Operations", copy=True, readonly=False)
    internal_notes = fields.Html("Internal Notes")
    quotation_notes = fields.Html("Quotation Notes")
    user_id = fields.Many2one(
        "res.users", string="Responsible", default=lambda self: self.env.user, check_company=True
    )
    ...
    def onchange_product_id(self):
        ...
        if (
            self.product_id and self.lot_id and self.lot_id.product_id != self.product_id
    ...
    def action_validate(self):
        ...
        available_qty_owner = self.env["stock.quant"]._get_available_quantity(
            self.product_id, self.location_id, self.lot_id, owner_id=self.partner_id, strict=True
        )
    ...
    def _create_invoices(self, group=False):
        ...
        for repair in repairs:
            ...
            for fee in repair.fees_lines:
                ...
                account = fee.product_id.product_tmpl_id.get_product_accounts(fiscal_pos=fpos)[
                    "income"
                ]
                if not account:
    ...
    def action_repair_done(self):
        ...
        for repair in self:
            ...
            if (
                float_compare(available_qty_owner, repair.product_qty, precision_digits=precision)
                >= 0
            ):
...
class RepairFee(models.Model):
    ...
    repair_id = fields.Many2one(
        "repair.order", "Repair Order Reference", index=True, ondelete="cascade", required=True
    )
