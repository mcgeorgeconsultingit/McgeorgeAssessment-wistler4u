...
class HrExpenseSplit(models.TransientModel):
    ...
    tax_ids = fields.Many2many(
        "account.tax", domain="[('company_id', '=', company_id), ('type_tax_use', '=', 'purchase')]"
    )
    ...
    product_has_tax = fields.Boolean(
        "Whether tax is defined on a selected product", compute="_compute_product_has_tax"
    )
    product_has_cost = fields.Boolean(
        "Is product with non zero cost selected", compute="_compute_from_product_id", store=True
    )
