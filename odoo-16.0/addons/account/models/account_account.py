...
class AccountAccount(models.Model):
    ...
    def _get_most_frequent_accounts_for_partner(
        self, company_id, partner_id, move_type, filter_never_user_accounts=False, limit=None
    ):
        """
    ...
    def copy(self, default=None):
        ...
        try:
            ...
            while self.env["account.account"].search(
                [
                    ("code", "=", default["code"]),
                    ("company_id", "=", default.get("company_id", False) or self.company_id.id),
                ],
                limit=1,
            ):
    ...
    def _unlink_except_contains_journal_items(self):
        if self.env["account.move.line"].search([("account_id", "in", self.ids)], limit=1):
            raise UserError(
                _("You cannot perform this action on an account that contains journal items.")
            )
    ...
    def _unlink_except_account_set_on_customer(self):
        ...
        partner_prop_acc = (
            self.env["ir.property"].sudo().search([("value_reference", "in", values)], limit=1)
        )
        if partner_prop_acc:
