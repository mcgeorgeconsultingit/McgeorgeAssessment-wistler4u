...
class TestQWebTField(TransactionCase):
    ...
    def test_default_value(self):
        ...
        result = """
                <div></div>
        """
        rendered = self.env["ir.qweb"]._render(t.id, {"record": Partner.new({"name": ""})})
        self.assertEqual(str(rendered.strip()), result.strip())
...
class TestQWebNS(TransactionCase):
    ...
    def test_render_dynamic_xml_with_namespace_2(self):
        ...
        self.assertEqual(
            etree.fromstring(self.env["ir.qweb"]._render(view1.id, {"version_id": 1.0})),
            expected_result,
        )
...
class TestQWebBasic(TransactionCase):
    ...
    def test_foreach_as_error_3(self):
        ...
        try:
            ...
            error = str(e)
            self.assertIn(
                "The varname 'b-2' can only contain alphanumeric characters and underscores", error
            )
    ...
    def test_set_error_2(self):
        ...
        try:
            ...
            error = str(e)
            self.assertIn(
                "The varname can only contain alphanumeric characters and underscores", error
            )
    ...
    def test_render_barcode(self):
        partner = self.env["res.partner"].create({"name": "bacode_test", "barcode": "test"})
        ...
        self.assertRegex(
            rendered, r'<div><img alt="Barcode test" src="data:image/png;base64,\S+"></div>'
        )
...
class TestQwebCache(TransactionCase):
    ...
    def test_render_xml_cache_different(self):
        ...
        result = etree.fromstring(
            IrQweb._render(
                view1.id,
                {"cache_id": (1,), "cache_id2": (1,), "value": [1, 2, 3], "value2": [10, 20, 30]},
            )
        )
        self.assertEqual(
            result,
    ...
    def test_render_xml_conditional_cache(self):
        ...
        result = etree.fromstring(
            IrQweb._render(view1.id, {"cache_id": 1, "condition": True, "value": [1, 2, 3]})
        )
        self.assertEqual(result, expected_result, "First rendering (add in cache)")

        result = etree.fromstring(
            IrQweb._render(view1.id, {"cache_id": 1, "condition": True, "value": [10, 20, 30]})
        )
        self.assertEqual(result, expected_result, "Next rendering use cache")
    ...
    def test_render_xml_cache_and_inherit_view(self):
        ...
        self.assertEqual(
            result, expected_result, "First rendering create cache from company and the value 1"
        )
        ...
        self.assertEqual(
            result, expected_result, "Next rendering create cache from company and the value 10"
        )
...
class TestQWebStaticXml(TransactionCase):
    ...
    def run_test_file(self, path):
        ...
        for template in loader:
            ...
            result = remove_space(doc.find('result[@id="{}"]'.format(template)).text or "").replace(
                "&quot;", "&#34;"
            )
