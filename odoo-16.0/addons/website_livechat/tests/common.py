...
class TestLivechatCommon(tests.TransactionCase):
    def setUp(self):
        ...
        self.open_chat_params = {
            "params": {"channel_id": self.livechat_channel.id, "anonymous_name": "Wrong Name"}
        }
        ...
        self.patch(
            type(self.env["website.visitor"]), "_get_visitor_from_request", get_visitor_from_request
        )
    ...
    def _send_rating(self, channel, visitor, rating_value, reason=False):
        ...
        res_model_id = (
            self.env["ir.model"].sudo().search([("model", "=", channel._name)], limit=1).id
        )
