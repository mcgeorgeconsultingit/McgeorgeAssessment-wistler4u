...
class AccountMove(models.Model):
    ...
    def _check_invoice_type_document_type(self):
        ...
        docs_used_for_inv_and_ref = self.filtered(
            ...
            and x.l10n_latam_document_type_id.code in self._get_l10n_ar_codes_used_for_inv_and_ref()
        ...
        for inv in self.filtered(lambda x: x.company_id.l10n_ar_company_requires_vat):
            ...
            for line in inv.mapped("invoice_line_ids").filtered(
                ...
                if len(vat_taxes) != 1:
                    ...
                    raise UserError(
                        _('On invoice id "%s" you must use VAT Not Applicable on every line.')
                        % inv.id
                    )
                elif (
                    purchase_aliquots == "not_zero"
    ...
    def _onchange_partner_journal(self):
        ...
        for rec in self.filtered(
            ...
            if (
                ...
                journal = journal.search(
                    domain + [("l10n_ar_afip_pos_system", "not in", expo_journals)], limit=1
                )
                msg = _(
        ...
        for rec in to_review:
            ...
            if current_pos != rec.journal_id.l10n_ar_afip_pos_number:
                invoices = self.search(
                    [("journal_id", "=", rec.journal_id.id), ("posted_before", "=", True)], limit=1
                )
    ...
    def _l10n_ar_get_amounts(self, company_currency=False):
        ...
        return {
            ...
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "07"
                ).mapped(amount_field)
            ),
            "mun_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "08"
                ).mapped(amount_field)
            ),
            "intern_tax_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "04"
                ).mapped(amount_field)
            ),
            "other_taxes_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "99"
                ).mapped(amount_field)
            ),
            ...
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "06"
                ).mapped(amount_field)
            ),
            "other_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "09"
