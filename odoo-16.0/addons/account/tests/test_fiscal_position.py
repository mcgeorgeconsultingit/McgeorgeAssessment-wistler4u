...
class TestFiscalPosition(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.jc = cls.res_partner.create(dict(name="JCVD", vat="BE0477472701", country_id=be.id))
        ...
        cls.be_nat = cls.fp.create(
            dict(name="BE-NAT", auto_apply=True, country_id=be.id, vat_required=False, sequence=10)
        )
    ...
    def test_10_fp_country(self):
        def assert_fp(partner, expected_pos, message):
            self.assertEqual(self.fp._get_fiscal_position(partner).id, expected_pos.id, message)
        ...
        self.fp2m = self.fp.create(
            {
                "name": "FP-TAX2TAXES",
                "tax_ids": [
                    (0, 0, {"tax_src_id": self.src_tax.id, "tax_dest_id": self.dst1_tax.id}),
                    (0, 0, {"tax_src_id": self.src_tax.id, "tax_dest_id": self.dst2_tax.id}),
                ],
            }
        )
