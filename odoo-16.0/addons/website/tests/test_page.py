...
class TestPage(common.TransactionCase):
    ...
    def test_copy_page(self):
        ...
        self.assertEqual(
            len(cloned_page), 1, "A page with an URL /page_specific-1 should've been created"
        )
        ...
        self.assertEqual(
            len(cloned_page_about_us), 1, "A page with an URL /about-us should've been created"
        )
        self.assertEqual(
    ...
    def test_cou_page_frontend(self):
        ...
        self.assertTrue(
            website_id
            not in View.search([("name", "in", ("Base", "Extension"))]).mapped("website_id").ids,
            "Same for views",
        )
...
class WithContext(HttpCase):
    ...
    def test_unpublished_page(self):
        ...
        specific_page.write(
            {
                "is_published": False,
                "arch": self.page.arch.replace("I am a generic page", "I am a specific page"),
            }
        )
        ...
        self.assertEqual(
            "I am a specific page" in r.text, True, "Admin should see the specific unpublished page"
        )
    ...
    def test_03_error_page_debug(self):
        with MockRequest(self.env, website=self.env["website"].browse(1)):
            ...
            self.assertIn(
                "ZeroDivisionError: division by zero", r.text, "Error should be shown in debug."
            )
    ...
    def test_06_homepage_url(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.url, contactus_url_full, "Menu fallback should be a redirect, not a reroute"
        )
        self.assertIn(contactus_content, r.content)
    ...
    def test_07_alternatives(self):
        ...
        with self.subTest(url="/page_1"):
            ...
            canonical_url = root_html.xpath('//link[@rel="canonical"]')[0].attrib["href"]
            alternate_en_url = root_html.xpath('//link[@rel="alternate"][@hreflang="en"]')[
                0
            ].attrib["href"]
            alternate_fr_url = root_html.xpath('//link[@rel="alternate"][@hreflang="fr"]')[
                0
            ].attrib["href"]
        ...
        specific_page.write(
            {
                "arch": self.page.arch.replace(
                    "I am a generic page", "I am a specific page not available for visitors"
                ),
                "is_published": True,
    ...
    def test_page_url_case_insensitive_match(self):
        ...
        self.assertEqual(
            r2.status_code, 303, "URL exists only in different casing, should redirect to it"
        )
        self.assertTrue(
            r2.headers.get("Location").endswith("/page_1"), "Should redirect /Page_1 to /page_1"
        )
