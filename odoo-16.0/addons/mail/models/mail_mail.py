...
class MailMail(models.Model):
    ...
    mail_message_id = fields.Many2one(
        "mail.message", "Message", required=True, ondelete="cascade", index=True, auto_join=True
    )
    ...
    to_delete = fields.Boolean(
        "To Delete", help="If set, the mail will be deleted during the next Email Queue CRON run."
    )
    ...
    def send(self, auto_commit=False, raise_exception=False):
        ...
        for mail_server_id, smtp_from, batch_ids in self._split_by_mail_configuration():
            ...
            try:
                ...
                _logger.info(
                    "Sent batch %s emails via mail server ID #%s", len(batch_ids), mail_server_id
                )
            finally:
                if smtp_session:
                    smtp_session.quit()
