...
class Partner(models.Model):
    ...
    membership_amount = fields.Float(
        string="Membership Amount", digits=(16, 2), help="The price negotiated by the partner"
    )
        ...
        for partner in self:
            ...
            for mline in partner.member_lines:
                if (mline.date_to or date.min) >= today and (mline.date_from or date.min) <= today:
                    ...
                    if mline.account_invoice_id and mline.account_invoice_id.payment_state in (
                        "in_payment",
                        "paid",
                    ):
