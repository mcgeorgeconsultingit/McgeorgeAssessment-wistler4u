...
class TestAnalyticAccount(TestMrpAnalyticAccount):
    ...
    def test_workcenter_different_analytic_account(self):
        ...
        self.assertEqual(mo.workorder_ids.mo_analytic_account_line_id.amount, -10.0)
        self.assertEqual(
            mo.workorder_ids.mo_analytic_account_line_id.account_id, self.analytic_account
        )
    ...
    def test_changing_mo_analytic_account(self):
        ...
        Form(
            self.env[(wizard_dict.get("res_model"))].with_context(wizard_dict["context"])
        ).save().process()
        self.assertEqual(mo.state, "done")
        ...
        self.assertEqual(
            mo.move_raw_ids.analytic_account_line_id.account_id.id, new_analytic_account.id
        )
