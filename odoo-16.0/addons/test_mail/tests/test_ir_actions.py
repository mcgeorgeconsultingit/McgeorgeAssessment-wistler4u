...
class TestServerActionsEmail(TestMailCommon, TestServerActionsBase):
    ...
    def test_action_email(self):
        # initial state
        self.assertEqual(len(self.test_partner.message_ids), 1, "Contains Contact created message")
    ...
    def test_action_message_post(self):
        ...
        self.assertEqual(
            self.test_partner.message_ids[0].email_from, self.partner_root.email_formatted
        )
        self.assertFalse(self.test_partner.message_partner_ids)
        ...
        with self.assertSinglePostNotifications(
            ...
            self.action.with_context(self.context).run()
        self.assertEqual(len(self.test_partner.message_ids), 3, "2 new messages produced")
