...
class StockScrap(models.Model):
    ...
    def _get_default_scrap_location_id(self):
        ...
        return (
            ...
            .search(
                [("scrap_location", "=", True), ("company_id", "in", [company_id, False])], limit=1
            )
            .id
        )
    ...
    package_id = fields.Many2one(
        "stock.quant.package", "Package", states={"done": [("readonly", True)]}, check_company=True
    )
    owner_id = fields.Many2one(
        "res.partner", "Owner", states={"done": [("readonly", True)]}, check_company=True
    )
    move_id = fields.Many2one(
        "stock.move", "Scrap Move", readonly=True, check_company=True, copy=False
    )
    picking_id = fields.Many2one(
        "stock.picking", "Picking", states={"done": [("readonly", True)]}, check_company=True
    )
    ...
    def _onchange_serial_number(self):
        if self.product_id.tracking == "serial" and self.lot_id:
            message, recommended_location = self.env["stock.quant"]._check_serial_number(
                self.product_id,
                self.lot_id,
                self.company_id,
                self.location_id,
