...
class Channel(models.Model):
    ...
    def _generate_random_token(self):
        ...
        return "".join(
            choice("abcdefghijkmnopqrstuvwxyzABCDEFGHIJKLMNPQRSTUVWXYZ23456789") for _i in range(10)
        )

    # description
    name = fields.Char("Name", required=True)
    active = fields.Boolean(
        default=True, help="Set active to false to hide the channel without removing it."
    )
    ...
    def _search_channel_partner_ids(self, operator, operand):
        return [
            (
                "channel_member_ids",
                "in",
                self.env["mail.channel.member"].sudo()._search([("partner_id", operator, operand)]),
            )
        ]
    ...
    def _compute_member_count(self):
        read_group_res = self.env["mail.channel.member"]._read_group(
            domain=[("channel_id", "in", self.ids)], fields=["channel_id"], groupby=["channel_id"]
        )
        ...
        for channel in self:
            ...
            if channel.group_public_id:
                invalid_partners = partners.filtered(
                    lambda partner: channel.group_public_id not in partner.user_ids.groups_id
                )
                if invalid_partners:
                    raise UserError(
                        _(
            ...
            existing_members = self.env["mail.channel.member"].search(
                expression.AND(
                    [
                        [("channel_id", "=", channel.id)],
                        expression.OR(
                            [[("partner_id", "in", partners.ids)], [("guest_id", "in", guests.ids)]]
                        ),
                    ]
                )
            )
            ...
            notifications.append(
                (
                    ...
                    {
                        "channelMembers": [
                            ("insert", list(new_members._mail_channel_member_format().values()))
                        ],
                        "id": channel.id,
                    ...
                    (
                        ...
                        "mail.channel/insert",
                        {
                            "channelMembers": [
                                (
                                    "insert",
                                    list(existing_members._mail_channel_member_format().values()),
                                )
                            ],
                            "id": channel.id,
                            "memberCount": channel.member_count,
    ...
    def _message_compute_parent_id(self, parent_id):
        ...
        return (
            ...
            .search([("id", "=", parent_id), ("model", "=", self._name), ("res_id", "=", self.id)])
            .id
        )

    @api.returns("mail.message", lambda value: value.id)
    def message_post(self, *, message_type="notification", **kwargs):
        self.filtered(lambda channel: channel.is_chat or channel.channel_type == "group").mapped(
            "channel_member_ids"
        ).sudo().write(
            {
                "is_pinned": True,
        ...
        return super(
            Channel, self.with_context(mail_create_nosubscribe=True, mail_post_autofollow=False)
    ...
    def _message_subscribe(self, partner_ids=None, subtype_ids=None, customer_ids=None):
        ...
        considered."""
        raise UserError(
            _("Adding followers on channels is not possible. Consider adding members instead.")
        )
    ...
    def channel_info(self):
        ...
        all_needed_members = self.env["mail.channel.member"].search(
            expression.AND([[("channel_id", "in", self.ids)], all_needed_members_domain]),
            order="id",
        )
        ...
        for channel in self:
            ...
            if current_partner or current_guest:
                ...
                ).with_prefetch([m.id for m in member_of_current_user_by_channel.values()])
                if member:
            ...
            if channel.channel_type != "channel":
                ...
                channel_data["channelMembers"] = [
                    (
                        "insert",
                        list(members_by_channel[channel]._mail_channel_member_format().values()),
                    )
                ]
    ...
    def channel_get(self, partners_to, pin=True):
        ...
        if len(partners_to) > 2:
            raise UserError(
                _("A chat should not be created with more than 2 persons. Create a group instead.")
            )
            ...
            channel = self.create(
                {
                    ...
                    "channel_type": "chat",
                    "name": ", ".join(
                        self.env["res.partner"].sudo().browse(partners_to).mapped("name")
                    ),
                }
            )
    ...
    def _set_last_seen_message(self, last_message):
        ...
        channel_member_domain = expression.AND(
            [
                ...
                [("partner_id", "=", self.env.user.partner_id.id)],
                expression.OR(
                    [[("seen_message_id", "=", False)], [("seen_message_id", "<", last_message.id)]]
                ),
            ]
        )
    ...
    def create_group(self, partners_to, default_display_mode=False, name=""):
        ...
        channel = self.create(
            {
                "channel_member_ids": [
                    Command.create({"partner_id": partner_id}) for partner_id in partners_to
                ],
                "channel_type": "group",
    ...
    def load_more_members(self, known_member_ids):
        ...
        unknown_members = self.env["mail.channel.member"].search(
            domain=[("id", "not in", known_member_ids), ("channel_id", "=", self.id)], limit=100
        )
    ...
    def execute_command_help(self, **kwargs):
        ...
        if self.channel_type == "channel":
            ...
            msg = _(
                ...
                _(" @").join(html_escape(member.partner_id.name) for member in channel_members)
                if channel_members
                else _("Anonymous"),
            )
