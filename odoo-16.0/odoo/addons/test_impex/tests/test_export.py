...
class test_m2o(CreatorCase):
    ...
    def test_path(self):
        ...
        record = self.env["export.integer"].create({"value": 42})
        self.assertEqual(
            self.export(record.id, fields=["value/.id", "value/value"]), [[str(record.id), 42]]
        )
...
class test_o2m(CreatorCase):
    ...
    def test_single_subfield(self):
        self.assertEqual(
            self.export([Command.create({"value": 42})], fields=["value", "value/value"]),
            [["export.one2many.child:42", 42]],
        )

    def test_integrate_one_in_parent(self):
        self.assertEqual(
            self.export([Command.create({"value": 42})], fields=["const", "value/value"]), [[4, 42]]
        )
...
class test_o2m_multiple(CreatorCase):
    ...
    def test_single_per_side(self):
        ...
        self.assertEqual(
            self.export(
                child1=[Command.create({"value": 43})], child2=[Command.create({"value": 42})]
            ),
    ...
    def test_single_integrate_subfield(self):
        ...
        self.assertEqual(
            self.export(child1=False, child2=[Command.create({"value": 42})], fields=fields),
            [[36, "", 42]],
        )

        self.assertEqual(
            self.export(child1=[Command.create({"value": 43})], child2=False, fields=fields),
            [[36, 43, ""]],
        )
...
class test_m2m(CreatorCase):
    ...
    def test_multiple_records(self):
        self.assertEqual(
            self.export(
                self.commands, fields=["const", "value/value"], context={"import_compat": False}
            ),
            [
                [4, 4],
                ["", 42],
